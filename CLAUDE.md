...
1. **`/軍議`** — 家老に偵察・設計を命じ、陣立て書＋**受け入れ条件**をまとめる。**上奏の前に Codex の軍議検分を既定で通す**（AC 一覧自体の漏れは実装後の検分では構造的に拾えないため。Codex が未ログイン・リミット停止等で使えないときのみスキップし、上奏に未実施＋理由を明記）
...
**表の列は必ず7つ（ID / 戦役 / 状態 / 依存 / 完了条件 / 証拠(PR/テスト) / 台帳）。** 補足事項は新しい列を増やさず既存列（多くは「証拠(PR/テスト)」）へ畳み込むこと。セル内で本物の `|` を使う場合は `\|` へエスケープする（コードスパン \` ... \` 内の `|` はエスケープ不要）。列数のズレは同じ番人クラス `TaskListCmpIdDuplicateGuardTest` の `cmpIdの行は列数が7である()` が CI で検出する。
...
| `/巡回` | ビルド・テスト監視 |
| `/撤収` | 開発環境の終了 |
| `/実機` | 実機E2E（ユーザー視点・モックなし。ロール横断必須） |
| `/アリシゼーション` | 住民として触り回り、決めたシナリオでは出ない欠陥（導線欠落）を掘る。実機E2E の後に必ず実施 |
| `/凱旋` | 検分→CI緑裏取り→マージ（`--admin`可）を一気通貫 |
| `/物見` | CI/PR 戦況の低頻度・単発確認（ポーリング枯渇回避） |
| `/陣払い` | 古い足軽 worktree の一括掃除 |
| `/OpenJDK整理` | メモリ常駐JVM（Gradleデーモン等）の実測→残骸のみ停止 |
