...
$mutating = 'git(\s+-{1,2}\S+(\s+\S+)?)*\s+(' + (@(
    ...
    # ただし**削除だけ**は本陣でも許す（凱旋後の後片付けのため）。CLAUDE.md は
    # 「worktree のクリーンアップなど git 管理操作そのもの」を本陣の例外に挙げて
    # いるが、#3082 で branch を保護対象へ広げた際にブランチ削除まで巻き込まれ、
    # マージ済みブランチの掃除ができなくなっていた（2026-09-04 実測）。
    #
    # なぜ -d だけでなく -D も許すのか:
    #   本リポジトリのマージは squash であり、マージ済みブランチのコミットは
    #   main の祖先にならない。よって `git branch -d` は常に「未マージ」として
    #   拒否され、後片付けに実際に使えるのは -D だけである。-d のみ許しても
    #   この問題は解決しない。
    #
    # なぜ許してよいと判断したか:
    #   - git 自身が「いずれかの作業木がチェックアウト中のブランチ」の削除を
    #     拒む。他セッションが作業中のブランチは構造的に消せない。
    #   - 削除するのはローカル参照だけで、作業木・インデックス・リモートは
    #     変わらない。消した参照も reflog から復元できる。
    #   - 本フックが守りたいのは「本陣の共有作業木が書き換わって他セッションの
    #     未コミット作業が消えること」であり、ブランチ削除はそこに触れない。
    #
    # 作成・改名・upstream 設定は従来どおり拒否のまま残す（共有リポジトリの
    # ローカル参照を増やす・付け替える操作は隔離してから行うべきため）。
    # 削除フラグと**ブランチ名以外の語が混じらない**ときだけ除外する。
    # -r/--remotes を伴う `git branch -dr` はリモート追跡参照の削除であり、
    # 後片付けの範囲を超えるため除外しない（拒否のまま）。
        # 末尾はセグメント終端だけでなく、パイプ・リダイレクトでも閉じてよい。
    # `git branch -D x 2>&1 | tail` のように後続が付く形が実運用では普通であり、
    # `$` だけで閉じると除外が成立せず拒否へ倒れる（2026-09-04 実測。最初の実装は
    # 素のコマンドしかテストしておらず、この形を取りこぼしていた）。
    # ブランチ名トークンからも | < > & を除き、名前がパイプを飲み込まないようにする。
'branch(?=(?:\s+-\S+)*\s+[^-\s])(?!(?:\s+(?:-[dD]|--delete))+(?:\s+[^-\s|<>&][^\s|<>&]*)+\s*(?:$|[|<>&]|\d>))',
    # 引数を取らない破壊的オプション
