...
| CMP-023 | 時刻設計の全域是正（瞬間はUTC・壁時計はTZ付き）＋テナントTZ導入 | 未着手 | #2700 | `LocalDateTime` の二義使用を解消し `TimeZoneConfig` の強制JSTを撤去した上で、チーム単位のタイムゾーンで業務ローカル時刻が解釈されること（Issue #2684 の締切判定2件を含む） | 設計 #2698・#2680（main済） | — |
