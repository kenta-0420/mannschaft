...
- **R2 オブジェクトキーの命名規約**（推奨）: `{feature}/{scope_type}/{scope_id}/{subpath}/{uuid}.{ext}` の階層構造とする。例:
  - タイムライン画像: `timeline/TEAM/1/posts/250/img-{uuid}.webp`
  - タイムライン動画: `timeline/TEAM/1/posts/250/video-{uuid}.mp4`
  - ギャラリー写真: `gallery/TEAM/1/album-8/photo-{uuid}.jpg`
  - ギャラリー動画: `gallery/TEAM/1/album-8/video-{uuid}.mp4`
  - ファイル共有: `files/TEAM/5/{uuid}.{ext}`（§26 準拠）
  - ブログ画像: `blog/TEAM/3/post-100/image-{uuid}.jpg`
  - ブログ動画: `blog/TEAM/3/post-100/video-{uuid}.mp4`
  - チャット添付: `chat/TEAM/5/channel-10/{uuid}.{ext}`
  - カルテ写真: `chart/TEAM/1/record-42/{photo_type}-{uuid}.jpg`
  - 下書き一時領域: `{feature}/{scope_type}/{scope_id}/tmp/{uuid}.{ext}`（7日後に R2 Object Lifecycle で自動削除）
- **Presigned URL フロー**: R2 は S3 互換 API を提供するため、AWS SDK v3 を Cloudflare R2 エンドポイント（`https://{accountId}.r2.cloudflarestorage.com`）に向けて設定することで、既存の S3 用 `getSignedUrl` / `HeadObject` / `CopyObject` ロジックがそのまま使える。Presigned PUT（アップロード）の有効期限はファイル種別ごとに調整（画像: 10分、動画: 15分）
- **配信 URL**: ファイル URL の配信は Cloudflare Workers 経由が定石である。Workers ルートが (a) ユーザー認証 + 対象スコープの認可チェック、(b) R2 バケットからのストリーミング（HTTP Range Request 対応、動画シーク用）、(c) 必要に応じて短命の R2 S3 互換 Presigned GET URL への 302 リダイレクトを担う。URL を公開バケット経由で直接露出させる運用は行わない
- **画像・動画の後処理**: 画像の WebP 変換・サムネイル生成・EXIF ストリップ、動画のメタデータ抽出（再生時間・解像度・コーデック）・ポスターフレーム抽出は Cloudflare Workers（ffmpeg.wasm）または Cloudflare Images / Cloudflare Stream の連携で行う。AWS Lambda は使用しない
- **クォータ連携**: `shared_files.storage_used_bytes` / `storage_subscriptions.used_bytes` 等は R2 上の実使用量と一致させる。計量ロジック・計算単位（バイト）は S3 時代から変更なし。詳細は [横断設計: 統合ストレージクォータ](cross-cutting/storage_quota.md) を参照
