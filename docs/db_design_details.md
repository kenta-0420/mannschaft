...
- **基盤**: プラットフォーム上の全ファイルストレージ（画像・動画・ドキュメント・添付ファイル等）は **Cloudflare R2** に集約する。AWS S3 / CloudFront / Lambda の構成は採用しない
- **バケット構成**: **単一バケット `mannschaft-storage` + プレフィックスで機能を分離** する方針に統一する。機能別にバケットを分ける運用は採用しない（クォータ集計・ライフサイクル管理・鍵運用の複雑化を避けるため）。全ての機能は同一バケット内の異なるプレフィックスに書き込む
- **オブジェクトキーの統一表記**: 設計書・コード上では `file_key` / `s3_key` / `photo_s3_key` / `cover_image_s3_key` 等のカラム名を歴史的経緯で維持するが、**格納される値は Cloudflare R2 バケット内のオブジェクトキー**である。新規テーブル追加時は `file_key` / `object_key` / `r2_key` など S3 を示唆しない命名を推奨するが、既存カラムは互換性のため無理にリネームしない（リネームは別タスクで一括検討）

### R2 単一バケット プレフィックス規約

```
mannschaft-storage/                                         ← 単一バケット
├── timeline/{scopeType}/{scopeId}/posts/{postId}/{uuid}.{ext}   # タイムライン投稿（画像・動画）
├── gallery/{scopeType}/{scopeId}/album-{albumId}/{uuid}.{ext}   # ギャラリー（写真・動画）
├── files/{scopeType}/{scopeId}/{uuid}.{ext}                     # ファイル共有
├── chat/{scopeType}/{scopeId}/channel-{channelId}/{uuid}.{ext}  # チャット添付
├── blog/{scopeType}/{scopeId}/post-{postId}/{uuid}.{ext}        # ブログメディア
├── chart/{scopeType}/{scopeId}/record-{recordId}/{uuid}.{ext}   # カルテ写真
├── thumbnails/{元のオブジェクトキー}.jpg                        # サムネイル（クォータ非計量）
└── tmp/{uuid}/                                                  # 一時ファイル（24時間で自動削除）
```

- **`scopeType`** の値は `PERSONAL` / `TEAM` / `ORGANIZATION` の3種類（`ScopeType` ENUM と一致させる。大文字固定）
- **`thumbnails/` プレフィックス**: Workers または Cloudflare Images が生成したサムネイル・WebP 変換物・動画ポスターフレームの保存先。命名規則は `thumbnails/{元のパス}.jpg`（例: 元動画が `gallery/TEAM/1/album-8/video-abc.mp4` ならサムネイルは `thumbnails/gallery/TEAM/1/album-8/video-abc.mp4.jpg`）。**ユーザーが制御できない後処理生成物のため、ストレージクォータには計上しない**
- **`tmp/` プレフィックス**: Multipart Upload の下書きや、Presigned URL 発行後に紐付けされなかった孤立オブジェクトの一時置き場。下書き・一時領域は従来 `{feature}/{scopeType}/{scopeId}/tmp/{uuid}.{ext}` の形式で各機能プレフィックス配下に置く運用もあるが、完全に DB 未登録の短命オブジェクトは `tmp/` 直下に置いて 24時間で自動削除する

### R2 Object Lifecycle Rule（サンプル JSON）

```json
{
  "Rules": [
    {
      "Id": "DeleteTmpAfter24Hours",
      "Status": "Enabled",
      "Filter": { "Prefix": "tmp/" },
      "Expiration": { "Days": 1 }
    },
    {
      "Id": "DeleteFeatureTmpAfter7Days",
      "Status": "Enabled",
      "Filter": { "Prefix": "" },
      "Expiration": {
      ...
      "Note": "各機能プレフィックス配下の `tmp/` サブパス（timeline/.../tmp/, gallery/.../tmp/ 等）は 7日間の下書き猶予期間後に削除する。実装時は `tmp/` を含むキーパターンでフィルタ"
    }
  ]
}
```

> 本番運用では Cloudflare R2 の Lifecycle API（S3 互換 `PutBucketLifecycleConfiguration`）で適用する。上記 JSON はサンプルであり、実際の prefix マッチ条件は Cloudflare の挙動に応じて調整する
>
> **[実装時判断]** R2 Lifecycle Rule の prefix マッチが `tmp/` サブパスに
> 正しく適用されるか、Cloudflare ダッシュボードまたは API で実機確認すること。
> S3 互換仕様ベースで記述しているが R2 実挙動と差異がある場合は修正が必要。

### Presigned URL フロー

- R2 は S3 互換 API を提供するため、AWS SDK v3 を Cloudflare R2 エンドポイント（`https://{accountId}.r2.cloudflarestorage.com`）に向けて設定することで、既存の S3 用 `getSignedUrl` / `HeadObject` / `CopyObject` ロジックがそのまま使える。Presigned PUT（アップロード）の有効期限はファイル種別ごとに調整（画像: 10分、動画: 15分）
- **単発 PUT の上限**: 1 リクエストで PUT できる最大サイズは **100MB**。理由: ブラウザメモリ・回線切断時のリトライコスト・タイムアウト耐性。100MB を超える場合は必ず Multipart Upload（`CreateMultipartUpload` → 各パートを Presigned PUT → `CompleteMultipartUpload`）を使用する
- **Multipart Upload の上限**: 1 オブジェクトあたり **最大 5TB**（R2 のハード上限）。パートサイズは 5〜10MB 推奨、最大 10,000 パート
- Multipart Upload フローは F04.1 タイムライン動画・F06.2 ギャラリー動画・F06.1 ブログ動画・F05.5 ファイル共有の全ての大容量アップロードで共通利用する（各機能ドキュメントは `docs/features/F05.5_file_sharing.md` の Multipart フローを参照する形で記載）

### 配信 URL

- ファイル URL の配信は Cloudflare Workers 経由が定石である。Workers ルートが (a) ユーザー認証 + 対象スコープの認可チェック、(b) R2 バケットからのストリーミング（HTTP Range Request 対応、動画シーク用）、(c) 必要に応じて短命の R2 S3 互換 Presigned GET URL への 302 リダイレクトを担う。URL を公開バケット経由で直接露出させる運用は行わない

### 画像・動画の後処理 — サムネイル生成戦略（ハイブリッド方式）

動画サムネイル生成は **動画長に応じて Workers 側とクライアント側を使い分けるハイブリッド方式** を採用する。Cloudflare Stream は R2 と別課金体系（月額 + エンコード時間課金）で割高になるため **採用しない**。画像最適化用途としては Cloudflare Images を採用する（ブログ画像・ギャラリー写真の WebP 変換・リサイズ配信）。

| 動画長 | 生成方式 | 実装場所 |
|---|---|---|
| **5 分以下（短尺）** | **Workers + ffmpeg.wasm** で R2 上にサムネイル生成 | バックエンド（Cloudflare Workers） |
| **5 分超（長尺）** | **クライアント側で `<video>` + Canvas によるフレーム抽出** → R2 にアップロード | フロントエンド（Vue コンポジション） |

振り分け閾値を **5 分（300 秒）** に統一し、全機能（F04.1 / F06.1 / F06.2 / F05.5）で同じ値を使う。

#### Workers 実装フロー（短尺動画用）

```
1. クライアントが R2 に動画をアップロード完了
   → バックエンド API（例: POST /api/v1/timeline）が動画メタデータを受信
2. バックエンドが動画長を HeadObject / メタデータ抽出で確認
3. 動画長 <= 300秒 の場合、`POST /internal/thumbnail-jobs` を Cloudflare Workers にトリガー
4. Worker が R2 から動画をストリーミング読込（Range Request）
5. ffmpeg.wasm で 1 秒目のフレームを JPEG 抽出
6. R2 の `thumbnails/{元のパス}.jpg` に PUT（サムネイルは `thumbnails/` プレフィックス配下でクォータ非計量）
7. DB の対応レコード（例: timeline_post_attachments.video_thumbnail_key）を UPDATE
8. processing_status を READY に遷移
```

#### クライアント実装フロー（長尺動画用）

```
1. ユーザーがファイル選択 → HTMLVideoElement で動画メタデータを読込
2. duration > 300秒 と判定されたらクライアント側ルートに分岐
3. <video> の currentTime を 1 秒に設定 → Canvas に描画 → toBlob で JPEG 生成
4. サムネイル → 動画本体 の順で R2 に Presigned URL 経由でアップロード
   （サムネイルアップロード先は `thumbnails/{動画のオブジェクトキー}.jpg`）
5. バックエンドに「動画 URL ・サムネイル URL ・メタデータ」を POST
6. バックエンドは HeadObject で両方の R2 上の実在を確認 → DB に INSERT
```

#### 失敗時のフォールバック

Workers 生成・クライアント生成のいずれも失敗した場合は、`processing_status = FAILED` とし、フロントエンドは **プレースホルダー画像**（汎用の動画アイコン）を表示する。手動リトライ API（`POST /api/v1/.../regenerate-thumbnail`）を各機能で提供し、SYSTEM_ADMIN およびアップロード本人がサムネイル再生成を要求できる。

#### 画像最適化（Cloudflare Images）

- **F06.1 ブログ**: 記事内画像の WebP 変換・サイズ別リサイズ配信に採用。`https://imagedelivery.net/{account_hash}/{image_id}/{variant}` 経由で配信
- **F06.2 ギャラリー**: 写真サムネイル（短辺 300px）生成と WebP 変換に採用
- **動画サムネイルの配信最適化**: Workers / クライアント生成後の JPEG サムネイルも Cloudflare Images に登録して配信最適化に利用する選択肢あり（Phase 8 での判断）

- **クォータ連携**: `shared_files.storage_used_bytes` / `storage_subscriptions.used_bytes` 等は R2 上の実使用量と一致させる。計量ロジック・計算単位（バイト）は S3 時代から変更なし。`thumbnails/` プレフィックス配下および Cloudflare Images が自動生成した variant はクォータ計上対象外。詳細は [横断設計: 統合ストレージクォータ](cross-cutting/storage_quota.md) を参照
...
- `users.archived_at DATETIME nullable`: 最終ログインから6ヶ月経過時にバッチ処理で付与。null = アクティブ。ログイン成功時にクリアする。アーカイブ中はリスト系クエリから除外し、Valkey キャッシュも保持しない
...
- **週次送信制限**: チーム/組織単位で週3回まで。制限カウントは Valkey（`dm:weekly:{scopeType}:{scopeId}:{isoYearWeek}`、TTL 8日）で管理。Phase 8 の課金導入以降に上限緩和・無制限プランを検討する
...
- `ads`（将来）: 広告クリエイティブ（`title`, `image_url`（Cloudflare R2 保存）, `click_url`, `click_type` ENUM: INTERNAL_ORG/EXTERNAL_URL/AFFILIATE, `target_organization_id` nullable（INTERNAL_ORG時）, `is_active`）。SYSTEM_ADMINが審査・管理
...
- `timeline_post_attachments`: `attachment_type` は `IMAGE` / `VIDEO_FILE` / `VIDEO_LINK` / `LINK_PREVIEW` の VARCHAR。`IMAGE` と `VIDEO_FILE` は Cloudflare R2 に実体を保存し、`file_key` に R2 オブジェクトキーを格納。`VIDEO_LINK` は `video_url`（外部URL: YouTube / Vimeo）・`video_thumbnail_url`・`video_title` カラムを持ち、R2 ストレージは使用しない。`VIDEO_FILE` は動画メタデータ（`video_duration_seconds` / `video_codec` / `video_width` / `video_height`）と後処理ステータス（`video_processing_status`）を保持し、Cloudflare Workers が非同期でメタデータ抽出・サムネイル生成を実施する
...
- `chart_photos`: ビフォーアフター写真（`photo_type`: BEFORE/AFTER, `is_shared_to_customer`）。写真は **Cloudflare R2** に保存し、**Cloudflare Workers 経由の署名付き URL** でのみアクセス可能にする（医療・美容記録のため公開 URL は使用しない。Workers が認可チェック + R2 の S3 互換 Presigned GET URL 発行もしくはストリーミング配信を行う）。1カルテあたり最大20枚を推奨上限とする
...
- SNS フィードキャッシュ（Instagram/X API レスポンス）は揮発性データのため MySQL テーブルではなく **Valkey**（key: `sns_feed:{teamId}:{provider}`、TTL: 15分）で管理する
