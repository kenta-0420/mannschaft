...
> **ステータス**: 🟡 設計中（Phase 3 基盤 完了 / Phase 4 機能別統合 α〜ε・ζ 完了 / **Phase 5 R2パス命名規則変更 設計完了・実装待ち** / Phase 8 課金 未着手）
...
- [x] Phase 5-b: 移行バッチが完了し、旧パスのファイルが新パスに移行されている。`storage_migration_errors` の未解決件数がゼロ
...
| 2026-05-05 | **Phase 5-b 実装完了**: 既存ファイル移行バッチ実装。`R2StorageService.copyObject`（CopyObjectRequest）を追加。`StorageMigrationErrorEntity` + `StorageMigrationErrorRepository` + Flyway `V14.020` を追加。`StoragePathMigrationBatchService`（CHAT / FILE_SHARING / CIRCULATION / SCHEDULE_MEDIA / PERSONAL_TIMETABLE_NOTES の5機能対応、ページング500件・REQUIRES_NEW トランザクション・エラー記録＆スキップ戦略）を実装。`StorageMigrationAdminController`（GET /status・POST /run）を追加。`StorageMigrationStatus` レスポンス型を追加。`StoragePathMigrationBatchServiceTest`（isOldPath判定・新パス生成・移行正常系・エラー時スキップ・Repository件数テスト）を追加。旧パス削除は行わず R2 ライフサイクルルールで 30 日後に自動削除。 |
