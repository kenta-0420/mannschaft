...
Google の統合ストレージモデル（Gmail + Drive + Photos で合算15GB）に倣い、プラットフォーム上の全機能が消費する S3 ストレージを**組織・チーム・個人**の3レベルで統合的に計量し、SYSTEM_ADMIN が設定した無料枠を超過した場合にアップロードを制限する仕組み。無料枠・有料プラン・超過料金は SYSTEM_ADMIN がプラットフォーム管理画面から自由に設定・変更できる。
...
| 対象 | カウント | 理由 |
|------|---------|------|
| ユーザーがアップロードしたオリジナルファイル | **する** | ユーザーが制御可能な消費 |
| Lambda 生成のサムネイル/WebP 変換 | **しない** | システム都合で自動生成。ユーザーが制御不可 |
| S3 上のバージョン管理ファイル（F05.5） | **する** | 全バージョンが容量を消費（最大20世代） |
...
- `used_bytes` の整合性は週次バッチで実際の S3 使用量と突合（ドリフト検出・自動修正）
...
- `DRIFT_CORRECTION`: 週次バッチで `used_bytes` と実際の S3 使用量に差異があった場合の自動修正レコード
...
```
1. アップロードリクエストを受信（Pre-signed URL 発行前 or multipart 受信時）
2. スコープの特定:
...
4. アップロード許可の場合:
   a. Pre-signed URL を発行 / ファイル受信を続行
   b. アップロード完了後:
...
2. 全 storage_subscriptions を走査:
   a. S3 上の実ファイルサイズを feature_type 別に集計
      （カウント対象ルールに基づき、Lambda 生成ファイル・アバター等を除外）
...
- **クォータチェックのバイパス防止**: Pre-signed URL 発行前に必ずクォータチェックを実行。Pre-signed URL を使い回して容量超過状態でアップロードされた場合、S3 イベント → Lambda でサイズチェックし、超過分を自動削除 + ADMIN に通知
...
| 機能 | ドキュメント | 対象 API | スコープ帰属 |
|------|------------|---------|------------|
| F04.1 タイムライン | F04.1_timeline.md | 画像投稿 | scope_type に従う |
| F04.2 チャット | F04.2_chat.md | ファイル添付 | チャットルームの scope_type に従う。DM は送信者の個人クォータ |
| F05.1 掲示板 | F05.1_bulletin_board.md | 添付ファイル | scope_type に従う |
| F05.2 回覧板 | F05.2_circular.md | 添付ファイル追加 | scope_type に従う |
| F05.5 ファイル共有 | F05.5_file_sharing.md | ファイルアップロード・バージョン追加 | scope_type に従う（**既存の `team_storage_subscriptions` を `storage_subscriptions` に統合**） |
| F06.1 CMS/ブログ | F06.1_cms_blog.md | 画像アップロード | scope_type に従う |
| F06.2 メンバー紹介 | F06.2_member_gallery.md | 写真アップロード | scope_type に従う |
...
- [ ] **組織解散・チーム解散時のストレージ解放** → S3 ファイルの物理削除タイミング。提案: 30日の猶予期間後にバッチで物理削除
