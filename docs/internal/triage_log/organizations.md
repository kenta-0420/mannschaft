...
F05.6 §4 該当 3 行を実装に合わせて書き換える。

### D-2. F07.6 インシデント / F09.10 サイネージ: legacy prefix 注記追加（A-3, A-4）

各設計書 §4 セクション冒頭に「※現実装は legacy URL prefix で稼働中」の注記を追加。

### D-3. 状態列導入（案 A 採用）

triage_rules.md §B 案 A に従い、対象設計書の §4 API 仕様表に **状態列を先頭追加** する方針。
ただし全 26 設計書への展開は本足軽の時間枠を超えるため、F09.10 + F07.6 の 2 件のみ
状態列マイグレーションを実施し、残りは引き継ぎ事項とする。

### D-4. F06.5 知識ベース: organizations → teams へリネーム（A-1）

F06.5 §4 該当 8 行を `/api/v1/teams/{teamId}/knowledge-base/...` に書き換える。

### D-5. F07.5 スキル認証: organizations → teams へリネーム（A-2）

F07.5 §4 該当 15 行を `/api/v1/teams/{teamId}/skills/...` に書き換える。
