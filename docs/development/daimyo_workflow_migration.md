...
Dynamic Workflows は大名システムを**置き換えるものではなく、出陣・検分フェーズの「中身」を決定論的ファンアウトに差し替える道具**である。

- **軍議は対話スキルのまま残す** — Workflow は起動後に承認待ちで停止できず最後まで自走するため、`マスター御裁可` ゲートを内包できない。
- **出陣・検分は Workflow 化が有効** — 独立足軽の並列実装（`pipeline` + worktree）、次元別レビュー→敵対的検証（find→verify）は Workflow の本命パターン。
...
| 家老（設計・検分） | 軍議は `Plan`/`Explore` サブエージェントのまま。検分は Workflow の verify ステージ群 |
...
| `/検分` | **Workflow 化（本命）** | 次元別レビュー→敵対的検証の find/verify が最も効く |
...
[Workflow] /出陣 を1本起動（足軽の並列実装＋ビルド／テスト）→ 殿が結果を読む
            │
[Workflow] /検分 を1本起動（次元別レビュー→敵対的検証）→ 殿が判定
            │
...
| 用途 | model | effort | 根拠 |
|---|---|---|---|
| 殿の main loop（Workflow 起動元） | opus 4.8 | — | 采配・最終判断 |
| 家老（軍議・設計） | opus 4.8 | high | 設計の質がボトルネック |
...
1. **第1段: `/検分` を Workflow 化** — 既存の検分フローは独立性が高く、承認ゲートを内包しないため最も安全な初手。`code-review` スキルの思想を時代劇運用に合わせて移植。
...
- **Workflow はオプトイン**。スキル（`/出陣` `/検分`）内から呼ぶか、マスターが明示依頼した時のみ起動する。殿が普通の依頼で勝手にファンアウトしない。
