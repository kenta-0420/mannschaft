...
`:8082` が**使用中のときは所有者を問わず別ポートを指定して回避する**
（他セッションでも、自分が並行して走らせている生成でも同じ）。詳しい診断は下の「2.」を読むこと。
...
混同しないこと。**避ける**のはポートの話、**停止する**のはプロセスの話である。
そして**「避ける」の条件に所有者は関係ない**。所有者が効くのは「停止してよいか」だけである。

| 軸 | 条件 | 所有者は効くか |
|---|---|---|
| **別ポートへ避ける** | そのポートが**使えないとき全部**<br>・**使用中**（誰のものであろうと）<br>・**確認不能**（`[0]` / `[3]`、誰が使っているか分からない） | **効かない**。自分のものでも避ける |
| **プロセスを停止する** | AND 判定（条件1・2・3）をすべて満たす**自分の worktree の孤児**のときだけ | **効く**。自分のものに限る |

**使用中なら、所有者が誰であっても避ける。** 他 worktree のフォーク、他セッションのアプリ、
Docker の公開ポート、root のプロセスはもちろん、**自分が今まさに走らせている生成**や
**自分の別用途のプロセス**も同じである。後者は「自分のもの」だが**孤児ではない**ので
停止もできず、ポートも空かない。**避けるしかない。**

**避けるのはいつでも安全**なので、判断に迷う場面はすべて「避ける」で済ませてよい。
停止は、自分の孤児だと確定したときに限った最後の手段である。

##### 境界の確認（当てはまらない例）

条件は、**当てはまらない例**を並べると境界がはっきりする。

**「避ける」に当てはまらない**のは、**両側とも空きだと確認できたとき**だけ:

- `:8097` / `:8098` / `:8099` がいずれも Windows・WSL 両側で空き → そのまま使ってよい

逆に言えば、**使用中も確認不能もすべて避ける側**である。所有者は問わない。

**「停止する」に当てはまらない**のは、条件1・2・3 のどれかを欠くとき:

| 例 | 欠ける条件 |
|---|---|
| 他 worktree の openapi-gen フォーク（`fix-1525` 等） | 条件2（worktree 不一致） |
| 他 worktree の Gradle / アプリのプロセス | 条件1・2 |
| **自分の worktree の Gradle デーモン** | 条件1（`openapi-gen` ではない） |
| **自分が今まさに走らせている生成のフォーク** | 条件3（実行中であって孤児ではない） |
| 所有者不明のポート（`:3306` 等） | 条件2（確認できない） |

最後の2つに注意。**自分のものでも停止対象とは限らない**し、
**停止できないからといってポートが使えるわけでもない**（避けるだけ）。
...
| 出力 | 意味 | 対処 |
|---|---|---|
| `[0] 確認不能` | **`ss` が無い、または実行に失敗した**（プローブ自体が動かなかった） | **停止せず別ポートへ避ける**。空＝不在と読まない |
...
所有者を特定できた場合も、そのポートが使用中である以上**避ける対象である**
（他 worktree のフォークでも、自分が並行して走らせている生成でも変わらない）。
避ける／停止するの切り分けは上の「「避ける」と「停止する」は別の軸」に従うこと。
