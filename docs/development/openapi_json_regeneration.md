...
| 軸 | いつ | 対象 |
|---|---|---|
| **別ポートへ避ける** | ①**確認不能**（`[0]` / `[3]`、誰が使っているか分からない）<br>②**確認できて使用中**（他 worktree のフォーク、他セッションのアプリ、Docker 公開ポート等、自分のものではない） | そのポートを使うのをやめる |
| **プロセスを停止する** | AND 判定（条件1・2・3）をすべて満たす**自分の worktree の孤児**のときだけ | そのプロセスを終了させる |

**避けるのはいつでも安全**なので、判断に迷う場面はすべて「避ける」で済ませてよい。
停止は、自分のものだと確定したときに限った最後の手段である。
...
他 worktree のフォークを特定できた場合は、確認不能ではないが**避ける対象である**。
