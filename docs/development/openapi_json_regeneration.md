...
`:8082` が**使用中のときは所有者を問わず別ポートを指定して回避する**。詳しい診断は下の「2.」を読むこと。

**ただし例外がひとつある。同じ worktree で `generateOpenApiDocs` が既に走っているときは、
別ポートで二重起動してはならない。** `-PopenApiPort` が変えるのは**ポートだけ**で、
出力先は変わらないからである（`outputDir` / `outputFileName` は固定。`backend/build` も共有）。
二重起動すると 2 つのタスクが同じ `docs/openapi.json` と `backend/build` へ書き込み、
古い方が後から上書きする・ビルド出力が壊れる、といった事故になる。
**この場合は待つこと。**
...
if [ "$WIN_STATE" = "空き" ] && [ "$WSL_STATE" = "空き" ]; then
  ...
  echo "==> 使用中または確認不能。別ポートへ避ける（主が自分の worktree の生成なら待つ）"
fi
```

**「使ってよい」と言えるのは両側とも空きのときだけ。** 片側でも使用中なら、
`-PopenApiPort=<別ポート>` で避けるのが最短で、主の特定は必要なときだけ行えばよい。
**ただし主が自分の worktree の生成そのものなら、避けずに待つこと**（下記「3 つを使い分ける」）。
...
| 選ぶもの | 条件 | 理由 |
|---|---|---|
| **待つ** | 使用中の主が**自分の worktree の生成そのもの** | ポートを避けても出力先が競合する。二重起動しない |
| **別ポートへ避ける** | それ以外で**使用中**（所有者を問わない）、または**確認不能**（`[0]` / `[3]`） | 出力先は競合しないのでポートだけ避ければよい |
| **プロセスを停止する** | AND 判定（条件1・2・3）をすべて満たす**自分の worktree の孤児** | 孤児は誰の役にも立っていないので終了させてよい |

**「避ける」の条件に所有者は関係ない。** 所有者が効くのは「停止してよいか」と
「待つべきか」の 2 つだけである。他 worktree のフォークでも、他セッションのアプリでも、
Docker の公開ポートでも、root のプロセスでも、使用中なら等しく避ける。
...
これ以外は——使用中も確認不能も、所有者が誰であっても——**すべて避ける側**である。
...
| 所有者不明のポート（`:3306` 等） | 条件2（確認できない） | 避ける |
...
（主が自分の worktree の生成なら、避けずにその完了を待つ）。
