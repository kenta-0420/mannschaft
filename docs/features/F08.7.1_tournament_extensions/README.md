...
| enum 文字列追加（値追加。chat 以外は桁拡張不要） | bulletin `ScopeType` に `TOURNAMENT`（10字）/ `TOURNAMENT_DIVISION`（19字）＝`scope_type VARCHAR(20)` に収まり **MODIFY 不要**（[01](./01_communication.md) §2.2）、chat `ChannelType` に `TOURNAMENT_CHAT`（15字）/ `TOURNAMENT_DIVISION_CHAT`（24字）＝**`channel_type` は上記 MODIFY 必須**（[01](./01_communication.md) §2.3）、`shared_folders.scope_type`（`VARCHAR(20)`・JPA enum は `com.mannschaft.app.filesharing.FileScopeType`。04 では概念名 `StorageScopeType` と記載）に `TOURNAMENT` / `TOURNAMENT_DIVISION`＝19 字で収まり MODIFY 不要（[04](./04_file_storage.md) §2）、`workflow_requests.source_type`（`VARCHAR(30)`）に `TOURNAMENT_SUBMISSION`（21字）＝収まる（[06](./06_document_submission.md) §2.1） | 桁確認済：chat のみ要 MODIFY |
