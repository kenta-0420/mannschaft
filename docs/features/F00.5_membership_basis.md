...
### 2.5 課題#5「応援者はメンバーか」— 可視性軸での決着（2026-06-05・W0 設計統一）

§1.1 #5 で挙げた **「サポーターはメンバーか？」** の問いに一貫した答えを与える。本案件（メンバーシップ基盤）は
`memberships.role_kind ENUM('MEMBER','SUPPORTER')` で両者を**別区分のレコード**として正規化したが、
「コンテンツ可視性」の文脈での包含関係は F00 ContentVisibilityResolver の `_AND_ABOVE` 統一（W0）で次のとおり確定する:

> **応援者(SUPPORTER)は `SUPPORTERS_AND_ABOVE` には含むが `MEMBERS_AND_ABOVE` には含まない。
> 『メンバー』= `memberships.role_kind=MEMBER` 相当（RolePriority ≦ 4）。**

- `SUPPORTERS_AND_ABOVE`（SUPPORTER 以上の閾値）… ADMIN/DEPUTY_ADMIN/MEMBER/**SUPPORTER**。応援者を含む（GUEST 除外）
- `MEMBERS_AND_ABOVE`（MEMBER 以上の閾値・F00 W0 で新設）… ADMIN/DEPUTY_ADMIN/MEMBER。**応援者を除外**した内輪
- `SCOPE_AFFILIATED`（直接所属の別軸＝旧 F00 `MEMBERS_ONLY`）… 直接所属していれば SUPPORTER/GUEST も含め誰でも

すなわち「応援者はメンバーか」は文脈で答えが分かれる:
- **権限ロール（RBAC）の文脈** → SUPPORTER は MEMBER とは別区分（本案件 `role_kind`）。「メンバーではない」
- **可視性の閾値（_AND_ABOVE）の文脈** → SUPPORTER は `SUPPORTERS_AND_ABOVE` に含むが `MEMBERS_AND_ABOVE` には含まない。「『メンバー以上』には入らない」
- **直接所属（SCOPE_AFFILIATED）の文脈** → SUPPORTER も「そのスコープに所属している人」には含む

詳細は F00 `F00_content_visibility_resolver.md` §5.1.5（命名規約・別軸）/ §5.1（enum 定義）を参照。
本決着は可視性軸の正準化であり、本案件の `role_kind` 設計（MEMBER/SUPPORTER 別区分）と矛盾しない。

> **移行ステータス注記**: `MEMBERS_AND_ABOVE` は F00 W0 で確定した**目標仕様**であり、コード（F00 の `StandardVisibility` enum・各 Mapper）は後続 wave で追従する。
...
### 8.3 SUPPORTER 階層 ✅ 実装済み（統合ロール解決）

`hasRoleOrAbove("SUPPORTER")` / `hasRoleOrAbove("MEMBER")` 系のチェックを次の意味論で再定義し、**実装を完了した**:

```
旧: user_roles.role.priority <= roles[name=required].priority
新（実装済み）: 「権限ロール（user_roles 由来）」と「所属ロール（memberships.role_kind 由来 = MEMBER/SUPPORTER）」を
              UNION し、roles テーブルの priority が最小（= 最強）のロールを採用して判定する
```

#### 実装の核心: `AccessControlService.resolveEffectiveRoleName(userId, scopeId, scopeType)`

`V60.010__delete_member_supporter_from_user_roles.sql` で user_roles から MEMBER / SUPPORTER 行が削除されたため、
旧 `getRoleName` / `hasRoleOrAbove`（user_roles 専属）では **memberships 専属の MEMBER / SUPPORTER が
null（= 非メンバー）扱いになり、SCOPE_AFFILIATED / SUPPORTERS_AND_ABOVE / MEMBERS_AND_ABOVE が
誤って不可視になる過小権限バグ**が発生していた。

これを根治するため、統合ロール解決メソッド `resolveEffectiveRoleName` を新設した:

- **権限ロール**（user_roles 由来: ADMIN / DEPUTY_ADMIN / GUEST 等）と
  **所属ロール**（memberships.role_kind 由来: MEMBER / SUPPORTER）の両系統を集め、
  `roles` テーブルの priority が最小（最強）のロール名を返す。どちらにも無ければ null。
- `getRoleName` は `resolveEffectiveRoleName` に委譲（既存の呼出元は無改修）。
- `hasRoleOrAbove` は解決した有効ロールの priority で「以上」判定するよう書き換え。
- `isAdmin` / `isAdminOrAbove` / `isMember` / `isSupporter` の挙動は**不変**
  （ADMIN/DEPUTY は user_roles 由来で維持、isMember/isSupporter は memberships のまま）。
  ADMIN(user_roles) ＋ MEMBER(memberships) が併存する場合は priority 最強の ADMIN が採用されるため回帰しない。

#### F00 共通可視性基盤側の追従

`MembershipBatchQueryService.resolveDirectMembership` / `resolveOrgMembership` でも、
`UserScopeRoleSnapshot.roleByScope` / `orgMemberOf` に user_roles 由来のロールへ加えて
memberships の MEMBER / SUPPORTER を**マージ**（同一スコープに両方あれば priority 最小を優先）する。
N+1 を避けるため `MembershipRepository.findActiveRoleKindsByUserAndScopes(...)` で
全スコープ分の role_kind を 1 SQL でまとめ取りする。これにより
`AbstractContentVisibilityResolver` の D-1〜D-4（SCOPE_AFFILIATED / SUPPORTERS_AND_ABOVE /
MEMBERS_AND_ABOVE / ORGANIZATION_WIDE）が memberships 専属ユーザーで正しく可視になる。
`UserScopeRoleSnapshot` / `RolePriority` / `AbstractContentVisibilityResolver` / `RoleResolver` /
全呼出元は**無改修**。
...
| `AccessControlService` | 権限判定（isMember/isAdmin 等）＋統合ロール解決（`resolveEffectiveRoleName` で user_roles の権限ロールと memberships の所属ロールを UNION し priority 最小を採用。§8.3 実装済み）。内部で MembershipService と RoleService を組み合わせる | 両方を読むが、書き込みはしない |
...
- F00 の StandardVisibility.SCOPE_AFFILIATED 判定（旧 MEMBERS_ONLY ＝ isMemberOf）で「ユーザーが当該スコープのメンバーか」を問う場合、本キャッシュ経由で `O(1)` で判定可能となる（既存の N+1 を引き続き回避）
...
| バージョン | 日付 | 内容 |
|---|---|---|
| v1.6 | 2026-06-05 | **課題#5「応援者はメンバーか」を可視性軸で決着（§2.5 追記・F00 W0 連携）**: F00 の `_AND_ABOVE` 統一に合わせ、「SUPPORTER は `SUPPORTERS_AND_ABOVE` に含むが `MEMBERS_AND_ABOVE` には含まない。『メンバー』= role_kind=MEMBER 相当（priority ≦ 4）」と確定。RBAC/可視性閾値/直接所属の 3 文脈で答えが分かれることを明文化。`MEMBERS_AND_ABOVE` は目標仕様でコードは後続 wave 追従（移行ステータス注記付き）。 |
