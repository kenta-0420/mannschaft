...
> **ステータス**: 🟡 設計レビュー中（v0.9 ドラフト）
> **実装フェーズ**: Phase 1（軍議）
> **最終更新**: 2026-05-04
> **モジュール種別**: コア機能（メンバーシップ基盤）
> **関連ドキュメント**: F01.2 / F00 / F08.7 / F13.1 / F02.2
> **旧称**: なし（新規）
...
| **GDPR 完全対応** | 退会＝left_at セット / GDPR 削除＝user_id NULL マスキング / 30 日後物理削除バッチ。履歴は壊さない |
...
| GDPR | 退会 = left_at セット / GDPR 削除 = user_id NULL マスキング / 30 日後物理削除バッチ | 履歴は壊さない |
...
| FR-09 | GDPR 削除時、user_id を NULL マスキングし、30 日後の物理削除バッチでレコード削除 | MUST |
...
| NFR-05 | GDPR 削除完了時間 | マスキング即時、物理削除は 30 日後の日次バッチ |
...
- `joined_at` と `left_at` の逆転は CHECK 制約 `chk_memberships_period` を §5.5 のインデックス節で追加（DDL ブロックに混ぜず分離）
...
将来的に「user_roles から MEMBER/SUPPORTER 行が完全に消えた」状態が 30 日連続で確認された場合（Phase 5 末尾）、roles マスタの MEMBER / SUPPORTER 行は当面残置する（コードからの参照漏れ検知のため）。物理削除は 1 年後の Phase 8 以降で別途検討（OoS）。
...
-- joined_at と left_at の整合性
ALTER TABLE memberships
    ADD CONSTRAINT chk_memberships_period CHECK (
...
2. **Day 0〜30**: マスキング状態。memberships 行は user_id NULL で残るが、scope_type / scope_id / joined_at / left_at / role_kind 等の「組織側統計に必要な情報」は保持される
3. **Day 30**: 日次バッチ `MembershipPurgeBatch` が `WHERE gdpr_masked_at < NOW() - INTERVAL 30 DAY` を物理削除
...
| `MEMBERSHIP_GDPR_MASKED` | scopeType, scopeId, originalUserId（暗号化） |
...
- 物理削除: 30 日後の日次バッチ `MembershipPurgeBatch`
- バッチ失敗時の安全策: トランザクション内で実行し、件数が想定外（前日比 ±10% 以上）の場合は SLACK 通知 + 中断
- マスキングは **逆引き不能** とする（user_id を暗号化保存しない）。「あの人は過去に X チームに居た」を証明する手段は組織側のバックアップ（Phase 8 検討）以外存在しない設計とする
...
`POST /api/v1/memberships` のリクエスト DTO は `MembershipCreateRequest` の専用クラスを定義し、エンティティクラスに直接マッピングしない。`gdpr_masked_at` などサーバ管理項目はリクエストに含めても無視する。
...
| EC-18 | 役職 P を持ったまま user A が GDPR マスキング → 30 日後物理削除 | member_positions は ON DELETE CASCADE で自動削除される。positions マスタは無傷 |
...
graph LR
    ...
    P4 --> P5[Phase 5<br/>SUPPORTER 39 箇所書き換え<br/>BE 13 + FE 11 + Migration 15]
    P5 --> P6[Phase 6<br/>F08.7 正規化<br/>HourlyRateAggregationService]
    P5 --> P7[Phase 7<br/>11 feature docs 書き換え]
    P6 --> P8[Phase 8<br/>(将来) MEMBER/SUPPORTER role 行物理削除]
```
...
2. 差分件数が **7 日連続でゼロ** であれば Phase 4 ゴーサイン
3. API レスポンス比較バッチ `MembershipApiComparisonBatch` が `GET /teams/{id}/members` を旧実装と新実装の両方で叩き、レスポンス JSON の diff を計測。7 日連続 diff=0 で Phase 4 ゴーサイン
...
`MembershipIntegrationTest`（Spring Boot Test + Testcontainers MySQL）:
...
シナリオ 4: GDPR 削除請求 → マスキング → 30 日後物理削除のシミュレーション
```
...
本ドラフト v0.9 時点で残っている疑問・要再確認事項。**Phase 1 軍議完了時に全件決着し、ゼロ件にして v1.0 を確定する**。

| ID | 問題 | 候補 | 決着先 |
|---|---|---|---|
| OQ-1 | Phase 4 で旧 user_roles 行を削除した後、`MembershipChangedEvent` の発火元はどこに置くべきか。RoleService 残置か、MembershipService 移管か | (A) RoleService に薄い委譲メソッドを残す / (B) MembershipService に完全移管し、購読側のメソッドシグネチャは不変 | Phase 1 軍議で確定。本ドラフトは (B) 方針で書いているが、購読側 8 箇所の影響評価が未完 |
| OQ-2 | `members[].roleName` DTO の値はどう決めるか。memberships に MEMBER 行と user_roles に ADMIN 行が両方ある場合、どちらを返す？ | (A) ADMIN を優先 / (B) memberships の role_kind と user_roles の role の両方を返す（DTO 拡張） | (A) で当面進めるが、フロント側で priority 表示が必要なら DTO 拡張 |
| OQ-3 | `team_org_memberships`（チーム⇔組織所属）と本案件 memberships の命名衝突。混乱を避けるため改名が必要では？ | (A) 既存 team_org_memberships を `team_org_associations` 等に改名 / (B) 本案件の memberships は `user_memberships` に改名 / (C) 衝突を許容（ER 図で区別が明確なため） | Phase 1 軍議で確定 |
| OQ-4 | positions マスタへの seed は組織種別ごとにテンプレを用意するか？例: NPO 組織には「理事長」「監事」、企業組織には「CEO」「CFO」 | (A) 組織種別×役職テンプレを別表で管理 / (B) seed なし。各 ADMIN が手動登録 / (C) is_system フラグで予約名のみ seed | Phase 2 着手前に決定 |
| OQ-5 | role_kind に MEMBER / SUPPORTER 以外を将来追加する余地はあるか？例: PROVISIONAL_MEMBER（仮メンバー）等 | ENUM のため拡張は ALTER TABLE 必要。Phase 8 以降の検討事項 | Phase 1 で「Phase 8 以降検討」と明記して closed |
| OQ-6 | `member_positions` のスコープ越境チェックは DB 制約で表現できないが、運用で防ぎきれるか？ | (A) アプリ層検証 + 監査バッチで日次チェック / (B) Trigger を使う | (A) で進める。Trigger は MySQL 8.0 では運用負債が大きい |
| OQ-7 | team_officers / organization_officers と member_positions の統合可否 | 役員「名簿」（ユーザー紐付なし）と「役職」（ユーザー紐付あり）は別概念のため統合しない方針だが、UI で「役員一覧」が member_positions の現役行だけで足りるならテーブル削除も視野 | OoS（別案件）。本ドラフトでは「並存」方針 |
| OQ-8 | GDPR 物理削除バッチの 30 日という期間は GDPR 規約上正しいか？法務確認が必要 | (A) 30 日 / (B) 即時削除 / (C) 14 日 | 法務レビュー結果次第。本ドラフトは「30 日」前提で書いているが、変更可能 |
| OQ-9 | 二重書き込み期間中、user_roles 側の行と memberships 側の行で `created_at` が 1 ミリ秒ずれた場合、§13.5 の整合性チェックバッチは false positive を出すか | チェックロジックは PK ペアの存在のみを比較し、created_at は比較しない方針。要明文化 | Phase 2 着手時に検証スクリプトとして固める |
| OQ-10 | `GET /teams/{id}/members?role=ADMIN` のような既存クエリの挙動。ADMIN は memberships 側に存在しないため、内部実装は user_roles 側を見る必要がある。これはコード上どう統合するか | サービス層で `roleName` の値に応じて memberships か user_roles かを切り替える 1 段ディスパッチを入れる | Phase 2 で実装方針を固める |
...
| 優先度 | OQ | 理由 |
|---|---|---|
| 高 | OQ-1 | MembershipChangedEvent の発火元が定まらないと Phase 3 の Service 切替がブロック |
| 高 | OQ-3 | テーブル名衝突は Phase 2 着手前に解決必須（ALTER TABLE は後から痛い） |
| 高 | OQ-10 | Phase 2 で実装する `GET /members?role=ADMIN` の挙動仕様 |
| 中 | OQ-2 | Phase 3 で発覚すると DTO 拡張のリリースが追加で必要 |
| 中 | OQ-4 | Phase 2 リリース時の seed 内容を決める必要 |
| 中 | OQ-9 | 整合性チェックバッチの実装方針 |
| 低 | OQ-5 | Phase 8 以降の課題として保留可 |
| 低 | OQ-6 | 実装方針はほぼ確定（アプリ層検証） |
| 低 | OQ-7 | 別案件として切り出し可 |
| 法務 | OQ-8 | 別ルートで法務確認 |

### 15.2 軍議で確定したい質問の例

- Q1（OQ-1 関連）: MembershipChangedEvent は MembershipService からの発火に統一してよいか？ 既存の 7 箇所の publishEvent コードを移管する負荷を許容するか？
- Q2（OQ-2 関連）: `members[].roleName` は Phase 2-3 で「ADMIN > DEPUTY_ADMIN > MEMBER > SUPPORTER > GUEST」の優先度のいずれかを返す方式で良いか？
- Q3（OQ-3 関連）: テーブル名衝突を避けるため、本案件のメインテーブルは `memberships` のままで進めて良いか？ もし `user_memberships` に変えるなら今しかない
- Q4（OQ-4 関連）: positions マスタの seed は「組織種別ごとのテンプレ表」を作るか、空（手動登録）から始めるか？
- Q5（OQ-10 関連）: ADMIN 一覧取得は `roleName=ADMIN` で絞ったときに user_roles 直参照する分岐を入れる方針で良いか？
...
| v1.0 | 未定 | Phase 1 軍議で未解決事項 10 件を全決着後、確定版へ昇格 |
