...
- **価格 revision / band の状態に `CANCELLED` を追加（2026-09-24 御裁可）**: `billing_price_versions.status` / `billing_price_band_versions.status` の CHECK（`chk_bpv_status` / `chk_bpbv_status`）に `CANCELLED` を加える（V224 の band provision 追跡 migration で CHECK を張り直す）。単一 future 制限の生成列 `future_reservation_key` は DRAFT/PROVISIONING/PROVISION_FAILED/READY/SCHEDULED のみ非 NULL とし、`CANCELLED`（と ACTIVE/RETIRED）は NULL＝future 枠を占有しない。取り消しは修復できない PROVISION_FAILED が商品の future 枠を永久に塞ぐのを解く出口。
