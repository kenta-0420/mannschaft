...
- **band の税 snapshot（価格改定戦役）**: `billing_price_band_versions.tax_code_snapshot` は税コードマスタ `billing_tax_codes` の**内部 `code`**（例 `JP_STANDARD_10`。API・DTO の `taxCode`）を保持する。`tax_master_snapshot`（JSON）は revision create 時に band の `effectiveFrom` 時点で解決したマスタ行を `{code, displayName, rateBasisPoints, stripeTaxCode}` で固定する。**Stripe Product の `tax_code`・Product 解決キー（`billing_stripe_products.stripe_tax_code`）・reconcile の Product `tax_code` 照合には、この snapshot の `stripeTaxCode`（`txcd_...`。未設定なら null＝tax_code を付けない）を使い、内部 `code` は Stripe へ渡さない**（Provision 時にマスタを引き直さない＝決定8。マスタの `stripe_tax_code` を後から更新しても作成済み revision の税分類は変わらない）。当初 Provision が内部 `code` を Stripe の `tax_code` に渡していた欠陥（2026-09-24 Codex 検分）の根治。`stripeTaxCode` キー自体を持たない旧形式の snapshot（この修正前に作られた band）は「未設定」と区別できないため fail-closed（band を `PROVISION_FAILED`＋`TAX_SNAPSHOT_LEGACY_FORMAT`）とし、回復は revision を取り消して（`CANCELLED`）作り直す。
- **価格 revision / band の状態に `CANCELLED` を追加（2026-09-24 御裁可）**: `billing_price_versions.status` / `billing_price_band_versions.status` の CHECK（`chk_bpv_status` / `chk_bpbv_status`）に `CANCELLED` を加える（V222 の band provision 追跡 migration で CHECK を張り直す）。単一 future 制限の生成列 `future_reservation_key` は DRAFT/PROVISIONING/PROVISION_FAILED/READY/SCHEDULED のみ非 NULL とし、`CANCELLED`（と ACTIVE/RETIRED）は NULL＝future 枠を占有しない。取り消しは修復できない PROVISION_FAILED が商品の future 枠を永久に塞ぐのを解く出口。
