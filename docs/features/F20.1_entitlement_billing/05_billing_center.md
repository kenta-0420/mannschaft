...
CREATE TABLE billing_price_versions (
 ...
 CONSTRAINT chk_bpv_status CHECK(status IN ('DRAFT','PROVISIONING','PROVISION_FAILED','READY','SCHEDULED','ACTIVE','RETIRED','CANCELLED')), CONSTRAINT chk_bpv_source CHECK((creation_source='OPERATOR' AND created_by IS NOT NULL) OR creation_source='SYSTEM_BACKFILL')
...
`GET /me/billing/summary` は`PendingChange={id,contractId,status:'PENDING_PAYMENT'|'REQUIRES_ACTION'|'CREATING_SCHEDULE'|'SCHEDULED',paymentActionRequired:boolean,effectiveAt}`を返す。`GET /me/billing/contracts/{id}/payment-action` はBillingAccessGuard後にStripe最新Invoice/PaymentIntentを取得し、`requires_action`かつそのactor/scope/contractのchangeだけへfresh clientSecretを再発行する。同時にサーバーは`Set-Cookie: billing_payment_action_state=...; HttpOnly; Secure; SameSite=Lax; Path=/billing/payment-action/return`を発行し、cookie内stateのpurposeは`PAYMENT_ACTION_RETURN`、有効期限は`min(pending_update.expires_at, now+15分)`とする。**この cookie 名 `billing_payment_action_state` は、CHECKOUT_SUCCESS/CHECKOUT_CANCEL/PORTAL_RETURN が使う `billing_return_state` とは別名である**（PR6b-1 の設計判断。§5.2 E3' 参照）。BillingAccessGuard は同一scopeであっても `billing_contract_changes.created_by` と actor の不一致を404に畳む（§5.2 E5' 参照）。confirmPaymentの`return_url`は固定 `/billing/payment-action/return` で、redirect先のBackend/Nitro callback自身がcookieをserver-sideでconsumeし、HMAC→purpose/expiry→actor/Guard→nonce CAS後にclean 303 `/billing?...`へ遷移する。Stripe/issuer起点top-level GETのOrigin/Refererは要求しない。bodyを送るconsume APIは設けない。他状態は409でsecret/cookieを返さない。`invoice.payment_failed`はPaymentIntentが`requires_action`又はpending_updateが存続中ならREQUIRES_ACTIONを単調維持し、definitive decline/void/pending_update_expiredだけFAILEDにする。
...
### 5.2 upgrade（3DS/pending_update）と PLAN_CHANGE 回収の確定仕様（Billing Center PR6b-1 の設計判断 E1F〜E9'）

本節は PR6b-1（upgrade の 3DS 対応と `PLAN_CHANGE` operation の回収 worker）で確定した判断である。上位の記述と食い違う箇所は本節が優先する。

**E1F upgrade の耐久 lease と回収 worker の権限**: upgrade の `PLAN_CHANGE` operation は、決着（`APPLIED`/`FAILED`/`CANCELLED`）まで `active_billing_contract_operation_pointers` の pointer を保持したまま `CALLING_STRIPE` に留まる（§5 の「唯一の耐久 lease は pointer」という既存原則の upgrade への適用）。**回収 worker（D8 の停止窓回収と同じ仕組みを upgrade へ拡張したもの）は `PLAN_CHANGE` を「まだ Stripe 側の決着を待つ」か「決着不能と判断して `FAILED` 確定する」の2つでしか扱わず、支払いの成否そのものを確定しない**。支払いの確定（`APPLIED` への遷移）は §4 の `invoice.paid` webhook だけが行う（E9' 参照）。回収周期で Stripe の Invoice/PaymentIntent を参照できなかった（Stripe API 障害・レート制限等）場合は、その周は**検疫（`RECONCILIATION_REQUIRED` への遷移や `FAILED` 確定）をせず見送り**、次周期に再試行する。参照できない状態のまま憶測で確定させないことが目的である。

**E2' `pending_update` の照合は保存済みスナップショット基準**: `pending_update` は Stripe の独立 object ではなく、適用が完了した Subscription からは live な pending_update を取得できない。そのため照合は、当該 change 行に**保存済みの** `pending_update_expires_at` / `pending_update_target_snapshot`（§5 DDL の `billing_contract_changes` 列）と、その時点で取得した Stripe Subscription の**現在の items**（価格・数量）を突き合わせて行う。Stripe から都度 `pending_update` を再取得して比較する設計は採らない。

**E3' payment-action cookie は既存 cookie と別名**: `GET /me/billing/contracts/{id}/payment-action` が発行する cookie は **`billing_payment_action_state`** とし、CHECKOUT_SUCCESS/CHECKOUT_CANCEL/PORTAL_RETURN が使う既存 `billing_return_state` とは異なる名前にする（§7 で反映済み）。`Path=/billing/payment-action/return` に限定し、有効期限は `min(pending_update.expires_at, now+15分)` とする。**正本の従来の記述（`billing_return_state` を PAYMENT_ACTION_RETURN にも流用する版）はこの設計判断により置き換えた**（下記「書き換えた既存記述」参照）。

**E5' payment-action は actor 不一致も404**: `GET /me/billing/contracts/{id}/payment-action` の `BillingAccessGuard` は、他 scope の contract id を404にする既存規約（D6 と同型）に加えて、**同一 scope 内であっても actor が当該 change の作成者でない場合を404に畳む**。判定は `billing_contract_changes.created_by` と要求 actor の一致とする（同一 scope の別 DEPUTY_ADMIN が他人の 3DS confirm を横取りできないようにするため。403 ではなく 404 とし、対象 change の存在オラクルを残さない）。

**E6' `pending_if_incomplete` の意味論を一点に集約**: Stripe の `pending_if_incomplete` は、**支払いが即座に成功すれば upgrade をそのまま即時適用し、`pending_update` が返るのは支払いが失敗（3DS 要求含む）した場合だけ**である。同期的に成功した場合と 3DS で `requires_action` になった場合のどちらでも `invoice.paid` イベントは必ず後続で飛ぶため、**upgrade の権利発行（APPLIED 確定）を判定する唯一の確定点を `invoice.paid` 一点に一本化する**。同期成功時に別経路で先行 APPLIED 化する分岐は設けない。

**E8 upgrade 請求と契約本体の状態遷移を混ぜない**: upgrade の差額請求 invoice の成否を契約本体（`billing_contracts`）の状態遷移に流用しない。`billing_contract_changes` の `payment_failed`（`pending_update` 消滅）で契約を `PAST_DUE` にしてはならず（`PAST_DUE` は renewal invoice 専用の遷移である。§4）、`billing_contract_changes` の `paid` で契約の `current_period_end`/`valid_until` を延長してもならない（期間延長は renewal invoice の役目であり、upgrade invoice は差額請求に閉じる）。

**E9' `APPLIED` を確定できるのは webhook だけ**: upgrade `PLAN_CHANGE` change の `APPLIED` 確定は、`invoice.paid`（及びそれを再確認する `customer.subscription.pending_update_applied`）webhook のハンドラだけが行う。回収 worker、同期 API レスポンスパス、他の operation（cancel/resume/migration 等）のハンドラは `APPLIED` へ CAS してはならない。

**支払い未完了の 409 通知（マスター御裁可）**: upgrade `PENDING_PAYMENT`/`REQUIRES_ACTION` 中に別の change/cancel/downgrade を試みて409になった場合、FE はその理由が判別できる形（専用 `BillingConflictDetails.reason` 等）でエラーを受け取り、「プラン変更の支払いが完了していません」と明示する。加えて、中断した3DS確認を再開する導線（`GET /payment-action` を再度呼ぶ動線）と、`pending_update` の失効時刻（E3' の cookie 有効期限と同じ `min(pending_update.expires_at, now+15分)` を導出元とする表示用の期限）を示す。

**E10（修繕2巡目で確定）支払い待ちの再開・期限・変更先候補の表示契約**: `GET /api/v1/me/entitlements` が返す `BillingActiveContract.pendingChange` は `{changeId, status, effectiveAt, paymentActionRequired, pendingUpdateExpiresAt?}` とする。**`changeId` は必須である**——ページ再読込・再ログイン・別端末では FE のローカル状態が失われ、この投影が `GET .../changes/{changeId}/payment-action` を組み立てる唯一の手掛かりになるため、これが無いと再開（本節の再開要件）が構造的に成立しない。**利用者へ示す支払期限の根拠は `billing_contract_changes.pending_update_expires_at` だけ**とし、`effectiveAt`（変更行を作った時刻）を期限として表示してはならない（「現在時刻までに払え」と読ませる誤表示になる）。期限が存在しない検体（pending_update を伴わない同期成功等）では期限を断定せず、取り消されうることだけを伝える文言へ倒す。また **変更先候補は実際に upgrade になるもの（価格上位）だけ**を出す——見積りは変更先金額が現在以下なら必ず409（`CHANGE_CONFLICT`）で拒否するため、下位・同額を候補に並べると「選べるが必ず失敗する」項目になる（下位への変更は PR6b-2）。

**E10'（Codex 検分3巡目で訂正）候補の決定は BE 側に一本化する**: 上記の候補判定を FE がカタログの `baseMonthlyPriceJpy` で推測する実装は誤りだった。`baseMonthlyPriceJpy` は販売価格の正本ではなく、実際の upgrade 判定は現行 revision の `billing_price_band_versions.amount_including_tax` と現在の人数で行われる（§5 band 解決）。基準額が null・古い、あるいは TEAM/ORG のように人数 band で価格が変わる構成では、この推測は「有効な上位プランを隠す」か「必ず409になる候補を出す」のどちらかを起こす。したがって `BillingActiveContract` に `changeablePlanKeys: string[]`（PLAN 契約向け・ADDON は常に空配列）を追加し、BE（`BillingCurrentBandResolver`）が見積り（`BillingPlanChangePreviewService`）と**同じ band 解決**で候補を確定してから返す。FE はこの一覧をそのまま候補として使い、カタログは表示名（`displayNameKey`）の解決にのみ用いる。新規エンドポイントは追加しない（既存の権利サマリ投影に載せる）。
...
return state はサーバーだけが作る `opaque HMAC` tokenであり、payload は `{purpose:'CHECKOUT_SUCCESS'|'CHECKOUT_CANCEL'|'PORTAL_RETURN'|'PAYMENT_ACTION_RETURN',scopeKind,scopeId,tab,quoteId?,sessionId?,billingCustomerId?,iat,exp,nonce}`。HMAC secretは環境変数でkey-id付きrotationを行う。Checkout success/cancel、Portal return、3DS returnはそれぞれBackend/Nitro所有の固定同一origin route（`/billing/checkout/success`、`/billing/checkout/cancel`、`/billing/portal/return`、`/billing/payment-action/return`）でHMAC/期限/purposeを検証する。Stripe/issuer起点のtop-level GETでOrigin/Referer同一originを要求しない。未認証ならnonceを消費せずsigned stateをHttpOnly/Secure/SameSite=Lax短命cookieへ保存しloginへ303する。**cookie名は purpose によって2系統に分かれる**: CHECKOUT_SUCCESS/CHECKOUT_CANCEL/PORTAL_RETURNの3purposeは`billing_return_state`（Stripe URLへ埋め込まれ、query paramとの併存を前提とする）、PAYMENT_ACTION_RETURNだけは`billing_payment_action_state`（`GET /payment-action`のレスポンスでだけ発行され、Stripe URLへは埋め込まない）である。両者は名前が異なるため同一ブラウザに同時に存在しても衝突しない。このときloginの`next`は**その callback 自身のパス**（`/billing/checkout/success` 等）とし、再認証後にcallbackが必ずもう一度呼ばれるようにする（`next=/billing`ではnonceが消費されず復帰導線が失われる）。4入口すべてがcookieからstateを受け取れる。query paramとcookieが両方来た場合は**query paramを優先**し（Stripe/issuerからの直接復帰が正、cookieは退避経路）、`PAYMENT_ACTION_RETURN`だけはquery paramを一切読まない。cookieが存在した要求では消費成否に関わらずcookieを失効させる。再認証後は同じBackend/Nitro callbackが**bodyを読まず**該当するcookie名をserver-sideで復号・削除し、HMAC→purpose/expiry→actor一致→BillingAccessGuard→nonce CASの順で消費してclean 303 `Location: /billing?...`を返す。別のconsume APIは設けない。3DS後loginでも`PAYMENT_ACTION_RETURN`が同じ手順で一回だけ消費される。Stripe URL作成成功後のDB更新失敗はmetadataのsession/customer refで照合キューが回収する。不正/期限切れ/nonce再利用はUSER hubのgeneric errorへ安全遷移する。token `exp` はCheckoutではSession expiry+15分（発行から最大24時間）、Portalでは`issuedAt+30分`、PAYMENT_ACTION_RETURNは`min(pending_update.expires_at, now+15分)`である。**Checkout/Portal stateだけ**をサーバーがStripe URLへ埋め込み、PAYMENT_ACTION_RETURNは`GET /payment-action`の`Set-Cookie`（HttpOnly、`billing_payment_action_state`、`Path=/billing/payment-action/return`）だけで発行する。後者のtokenはJSON body・URL・JavaScriptへ公開しない。期限又は消費済み行は7日後cleanupとする。
...
| BC-18 | price revision/Provision IT: SYSTEM_ADMINが不変catalogRevision/revisionNoとlockVersionを持つDRAFT revision+bandsを作成し、範囲/有効期間overlapを拒否する。Provisionの正常、Stripe失敗、部分成功、retry、同時activate CAS、全band READY後だけfuture BをSCHEDULED、開始時に現行AをRETIRED/BをACTIVE、次future Cも予約できることを観測する。公開pricing/quote共用now selectorがdue SCHEDULED BとAをrow lockして冪等lazy-promote→再読込し、同時二重promoteでも一回だけA→RETIRED/B→ACTIVEとなること、23:55→00:00とscheduler遅延、A invoiceがB開始後に先着する順不同でも保存FK/Stripe ref/snapshotでRETIRED Aを受理することを観測する。既存契約は次周期だけ新bandを選ぶ。FlywayがStripe APIを呼ばず、既存subscription itemのreconcile不一致は販売停止。**【価格改定戦役（price-revisions）第6版・注記】** 上記「次future Cも予約できる」（＝同一 `(productKind, productKey, scopeKind)` に futureを複数同時に予約できること）は、**本戦役では実装しない**。理由: 検分5巡目でB・Cが同時にfutureとして併存する状態を前提とする重大欠陥が2件（①Bを未昇格のままRETIREDにすると価格が0件になる瞬間が生じる、②CをB開始前に予約するとBが永久にSCHEDULEDで残る）指摘され、futureを「同時に1本まで」に制限することでこの併存自体を構造的に無くし根治した。代償として、この行が元々求めていた「次future Cも予約できる」（＝A→B→Cの多重予約）は満たさない。本戦役では同一`(productKind, productKey, scopeKind)`の future（DRAFT/PROVISIONING/PROVISION_FAILED/READY/SCHEDULEDのいずれか。PROVISIONING・PROVISION_FAILEDはDRAFT→READYの途中状態として含める＝2026-09-24御裁可）。修復できない PROVISION_FAILED で枠が永久に塞がらないよう、DRAFT/READY/PROVISION_FAILED は `POST /price-revisions/{id}/cancel` で `CANCELLED`（future 枠を占有しない状態）へ取り消せる（2026-09-24御裁可）は常に高々1本であり、既存futureがある間の2本目のcreateは`effectiveFrom`の値に関わらず409になる。複数future予約（A→B→C）は**後続の戦役で扱う**。詳細は `.claude/campaigns/price-rev-plan-v3.md`（第6版）を参照 |
