...
> `/api/v1/residence-status/...` として設計されたが、現実装は **組織スコープ**
> `/api/v1/organizations/{orgId}/residence-status/...` に統一されている。
> 該当 Controller: `AnnualReviewController` (3 件) /
> `MonitoringCommitteeVisitController` (2 件) / `ResidenceStatusController` (2 件) /
> `AnnualReviewResponseController` (1 件) / `OrgWideSafetyCheckController` (1 件)。
...
{
  "organizationId": 1,
  "triggerReason": "震度5地震発生のため組合員全戸の安否を確認"
}
```
...
`POST /api/v1/organizations/{orgId}/residence-status/org-wide-safety-checks` は ADMIN のみ。組織全体への発動は管理組合判断（理事長）に限定。
...
- POST /api/v1/organizations/{orgId}/residence-status/org-wide-safety-checks（F03.6 連携）
- ダッシュボード Redis キャッシュ
- E2E テスト
- 🟢 ステータス更新
