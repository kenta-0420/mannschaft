...
| メソッド | パス | 認可 | 用途 |
|---|---|---|---|
| POST | `/api/v1/residence-status/annual-reviews` | ADMIN | 年次更新キャンペーン起動 |
| GET | `/api/v1/residence-status/annual-reviews` | ADMIN/DEPUTY_ADMIN | キャンペーン一覧 |
| GET | `/api/v1/residence-status/annual-reviews/{id}` | ADMIN/DEPUTY_ADMIN | キャンペーン詳細（回答状況） |
| POST | `/api/v1/residence-status/annual-reviews/{id}/close` | ADMIN | キャンペーン手動クローズ |
| GET | `/api/v1/residence-status/annual-reviews/{id}/responses` | ADMIN/DEPUTY_ADMIN | 全回答一覧 |
| GET | `/api/v1/residence-status/annual-reviews/my` | MEMBER | 自分の回答対象キャンペーン取得 |
| PUT | `/api/v1/residence-status/annual-reviews/{id}/responses/me` | MEMBER | 自分の回答送信 |
| GET | `/api/v1/residence-status/activity-snapshots/{residentId}` | ADMIN/WATCHER | 推定スコア + アクティビティ集計（本人非開示）|
| POST | `/api/v1/residence-status/monitoring-visits` | WATCHER | 訪問記録の入力 |
| GET | `/api/v1/residence-status/monitoring-visits` | WATCHER/ADMIN | 訪問記録一覧 |
| PUT | `/api/v1/residence-status/monitoring-visits/{id}` | WATCHER | 訪問記録の更新（24h 以内のみ） |
| POST | `/api/v1/residence-status/org-wide-safety-checks` | ADMIN | 管理組合横展開安否確認の発動 |
| GET | `/api/v1/residence-status/dashboard` | ADMIN/DEPUTY_ADMIN | 居住実態ダッシュボード統計 |

### 6.2 主要エンドポイント仕様（抜粋）

#### POST `/api/v1/residence-status/annual-reviews`
...
#### POST `/api/v1/residence-status/org-wide-safety-checks`
...
#### GET `/api/v1/residence-status/activity-snapshots/{residentId}`
...
- 全 5 UUIDv7 テーブル DDL（V9.030〜V9.034）
- F03.6 `source_type=ORG_WIDE` 値追加
- F03.12 relationship enum 追加
- F04.10 `purpose_tag=MONITORING` + `WATCHER` ロール追加（V9.035）
- F14.2 `COVENANT_BLOCK` フィールド型対応
- `MANAGE_RESIDENCE_STATUS` permission 追加（V9.036）
...
| バージョン | 内容 | 状態 |
|---|---|---|
| V9.103 | `resident_registry` 居住実態関連 5 カラム拡張（ALTER TABLE。`occupancy_status` / `last_annual_review_at` / `annual_review_due_at` / `is_secondary_home` / `age_estimated`） | ✅ S1-C 完了 (2026-05-12) |
| V9.030 | `resident_activity_snapshots` テーブル新設 | 仮予約 |
| V9.031 | `annual_reviews` テーブル新設 | 仮予約 |
| V9.032 | `annual_review_responses` テーブル新設 | 仮予約 |
| V9.033 | `monitoring_committee_visits` テーブル新設 | 仮予約 |
| V9.034 | `org_wide_safety_checks` テーブル新設 | 仮予約 |
| V9.035 | F03.6 `safety_checks.source_type` に `ORG_WIDE` / F03.12 relationship enum 拡張 / F04.10 `purpose_tag=MONITORING` + `committee_members.role=WATCHER` | 仮予約 |
| V9.036 | `MANAGE_RESIDENCE_STATUS` permission 追加 | 仮予約 |

> V9.030〜V9.036 は **仮予約**。実装着手時に Phase 9 系の連番衝突を確認する。S1-C では `resident_registry` への居住実態拡張カラム（5 本）を新たに V9.103 として追加した。設計書 §5 にカラム仕様の正式記載がなかったため、`F09.1_resident_registry.md` §3 と同等の精度（VARCHAR(30) / BIGINT UNSIGNED / SMALLINT UNSIGNED）で実装している。
...
| Flyway 連番確定 | 実装着手時に Phase 9 系の予約状況を再確認のうえ確定（仮予約 V9.030〜V9.036） |
