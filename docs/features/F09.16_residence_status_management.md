...
| バージョン | 内容 |
|---|---|
| V9.030 | `resident_activity_snapshots` テーブル新設 |
| V9.031 | `annual_reviews` テーブル新設 |
| V9.032 | `annual_review_responses` テーブル新設 |
| V9.033 | `monitoring_committee_visits` テーブル新設 |
| V9.034 | `org_wide_safety_checks` テーブル新設 |
| V9.035 | F03.6 `safety_checks.source_type` に `ORG_WIDE` / F03.12 relationship enum 拡張 / F04.10 `purpose_tag=MONITORING` + `committee_members.role=WATCHER` |
| V9.036 | `MANAGE_RESIDENCE_STATUS` permission 追加 |

> V9.030〜V9.036 は **仮予約**。実装着手時に Phase 9 系の連番衝突を確認する。
