...
| **見守り委員会**（マンション管理組合の高齢区分所有者見守り・F09.16 連携・WATCHER ロール任命）| 中 |

### 委員会テンプレート: 見守り委員会（2026-05-12 追加）

`committees.purpose_tag = MONITORING` で識別される。マンション管理組合での高齢区分所有者の見守り活動を担う。

| 項目 | 値 |
|---|---|
| `name` | 例: 「2026年度 見守り委員会」 |
| `purpose_tag` | `MONITORING` |
| 委員ロール | CHAIR（理事長等）+ WATCHER 複数名（民生委員・自治会有志等） |
| 訪問対象 | MONITORING_CONSENT 誓約済みの区分所有者のみ |
| 連携機能 | F09.16 `monitoring_committee_visits`（訪問記録）/ F09.15 `succession_covenants`（同意確認）|

`purpose_tag` カラム / `committee_members.role` ENUM の値追加は **Flyway V9.035 で実施予定**（F09.16 側で採番）。
...
| WATCHER（見守り委員）| **F09.16 連携で追加（2026-05-12）**。`purpose_tag = MONITORING` の委員会専用ロール。担当区分所有者の訪問記録入力（`monitoring_committee_visits`）・推定スコア閲覧（自分の担当者のみ）・委員会内コンテンツ閲覧。**MONITORING_CONSENT 同意済みの区分所有者にのみ訪問可**。`committee_members.role` ENUM 拡張（Flyway V9.035 予定）|
...
| `POST /api/v1/committees/{committeeId}/surveys` | `scope_type` を path セグメント化（F05.4 §4 スコープ統一に合わせる。Stage 3 5-γ 2026-05-17 で path 体系整流済）。Controller 側で `scopeType=committees` を validation 拡張する必要あり |
| `POST /api/v1/organizations/{orgId}/circulations` | `committee_id` フィールド受容（旧: `/api/v1/circulation-documents`、F05.2 §4.1 で URL 体系整流済 2026-05-17）|
| `POST /api/v1/confirmable-notifications` | `scope_type: 'COMMITTEE'` 受容 |
| `POST /api/v1/activities` | `scope_type: 'COMMITTEE'` 受容（F06.4 活動記録作成 API を共有。旧 `/api/v1/activity-records` 表記は F06.4 開発初期の旧名称で、最終 URL は `/api/v1/activities` に決定） |
...
🔵 Phase 2 未着工。`GET /api/v1/admin/dashboard-stats` エンドポイントは F10.1 管理ダッシュボード Phase 2 で実装予定。F04.10 委員会機能が完成し次第、当該エンドポイントに `committee` セクションを追加する。
