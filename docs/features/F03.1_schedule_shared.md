...
| `team_breakdown_enabled` | BOOLEAN | NO | FALSE | 出欠集計をチーム別内訳（by_team）でも収集・表示するか（作成時トグル。既定 FALSE = 従来挙動＝全体集計のみ。組織スコープ＋TRUE のときのみ by_team を算出）(V117.001) |
...
4. schedule_attendances を取得し、user_id から memberships（scope_type='TEAM' AND left_at IS NULL）でチームを解決
5. チームごとに attending / partial / absent / undecided を集計:
   - チームに所属しない組織直接メンバー（memberships に scope_type='TEAM' のアクティブエントリなし）は team_id = null として集計
