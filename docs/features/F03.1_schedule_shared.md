...
チーム・組織の練習・試合・イベント等のスケジュールを管理し、出欠確認・アンケートを一体化した出席管理機能を提供する。繰り返しスケジュール（週次練習等）をサポートし、「この回のみ変更」「この回以降を変更」「全回変更」の3段階編集に対応する。MEMBER はデフォルトで `MANAGE_SCHEDULES` 権限を持ちスケジュールの作成・編集が可能。ユーザーは任意でチーム・組織のスケジュールを個人の Google カレンダーへ自動同期できる。試合スケジュールは相手チーム・組織を招待してカレンダーを共有できる（クロスチームスケジュール）。出欠アンケートにはリマインダーを設定でき、未回答メンバーに期日前通知を送れる。組織スケジュールの出欠は所属チーム別に集計して参照できる。スケジュール終了後には実績日時・移動距離・結果サマリーを事後入力でき、経費精算機能（F13.x）やブログ活動記録（F06.1）との連携データソースとして機能する。
...
| `distance_km` | DECIMAL(8,2) UNSIGNED | YES | NULL | 移動距離（km）。MATCH・PRACTICE等の移動を伴うイベントでの移動距離を記録。経費精算・活動記録との連携用。小数点2桁精度（0.00〜999999.99 km） |
| `distance_notes` | VARCHAR(500) | YES | NULL | 移動距離の補足メモ（例: 「電車＋徒歩」「高速道路利用」「往復」）。経費申請時の補足情報として使用 |
| `actual_start_at` | DATETIME | YES | NULL | 実際の開始日時（予定日時との乖離記録用。事後入力。NULL = 未記録）|
| `actual_end_at` | DATETIME | YES | NULL | 実際の終了日時（同上）|
| `result_summary` | VARCHAR(1000) | YES | NULL | 結果サマリー（試合スコア・練習内容・イベント結果等の事後メモ。NULL = 未記録）|
...
INDEX idx_sch_distance (team_id, distance_km)             -- 移動距離集計用（経費精算連携）
```
...
- `distance_km` は UNSIGNED のため負値不可。0.00 = 移動なし（在宅・オンライン等）、NULL = 未入力
- `actual_start_at` / `actual_end_at` は `start_at` / `end_at` より前後の日時も許容（早退・延長対応）
- `result_summary` は Markdown 非対応の plain text。フロントエンドでの改行は `\n` で表現
...
```json
{
  ...
  "distanceKm": 12.5,
  "distanceNotes": "電車＋徒歩",
  "actualStartAt": "2026-04-17T18:10:00",
  "actualEndAt": "2026-04-17T20:45:00",
  "resultSummary": "3-1 で勝利。次回は守備の連携を強化する。"
}
```
...
> | フィールド | 型 | 必須 | 説明 |
> |-----------|---|------|------|
> | `distanceKm` | number | 任意 | 移動距離（km）。0〜999999.99。null で削除 |
> | `distanceNotes` | string | 任意 | 移動メモ。最大500文字 |
> | `actualStartAt` | string (ISO 8601) | 任意 | 実際の開始日時 |
> | `actualEndAt` | string (ISO 8601) | 任意 | 実際の終了日時 |
> | `resultSummary` | string | 任意 | 結果サマリー。最大1000文字 |
...
-- F03.12 連携（移動距離・実績カラム追加 / スケジュールメディアテーブル作成）
V13.014__alter_schedules_add_distance_and_result.sql  -- 移動距離・実績日時・結果サマリーカラム追加
```

```sql
-- V13.014__alter_schedules_add_distance_and_result.sql
ALTER TABLE schedules
    ADD COLUMN distance_km       DECIMAL(8,2) UNSIGNED NULL   COMMENT '移動距離(km)。経費精算・活動記録連携用' AFTER location,
    ADD COLUMN distance_notes    VARCHAR(500)          NULL   COMMENT '移動距離の補足メモ' AFTER distance_km,
    ADD COLUMN actual_start_at   DATETIME              NULL   COMMENT '実際の開始日時（事後入力）' AFTER updated_at,
    ADD COLUMN actual_end_at     DATETIME              NULL   COMMENT '実際の終了日時（事後入力）' AFTER actual_start_at,
    ADD COLUMN result_summary    VARCHAR(1000)         NULL   COMMENT '結果サマリー（試合スコア等）' AFTER actual_end_at,
    ADD INDEX  idx_sch_distance  (team_id, distance_km);
```
...
- V13.014 は V3.007（schedules）完了後に実行。V13.015（schedule_media_uploads 作成）の前に適用すること（F03.12_schedule_media.md 参照）
...
- [x] ~~移動距離の単位を km に統一するか、m も許容するか~~ → km 単位・DECIMAL(8,2) で確定（精度0.01km = 10m）
- [ ] F13.x 経費精算機能実装時に schedules.distance_km を自動読み込みするかどうか（Phase 13 設計時に判断）
- [ ] actual_start_at / actual_end_at の入力をカレンダーUIに統合するか、事後記録専用UIにするか（F03.1 Phase 3 実装時に判断）
...
| 日付 | 変更内容 |
|------|---------|
| 2026-04-17 | 移動距離欄（distance_km / distance_notes）・実績日時（actual_start_at / actual_end_at）・結果サマリー（result_summary）追加。F03.12 連携コメント追記。Flyway V13.014 追加。未解決事項3件追加 |
