...
> **ステータス**: 🟢 設計完了（F01.7 `CUSTOM_TEMPLATE` 拡張は 🟡 設計中）
> **実装フェーズ**: Phase 3
> **最終更新**: 2026-06-02
> **関連ドキュメント**: 個人スケジュール → `F03.2_schedule_personal.md` / 外部連携 → `F08_external_integration.md` / F01.7 カスタム公開範囲テンプレート / アンケート → `F05.4_survey_vote.md` / スケジュール変更フィード → `F03.18_schedule_activity_feed.md`

---

## 1. 概要

チーム・組織の練習・試合・イベント等のスケジュールを管理し、出欠確認・アンケートを一体化した出席管理機能を提供する。繰り返しスケジュール（週次練習等）をサポートし、「この回のみ変更」「この回以降を変更」「全回変更」の3段階編集に対応する。MEMBER の `MANAGE_SCHEDULES` は初期値 OFF。対象チーム・組織の ADMIN が許可した場合にスケジュールの作成・編集が可能。ユーザーは任意でチーム・組織のスケジュールを個人の Google カレンダーへ自動同期できる。試合スケジュールは相手チーム・組織を招待してカレンダーを共有できる（クロスチームスケジュール）。出欠アンケートにはリマインダーを設定でき、未回答メンバーに期日前通知を送れる。組織スケジュールの出欠は、作成時トグル `team_breakdown_enabled`（既定 OFF = 従来挙動＝全体集計のみ）を ON にした場合に限り、所属チーム別の内訳（by_team）でも集計して参照できる。配下の複数チームに所属する回答者は所属全チームへ 1 票ずつ計上するため、by_team 各チームの合計（のべ人数）は全体 total（実人数・DISTINCT で別建て）以上になりうる。
...
| MEMBER | `MANAGE_SCHEDULES` は初期値 OFF。ADMIN が許可した場合に作成・編集が可能。他者のスケジュール削除は `DELETE_OTHERS_CONTENT` 権限が必要。`min_response_role IN ('SUPPORTER+', 'MEMBER+')` のスケジュールに出欠回答可・出欠集計サマリー（件数のみ）参照可 |
...
| `schedule_attendance_reminders` | 出欠リマインダー設定（未回答者への期日前通知）。機能55でRELATIVE/ABSOLUTE両対応 | なし |
| `schedule_cross_refs` | クロスチーム・組織スケジュール招待（試合マッチング等）| なし |
| `schedule_scheduled_tasks` | 予約作成タスク（機能55）。アンケート/出欠募集を指定時刻に遅延 materialize する | あり |
...
| `include_supporters` | BOOLEAN | NO | FALSE | 出欠確認の配信母集団にサポーター（応援者）を含めるか（per-schedule トグル。組織配信時は既定除外）(V101.002) |
| `team_breakdown_enabled` | BOOLEAN | NO | FALSE | 出欠集計をチーム別内訳（by_team）でも収集・表示するか（作成時トグル。既定 FALSE = 従来挙動＝全体集計のみ。組織スコープ＋TRUE のときのみ by_team を算出）(V119.001) |
...
| `distance_km` | DECIMAL(8,2) UNSIGNED | YES | NULL | 移動距離（km）。往復の場合は往復合計 |
| `distance_notes` | VARCHAR(255) | YES | NULL | 移動手段・経路のメモ（例: 「電車+徒歩」「自家用車」）|
| `actual_start_at` | DATETIME | YES | NULL | 実績開始日時（事後記録用）|
| `actual_end_at` | DATETIME | YES | NULL | 実績終了日時（事後記録用）|
| `result_summary` | TEXT | YES | NULL | 予定の結果サマリー（試合結果・練習内容メモなど）|
| `allow_proxy_attendance` | BOOLEAN | NO | FALSE | 代理出席を許可するか (F03.10) |
| `is_proxy_auto_accept` | BOOLEAN | NO | FALSE | 代理人の承認不要か (F03.10) |
...
**§DB 実装状況（実装と本書の差分・正直な棚卸し）**

- **`min_view_role` の型・既定値**: 本書は `ENUM('ANYONE', 'SUPPORTER+', 'MEMBER+', 'ADMIN_ONLY')` / 既定 `'MEMBER+'` と表記しているが、実際のカラム定義（`V3.040__create_schedules_table.sql`）は `VARCHAR(20) NOT NULL DEFAULT 'MEMBER_PLUS'` であり、値も Java 側 `MinViewRole` enum（`ANYONE` / `SUPPORTER_PLUS` / `MEMBER_PLUS` / `ADMIN_ONLY`）に合わせて `_PLUS` サフィックス表記が正。本書中の `'SUPPORTER+'` `'MEMBER+'` という表記は可読性のための略記であり、実際の格納値ではない。
- **`min_view_role` は UI から設定不能**: バックエンドは `min_view_role` を作成・更新の両 API で受け付け、閲覧判定にも生きた軸として使用する（CMP-017b で根治）が、フロントエンド（`frontend/app`）側には `minViewRole` を編集する UI が存在しない（生成型 `frontend/app/types/generated/index.ts` にのみ現れ、Vue コンポーネントからの参照はゼロ）。そのため実運用上は常に既定値 `MEMBER_PLUS` のまま作成され、チーム/組織の `default_schedule_min_view_role` 継承や `ANYONE` / `SUPPORTER_PLUS` / `ADMIN_ONLY` への変更は API を直接叩かない限り行えない。UI 対応は起票済みの別課題として扱う。
...
- **【重要】配信対象（distribution）と可視性評価（visibility）は独立した概念**:
  - **配信（distribution）**: 出欠募集・通知の送信対象を決定する。「誰にレコードを作るか」の問題。
  - **可視性評価（visibility）**: スケジュールの閲覧権限を決定する。「誰が見られるか」の問題。
  - 組織スコープにおいて、この2つは**混同しやすいが別々に評価する**こと（設計の穴の温床になるため）。
- **出欠配信の対象範囲（組織スコープ・Phase A 以降）**:
  - 組織スコープのスケジュールで `attendance_required = TRUE` の場合、配信対象は「組織直属メンバー ∪ 配下の参加チーム（`team_organization_memberships.status = 'ACTIVE'`）に所属するメンバー」の重複除外集合とする（Phase A: #1598 実装済み）。
  - サポーター（SUPPORTER ロール）は配信母集団から**既定除外**する。`schedules.include_supporters = TRUE` を設定した場合のみ含める（per-schedule トグル）。
  - 可視性評価（view）は引き続き「スケジュールを所有するチーム/組織への**直接所属ロール**」で行う（配信対象の拡大が可視性の緩和を意味しない）。
...
- **SUPPORTER と `visibility = 'ORGANIZATION'` の組み合わせ**:
  ...
  - `visibility = 'ORGANIZATION'` は**チームスコープのスケジュールにのみ有効**（組織スコープのスケジュールでは `visibility` の値にかかわらず可視性評価は組織への直接所属ロールで行う）
  - **⚠️ 既知の矛盾（未解消・起票済み）**: 上記「親グループのロールは子グループのスケジュールに継承しない」という記述は、`ScheduleVisibilityResolver`（F00 可視性基盤）フェーズ M2 で実装された下向き再帰（`StandardVisibility#ORGANIZATION_AND_DESCENDANTS`。組織スコープ・`visibility = 'ORGANIZATION'` のスケジュールを、配下の子孫組織に所属するメンバーにも閲覧可能にする挙動）と矛盾する。どちらを正とするか未確定のため、本書はどちらか一方の記述に寄せず矛盾を明記するに留める。起票済み: `docs/task-list.md` CMP-027（フェーズM2の下向き再帰が本番で届いていない疑い）。
...
出欠確認が必要なスケジュールに対し、未回答メンバーへの通知を送るためのリマインダー設定テーブル。機能55 第一陣（V72.002）で相対指定（開始N分前）に対応し、`reminder_kind` で RELATIVE/ABSOLUTE を切り替えられるようになった。

| カラム名 | 型 | NULL | デフォルト | 説明 |
|---------|---|------|-----------|------|
| `id` | BIGINT UNSIGNED | NO | AUTO_INCREMENT | PK |
| `schedule_id` | BIGINT UNSIGNED | NO | — | FK → schedules（ON DELETE CASCADE）|
| `remind_at` | DATETIME | **YES** | NULL | 絶対日時（`ABSOLUTE` 時に使用。`RELATIVE` 時は未確定のため NULL 可）|
| `remind_before_minutes` | INT | YES | NULL | 相対指定：開始N分前（`RELATIVE` 時に使用）|
| `reminder_kind` | VARCHAR(10) | NO | 'ABSOLUTE' | 指定方式。`RELATIVE`（相対）/ `ABSOLUTE`（絶対）。既定は ABSOLUTE（後方互換）|
| `is_sent` | BOOLEAN | NO | FALSE | 送信済みフラグ |
| `sent_at` | DATETIME | YES | NULL | 実際の送信完了日時 |
| `created_at` | DATETIME | NO | CURRENT_TIMESTAMP | |

> V72.002 での変更点: `remind_at` を NOT NULL → NULL 可に変更、`remind_before_minutes` / `reminder_kind` を追加。
...
**実効時刻の解決ロジック**
- `RELATIVE`: `effectiveRemindAt = schedules.start_at - remind_before_minutes 分`
- `ABSOLUTE`: `effectiveRemindAt = remind_at`
- `ScheduleReminderBatchService` が 1 分間隔で実効時刻 ≤ NOW() かつ未送信のリマインダーを走査して通知を発火する。通知種別は `SCHEDULE_REMINDER`。

**制約・備考**
- 1スケジュールにつき最大5件（アプリ層でバリデーション）
- `attendance_required = FALSE` のスケジュールへの設定は不可（アプリ層でバリデーション）
- `ABSOLUTE` の場合、`remind_at` は `attendance_deadline` より前でなければならない（deadline が NULL の場合は `start_at` より前）
- `RELATIVE` の場合、`remind_before_minutes > 0` であること（アプリ層バリデーション）
...
#### `schedule_scheduled_tasks`（機能55 予約作成タスク）

予定作成時に「指定日時になったらアンケートを生成・公開する」「指定日時になったら出欠募集を開始する」という予約を保持するテーブル。`scheduled_at` 到来時に `ScheduleScheduledTaskBatchService`（1分間隔バッチ）が `payload_json` のスナップショットを元に実体を materialize する。

主キーは UUIDv7（CLAUDE.md 原則6）。`schedule_id` / `scope_id` / `created_by` はクロスドメイン論理参照のため FK なし・index のみ（原則1）。`organization_id` をテナントキーとして保持する（原則7）。

| カラム名 | 型 | NULL | デフォルト | 説明 |
|---------|---|------|-----------|------|
| `id` | BINARY(16) | NO | — | PK（UUIDv7）|
| `schedule_id` | BIGINT | NO | — | 親予定 schedules.id（FK制約なし・論理参照）|
| `organization_id` | BIGINT | NO | — | テナントキー。team予定なら所属組織のid（原則7）|
| `scope_type` | VARCHAR(20) | NO | — | `TEAM` / `ORGANIZATION` |
| `scope_id` | BIGINT | NO | — | スコープ実体ID（team_id または organization_id）|
| `task_type` | VARCHAR(20) | NO | — | `SURVEY`（アンケート生成）/ `ATTENDANCE`（出欠募集開始）|
| `scheduled_at` | DATETIME | NO | — | この時刻に materialize する |
| `status` | VARCHAR(20) | NO | 'PENDING' | `PENDING`（待機中）/ `CREATED`（materialize済み）/ `CANCELLED`（取消）/ `FAILED`（試行打ち止め）|
| `payload_json` | JSON | NO | — | アンケート定義/出欠設定のスナップショット（materialize 時に deserialize）|
| `materialized_entity_id` | BIGINT | YES | NULL | 生成後の実体id（surveys.id / schedule_attendances の scheduleId 等）|
| `attempt_count` | INT | NO | 0 | materialize 試行回数（上限5回で FAILED 確定）|
| `last_error` | VARCHAR(1000) | YES | NULL | 最終失敗理由 |
| `created_by` | BIGINT | YES | NULL | 作成者 users.id（FK制約なし）|
...
```sql
PRIMARY KEY (id)
INDEX idx_sst_status_scheduled (status, scheduled_at)  -- バッチ: PENDING かつ scheduledAt 到来済みを取得
INDEX idx_sst_schedule (schedule_id)                   -- 予定キャンセル時の一括取消用
INDEX idx_sst_org_deleted (organization_id, deleted_at) -- テナントスコープ取得用
```

**materialize バッチ仕様**

`ScheduleScheduledTaskBatchService.runBatch()` が 1 分間隔（`@Scheduled fixedDelay=60秒`）で実行される。ShedLock（`scheduleScheduledTaskBatch`）により複数インスタンス起動時の二重実行を防ぐ。

- `status = PENDING` かつ `scheduled_at <= NOW()` かつ `deleted_at IS NULL` のタスクを全件取得
- 各タスクを **独立したトランザクション**（`@Transactional(REQUIRES_NEW)`）で materialize する（1件の失敗が他件をロールバックしない）
- `SURVEY` タスク: `payload_json` を `CreateSurveyRequest` にデシリアライズ → `SurveyService.createSurvey()` → `SurveyService.publishSurvey()` → 集計可能なアンケートとして生成・公開される（`F05.4_survey_vote.md` 相互参照）
- `ATTENDANCE` タスク: `ScheduleAttendanceService.openAttendanceSolicitation(scheduleId)` を呼び出し、出欠レコードの生成と募集通知の配信を開始する
- 失敗時: `attempt_count` を加算。上限（`MAX_ATTEMPTS = 5`）到達で `status = FAILED`（打ち止め）、未達なら `status = PENDING` のまま次回バッチで再試行
- materialize 成功時: `status = CREATED`、`materialized_entity_id` に生成実体の ID を記録

**出欠募集の即時/予約切り替え**

`ScheduleCreatedEvent` を `ScheduleAttendanceSolicitationEventListener`（AFTER_COMMIT + @Async）が受信し、即時出欠募集を開始する。ただし当該予定に `status=PENDING` の `ATTENDANCE` 予約タスクが存在する場合は、バッチ委譲（即時募集スキップ）とする（二重募集防止）。

**制約・備考**
- PENDING のタスクのみ取消可能（→ CANCELLED）。CREATED/FAILED/CANCELLED は取消不可（409）
- スコープ不一致（パスの scopeType/scopeId と task の scopeType/scopeId が異なる）は IDOR 対策で 404 として扱う
...
schedules (1) ──── (N) schedule_scheduled_tasks    ※ 論理参照（FK なし・機能55）
...
### 3.X F01.7 `CUSTOM_TEMPLATE` visibility 拡張（2026-04-20 追記 / Phase 拡張）

F01.7 カスタム公開範囲テンプレート導入に伴い、`schedules.visibility` ENUM を拡張する。F03.11 と並ぶ F01.7 の「第一次適用対象」の一つ。

```sql
-- 3.X: F01.7 連携マイグレーション（F01.7 Phase 3 で追加）
ALTER TABLE schedules
  MODIFY COLUMN visibility ENUM('MEMBERS_ONLY','ORGANIZATION','CUSTOM_TEMPLATE') NOT NULL DEFAULT 'MEMBERS_ONLY';

ALTER TABLE schedules
  ADD COLUMN visibility_template_id BIGINT UNSIGNED NULL AFTER visibility,
  ADD CONSTRAINT fk_sch_visibility_template
    FOREIGN KEY (visibility_template_id) REFERENCES visibility_templates(id) ON DELETE SET NULL,
  ADD INDEX idx_sch_visibility_template (visibility_template_id);

-- DB レベルの整合性 CHECK 制約（アプリ層検証との二重保護）
ALTER TABLE schedules
  ADD CONSTRAINT chk_sch_custom_template_consistency CHECK (
    (visibility = 'CUSTOM_TEMPLATE' AND visibility_template_id IS NOT NULL)
    OR (visibility != 'CUSTOM_TEMPLATE' AND visibility_template_id IS NULL)
  ),
  ADD CONSTRAINT chk_sch_custom_template_scope CHECK (
    -- 個人スコープ（user_id IS NOT NULL）では CUSTOM_TEMPLATE 禁止
    (user_id IS NULL) OR (visibility != 'CUSTOM_TEMPLATE')
  );
```

**ビジネスルール**:
- 上記 CHECK 制約で DB レベル強制。Service 層では 400 エラーで事前検証しユーザーに分かりやすいエラー返却
- 作成/編集時、`visibility_template_id` で指定されたテンプレートが **作成者所有 or システムプリセット** であることを検証（他人のテンプレートは 400 / `SCHEDULE_VISIBILITY_TEMPLATE_INVALID_OWNER`）
- **`min_view_role` との協調**: `visibility = 'CUSTOM_TEMPLATE'` のとき `min_view_role` は引き続き評価される（テンプレートが返す受信者候補のうち、`min_view_role` を満たす者だけが閲覧可能）。テンプレートと min_view_role は **AND 結合**
- **テンプレート not found 時の処理**:
  ...
  - 出欠依頼配信はテンプレート削除時点で停止（`resolveRecipients` は空集合を返す）
- **既存レコードの扱い**: マイグレーション時、既存 `visibility IN ('MEMBERS_ONLY','ORGANIZATION')` のレコードは一切変更しない（visibility は保持、`visibility_template_id` は NULL のまま）
- **クロスチーム招待 mirror schedule の扱い（SOURCE_DERIVED 方式）**: 元スケジュールが `visibility = 'CUSTOM_TEMPLATE'` の場合でも、mirror レコードの `visibility` は相手チーム向けに `'MEMBERS_ONLY'` へ書き換え、`visibility_template_id = NULL` とする。mirror 側は普通のメンバー判定で動作し、テンプレートを参照しない（owner 不整合・相手チーム視点の分かりやすさの両立）

**評価フロー**:
- スケジュール閲覧可否判定時に `VisibilityTemplateEvaluator.matches(template_id, viewer_user_id)` を呼び出し、`min_view_role` チェックと AND で結合
- 出欠依頼通知配信時に `VisibilityTemplateEvaluator.resolveRecipients(template_id)` で候補を解決し、`min_response_role` でさらに絞り込む

**Phase 実装順序**:
- **F01.7 Phase 3 と同時リリース**（カスタム公開範囲テンプレート本体導入時）
- 既存 `visibility IN ('MEMBERS_ONLY','ORGANIZATION')` の挙動は一切変更しない
- クロスチームスケジュール（相手チーム招待）に対する CUSTOM_TEMPLATE の挙動は要検討事項として未解決事項に追加（精査で再整理）

### 出欠コメント（`schedule_attendances.comment`）と予定コメントスレッド（F03.16）の棲み分け

> ⚠️ 本書の「コメント」（`schedules.comment_option` / `schedule_attendances.comment`）は **出欠回答に添える一言**であり、[`F03.16_schedule_comment_thread.md`](F03.16_schedule_comment_thread.md) が新設する**予定コメントスレッド**とは別物である。両者は併存し、カラム・API・権限を一切共有しない。

| 観点 | 出欠コメント（本書） | 予定コメントスレッド（F03.16） |
|---|---|---|
| 実体 | `schedule_attendances.comment VARCHAR(500)` | `schedule_comments` テーブル（新設） |
| 件数 | 1ユーザーにつき1件（`uq_sa_schedule_user` で構造的に1件） | 1ユーザーが何件でも |
| 目的 | 「15分遅刻します」など出欠回答の補足 | 「持ち物は？」など予定についての会話 |
| 返信 | 不可 | 可（`parent_id` / `root_id` / `depth`） |
| 表示制御 | `schedules.comment_option`（HIDDEN/OPTIONAL/REQUIRED） | `schedules.comments_enabled`（F03.16 で追加。`comment_option` とは無関係） |
| 書き込み条件 | `min_response_role` 以上（出欠回答権限） | 親スケジュールを閲覧できること（GUEST を除く） |
| 通知 | なし | メンション・返信で通知（F04.3 / F04.11） |

**設計上の帰結**: `comment_option = 'HIDDEN'` を設定していても予定コメントスレッドは独立して機能し、逆にスレッドを閉じても出欠コメントは従来どおり動く。**片方の改修がもう片方を壊さない**ようにするための構造的分離であり、将来「コメント機能を統合しよう」という判断をする場合は、この2つが別目的であることを踏まえて再設計すること。
...
| DELETE | `/api/v1/teams/{id}/schedules/{scheduleId}/scheduled-tasks/{taskId}` | 必要（MANAGE_SCHEDULES 以上）| 予約タスク取消（機能55）。PENDING のみ取消可（204）。取消済み/materialize済みは 409 |
...
| DELETE | `/api/v1/organizations/{id}/schedules/{scheduleId}/scheduled-tasks/{taskId}` | 必要（MANAGE_SCHEDULES 以上）| 予約タスク取消（機能55）。PENDING のみ取消可（204）。取消済み/materialize済みは 409 |
| GET | `/api/v1/organizations/{id}/schedules/{scheduleId}/attendances` | 必要（ADMIN）| 組織出欠回答一覧（個別 user_id・ステータス・コメント・回答日時）|
| GET | `/api/v1/organizations/{id}/schedules/{scheduleId}/attendances/export` | 必要（ADMIN）| 組織出欠回答一覧CSVエクスポート（個別行）|
| GET | `/api/v1/organizations/{id}/schedules/{scheduleId}/attendances/team-breakdown` | 必要（ADMIN）| 組織出欠チーム別内訳集計（by_team・個人情報なし。`team_breakdown_enabled` ON 時のみ by_team を算出）|
| GET | `/api/v1/organizations/{id}/schedules/{scheduleId}/attendances/team-breakdown/export` | 必要（ADMIN）| 組織出欠チーム別内訳CSVエクスポート |
...
```json
{
  ...
  "reminders": [
    ...
    { "remind_at": "2026-04-02T09:00:00", "reminder_kind": "ABSOLUTE" },
    { "remind_before_minutes": 60, "reminder_kind": "RELATIVE" }
  ],
  "scheduled_surveys": [
    {
...
> **機能55 拡張フィールド**
> - `reminders[].reminder_kind`: `ABSOLUTE`（絶対日時・既定）/ `RELATIVE`（開始N分前）。`RELATIVE` 時は `remind_at` の代わりに `remind_before_minutes`（正の整数）を使用する。既存の `remind_at` のみのフォーマットは後方互換で `ABSOLUTE` とみなす
> - `scheduled_surveys`: 予約アンケート（最大10件）。`scheduled_at` 到来時に `SurveyService` 経由でアンケートを自動生成・公開する（`F05.4_survey_vote.md` 相互参照）。設問定義・設問タイプ・匿名設定等はアンケート作成と同一の `CreateSurveyRequest` 形式で指定する
> - `scheduled_attendance`: 予約出欠募集（単一）。`scheduled_at` 到来時に出欠レコードを生成し対象メンバーへ募集通知を配信する。`attendance_required = true` の場合に使用する。予約タスクがある場合は `ScheduleCreatedEvent` リスナーによる即時出欠募集はスキップされ、バッチが委譲先となる
>   - `scheduled_at` / `attendance_deadline` はいずれも **タイムゾーンオフセット付き**（例 `2026-08-01T09:00:00+09:00` / `Z`）で受け取り、サーバー側で Asia/Tokyo に変換して保存する。オフセット無しの旧形式は JST として解釈する（後方互換）
>   - `attendance_deadline` / `comment_option` / `min_response_role` は予約タスクの `payload_json` にスナップショットとして保持され、**materialize 時に予定本体（`schedules`）の出欠属性へ適用される**。未指定の項目は予定の既存値を保つ（PATCH セマンティクス）
>
> **(B) 組織→参加チーム配信 拡張フィールド**
> - `include_supporters`（任意・既定 false）: 出欠確認の配信母集団にサポーター（応援者）を含めるか。組織配信時は既定で除外（per-schedule トグル）
> - `team_breakdown_enabled`（任意・既定 false）: 出欠集計をチーム別内訳（by_team）でも収集・表示するか。既定 false = 従来挙動（全体集計のみ）。ON かつ組織スコープのときのみ `.../attendances/team-breakdown` が by_team を算出する
...
```json
{
  "data": {
    ...
    "is_exception": false,
    "created_by": { "id": 1, "display_name": "田中 太郎" },
    "created_at": "2026-03-01T10:00:00Z",
    ...
    "reminders": [
      { "id": 1, "reminder_kind": "ABSOLUTE", "remind_at": "2026-03-29T09:00:00", "remind_before_minutes": null, "is_sent": false, "sent_at": null },
      { "id": 2, "reminder_kind": "RELATIVE", "remind_at": null, "remind_before_minutes": 60, "is_sent": false, "sent_at": null }
    ],
    "scheduled_tasks": [
      {
        "id": "0190f9a0-1234-7abc-def0-000000000001",
        "task_type": "SURVEY",
        "scheduled_at": "2026-04-06T09:00:00",
        "status": "PENDING",
      ...
      {
        "id": "0190f9a0-1234-7abc-def0-000000000002",
        "task_type": "ATTENDANCE",
        "scheduled_at": "2026-03-20T09:00:00",
        "status": "CREATED",
        "materialized_entity_id": 20
      }
    ],
    "cross_invitations": [
      {
...
> - `attendance_summary`: 当該スケジュールの `min_response_role` 以上のロールを持つリクエスト者のみ返す（min_response_role 未満の場合は null）。内容は件数集計のみで個人情報は含まない
> - `reminders`: スケジュール作成者 / ADMIN のみ返す。機能55で `reminder_kind`（ABSOLUTE/RELATIVE）・`remind_before_minutes` フィールドが追加された
> - `scheduled_tasks`: 予約タスク一覧（機能55）。PENDING/CREATED/CANCELLED/FAILED の全状態を返す。FE が「○月○日に出欠募集を自動開始予定」「アンケート作成済み」などの表示を出し分けられる。スケジュール作成者 / ADMIN のみ返す
...
チームスケジュール一覧（`GET /api/v1/teams/{id}/schedules`）と同様。クエリパラメータ（`from` / `to` / `q` / `event_type` / `status` / `cursor` / `size`）・レスポンス形式（`meta` 含む）は同一。`organization_id` に紐づくスケジュールを返す。
...
#### `PATCH /api/v1/teams/{id}/schedules/{scheduleId}/attendances/bulk`

ADMIN がメンバーの出欠を一括更新する。連絡なしの未回答者を一括「欠席」処理する等、大会前の最終確認に使用する。
...
> - `user_ids` は最大50件。超過する場合は 400 Bad Request
> - 対象ユーザーの `schedule_attendances` が存在しない場合はスキップ（部分成功）
> - `comment` は省略可。指定した場合は全対象ユーザーに同一コメントを設定
...
| 403 | ADMIN 権限なし |
| 404 | スケジュールが存在しない / 論理削除済み |
| 422 | `attendance_required = FALSE` のスケジュール |
| 422 | `status = CANCELLED` のスケジュール |

---

#### `GET /api/v1/teams/{id}/schedules/{scheduleId}/attendances/export`

チームスケジュールの出欠一覧をCSVでエクスポートする。ADMIN のみ参照可能。

**レスポンス（200 OK）**
- Content-Type: `text/csv; charset=UTF-8`
- Content-Disposition: `attachment; filename="attendance_{scheduleId}_{date}.csv"`
- BOM 付き UTF-8（Excel互換）

**CSVカラム**
```
氏名,ステータス,コメント,回答日時,アンケート設問1,アンケート設問2,...
田中 太郎,出席,,2026-04-01T10:00:00Z,true,
鈴木 花子,欠席,体調不良,2026-03-31T15:00:00Z,false,
山田 次郎,未回答,,,
```

> - ステータスは日本語表記（出席 / 一部参加 / 欠席 / 未回答）
> - アンケート設問カラムは `event_surveys` の `sort_order` 順で動的に追加
> - UNDECIDED メンバーも含む
...
#### `POST /api/v1/teams/{id}/schedules/{scheduleId}/duplicate`

既存スケジュールを複製して新規スケジュールを作成する。日時を指定して他の設定をコピーできる。

**リクエストボディ**
```json
{
  "start_at": "2026-05-10T13:00:00",
  "end_at": "2026-05-10T17:00:00",
  "title": null
}
```

> - `start_at` / `end_at` は必須（複製元の日時をそのまま使用するケースは稀なため）
> - `title` を指定した場合は上書き。`null` または省略時は複製元の `title` を使用
> - コピー対象フィールド: `title`・`description`・`location`・`all_day`・`event_type`・`visibility`・`min_view_role`・`min_response_role`・`attendance_required`・`comment_option`・`surveys`（設問定義のみ。回答はコピーしない）・`reminders`（`remind_at` は `start_at` からの相対時間で再計算）
> - コピー対象外: `recurrence_rule`（繰り返し設定は複製しない。単発スケジュールとして作成）・`attendance_deadline`（新しいスケジュールで改めて設定が必要）
> - 作成後のフローは `POST /teams/{id}/schedules`（単発）と同一（出欠レコード生成・Google 同期・audit_logs 等）

**レスポンス（201 Created）**: `POST /teams/{id}/schedules` の単発レスポンスと同一スキーマ

**エラーレスポンス**
| ステータス | 条件 |
|-----------|------|
| 403 | MANAGE_SCHEDULES 権限なし |
| 404 | 複製元スケジュールが存在しない / 論理削除済み |
| 422 | 対象チームがアーカイブ済み |
...
組織スケジュールの出欠回答一覧（個別メンバーの `user_id`・ステータス・コメント・回答日時）を返す。
チーム別の集計（by_team）は別エンドポイント `.../attendances/team-breakdown` を使用する。

> 実装メモ: 本エンドポイントは個別出欠行の一覧（`List<AttendanceResponse>`）を返す。チーム別集計が
> 必要なフロントエンドは次節の `team-breakdown` を呼ぶこと。

---

#### `GET /api/v1/organizations/{id}/schedules/{scheduleId}/attendances/team-breakdown`

組織スケジュールの出欠を「全体集計（`total`）＋チーム別内訳（`by_team`）」で返す（(B) フェーズB）。
個別メンバーの出欠情報（`user_id`・名前・個別ステータス）は含まない。

**認可**: チーム別内訳は組織の運用管理データのため、当該組織の **ADMIN / DEPUTY_ADMIN** のみ参照可能
（`AccessControlService.checkAdminOrAbove(userId, orgId, "ORGANIZATION")`・兄弟の組織管理 EP と同じ正準パターン）。
非 ADMIN は **403（`COMMON_002`）**。export EP（`.../team-breakdown/export`）も同一認可。

**作成時トグル `team_breakdown_enabled` による条件分岐（後方互換）**
- トグル **OFF（既定）または非組織スコープ**: `by_team = null`（省略）＝従来挙動。`total`（実人数集計）のみ返す。
- トグル **ON かつ組織スコープ**: `by_team` を算出して返す。

**御裁可A（全チーム計上・重複あり / `total` は DISTINCT で別建て）**
- `total`: 配信母集団の**実人数（DISTINCT user_id）**を母数とした集計。1 ユーザーは 1 票のみ。
- `by_team`: 配下の複数チームに所属する回答者を**所属全チームへ 1 票ずつ計上**する（重複計上あり）。
  組織直属かつチーム所属を兼ねるユーザーは `team_id = null` 枠とチーム枠の両方に計上される。
  したがって **`by_team` 各チームの合計（のべ人数）≧ `total`（実人数）** になりうる。
- 母集団の SUPPORTER 包含は配信時と同じく `schedules.include_supporters` トグルに従う（配信＝集計の母集団一致）。
...
> - `team_id: null`「チーム未所属（組織直接メンバー）」グループ: 組織に直接所属しているがいずれの配下チームにも属さないメンバー（組織管理者・事務局スタッフ等）の集計。`team_name` は BE では `null` を返し、表示ラベル（例: 「チーム未所属（組織直接メンバー）」）はフロントエンド/i18n が決める。出欠通知・回答・集計は他チームメンバーと同一フローで処理される
> - 上記サンプルは重複計上の例: by_team のべ人数（出席 20+25+5=50）は total 実人数（出席 45）以上になりうる（御裁可A）
> - トグル OFF / 非組織スコープでは `by_team` は `null`（省略）

---

#### `GET /api/v1/organizations/{id}/schedules/{scheduleId}/attendances/team-breakdown/export`

組織スケジュールの出欠チーム別内訳をCSVでエクスポートする。チーム別の集計値＋末尾「合計」行を出力する（個別メンバー情報は含まない）。

**レスポンス（200 OK）**
- Content-Type: `text/csv; charset=UTF-8`
- Content-Disposition: `attachment; filename="org_attendance_team_breakdown_{scheduleId}.csv"`
- BOM 付き UTF-8（Excel互換）

**CSVカラム**（合計行は `total`＝実人数・DISTINCT。チーム行ののべ合計 ≧ 合計行になりうる＝御裁可A）
```
チーム名,出席,一部参加,欠席,未回答,合計
Aチーム,20,3,4,6,33
Bチーム,25,2,6,9,42
チーム未所属（組織直接メンバー）,5,0,1,0,6
合計,45,5,10,15,75
```

> トグル OFF / 非組織スコープではチーム行は出力されず、ヘッダ＋「合計」行（実人数 total）のみを出力する。

---

#### `POST /api/v1/organizations/{id}/schedules/{scheduleId}/duplicate`

チームスケジュール複製（`POST /api/v1/teams/{id}/schedules/{scheduleId}/duplicate`）と同様。リクエストボディ・コピーフィールドルール・レスポンスは同一。内部では `organization_id` が設定される。

---

#### `GET /api/v1/organizations/{id}/schedule-invitations`

チーム版（`GET /api/v1/teams/{id}/schedule-invitations`）と同様。クエリパラメータ（`status`）・レスポンス形式は同一。`target_type = 'ORGANIZATION'` かつ `target_id = {id}` の招待を返す。

---

#### `POST /api/v1/organizations/{id}/schedule-invitations/{invitationId}/accept`

チーム版（`POST /api/v1/teams/{id}/schedule-invitations/{invitationId}/accept`）と同様。承認フローは同一だが、ミラースケジュールは `organization_id = {id}` で作成される。組織の `visibility` に基づく `AWAITING_CONFIRMATION` フローも適用する（`PRIVATE` 相当の組織は非公開設定で判定）。

---

#### `POST /api/v1/organizations/{id}/schedule-invitations/{invitationId}/reject`

チーム版（`POST /api/v1/teams/{id}/schedule-invitations/{invitationId}/reject`）と同様。レスポンス・エラーレスポンスは同一。
...
> - **レート制限**: 同一スケジュールに対して1回/時間。制限超過時は `429 Too Many Requests`（`Retry-After` ヘッダーで次回実行可能時刻を返す）
...
> - `deadline_approaching`: `attendance_deadline` が設定されており、現在時刻から24時間以内に到来する場合 `true`（フロントエンドが「まもなく締切」バッジを表示するために使用）。`attendance_deadline = null` / `attendance_required = false` / `scope_type = "PERSONAL"` の場合は `false`
> - `is_overdue`: `attendance_deadline < NOW()` の場合 `true`（期限切れ）。`attendance_deadline = null` の場合は `false`
...
#### `GET /api/v1/teams/{id}/attendance-stats/export`

チームメンバー全員の出席統計をCSVでエクスポートする。ADMIN のみ参照可能。月次報告や保護者会資料に使用する。

**クエリパラメータ**: `GET /teams/{id}/attendance-stats` と同一（`from` / `to` / `user_id`）

**レスポンス（200 OK）**
- Content-Type: `text/csv; charset=UTF-8`
- Content-Disposition: `attachment; filename="attendance_stats_{teamId}_{from}_{to}.csv"`
- BOM 付き UTF-8（Excel互換）

**CSVカラム**
```
氏名,対象スケジュール数,出席,一部参加,欠席,未回答,出席率(%),完全出席率(%)
田中 太郎,24,16,4,4,0,83.3,66.7
鈴木 花子,24,18,2,3,1,87.0,78.3
```
...
|-----------|------|
| 403 | ADMIN 権限なし |
| 400 | `from` 〜 `to` が 12ヶ月超過 |

---

#### `GET /api/v1/organizations/{id}/attendance-stats/export`

組織内チーム別の出席統計をCSVでエクスポートする。ADMIN のみ参照可能。

**クエリパラメータ**: `GET /organizations/{id}/attendance-stats` と同一（`from` / `to` / `team_id`）

**レスポンス（200 OK）**
- Content-Type: `text/csv; charset=UTF-8`
- Content-Disposition: `attachment; filename="org_attendance_stats_{orgId}_{from}_{to}.csv"`
- BOM 付き UTF-8（Excel互換）

**CSVカラム**
```
チーム名,対象スケジュール数,メンバー数,平均出席率(%),平均完全出席率(%)
FC Tokyo Youth A,24,20,82.5,66.7
FC Tokyo Youth B,24,18,78.0,62.2
```
...
8. attendance_required = TRUE の場合:
   ...
   b. 以下を @Async で実行（メインのレスポンスをブロックしない）:
      i.  スコープに応じて、min_response_role 以上のロールを持つ対象ユーザーを取得:
          ...
          - 組織スコープ（Phase A 以降）: 以下の和集合から重複を除外した全員のうち min_response_role 以上のロールを持つ者
              * 組織直属メンバー（team_organization_memberships 未経由で組織に直接所属するメンバー）
              * 配下の参加チーム（team_organization_memberships.status = 'ACTIVE'）に所属するメンバー
            ※ SUPPORTER ロールのメンバーは既定除外。schedules.include_supporters = TRUE の場合のみ含める
...
### 出欠リマインダーバッチフロー（機能55 根治後）

`ScheduleReminderBatchService` が 1 分間隔（`@Scheduled fixedDelay=60秒`）で実行される。ShedLock（`scheduleReminderBatch`）により複数インスタンス起動時の二重実行を防ぐ。

**根治経緯**: 第一陣（V72.002）以前は `@Scheduled` も呼び出し元も存在せず、共有予定のリマインダーは永遠に発火しなかった（`schedule_attendance_reminders` レコードが積み上がるだけの非稼働状態）。機能55 第二陣で `ScheduleReminderBatchService` を新設してこの欠落を根治した。

```
1. schedule_attendance_reminders から is_sent = FALSE のレコードを全件取得
2. 各リマインダーについて:
   a. 実効リマインド時刻を解決する:
      - reminder_kind = ABSOLUTE: effectiveRemindAt = remind_at
      - reminder_kind = RELATIVE: effectiveRemindAt = schedules.start_at - remind_before_minutes 分
   b. effectiveRemindAt > NOW() の場合はスキップ（まだ時刻に達していない）
   c. 紐づくスケジュールが status = SCHEDULED かつ deleted_at IS NULL か確認（CANCELLED / COMPLETED はスキップ）
   d. attendance_deadline が設定されている場合: deadline < NOW() であれば期限切れとしてスキップ
      attendance_deadline = NULL（無期限）の場合はスキップしない（必ず通知する）
   e. schedule_attendances から status = UNDECIDED の user_id 一覧を取得
   f. ReminderNotificationEvent を発火 → ScheduleReminderNotificationListener が受信し、
      NotificationHelper.notifyAll() で通知種別 SCHEDULE_REMINDER の IN_APP + PUSH を配信
   g. schedule_attendance_reminders.is_sent = TRUE、sent_at = NOW() に UPDATE
3. audit_logs には記録しない（通知インフラのログで管理）
```

---

### 予約タスク materialize バッチフロー（機能55）

`ScheduleScheduledTaskBatchService` が 1 分間隔で実行。詳細は `schedule_scheduled_tasks` テーブル定義の「materialize バッチ仕様」を参照。
...
### 組織スケジュール出欠集計フロー（チーム別内訳 by_team）

> **2026-06-19 改稿（実装一致）**: 出欠の「全体集計＋チーム別内訳（by_team）」は専用 EP
> `GET /api/v1/organizations/{orgPublicId}/schedules/{scheduleId}/attendances/team-breakdown` が担当する。
> 従来 `GET .../attendances` は **個別の出欠行一覧**（user_id ベース）を返す実装に整理済みであり、
> by_team 集計はこの専用 EP からのみ取得する。CSV は `.../attendances/team-breakdown/export`。

```
1. GET /api/v1/organizations/{orgPublicId}/schedules/{scheduleId}/attendances/team-breakdown を受付
2. 操作者が当該組織の ADMIN（ADMIN / DEPUTY_ADMIN）か確認（checkAdminOrAbove・非 ADMIN は 403 COMMON_002）
3. スケジュールが当該組織スコープ（organization_id 一致）か確認
4. total（全体集計）を常に算出: schedule_attendances のステータス別件数をそのまま実人数（DISTINCT 母数・1 ユーザー 1 行）として集計
5. by_team の算出可否を判定:
   - 作成時トグル team_breakdown_enabled = FALSE（既定）または非組織スコープ → by_team = null（従来挙動＝全体集計のみ）。step 8 へ
   - team_breakdown_enabled = TRUE かつ組織スコープ → step 6〜7 で by_team を算出
6. 母集団（userId → 所属チーム）を OrganizationMembershipService.resolveMemberTeams で解決
   （SUPPORTER 包含は配信トグル include_supporters に従う＝配信と集計の母集団一致）。
   チームに所属しない組織直接メンバーは team_id = null 枠として集計
7. 各ユーザーのステータスを所属全チームへ 1 票ずつ計上（御裁可A・重複計上あり。複数チーム所属者は
   所属全チームに計上されるため by_team 各チーム合計は total 以上になりうる）
8. 集計結果のみ返す（個別 user_id・名前は含まない）。200 OK
```
...
| スケジュール複製 | `POST /teams/{id}/schedules/{scheduleId}/duplicate`・`POST /organizations/{id}/schedules/{scheduleId}/duplicate` |
| 出欠一括更新 | `PATCH /teams/{id}/schedules/{scheduleId}/attendances/bulk` |
| 招待承認 | `POST /teams/{id}/schedule-invitations/{invitationId}/accept`・`POST /organizations/{id}/schedule-invitations/{invitationId}/accept`（承認するチーム/組織がアーカイブ済みの場合）|
...
- **回答権限制御（min_response_role）**: `min_response_role` によるチェックを `PATCH /schedules/{scheduleId}/responses` で実施する。`min_view_role`（閲覧可否）とは独立した2軸制御。回答権限がないユーザーはスケジュールを閲覧可能だが 403 を返す。`attendance_required = TRUE` 時の `schedule_attendances` 一括 INSERT も `min_response_role` 以上のロールを持つメンバーのみを対象とする
  - **実装（サーバ側 enforcement / 2026-06 セキュリティ根治）**: 出欠回答の入口 `ScheduleAttendanceService.respondAttendance`（`PATCH /schedules/{scheduleId}/responses` の本体）で `validateMinResponseRole` を必ず通す。スケジュールの所属スコープを `team_id` / `organization_id` から TEAM / ORGANIZATION として解決し、`AccessControlService` で当該スコープの実ロールを判定する:
    - `SUPPORTER+` → `hasRoleOrAbove(userId, scopeId, scopeType, "SUPPORTER")`
    - `MEMBER+` → `hasRoleOrAbove(userId, scopeId, scopeType, "MEMBER")`
    - `ADMIN_ONLY` → `isAdminOrAbove(userId, scopeId, scopeType)`（ADMIN / DEPUTY_ADMIN のみ）
    - SYSTEM_ADMIN（`isSystemAdmin(userId)`）は横断で常に許可
    - ロール不足は `COMMON_002`（403）。本根治以前は enum 定義・DB 保存・レスポンス出力はあったが gate に読む箇所が無く、`ADMIN_ONLY` / `MEMBER_PLUS` 制限が一般メンバー全員に開放されていた過小判定であった
  - **後方互換**: `min_response_role = NULL`（移行前データ）は従来どおりメンバー全員の回答を許可し、本チェックをスキップする。PERSONAL スコープ（`team_id` / `organization_id` がいずれも null）はロール概念が無いため enforcement 対象外
...
- **即時リマインド通知のレート制限**: `POST /schedules/{id}/remind` は同一スケジュールに対して1回/時間。ADMIN の連打による大量通知送信を防止する
...
V3.016__add_user_id_to_schedules.sql                    -- schedules.user_id カラム追加・INDEX・CHECK 制約（F03.2_schedule_personal.md と共有）
...
V13.014__add_distance_and_result_to_schedules.sql         -- schedules に distance_km / distance_notes / actual_start_at / actual_end_at / result_summary カラム追加

-- 機能55 予定の予約作成（#1256/#1261/#1262/#1265 で main マージ済み）
V72.001__create_schedule_scheduled_tasks.sql
  -- schedule_scheduled_tasks テーブル新規作成（UUIDv7主キー・組織テナントキー・PENDING/CREATED/CANCELLED/FAILED ステータス・payload_json）
  -- 主キー BINARY(16)・INDEX idx_sst_status_scheduled (status, scheduled_at)・idx_sst_schedule (schedule_id)・idx_sst_org_deleted (organization_id, deleted_at)
V72.002__alter_schedule_attendance_reminders_add_relative.sql
  -- schedule_attendance_reminders に remind_before_minutes INT NULL・reminder_kind VARCHAR(10) NOT NULL DEFAULT 'ABSOLUTE' を追加
  -- remind_at を NULL 可に変更（RELATIVE 時は materialize 前に未確定のため）
V72.003__alter_personal_schedule_reminders_add_absolute.sql
  -- personal_schedule_reminders に remind_at DATETIME NULL・reminder_kind VARCHAR(10) NOT NULL DEFAULT 'RELATIVE' を追加
  -- remind_before_minutes を NULL 可に変更（ABSOLUTE 時は未使用のため）
  -- ※ personal_schedule_reminders は F03.2_schedule_personal.md が主管

-- 出欠確認の配信母集団トグル（include_supporters）
V101.002__add_include_supporters_to_schedules.sql
  -- schedules に include_supporters BOOLEAN NOT NULL DEFAULT FALSE を追加（配信母集団に SUPPORTER を含めるか）

-- (B) 組織→参加チーム配信 フェーズB（出欠のチーム別内訳 by_team）
V119.001__add_team_breakdown_enabled_to_schedules.sql
  -- schedules に team_breakdown_enabled BOOLEAN NOT NULL DEFAULT FALSE を追加（作成時トグル・既定 OFF = 従来挙動）
  -- ※ 当初 V117.001 で起票したが main 既存 phase5c/phase5d（V117.001/V118.001）と採番衝突したため V119.001 へリネーム
...
- V3.016 は V3.007（schedules）および V1.005（users）完了後（F03.2_schedule_personal.md と共有）
- Google Calendar 連携マイグレーション（V3.013〜V3.015・V3.017）の依存関係は F08_external_integration.md Section 6 を参照
- V72.001 は V2.001（organizations）完了後（organization_id はクロスドメイン論理参照のため FK なし）
- V72.002 / V72.003 は V3.011 / V3.018 の完了後（ALTER 対象テーブルが存在することが前提）
...
- [x] ~~Google Calendar 管理者レベル共有連携（`google_calendar_event_id` カラムの用途）~~ → **Phase 9 スコープ外と確定（F03.3 と同一の判断）**。F09 系の設計が完了した結果、共有カレンダー連携の需要は F03.3 の個人同期で十分にカバーされる。`google_calendar_event_id` は個人同期でのイベント紐付けに使用。管理者レベル共有連携は運用ニーズが顕在化した段階で検討
- [x] ~~`event_type` の選択肢はテンプレートに応じて表示切替するか~~ → **バックエンドは全 `event_type` を常に受け付ける（制限しない）。フロントエンドでテンプレートに応じて表示をフィルタリング**する設計に確定。スポーツテンプレートでは `PRACTICE` / `MATCH` を優先表示、ビジネステンプレートでは `MEETING` を優先表示、全テンプレートで `EVENT` / `OTHER` を共通表示。F01.3 の `WIDGET_MODULE_MAP` と同様の FE 側マッピング定数（`EVENT_TYPE_TEMPLATE_MAP`）で制御。DB・API 変更なし
- [x] 組織スコープのクロスチーム招待受信エンドポイントが未定義 → チーム版と同様のエンドポイント3本（`GET /organizations/{id}/schedule-invitations`・`POST .../accept`・`POST .../reject`）を追加。リクエスト/レスポンスはチーム版と同一仕様。承認時のミラースケジュールは `organization_id = {id}` で作成。`AWAITING_CONFIRMATION` フローも適用（Section 4 参照）
...
- [x] 組織スケジュールの出欠確認: チームに所属しない組織直接メンバーへの出欠通知フローを確定する: 既存フローで対応済み。スケジュール作成フロー step 8i「全所属チームのメンバー + 組織直接所属メンバー（重複除外）」に含まれるため通知・schedule_attendances 生成は自動的に行われる。出欠回答フロー step 6 で組織直接所属ロールによる回答権限を確認。集計は専用 EP `GET /organizations/{orgPublicId}/schedules/{scheduleId}/attendances/team-breakdown`（作成時トグル `team_breakdown_enabled = TRUE` の組織スケジュールのみ by_team を算出。トグル OFF / 非組織は `by_team = null`）の `team_id: null`「チーム未所属（組織直接メンバー）」枠で区別表示。当該 EP は ADMIN / DEPUTY_ADMIN のみ参照可。ダッシュボードへの表示は `GET /my/calendar` の横断ビューで対応（専用セクション表示はフロントエンド判断）
...
- [x] **F01.7 `CUSTOM_TEMPLATE` visibility 拡張 — mirror schedule 処理方針確定** (2026-04-21): **SOURCE_DERIVED 方式** を採用。mirror schedule は元スケジュールの visibility 判定を継承し、mirror 側は独自テンプレート指定不可。具体仕様:
  - mirror schedule 作成時、`visibility = 'CUSTOM_TEMPLATE'` であっても、mirror レコードの `visibility_template_id` は **NULL** を設定し、`visibility` は相手チーム向けに `'MEMBERS_ONLY'` へ書き換え（相手チームメンバー全員が閲覧可）
  - 閲覧判定時、mirror 側は普通のメンバー判定で動作（テンプレートを参照しない）
  - 元スケジュールのテンプレートが削除されても mirror は独立して存続
  - **根拠**: owner 不整合回避・設計簡素化・相手チーム視点の分かりやすさ

---

## 8.5 将来検討（TimeTree比較より）

以下は TimeTree との比較検討で洗い出された候補であり、**🔵 検討事項（未着手・設計未了）**。本書のスコープには含めず、着手時に別途軍議・設計書化する。

- **(a) 公開カレンダー**: 組織・店舗が外部の非会員へ予定を一方向配信し、フォロー＋開催通知を受けられる仕組み。村機能（F17系）の「公開行事告知」・広告機能（F09.7系）の「店舗の集客カレンダー」との接続候補。認証不要の外部公開を伴うため、§8 の「未認証パブリック公開は SEO 目的コンテンツ（F06 ブログ等）に留める」方針との整合を検討時に再確認する必要がある。
- **(b) 軽量参加経路**: アカウントなし・招待リンクのみで「カレンダー閲覧・出欠回答だけ」できる低摩擦オンボーディング。既存の招待・出欠フロー（§4・§5）はいずれもログイン済みユーザー前提であり、匿名/仮登録での出欠回答は本書の認可モデル（ロールベース `min_view_role`/`min_response_role`）を拡張する設計が必要になる。

いずれも本書の対象外（未着手）であり、上記2件を理由に §8 の未解決事項を追加することはしない（設計未了の記録に留める）。
...
| 日付 | 変更内容 |
|------|---------|
| 2026-08-05 | F03.16 予定コメントスレッド連携: §3 に「出欠コメント（`schedule_attendances.comment`）と予定コメントスレッドの棲み分け」節を新設（実体・件数・目的・返信可否・表示制御・書き込み条件・通知の7軸で分離。両者はカラム・API・権限を一切共有しない）。関連機能に F03.16 を追記。**本書の `comment_option`（HIDDEN/OPTIONAL/REQUIRED）は出欠回答の補足コメント専用**であり、F03.16 が追加する `schedules.comments_enabled`（スレッド開閉）とは無関係である点を明記 |
| 2026-08-05 | §8.5「将来検討（TimeTree比較より）」新設（公開カレンダー・軽量参加経路の2件を🔵検討事項として記録）。F03.18 スケジュール変更フィード設計書への相互参照を追加 |
| 2026-06-19 | (B) 組織→参加チーム配信 フェーズB（出欠のチーム別内訳 by_team）: (1) `schedules` テーブルに `team_breakdown_enabled BOOLEAN NOT NULL DEFAULT FALSE` を追加（作成時トグル・既定 OFF = 従来挙動。Flyway V119.001。当初 V117.001 で起票したが、main 既存の phase5c/phase5d（V117.001/V118.001）と採番衝突したため main 取込時に V119.001 へリネーム）。DDL 表に `include_supporters`（V101.002・従来未記載）も併記。(2) §1 概要を「組織出欠のチーム別集計は作成時トグル ON 時のみ」へ改稿（御裁可A: 複数チーム所属者は所属全チームへ 1 票ずつ計上＝by_team 合計のべ人数 ≧ total 実人数 DISTINCT 別建て）。(3) 出欠集計の by_team API を実装に合わせ専用 EP `GET .../{scheduleId}/attendances/team-breakdown` ＋CSV `.../team-breakdown/export` として記載（従来 `.../attendances` は個別出欠行一覧を返す実装に合わせ説明を訂正）。トグル OFF / 非組織は `by_team = null`（省略）。`team_id = null` 枠は組織直接メンバー（`team_name` は BE では null・表示ラベルは FE/i18n）。(4) スケジュール作成リクエストに `include_supporters` / `team_breakdown_enabled` を追記。FE は別フェーズ |
| 2026-06-02 | 機能55 予定の予約作成・リマインド根治の設計書同期（PR #1256/#1261/#1262/#1265）: (1) `schedule_scheduled_tasks` テーブル定義追加（UUIDv7・payload_json・status 4値・materialize バッチ仕様・出欠即時/予約切り替えロジック）。(2) `schedule_attendance_reminders` の `reminder_kind`（RELATIVE/ABSOLUTE）・`remind_before_minutes` カラム追加と実効時刻解決ロジックを記載。(3) 出欠リマインダーバッチを根治後の実装（1分間隔・実効時刻解決・通知種別 SCHEDULE_REMINDER）に更新（従来非稼働だった事実を明記）。(4) 予約タスク materialize バッチ仕様を追記（SURVEY→SurveyService/ATTENDANCE→openAttendanceSolicitation・per-item REQUIRES_NEW・最大5試行）。(5) API 一覧に取消 EP（DELETE .../scheduled-tasks/{taskId}）を追加。(6) スケジュール作成リクエストに `scheduledSurveys`・`scheduledAttendance`・`reminders[].reminder_kind`・`reminders[].remind_before_minutes` を追記。(7) 詳細レスポンスに `scheduled_tasks` フィールドを追加。(8) Flyway 節に V72.001/002/003 を記録 |
| 2026-05-25 | F03.10 代理出席連携: `schedules` テーブルに `allow_proxy_attendance` / `is_proxy_auto_accept` カラムを追加（Flyway 番号は F03.10 実装時に採番。詳細は F03.10 §9）。関連機能に F03.10 参照を追記 |
| 2026-04-30 | 移動距離・実績日時・結果サマリー欄を追加: `schedules` テーブルに `distance_km` / `distance_notes` / `actual_start_at` / `actual_end_at` / `result_summary` の5カラムを追加。Flyway V13.014 として管理 |
| 2026-04-21 | F01.7 軍議決着反映: (1) Phase 番号を「Phase 1 と同時」→「**Phase 3 と同時**」に訂正 (2) CHECK 制約 2 本追加（`chk_sch_custom_template_consistency` / `chk_sch_custom_template_scope`）で DB レベル強制 (3) テンプレート not found 時のフェイルセーフ挙動（PRIVATE 相当）を明記 (4) クロスチーム mirror は **SOURCE_DERIVED 方式**（mirror 側は元スケジュールの判定を継承、独自テンプレ不可）確定 (5) マイグレーション番号を V15.014 → V16 帯（F01.7 §7 参照）に変更予定 |
| 2026-04-20 | F01.7 カスタム公開範囲テンプレート連携: §3.X 新設（`schedules.visibility` ENUM に `CUSTOM_TEMPLATE` 追加、`visibility_template_id` カラム追加、`min_view_role` との AND 結合ルール、個人スコープでは CUSTOM_TEMPLATE 禁止を明記）。V15.014 マイグレーション予定。未解決事項に「クロスチーム招待時の mirror schedule 側の CUSTOM_TEMPLATE 処理」を追加 |
| 2026-03-18 | ステータスを 🟡 設計中 → 🟢 設計完了 に変更。未解決事項2件を解決: (1) Google Calendar 管理者レベル共有連携→Phase 9 スコープ外と確定（F03.3 と同一判断。個人同期で十分カバー） (2) event_type のテンプレート別表示切替→バックエンドは全 event_type を受け付け、フロントエンドで EVENT_TYPE_TEMPLATE_MAP 定数によりテンプレート別に優先表示をフィルタリングする設計に確定（DB・API 変更なし） |
| 2026-03-12 | 設計改善10件反映: ① スケジュール一覧にカーソルベースページネーション導入（`cursor` + `size` + `has_next` + `next_cursor`）。② キーワード検索パラメータ `q`（title/location 部分一致）追加。③ スケジュール複製API追加（`POST .../duplicate`、チーム/組織版）。④ 出欠一覧CSVエクスポート追加（`GET .../attendances/export`、チーム/組織版）。⑤ `POST /remind` にレート制限追加（1回/時間/スケジュール、429 対応）。⑥ ADMIN 出欠一括更新API追加（`PATCH .../attendances/bulk`、最大50件）。⑦ `GET /my/calendar` に `deadline_approaching`・`is_overdue` フラグ追加。⑧ スケジュール詳細の `created_by` をオブジェクト化（`id` + `display_name`）。⑨ 組織スコープのクロスチーム招待受信エンドポイント3本追加（未解決事項解決）。⑩ 出欠統計CSVエクスポート追加（`GET .../attendance-stats/export`、チーム/組織版） |
...
| 2026-02-21 | 初版作成 |
| 2026-04-18 | F04.10 組織委員会対応: `schedules` に `committee_id` カラムを追加し、team_id / organization_id / user_id / committee_id の 4 カラム XOR CHECK 制約に再定義。出欠確認（schedule_attendances）は `committee_id` 経由で自動的に委員会スコープ化される（詳細は F04.10_committee.md §3, §8 参照） |

---

## 予定対象者

- 対象者候補はメンバー一覧 API の `totalPages` まで `size=500` で全ページ取得する。候補母集団の人数と、一予定あたりの選択上限500名は別の制約である。
- 対象指定の形式・件数不正は `SCHEDULE_093`（HTTP 400）、対象メンバー不在・退会済み・別スコープは `SCHEDULE_094`（HTTP 404、存在秘匿）で返す。
- メンバーカレンダー色は Frontend の `MEMBER_CALENDAR_COLORS` と同じ12色（`#2563EB`, `#7C3AED`, `#DB2777`, `#DC2626`, `#EA580C`, `#CA8A04`, `#16A34A`, `#0D9488`, `#0891B2`, `#4F46E5`, `#64748B`, `#9333EA`）の固定パレットに限定する。色には氏名を必ず併記し、色だけを識別手段にしない。パレット外は `MEMBERSHIP_023`（HTTP 400）、対象メンバー不在は `MEMBERSHIP_024`（HTTP 404）とする。
- `UserAnonymizedEvent` で `schedule_targets.user_id` と `scope_member_calendar_settings.user_id` の行を各ドメインの独立 listener が即時削除する。退会対象者は `targets[]` から省略するが、通常の退会状態では保存済み行に基づく `targetCount` を維持する。全対象行が匿名化で消えた場合も `SELECTED_MEMBERS` を `ALL_MEMBERS` へ暗黙変更しない。

チーム・組織予定は、公開範囲とは独立して表示上の対象者を持つ。

- `targetMode=ALL_MEMBERS`: 対象IDを送らない。既存予定・省略時の後方互換値。
- `targetMode=SELECTED_MEMBERS`: `targetUserIds` に同一スコープの有効メンバーを重複なしで1〜500名指定する。
- 選択対象者は予定の `minViewRole` も満たさなければならない。対象指定によって可視性・ロール閾値を迂回しない。
- 応答は直下の `targetMode`、`targetCount`、`targets[]` を返す。スコープ外閲覧では氏名・アバター・色を含む `targets` を空にし、名簿を漏らさない。
- `schedule_targets.user_id` は user ドメインへの論理参照でありDB FKを持たない。`schedule_id` だけは同一scheduleドメインなのでCASCADE FKを持つ。
- 繰り返しの `THIS_ONLY` / `THIS_AND_FOLLOWING` / `ALL` は予定本体と同じ範囲で対象者を置換する。通常複製・年間コピーは対象者を複製し、クロススコープ招待は招待先の `ALL_MEMBERS` として作る。
- 出欠募集は `SELECTED_MEMBERS` の場合、従来の回答可能母集団と対象者集合の積集合へだけ生成する。

メンバー表示色は `scope_member_calendar_settings` にスコープ単位で保持する。未設定時は `(scopeType, scopeId, userId)` から固定パレットを決定し、管理者がメンバーAPIの `calendar-color` をPATCH／DELETEして上書き／リセットできる。予定背景色とは別の対象者チップ・ドットに使い、氏名を必ず併記する。

## 関連機能

- **F03.8 イベント管理**: スケジュールを拡張した大規模イベント管理。`events.schedule_id` で 1:1 紐付け
- **F03.17 キープ（日付未定の予定）**: `start_at` が決まる前のアイデアを貯める受け皿。`schedule_keeps.converted_schedule_id` で `schedules` へ 1タップ変換し、変換元は削除せず逆引きできる（`schedules` テーブル自体への変更は無い）
- **F03.10 代理出席**: スケジュールへの代理出席指定・承認フロー。`schedule_delegations` テーブルで管理
- **F04.10 組織委員会**: 委員会スコープのスケジュール。`schedules.committee_id` で連携
- **F08.3 議決権行使・委任状**: 投票セッションへの出欠確認連携
- **F04.3 プッシュ通知**: 出欠リマインダー・代理依頼通知の配信
- **F03.16 予定コメントスレッド**: 予定に紐づく汎用コメント・返信スレッド。`schedule_comments` テーブルで管理し、可視性は本書のスケジュール可視性を継承する。**出欠コメント（`schedule_attendances.comment`）とは別物**（§3「出欠コメントと予定コメントスレッドの棲み分け」参照）

---

## 付録: 組織委員会機能（F04.10）との連携

F04.10 で新設される組織委員会（Committee）は、スケジュール・カレンダー・出欠確認機能を流用する。

### スキーマ変更

`schedules` テーブルに `committee_id` カラムを追加し、既存の XOR CHECK 制約を拡張する。

```sql
ALTER TABLE schedules
  ADD COLUMN committee_id BIGINT UNSIGNED NULL AFTER user_id,
  ADD CONSTRAINT fk_schedules_committee
...
既存レコードは `committee_id = NULL` のままで XOR 制約を満たすため後方互換。

### 出欠確認の委員会スコープ化

- `schedule_attendances` テーブルそのものに変更はない（`schedule_id` 経由で委員会スコープを継承）
- 出欠期限・確認必須フラグ・最小回答ロール（`min_response_role`）の設定は既存機能をそのまま利用
- 委員会内のみで完結するため、委員会メンバー全員が対象になる（`min_response_role` は MEMBER+ 相当で事実上全員）

### Google Calendar 連携（F03.3）の扱い

- 委員会スケジュールの個人同期は **v1 では未対応**。委員会機能の運用実態を確認してから Phase 2 で検討
- `user_schedule_google_events` テーブルは `committee_id` をサポートしない（スコープが委員会の場合は同期スキップ）

### クロスチーム招待（schedule_cross_refs）との関係

- v1 では委員会スケジュールのクロスチーム招待は扱わない（委員会は閉じた場であるため）
- 必要な場合は `CommitteeDistributionService` 経由で通達する

### 関連マイグレーション

- V9.086: `schedules.committee_id` 追加 + XOR CHECK 再定義 + INDEX
