...
- したがって `F < A` の分岐は、**system 経路（§3.6）の中で「運営手数料の一部返金」を明示的に指定できる形**として実装する。既存 `refund()` のシグネチャ・挙動は変更しない。
- **⚠️ 実装時に必ず実測で確認すること（本書では未確認）**: `createConnectRefund` に渡している `refund_application_fee` は真偽値であり（`StripePaymentProvider` の既存シグネチャ）、「運営手数料のうち任意の額だけを返す」指定がこの真偽値で表現できるかは**未確認**である。表現できない場合は、運営手数料の返金を別の呼び出しに分ける必要がある。**Stripe の挙動を推測で実装せず、テスト環境で実測してから確定させること。**
...
1. **債権の放棄** — そのキャンセル料はもう請求しない
2. **申込ブロックの解除** — 未払いを理由とするブロック（`RecruitmentParticipantService.java:129-134`）が外れ、**そのユーザーは Mannschaft 上のあらゆる募集へ再び申し込めるようになる**

**2 の効果は免除した主催者の募集に閉じない。** 主催者は「自分の債権を放棄する」つもりで操作するが、効果は全体に及ぶ。ブロックの根拠が「未払いがあること」である以上、未払いが消えればブロックが外れるのは筋が通っており、この設計を変えるつもりはない。ただし**その性質を知らずに扱ってよいものではない**ため、ここに明記する。UI にもこの旨を表示すること（§12・i18n）。
...
| 効果 | `paymentStatus = WAIVED`・`notes` に理由を保存。§10.0 の 2 つの効果が生じる |
| 監査 | 必須（§10.4） |
...
```java
/** F03.4 募集キャンセル料が免除された（受取先側の管理者または SYSTEM_ADMIN による）。 */
...
| 16 | i18n | `common.json` × 6 言語 | 免除操作・`UNCOLLECTIBLE` 表示の文言。**免除の確認 UI には §10.0 の 2 つの効果（債権の放棄・申込ブロックの解除が全体に及ぶこと）を明示する文言を含める** |
