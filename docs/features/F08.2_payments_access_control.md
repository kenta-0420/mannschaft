...
> **実装フェーズ**: Phase 3
> **最終更新**: 2026-06-03
> **関連ドキュメント**: [F08.7.1/07 大会費用支払い](F08.7.1_tournament_extensions/07_tournament_payment.md)（本決済基盤の `payment_items`/`member_payments`/Stripe Checkout＋MANUAL/`team_access_requirements`/grace_period/返金を**大会参加費として再利用**。大会側は薄い連結 `tournament_fee` のみ新設し、汎用決済基盤は新規構築しない）
>
> ⚠️ **会費徴収の統一決済基盤への移行予定（2026-06-03・マスター承認済）**: 会費・月謝の徴収（会員→チーム/組織）は、現状は **Mannschaft 自社の Stripe アカウントへプラットフォーム集金**する方式だが、将来 [F22.1 統一決済プラットフォーム（`ConnectChargeService`）](F22.1_market/payment/README.md) の **`source_kind=MEMBERSHIP`・即時モード（Stripe Connect ダイレクトチャージ）へ移行予定**（F22.1 ロードマップ P2-e）。移行後はチーム/組織が直接受領（自社口座非経由＝資金移動業リスク回避）し、手数料は **5% を支払者2.5%・受取側2.5%で折半**する。`payment_items`/`member_payments` と `escrow_transactions` のマッピングは P2-e 軍議で確定する。移行までは本書記載のプラットフォーム集金方式を正とする。
>
> 📘 **上位拡張設計**: 会費の「払い手≠受益者（後見つきマルチ受益者）」「年齢段階つき後見切替」「ペイウォール（受益者キー判定）」「継続/期別課金（Stripe Subscription＋invoice 固定手数料上書き／単発）」「協会→加盟チーム請求」「領収書・税からくり」は [F08.9 会員決済・後見つきマルチ受益者・ペイウォール・継続課金](F08.9_membership_billing_paywall/README.md) で設計する（本書を上位拡張）。本書の `member_payments` は **受益者キー**を維持し、F08.9 で `payer_user_id`（払い手）を分離する。
...
> **モジュール利用可否（`module_level_availability`）**: `payment` モジュールは組織・チームの両レベルで有効化できる（PERSONAL は対象外）。
> 当初のシード（`V2.027`）は「基本 ORGANIZATION=false」という機械的既定のまま `payment` を ORGANIZATION=0 で残しており、
> 組織 ADMIN がモジュールを有効化しようとすると `TMPL_005`（このレベルではモジュールを利用できません）で拒否され、
> 組織サイドバーの領収書導線が表示されなかった。`V208.20260911005751__enable_payment_module_for_organization_level.sql` で是正済み。
> `module_definitions.payment` は `requires_paid_plan = 0` のため、**無料機能として解放**される（有料プラン判定 `TMPL_004` には掛からない）。
...
### F09.15 連携: PaymentStatusChangedEvent の発行（2026-05-12 追記）

F09.15 区分所有者承継支援の 5 段階滞納エスカレーション基盤として、`member_payments` の `status` 遷移（特に `PAID` ↔ `PENDING` / 滞納検知）時に **`PaymentStatusChangedEvent`** を発行する。**スキーマ変更なし**（既存テーブルへのカラム追加は不要）。
...
public class PaymentStatusChangedEvent {
    ...
    private final PaymentStatus oldStatus;  // PAID / PENDING / REFUNDED / CANCELLED
    private final PaymentStatus newStatus;
    private final LocalDate validUntil;
    private final LocalDateTime occurredAt;
    private final boolean isDelinquent;     // valid_until + grace_period_days < CURDATE() のとき true
}
```

**発行タイミング**:
- `member_payments.status` が変化したとき（Webhook / 手動入力 / バッチクローズ）
- `valid_until` が経過し猶予期間も終わったとき（滞納化）— 日次バッチで検知発行

**購読側**: F09.15 `DelinquencyEscalationListener` が購読し、滞納初日を `delinquency_escalations.delinquency_started_at` にセット、5 段階バッチの起点とする。

**ドメイン境界遵守**: payment ドメインから succession ドメインへは Spring の `ApplicationEventPublisher` 経由で **非同期イベント** として伝達。両ドメインの `@Transactional` は独立に閉じる（CLAUDE.md 原則 5 準拠）。
...
| 日付 | 変更内容 |
|------|---------|
| 2026-05-31 | [F08.7.1/07 大会費用支払い](F08.7.1_tournament_extensions/07_tournament_payment.md) が本決済基盤（`payment_items`/`member_payments`/`stripe_customers`/Stripe Checkout＋MANUAL/`team_access_requirements`/`content_payment_gates`/grace_period/webhook/返金 REFUNDED・CANCELLED）を**大会参加費として再利用**する旨の相互参照を追加。大会側は薄い連結 `tournament_fee` のみ新設し、金額・通貨・Stripe Product/Price・grace_period は本書の `payment_items` 側で一元管理（二重管理しない）。未払いゲートは `team_access_requirements`／提出受理（F08.7.1/06）／エントリー確定に適用可。Stripe Connect による協会間自動分配は対象外（本書の DDL・API 変更なし） |
...
| 2026-09-11 | `payment` モジュールの組織レベル利用可否を是正（`module_level_availability` の ORGANIZATION を 1 に更新、Flyway `V208.20260911005751`）。組織の支払い機能は全層で実装済みだったがシードの機械的既定で有効化できず、組織 ADMIN が `TMPL_005` で弾かれていた。`requires_paid_plan = 0` のため無料機能として解放される。契約IT `PaymentModuleOrganizationAvailabilityIT` を追加 |
