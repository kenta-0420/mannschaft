...
| ADMIN | シフトスケジュールの CRUD・公開・確定・スロット管理・希望一覧参照・統計閲覧・**【v2】自動割当実行/確定/破棄・勤務制約管理・D&D 割当操作**・**【v2.1】変更依頼の審査（承認/却下）・目視確認承認・オープンコール候補選定** |
| DEPUTY_ADMIN | `MANAGE_SHIFTS` 権限を持つ場合: シフトスケジュールの CRUD・公開・確定・スロット管理・希望一覧参照・統計閲覧・**【v2】自動割当実行/確定/破棄・勤務制約管理・D&D 割当操作**・**【v2.1】変更依頼の審査（承認/却下）・目視確認承認・オープンコール候補選定**。自分自身が割り当てられた枠については MEMBER と同様に変更依頼・交代依頼も作成可能 |
| MEMBER | 希望シフトの提出（**【v2】5段階の希望強度**）・編集・取り下げ。確定シフトの閲覧。シフト交代リクエストの作成・引き受け。デフォルト可否プロファイルの管理。**【v2】自分の勤務制約の閲覧**。**【v2.1】確定前の変更依頼作成（自分に関する範囲）・公開後のオープンコール作成/手挙げ** |
| SUPPORTER | 確定シフトの閲覧のみ（希望提出不可・変更依頼/交代依頼不可） |
| GUEST | 対象外 |
