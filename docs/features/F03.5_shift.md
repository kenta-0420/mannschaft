...
| GET | `/api/v1/shifts/schedules/{id}/summary` | 必要 | 日付別・ポジション別の充足状況サマリー（ADMIN/DEPUTY_ADMIN のみ） |
| POST | `/api/v1/shifts/schedules/{id}/remind` | 必要 | 未提出者への手動リマインド送信（ADMIN/DEPUTY_ADMIN のみ、COLLECTING のみ） |
...
> Phase 11 第二陣 2-α（2026-05-17）で実装完了。`ShiftScheduleService.getScheduleSummary()` がスロット・確定アサイン・希望提出を集計し「日付 × ポジション」マトリクスを返す。実装上の注意: 現行 shift_requests テーブルは slot_id を NULL で扱う運用が多いため、ポジション単位の希望集計（`requested`）は v1 では `0` 固定とし、day-level の延べ件数のみ `total_requested` で返す。slot_id 必須化後の Phase 12 で改めて精密化予定。
...
> Phase 11 第二陣 2-α（2026-05-17）で実装完了。`ShiftPreferenceReminderBatchService.triggerManualReminder()` が cron バッチと共通の「未提出者抽出 → 通知一斉送信」ロジックを再利用しつつ、`AuditEventType.SHIFT_MANUAL_REMINDER_SENT` で監査ログを必ず残す。通知種別は `SHIFT_REQUEST_REMINDER_MANUAL`（48h / 24h と区別）。COLLECTING ステータスのみ実行可能、それ以外は `SHIFT_012`（409 Conflict）。
...
| 2026-05-17 | **Phase 11 第二陣 2-α 残課題 2 件実装**: ① `GET /api/v1/shifts/schedules/{id}/summary` を `ShiftScheduleService.getScheduleSummary()` で実装（日付 × ポジションのマトリクス。スロット・確定アサイン・希望提出を集計し `total_required` / `total_confirmed` / `total_requested` を返す。slot_id 必須化前の暫定として `requested` は 0 固定とし day-level 合計のみ提供） ② `POST /api/v1/shifts/schedules/{id}/remind` を `ShiftPreferenceReminderBatchService.triggerManualReminder()` で実装（cron バッチと共通の未提出者抽出ロジックを extract、`SHIFT_REQUEST_REMINDER_MANUAL` で通知、`AuditEventType.SHIFT_MANUAL_REMINDER_SENT` で監査ログ必須化、COLLECTING のみ実行可能で他ステータスは `SHIFT_012` 409） ③ GlobalExceptionHandler の ERROR_CODE_STATUS_MAP に `SHIFT_001`→404 / `SHIFT_012`→409 を明示マッピング追加 ④ AuditEventCategory に SHIFT を追加 ⑤ Controller 認可は `@PreAuthorize("hasRole('ADMIN')")` ⑥ 単体テスト 7 件（getScheduleSummary 3 件 + triggerManualReminder 4 件） + Controller MockMvc IT 6 件追加 |
