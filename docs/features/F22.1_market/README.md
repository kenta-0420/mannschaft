...
札の属性は混同しない。`owner_kind=PERSONAL/TEAM/ORGANIZATION`（札主区分）、`participation_type=INDIVIDUAL/TEAM`（応募形式）、`payee_kind=USER/TEAM/ORG`（謝礼受領者）は独立した軸である。
...
- 個人札の初期公開範囲は `FRIEND_TEAMS_ONLY` のみとし、公開市への掲載は後続 Phase 4 で公開範囲・モデレーションを確定してから扱う。
...
| 2 | 個人札主バックエンド | `PERSONAL`、本人固定の作成/編集/公開/取消/履歴/認可。決済は禁止 |
| 3 | 個人市 UI と公開市の札主区分表示・フィルタ | `/me/market`、マッチング状況、空状態、ページング |
| 4 | 個人向けテンプレート、公開範囲、通知、ペナルティ | 公開市掲載を含む拡張はこの Phase で判断・実装 |
...
既存 `/me/recruitment-listings` は応募・参加履歴であり、札主が立てた札の履歴へ意味を変えない。TEAM/ORGANIZATION の既存一覧・作成 API も後方互換で維持し、主体別管理市は追加エンドポイント/画面として導入する。未知の `owner_kind` は公開、作成、認可、決済の全経路で fail-closed とする。
