# F22.1 市（Market）謝礼決済 — 03. セキュリティ・コンプライアンス・未解決事項
...
| 返金 | **札主 scope ADMIN のみ** | `escrow.source` の scope 所有権検証（IDOR・§4） |
| エスクロー状態照会 | 札主 scope ADMIN ＋ **受領者本人** | 受領者は自分宛の払出のみ閲覧可。無関係 scope は 404 |
...
- **escrow_transaction の所有権検証**: `GET /escrow/{id}` / 返金は、`escrow.source_kind/source_id` から札を引き、その scope（team_id/organization_id）の所有権を `AccessControlService` で必ず照合。不一致は **404 秘匿**（存在を漏らさない）。
...
- **`application_fee_amount`（プラットフォーム手数料）のみが Mannschaft の収益**。これは収納代行手数料であり資金移動業の対象外。
...
- [x] **11. 手数料率（固定/率）** → Phase 2 後半は確定額を `application_fee_amount` に記録。率の確定は運用ポリシー（別途）。
- [x] **12. F13.1 との関係** → テーブル共有せず独立構築。`source_kind=JOBMATCHING` を確保し将来の流用余地を残す（README §5）。
- [x] **13. フリマ転用** → `source_kind=FLEAMARKET` を確保（フロー自体は別軍議）。
...
## 8. ステータス確定条件（🟢設計確定/実装未着手の根拠）

すべて充足済み。

- [x] マスター御裁可（受領者札ごと選択・案A）が DDL/API に反映されている（01 §3.2/§4.1・02 §2/§3/§5）。
- [x] §3' 未解決事項が全件解決方針確定（[x]）。税務のみ別建て論点として明示（[ ] のまま意図的に保留）。
- [x] DB 原則適合（01 §7）：クロスドメインFK禁止・CASCADE 同一ドメイン内・新規UUIDv7・テナント Repository。
- [x] PCI（SAQ-A）・Webhook 署名＋冪等・IDOR・GDPR/退会・資金移動業回避が網羅されている。
- [x] 既存 payment 資産の再利用点と Connect 増築点が明確（README §4・02 §8）。
- [x] F22.1 README §1 対応表の是正と相互参照追記が列挙されている（README §8）。
