...
- **返金時も自社口座を経由しない（支払者負担モデル・decouple・02 §6.1）**: 返金は **明示 TransferReversal（受取側 Connect 残高から支払者へ戻す額 R を巻き戻し）＋ `reverse_transfer:false` の Refund（支払者へ R を返金）** で行う。巻き戻し額＝返金額＝R を完全一致させることで **Mannschaft±0・受取側±0**（受取側が被る額＝支払者へ戻す額）を担保し、Mannschaft が立替・自社負担することはない。R は受取側の受取正味＝transferAmount ベース（支払上乗せ 2.5% は含まない）。`refund_application_fee:false` で徴収済み手数料（1.4%相当）も返さない（設定A）。比例 reverse（`reverse_transfer:true`）は送金巻き戻し額が返金額と一致せず Mannschaft 持ち出しになるため採用しない。
...
- **手数料の非返還（支払者負担モデル・02 §6.1）**: 「ご返金が発生した場合でも、**決済代行会社（Stripe）の決済手数料はご返金されません。ご返金額は決済手数料を差し引いた額（受取側が実際に受け取った額）となります**」。つまり全額返金でも戻るのは受取側が受け取った正味（額面 10,000 円 → 9,750 円）であり、支払上乗せ手数料（2.5%）と決済手数料は支払者の負担となる。
...
- 「※お支払い合計には決済手数料が含まれます」「※ご返金が発生した場合でも、決済代行会社（Stripe）の決済手数料はご返金されません」「※ご返金額は決済手数料を差し引いた額（受取側が実際に受け取った額）となります」を**内訳ボックス直下に併記**する。
- これらの文言は i18n 6言語で管理（直書き禁止・04 §6 `market.payment.breakdown.{includesFeeNote,nonRefundableNote,refundExclFeeNote}`）。
