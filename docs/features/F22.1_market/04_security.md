...
## 6. 追加仕様: 主体別管理市の認可・プライバシー（Phase 0）

個人札の作成・編集・取消・履歴・マッチングは認証済み本人に自己スコープで束縛し、任意の user ID を受け付けない。TEAM/ORGANIZATION は既存の ADMIN / `MANAGE_RECRUITMENTS` 保有 DEPUTY_ADMIN の認可経路を変えず、別主体の URL・札 ID は 403 または存在秘匿 404 とする。未ログインは `/market` の公開閲覧だけを許可し、管理市・作成・履歴・マッチングは 401 とする。
