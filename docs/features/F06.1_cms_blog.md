...
- **メディアサイズ上限（容量課金上限は設けず、UX ガードのみ）**:
  ...
  - **技術的下限**: 単発 PUT は 100MB まで。100MB〜1GB のファイルは F05.5 の Multipart Upload フロー（`POST /api/v1/files/multipart/start`、`target_prefix = blog/...`）を利用してアップロードし、完了後に返却された `file_key` を本 `blog_media_uploads` に登録する
