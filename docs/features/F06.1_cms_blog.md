...
- **body サイズ制限**: `body` の最大文字数は **50,000文字**。超過時は 400 エラー。画像・動画は R2 の配信 URL を参照するため body に含まれるのは Markdown テキスト（`![alt](url)` / `<video src="url">` 等）のみ
...
**制約・備考**
- メディアアップロード時に `blog_post_id = NULL` で INSERT。記事保存時に `body` 内の R2 配信 URL を解析し、該当レコードの `blog_post_id` を更新
