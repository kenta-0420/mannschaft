...
| `blog_image_uploads` | ブログ画像・動画管理（R2 アップロード追跡・孤立ファイルクリーンアップ用。**将来的に `blog_media_uploads` へのリネーム提案あり**） | なし |
...
> **命名に関する提案**: 動画対応に伴いテーブル名は `blog_media_uploads` のほうが意味的に正確だが、既存マイグレーションとの互換性のため本 Phase ではリネームしない。`media_type` カラムで画像・動画を区別し、カラム命名のみ `s3_key` → `file_key` のリネーム案を将来検討する（本 Phase では維持）。
...
| `s3_key` | VARCHAR(500) | NO | — | **R2 オブジェクトキー**（カラム名は歴史的経緯で `s3_key` を維持） |
| `thumbnail_s3_key` | VARCHAR(500) | YES | NULL | 動画のポスターフレームサムネイル R2 オブジェクトキー（VIDEO のみ。Cloudflare Workers で非同期生成） |
...
| `processing_status` | ENUM('PENDING', 'PROCESSING', 'READY', 'FAILED') | NO | 'READY' | IMAGE は即時 READY。VIDEO はアップロード直後 PENDING → Workers 実行中 PROCESSING → 完了で READY に遷移（F04.1/F06.2 と同じ ENUM 値） |
| `created_at` | DATETIME | NO | CURRENT_TIMESTAMP | |

**インデックス**
```sql
INDEX idx_biu_post (blog_post_id)                          -- 記事別メディア一覧
INDEX idx_biu_orphan (blog_post_id, created_at)            -- 孤立メディアクリーンアップ用
UNIQUE KEY uq_biu_s3_key (s3_key)                          -- R2 キー一意性
```
...
- **メディアサイズ上限**:
  - 画像: 1枚あたり最大 **10MB**
  - 動画: 1本あたり最大 **300MB**（ブログは長文記事のため F04.1 タイムライン（200MB）より緩め。F06.2 ギャラリー（500MB）よりは抑える）
- 動画対応フォーマット: MP4 (H.264 + AAC) 推奨、WebM (VP9)、MOV（QuickTime）
- クォータ計上: IMAGE / VIDEO ともに `file_size` 分を `StorageQuotaService` 経由で記事スコープ（TEAM / ORGANIZATION / PERSONAL）に計上
...
blog_posts          (1) ──── (N) blog_image_uploads (R2 画像・動画管理)
...
■ 動画挿入:
   - ツールバーの動画ボタン or ドラッグ＆ドロップで Cloudflare R2 に Presigned PUT URL でアップロード
   - アップロード完了後、`<video src="{Workers 配信 URL}" controls poster="{サムネイル URL}"></video>` を自動挿入
   - Cloudflare Workers が非同期でポスターフレーム抽出を行い、完了後に `poster` 属性が有効化される（完了前は poster 未指定でもブラウザ側で自動生成される）
   - 1記事あたり動画は最大3本まで
```
...
- 処理:
  ...
  2. 対象レコードの s3_key（R2 オブジェクトキー）で R2 オブジェクトを削除
     - VIDEO の場合は thumbnail_s3_key も同時に削除
...
- 補足:
  ...
  - 記事保存時に body 内の R2 配信 URL（画像の `![](url)`、動画の `<video src="url">`）を解析し、
    blog_image_uploads.blog_post_id を紐付ける
...
- 処理:
  ...
  3. blog_image_uploads の関連メディアを R2 から削除 + StorageQuotaService.recordDeletion()
  4. 物理 DELETE 実行
- 補足:
...
V6.006__create_blog_image_uploads.sql          -- blog_image_uploads テーブル作成（画像・動画管理）
...
V6.024__add_blog_image_uploads_video_columns.sql  -- blog_image_uploads に media_type / thumbnail_s3_key / duration_seconds / processing_status カラム追加 + file_size を INT → BIGINT に拡張（動画300MB 対応）
```
...
- `V6.024` は `blog_image_uploads` テーブルへの ALTER。F06.2 ギャラリー（V6.016〜V6.023 使用中）・F06.4 活動記録（V6.010〜V6.015 使用中）との衝突を避けて V6.024 を採番。既存レコードは `media_type = 'IMAGE'` / `processing_status = 'READY'` がデフォルト適用され影響なし。本番稼働中の環境向けの ALTER 移行用。新規環境で V6.006 を新バージョン用に置き換える場合はこのスクリプトは不要
