...
※ タグ管理の実装粒度は表の記載より細かい。`BlogTagService`（`listTags`/`createTag`）の実測では、タグの**一覧・作成**は
`checkMembership` のみで MEMBER にも開放されている（タグはチーム/組織スコープの共有語彙であり、記事の作成者に紐付かない）。
**更新・削除**のみ `checkScopeAdmin`（ADMIN/DEPUTY_ADMIN）に限定される（`blog_post_tags` の外部キーが
`ON DELETE CASCADE` のため、タグ削除は他人の記事からもタグ付けを消す破壊的操作になる）。
CMP-260917-0041（`/admin/blog-management` の `BlogPostList.vue` への機能移植）で実測・FE 反映済み。
...
| `blog_media_uploads` | ブログ画像・動画管理（R2 アップロード追跡・孤立ファイルクリーンアップ用。旧名 `blog_image_uploads` から V6.025 でリネーム済み） | なし |
...
| `cover_image_url` | VARCHAR(500) | YES | NULL | カバー画像 URL（Cloudflare R2 / Workers 経由の配信 URL を保存） |
| `post_type` | VARCHAR(15) | NO | 'BLOG' | 記事種別。値: `BLOG`, `ANNOUNCEMENT` |
| `visibility` | VARCHAR(30) | NO | 'MEMBERS_ONLY' | 公開範囲。値: `PUBLIC`, `MEMBERS_ONLY`, `SUPPORTERS_AND_ABOVE`, `MEMBERS_AND_ABOVE`, `ADMINS_AND_ABOVE`, `SCOPE_AFFILIATED`, `FOLLOWERS_ONLY`（個人ブログ: フォロワー限定）, `PRIVATE`（個人ブログ: 非公開・下書き保管）。可視性ラダー統一(#1341)で新ラダー値名（`MEMBERS_AND_ABOVE`/`ADMINS_AND_ABOVE`/`SCOPE_AFFILIATED`）を追加。`MEMBERS_ONLY` は既存互換のため残置し `MEMBERS_AND_ABOVE` と同一の可視範囲へ写像される（`CmsVisibilityMapper`） |
...
- **非公開化の不変条件（issue #2616）**: `DRAFT` へ戻す遷移では、既に到来済み（`published_at <= NOW()`）の公開日時を **NULL に破棄する**。バッチの走査条件が「`DRAFT` かつ `published_at <= NOW()`」である以上、過去の公開日時を残したまま `DRAFT` にすると「意図的に非公開へ戻した記事」と「公開時刻に達した予約記事」が区別できず、非公開化した記事が毎分のバッチに勝手に再公開される。`BlogPostEntity#unpublish()`（`changeStatus(DRAFT)` からも委譲）が遷移側でこれを保証するため、API 経由の非公開化・リビジョン復元など経路が増えても漏れない。未来の `published_at` は予約設定そのものなので維持する。`ARCHIVED` など `DRAFT` 以外への遷移では公開履歴として `published_at` を保持する（バッチは `DRAFT` しか走査しないため再公開の危険はない）
...
- **body サイズ制限**: `body` の最大文字数は **50,000文字**。超過時は 400 エラー。body に含まれるのは Markdown テキストのみ
- **body に保存するのは URL ではなく R2 オブジェクトキー（r2Key）**: 画像・動画は `![alt](blog/TEAM/12/xxx.png)` / `<video src="blog/TEAM/12/xxx.mp4">` の形で **`blog/{scopeType}/{scopeId}/` 始まりの生キー**を保存する（先頭スラッシュなし）。配信は署名 URL（presigned URL）に統一しており、署名 URL には有効期限があるため body へ焼き込むことはできない。**取得時に `BlogBodyMediaResolver` が署名 URL へ解決する**
  - 解決する経路（表示）: `BlogPostService#getBySlug` / `#getBySlugWithPreviewToken` / `PublicPostQueryService`（公開・未認証詳細）
  - **解決しない経路（編集）**: `BlogPostService#getMyPostById`。編集画面へ署名 URL を渡すと保存時に期限付き URL が body へ永続保存され、数十分後に画像が恒久的に壊れるため
  - **越境防止の二段関門**: body は利用者が自由に編集できるため、他スコープの r2Key を手書きされても presign しないよう ①投稿自身の `blog/{scopeType}/{scopeId}/` 配下かをセグメント単位で厳密照合（`../` 脱出・`TEAM/12`→`TEAM/123` の前方一致誤判定を拒否）②`blog_media_uploads` に当該 `s3_key` が実在するか照合（1本文1クエリ・照会失敗時は fail-closed）の両方を通過したキーのみを解決対象とする
...
ブログ記事に挿入された画像・動画の管理テーブル。**Cloudflare R2** にアップロードされたメディアを追跡し、孤立メディアのクリーンアップに使用する。

> **テーブル名**: 旧名 `blog_image_uploads` は画像専用を示唆していたが、動画対応に合わせて `blog_media_uploads` にリネーム済み（V6.025 `RENAME TABLE blog_image_uploads TO blog_media_uploads`）。カラム命名は `s3_key` を歴史的経緯で維持するが、格納値は Cloudflare R2 オブジェクトキーである。カラム単位のリネーム（`s3_key` → `file_key` 等）は将来タスクとして別 Phase で検討する。
...
| `media_type` | ENUM('IMAGE', 'VIDEO') | NO | 'IMAGE' | メディア種別。既存レコードはすべて `IMAGE` |
| `s3_key` | VARCHAR(500) | NO | — | **R2 オブジェクトキー**（カラム名は歴史的経緯で `s3_key` を維持。単一バケット `mannschaft-storage` 内の `blog/` プレフィックス配下） |
| `thumbnail_r2_key` | VARCHAR(500) | YES | NULL | 動画のポスターフレームサムネイル R2 オブジェクトキー（VIDEO のみ。ハイブリッド方式で生成: 動画長 5 分以下は Cloudflare Workers が `thumbnails/` プレフィックスに書き出し、5 分超はクライアント側 Canvas 抽出を事前実行。S3 名残を避けるため新規カラム名は `thumbnail_r2_key` を採用） |
| `file_size` | BIGINT UNSIGNED | NO | — | ファイルサイズ（bytes）。動画対応のため INT → BIGINT に拡張 |
| `content_type` | VARCHAR(50) | NO | — | MIME タイプ。IMAGE: `image/jpeg`, `image/png`, `image/webp` 等 / VIDEO: `video/mp4`, `video/webm`, `video/quicktime`（iPhone で撮影した MOV 動画のアップロード対応） |
| `duration_seconds` | INT UNSIGNED | YES | NULL | 動画再生時間（秒。VIDEO のみ）。5 分（300 秒）を境にサムネイル生成経路を Workers / クライアントで振り分ける |
| `processing_status` | ENUM('PENDING', 'PROCESSING', 'READY', 'FAILED') | NO | 'READY' | IMAGE は即時 READY。VIDEO はアップロード直後 PENDING → Workers 実行中 PROCESSING → 完了で READY に遷移（F04.1/F06.2 と同じ ENUM 値）。FAILED 時はフロントでプレースホルダー画像を表示 |
| `created_at` | DATETIME | NO | CURRENT_TIMESTAMP | |

**インデックス**
```sql
INDEX idx_bmu_post (blog_post_id)                          -- 記事別メディア一覧
INDEX idx_bmu_orphan (blog_post_id, created_at)            -- 孤立メディアクリーンアップ用
UNIQUE KEY uq_bmu_s3_key (s3_key)                          -- R2 キー一意性
```

**制約・備考**
- メディアアップロード時に `blog_post_id = NULL` で INSERT。記事保存時に `body` 内の R2 オブジェクトキー（r2Key）を解析し、該当レコードの `blog_post_id` を更新
- 本テーブルは表示時の**認可台帳**も兼ねる。`BlogBodyMediaResolver` が body 中の r2Key を署名 URL へ解決する際、`s3_key` の実在をここで照合し、アップロード経路を通っていない捏造キーを弾く（`findByS3KeyIn` で本文 1 件につき 1 クエリ）
- **孤立メディアクリーンアップ**: `blog_post_id IS NULL AND created_at < NOW() - INTERVAL 72 HOUR` のレコードを日次バッチで R2 削除 + 物理削除 + クォータ減算
- 記事編集でメディアを本文から除去した場合、次回保存時に `blog_post_id` を NULL にリセット → 72時間後にクリーンアップ対象
- **1記事あたりの上限**:
  - 画像: 最大 **30枚**
  - 動画: 最大 **3本**（記事内に埋め込める動画数。視聴体験の観点から制限）
- **メディアサイズ上限（容量課金上限は設けず、UX ガードのみ）**:
  - 個々のファイルに対する容量課金上限は **設けない**（スコープ容量枠の範囲内なら自由）
  - **UX ガード**: 1 本あたり **1GB** を上限とする。これは容量課金ではなく **記事読み込み UX 観点** のソフトリミット（ブラウザが大容量動画をストリーム読み込みする際のメモリ負荷・初回表示レイテンシ対策）。1GB を超える動画は F05.5 ファイル共有にアップロードし、ブログ記事からは URL リンクで誘導する運用を推奨
  - **技術的下限**: 単発 PUT は 100MB まで。100MB〜1GB のファイルは F05.5 の Multipart Upload フロー（`POST /api/v1/files/multipart/start`、`target_prefix = blog/...`）を利用してアップロードし、完了後に返却された `file_key` を本 `blog_media_uploads` に登録する。**F05.5 Multipart API 呼び出し前に F06.1 側でブログ編集権限（スコープへのアクセス権）を確認すること（認可責任: 呼び出し元責任 — 案B 確定）**
- 動画対応フォーマット: MP4 (H.264 + AAC) 推奨、WebM (VP9)、MOV（QuickTime）
- **動画サムネイル生成（ハイブリッド方式）**: 詳細は `docs/db_design_details.md` の「サムネイル生成戦略」セクションを参照
  - `duration_seconds <= 300`（5 分以下）: Cloudflare Workers + ffmpeg.wasm が非同期にポスターフレーム抽出 → `thumbnails/{元のパス}.jpg` に書き出し → `thumbnail_r2_key` を UPDATE
  - `duration_seconds > 300`（5 分超）: クライアント側で `<video>` + Canvas 抽出を事前実行 → 本体アップロード前にサムネイルを先に PUT → バックエンドは HeadObject で両方の実在を確認
  - 失敗時はフロントエンドで汎用動画アイコンのプレースホルダー画像を表示
- **画像最適化**: IMAGE は Cloudflare Images に登録して記事本文内で WebP 変換・リサイズ配信（`https://imagedelivery.net/{account_hash}/{image_id}/{variant}`）を利用する選択肢あり（Phase 6 で段階導入）
- クォータ計上: IMAGE / VIDEO ともに `file_size` 分を `StorageQuotaService` 経由で記事スコープ（TEAM / ORGANIZATION / PERSONAL）に計上。サムネイル（`thumbnails/` 配下）は非計量
...
blog_posts          (1) ──── (N) blog_media_uploads (R2 画像・動画管理)
...
| POST | `/api/v1/blog/media/upload-url` | 必要 | 本文埋め込み用の画像・動画の R2 Presigned PUT URL 発行（`media_type`: IMAGE / VIDEO）|
...
>
> 2026-05-17 時点で F06.1 (CMS) は活動記録機能を扱いません。Phase 6 リファクタで `activity_results` / `activity_templates` / `activity_template_fields` テーブル群と関連 API は F06.4 に移管済みです。本セクションに以前掲載していた以下の旧 API は次の経路で代替されます:
>
> - `/api/v1/activities/custom-fields/*` 系 — F06.4 の `activity_templates` 内フィールド定義 (`activity_template_fields`) へ統合（テンプレート単位でカスタムフィールドを定義する設計）
> - `/api/v1/activities/stats/members/{userId}` — F06.4 `GET /api/v1/activities/stats?participant_user_id=...` で代替
> - `/api/v1/activities/stats/ranking` — F06.4 `GET /api/v1/activities/stats/fields` で代替（`aggregatable` フィールドの集計）
> - `/api/v1/activities/templates/*` — F06.4 `/api/v1/activity-templates/*` に URL リファクタ
> - `/api/v1/activities/templates/{id}/share` — 廃案。スコープ越境コピーは F06.4 `/api/v1/activity-templates/{id}/duplicate` で実現
> - `/api/v1/activities/templates/official` / `/import` — F06.4 `/api/v1/system-admin/activity-template-presets` + `/api/v1/activity-templates/import-preset` で再設計
> - `/api/v1/activities/generate-from-schedule` — v1 スコープ外（スケジュール → 活動記録の自動生成は F03 ドメインとの将来連携で別途軍議が必要）

#### 活動記録テンプレート

> 上記「活動記録」と同様に F06.4 へ移管済みです。
> 詳細は [docs/features/F06.4_activity_records.md](./F06.4_activity_records.md) §4「エンドポイント一覧」の
> 「テンプレート管理（ADMIN）」「プリセットテンプレート（SYSTEM_ADMIN）」を参照してください。
...
```json
{
  ...
  "cover_image_url": "https://cdn.example.com/blog/cover-123.jpg",
  "post_type": "BLOG",
  "visibility": "PUBLIC",
  "priority": "NORMAL",
  "tag_ids": [1, 3, 5],
...
```json
{
  ...
  "cover_image_url": "https://cdn.example.com/activities/cover-456.jpg",
  "participants": [
    {
      "user_id": 10,
...
- 非同期ジョブのダウンロードリンクは Cloudflare R2 Presigned GET URL（有効期限: 24時間、一時プレフィックスに置かれ7日後に R2 Object Lifecycle で自動削除）
...
■ ツールバーボタン一覧:
   ...
   | 番号付きリスト | `1. ` |
   | リンク | `[テキスト](URL)` |
   | 画像 | `![代替テキスト](URL)` ※ R2 アップロード連携 |
   | 動画 | `<video src="URL" controls></video>` ※ R2 アップロード連携。**Markdown 標準にない専用 HTML タグ**を許可し、DOMPurify のホワイトリストで `<video>` `<source>` `controls` `poster` 等を許容する |
   | 引用 | `> ` |
   | コードブロック | ``` ` ``` |
   | 水平線 | `---` |
...
■ 画像挿入:
   - ツールバーの画像ボタン or ドラッグ＆ドロップで **Cloudflare R2 に Presigned PUT URL でアップロード**
   - アップロード完了後、`![ファイル名](Workers 経由の配信 URL)` を自動挿入
   - バックエンドは `POST /api/v1/blog/media/upload-url`（下記）で Presigned URL を発行

■ 動画挿入（ハイブリッドサムネイル方式）:
   - ツールバーの動画ボタン or ドラッグ＆ドロップで動画を選択
   - ブラウザが `<video>` タグで動画メタデータを読み込み、`duration` を判定:
     * 動画長 ≤ 300 秒（5 分以下）: Cloudflare R2 に本体のみ Presigned PUT URL でアップロード → バックエンドが Cloudflare Workers + ffmpeg.wasm を非同期起動してポスターフレームを `thumbnails/` プレフィックスに書き出し
     * 動画長 > 300 秒（5 分超）: クライアント側で Canvas によりフレーム抽出 → サムネイル（JPEG）を先に R2 の `thumbnails/{動画のオブジェクトキー}.jpg` に Presigned PUT → 続けて動画本体を R2 にアップロード
   - いずれのフローでも 100MB 超の動画は F05.5 の Multipart Upload フロー（`POST /api/v1/files/multipart/start`、`target_prefix = blog/{scope_type}/{scope_id}/post-{postId}/`）を利用
   - アップロード完了後、`<video src="{Workers 配信 URL}" controls poster="{サムネイル URL}"></video>` を自動挿入
   - サムネイル生成失敗時は `poster` を未指定にし、ブラウザ側のフォールバック（最初のフレーム表示 or プレースホルダー画像）に任せる
   - 1記事あたり動画は最大3本まで、1本あたり 1GB（UX ガード）まで
   - サムネイル生成戦略の詳細は `docs/db_design_details.md` 参照
```
...
- 失敗ハンドリング:
  ...
  - 失敗回数は Valkey（`blog:schedule_retry:{postId}`、TTL 1時間）で管理
```

**実装状況（issue #2616・2026-08-12 時点）**

| 項目 | 状態 | 実体 |
|---|---|---|
| 処理①（予約公開） | ✅ 実装済み | `BlogScheduledPublishBatchService`（宣言・1分間隔・ShedLock `blogScheduledPublishBatch`）＋ `BlogScheduledPublishService`（抽出・1件ごと `REQUIRES_NEW` で遷移）。走査索引は `idx_bp_status_published_at`（V181） |
| 未来 `published_at` 指定時の据え置き | ✅ 実装済み | `BlogPostEntity#publish` が未来なら `DRAFT` のまま `published_at` だけ保持する。単票公開・一括 PUBLISH・セルフレビュー公開の3経路すべてが同じ判定を通る |
| 予約公開待ち記事の共有ガード | ✅ 実装済み | `BlogPostShareService#sharePost` が予約中（DRAFT＋未来 `published_at`）の共有を `CMS_026`（409）で拒否する。素の下書き（`published_at` なし）の共有は従来どおり許可 |
| 処理②（予約アーカイブ `archive_at`） | ⛔ 未実装 | issue #2616 のスコープ外。別途起票 |
| 3回連続失敗時の Valkey リトライ台帳 | ⛔ 未実装 | 同上。現状は失敗を `log.error` に残し、次回起動で再試行する（遷移条件は時刻経過のため自己修復する） |

> **`PostStatus.SCHEDULED` は新設しない。** 予約中は `DRAFT` のままであり、公開系クエリ
> （`status = PUBLISHED` の等値判定）は一切変更していない。予約中記事は「まだ DRAFT だから
> 公開系に出ない」という構造的な理由で漏れない（§155 / §949 / §2023 と同じ前提）。
...
- 実行間隔: 日次（深夜3:00）
- 処理:
  1. SELECT * FROM blog_media_uploads WHERE blog_post_id IS NULL AND created_at < NOW() - INTERVAL 72 HOUR
  2. 対象レコードの s3_key（R2 オブジェクトキー）で R2 オブジェクトを削除
     - VIDEO の場合は thumbnail_r2_key も同時に削除（R2 の `thumbnails/` プレフィックス配下）
  3. blog_media_uploads から物理削除
  4. StorageQuotaService.recordDeletion() で使用量減算
  5. 処理件数をログ出力
- 補足:
  - 72時間の猶予は、エディタでのメディアアップロード→記事保存までの十分な余裕を確保するため
  - 記事保存時に body 内の R2 配信 URL（画像の `![](url)`、動画の `<video src="url">`）を解析し、
    blog_media_uploads.blog_post_id を紐付ける
  - 記事編集でメディア参照を削除した場合、次回保存時に blog_post_id を NULL にリセット
```
...
- 処理:
  ...
  3. blog_media_uploads の関連メディアを R2 から削除 + StorageQuotaService.recordDeletion()
  4. 物理 DELETE 実行
- 補足:
...
- **XSS 対策**: `body` は Markdown テキストとして DB に保存し、HTML 変換はフロントエンドで実施する。レンダリングパイプライン: `markdown-it`（Markdown → HTML）→ `DOMPurify`（許可タグのホワイトリスト方式でサニタイズ。`<video>`, `<source>`, `controls`, `poster`, `src`, `preload` を許容）→ DOM 挿入。バックエンドは生 Markdown を保存するのみで HTML 変換は行わない
- **動画 src URL のバリデーション**: `<video>` タグの `src` / `<source>` の `src` は自ドメインの Cloudflare Workers 配信 URL のみ許可（DOMPurify のカスタム URL バリデータで外部 URL を拒否）。外部動画の埋め込みは F04.1 の VIDEO_LINK 同様に oEmbed を使う将来拡張候補
- **slug インジェクション防止**: slug は `[a-z0-9-]` のみ許可。最大200文字
- **カバー画像・本文埋め込みメディア**: Cloudflare R2 Presigned URL でアップロード。URL は Cloudflare Workers 経由の自ドメイン配信のみ許可（ドメインバリデーションで外部 URL を拒否）
- **レートリミット**: 記事作成 API に `Bucket4j` で 1分間に10回の制限。`POST /api/v1/blog/media/upload-url` は 1分間に20回（画像・動画の多重アップロード対応）
...
V6.006__create_blog_image_uploads.sql          -- blog_image_uploads テーブル作成（画像・動画管理。V6.025 で blog_media_uploads にリネーム）
V6.007__alter_blog_posts_add_personal.sql     -- blog_posts に user_id, social_profile_id, self_review_deadline カラム追加 + 3-way XOR 制約更新 + 個人ブログ用インデックス追加
V6.008__create_blog_post_shares.sql           -- blog_post_shares テーブル作成（個人ブログ共有）
V6.009__create_user_blog_settings.sql         -- user_blog_settings テーブル作成（セルフレビュー設定）
V6.024__add_blog_image_uploads_video_columns.sql  -- blog_image_uploads に media_type / thumbnail_r2_key / duration_seconds / processing_status カラム追加 + file_size を INT → BIGINT に拡張（動画対応・スコープ容量枠の範囲内で自由）
V6.025__rename_blog_image_uploads_to_blog_media_uploads.sql  -- blog_image_uploads → blog_media_uploads にテーブルリネーム（画像専用を示唆する旧名を動画対応に合わせて変更）
```

**V6.025 の DDL 例**:

```sql
-- V6.025__rename_blog_image_uploads_to_blog_media_uploads.sql
-- テーブル名を画像専用を示唆する blog_image_uploads から、動画も含む blog_media_uploads にリネームする
RENAME TABLE blog_image_uploads TO blog_media_uploads;

-- インデックス名も新規則にリネーム（任意。既存のインデックス名はリネーム不要だが、
-- 一貫性を優先するなら以下を実行）
ALTER TABLE blog_media_uploads RENAME INDEX idx_biu_post TO idx_bmu_post;
ALTER TABLE blog_media_uploads RENAME INDEX idx_biu_orphan TO idx_bmu_orphan;
ALTER TABLE blog_media_uploads RENAME INDEX uq_biu_s3_key TO uq_bmu_s3_key;
```

> **旧活動記録テーブル（V6.007-V6.014）は F06.4_activity_records.md に移管済み**。F06.4 の V6.010-V6.015 を参照。
...
- `blog_image_uploads`（後の `blog_media_uploads`）は `blog_posts` に依存するため V6.002 より後に作成（V6.006）
...
- `V6.007` は既存の `blog_posts` テーブルの ALTER。既存の 2-way XOR 制約 `chk_bp_scope` を DROP → 3-way XOR で再作成。`social_profile_id` の FK は `user_social_profiles`（F04.1 の V4.005 で作成済み）を参照
- `V6.008` の `blog_post_shares` は `blog_posts` に依存するため V6.007 より後に作成
- `V6.024` は `blog_image_uploads` テーブル（当時はまだリネーム前）への ALTER。F06.2 ギャラリー（V6.016〜V6.023 使用中）・F06.4 活動記録（V6.010〜V6.015 使用中）との衝突を避けて V6.024 を採番。既存レコードは `media_type = 'IMAGE'` / `processing_status = 'READY'` がデフォルト適用され影響なし。本番稼働中の環境向けの ALTER 移行用。新規環境で V6.006 を新バージョン用に置き換える場合はこのスクリプトは不要
- `V6.025` は `blog_image_uploads` → `blog_media_uploads` のテーブルリネーム。MySQL の `RENAME TABLE` は原子的に実行されるためダウンタイムは最小限。既存環境では V6.024 実行後に V6.025 を適用する順序を厳守（V6.024 で新カラムを追加してから V6.025 でリネーム）。新規環境で V6.006 を最初から `blog_media_uploads` として作成する場合は V6.025 は冪等性を保つために `IF EXISTS` 付きで書くか、V6.025 自体を no-op にする運用を検討する。アプリケーション層のエンティティクラス名（Java `BlogMediaUpload`）と JPA `@Table(name = "blog_media_uploads")` もマイグレーション実行と同タイミングでデプロイすること

---

## 7.1 お知らせウィジェット連携（F02.6）

チーム/組織ブログ記事（`team_id IS NOT NULL` または `organization_id IS NOT NULL`）は、作成・編集画面で「お知らせウィジェットに表示する」チェックを入れることで、ダッシュボード上部の横断お知らせウィジェット（F02.6）に表示できる。

### 表示フラグの扱い

- `blog_posts` テーブルには **専用カラムを追加しない**（F02.6 案B採用）。チェック状態は `announcement_feeds` テーブルに行が存在するかどうかで判定
- 編集画面を開いた際、フロントエンドは `GET /api/v1/teams/{teamId}/announcements?source_type=BLOG_POST` で自記事の既存登録を確認し、チェック状態を復元
- 公開状態（`status = PUBLISHED`）の記事のみお知らせ化可能。`DRAFT` / `PENDING_REVIEW` / `PENDING_SELF_REVIEW` のままお知らせ化しようとすると 400 エラー
- **個人ブログ（`user_id IS NOT NULL`）とソーシャルプロフィール投稿（`social_profile_id IS NOT NULL`）はお知らせ化対象外**（F02.6 の 6.1 IDOR 対策で拒否）

### 優先度のマッピング

ブログ記事の `priority` カラム（`NORMAL` / `IMPORTANT` / `CRITICAL`）は、お知らせウィジェットでは以下にマッピングされる:

| blog_posts.priority | announcement_feeds.priority |
|---|---|
| `CRITICAL` | `URGENT` |
| `IMPORTANT` | `IMPORTANT` |
| `NORMAL` | `NORMAL` |
...
- 記事の論理削除（`deleted_at` セット）時、`BlogPostDeletedEvent` を発行 → `AnnouncementSyncEventListener` が `announcement_feeds.source_deleted_at` をセット（ウィジェット表示から除外）
- 記事の更新（タイトル・excerpt・visibility・priority）時、`BlogPostUpdatedEvent` を発行 → `AnnouncementSyncEventListener` が `title_cache` / `excerpt_cache` / `visibility` / `priority` を同期更新
- `PUBLISHED` → `DRAFT` への遷移はブログ側で禁止されているため考慮不要。`PUBLISHED` → `ARCHIVED` の場合は `announcement_feeds` を物理削除

### UI 配置

`BlogPostForm.vue` の公開設定セクション配下に `AnnouncementToggle.vue` コンポーネントを配置。チェック ON 時に表示期限（任意）の日時ピッカーがスライドダウン表示される。ADMIN の場合のみピン留めオプションも表示。
...
| 2026-03-18 | ステータスを 🟢 設計完了 に変更。タイトルを「F06.1: CMS（ブログ・お知らせ）」に修正（活動記録は F06.4 に分離済み）。Flyway リナンバリング: 旧活動記録テーブル（V6.007-V6.014）を削除し F06.4 に移管、個人ブログ V6.015-V6.017 → V6.007-V6.009 に繰り上げ |
| 2026-04-11 | Cloudflare R2 移行 + ブログ記事への動画埋め込み対応: (1) ストレージ基盤を AWS S3 → Cloudflare R2 に全面差し替え（blog_image_uploads の `s3_key` カラム名は互換性のため維持。値は R2 オブジェクトキー）(2) `blog_image_uploads` に `media_type` ENUM('IMAGE', 'VIDEO') + `thumbnail_s3_key` + `duration_seconds` + `processing_status` カラム追加 + `file_size` を INT → BIGINT に拡張 (3) テーブル名を `blog_media_uploads` にリネームする提案を明記（本 Phase では互換性重視で見送り）(4) 本文への動画埋め込み方針: Markdown 中に `<video src="..." controls poster="..."></video>` タグを許可し、DOMPurify のホワイトリストで `<video>` `<source>` を許容 (5) 新 API `POST /api/v1/blog/media/upload-url`（媒体別の R2 Presigned PUT URL 発行）追加 (6) サイズ上限: 画像10MB / 動画300MB、1記事あたり画像30枚 + 動画3本 (7) 動画サムネイルは Cloudflare Workers で非同期にポスターフレーム抽出 (8) `<video src>` のドメインバリデーションで外部動画埋め込みを拒否 (9) 非同期ジョブダウンロードを R2 Presigned GET URL に変更 (10) Flyway V6.024 追加（F06.2/F06.4 の既存番号衝突回避） |
| 2026-04-17 | F02.6 お知らせウィジェット連携セクション追加: チーム/組織ブログ記事の「お知らせ表示」フラグは `announcement_feeds` テーブルで管理（blog_posts 側に新規カラム追加なし）。priority マッピング（CRITICAL→URGENT / IMPORTANT→IMPORTANT / NORMAL→NORMAL）、個人ブログ・ソーシャルプロフィール投稿は対象外、連動削除・同期は ApplicationEvent 経由 |
| 2026-04-11 | R2 移行 第2弾 — 上限緩和・テーブルリネーム・ハイブリッドサムネイル確定: (1) **テーブル名 `blog_image_uploads` → `blog_media_uploads` リネーム** を確定（V6.025 `RENAME TABLE` + インデックス名リネーム） (2) **`thumbnail_s3_key` カラムを `thumbnail_r2_key` にリネーム**（S3 名残削除。新規カラムのため互換性問題なし）(3) **動画上限を 300MB → 1GB に緩和**（容量課金ではなく記事読み込み UX 観点のガード。1GB 超は F05.5 へ誘導）(4) **画像 10MB の上限を撤廃**（スコープ容量枠の範囲内なら自由）(5) 技術的下限として単発 PUT 100MB を明記、100MB〜1GB は F05.5 の Multipart Upload フロー（`target_prefix = blog/...`）を利用 (6) **サムネイル生成をハイブリッド方式に確定**: 動画長 5 分以下は Workers + ffmpeg.wasm、5 分超はクライアント側 Canvas 抽出。失敗時はプレースホルダー画像を表示 (7) 画像最適化に Cloudflare Images 採用の選択肢を明記 (8) Flyway V6.025 追加 (9) R2 バケットは単一 `mannschaft-storage` の `blog/` プレフィックス配下 (10) 本文エディタの動画挿入フローをハイブリッド方式に更新 |
