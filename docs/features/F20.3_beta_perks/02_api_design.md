...
- **TZ 値が不正/空/NULL** のユーザーは既定 `Asia/Tokyo` へフォールバックする（WARN ログ）。一方、**TZ 解決自体が例外で失敗した場合は握り潰さず伝播**させる（「引けないので全員 0 日」は付与漏れを静かに生むため）。
  - **【2026-07-29 追記・Issue #2487 項目 4】不正値は入口で弾く**: 上記フォールバックは*後段の防御*であり、そもそも `users.timezone` に不正値を入れさせないことが根治である。プロフィール更新（`UpdateProfileRequest`）・新規登録（`RegisterRequest`）の `timezone` に `@ValidTimezone`（`TimezoneValidator`）を課し、**実在する IANA タイムゾーン名以外は 400 で拒否**する（`"+09:00"` のような固定オフセット表記も、夏時間を追随できないため拒否）。あわせて、更新で `timezone` / `locale` が変わったときに `UserTimezoneCache` / `UserLocaleCache` を **evict する結線が 1 箇所も無かった**（最大 5 分間は旧値が返っていた）ため `UserService#updateProfile` から呼ぶようにした。
  - **evict は必ずコミット確定後**（`TransactionSynchronizationManager` の `afterCommit`。F20.1 `BillingContractService#evictAfterCommit` と同型）。コミット前に evict すると、evict とコミットの隙に別スレッドがキャッシュミスを起こし、READ_COMMITTED 下で**未コミットの更新が見えない DB** を読んで**旧値を TTL 5 分ぶん再ポピュレート**する。evict で直そうとした症状が、タイミング依存の再現しにくい形で残ってしまう。ロールバック時は `afterCommit` が呼ばれないため evict もされず、キャッシュと DB が乖離しない。`UserServiceTest$UpdateProfile` の 4 テストがこの順序を機械的に固定している（コミット前 evict に戻すと 3 件が `NeverWantedButInvoked` で赤くなることを実測で確認済み）。
- **特殊オフセットの検証**（Issue #2487 項目 5）: 30 分/45 分刻み（`Asia/Kolkata` = +05:30、`Asia/Kathmandu` = +05:45）と +12:00 超（`Pacific/Apia` = +13:00、`Pacific/Kiritimati` = +14:00）でも現地の日境界で数えられることを `LoginActivityTimezoneIT`（AC-TZ6）が課す。オフセットを時間単位へ丸める退行が起きれば必ず落ちる（丸めたオフセットで 1 日に潰れることを同時に確認している）。
