...
> **収益化は進めるが急ぎではない。本書では「team オーナー変更イベント（B-4）」を初期実装スコープから外し Phase 2 へ保留する**（設計内容は削除せず温存＝将来そのまま使う。スコープのみ移動）。ベータ特典の**付与/判定・売買対策の主防壁（USER スコープ限定の構造）・規約による取消・手動 flag-review・称号バッジ**は**初期スコープに残す**。

| Phase 2 保留機能 | 本書での該当箇所 | 初期スコープでの代替 |
|---|---|---|
| **team オーナー変更イベント（B-4）**: 転売対策として team/organization のオーナー変更時に特典を再審査フラグ（`review_flag`）にする**専用の自動イベント新設**（`TeamOwnershipTransferredEvent`／`OrganizationOwnershipTransferredEvent` の発火＋購読リスナー） | §1.1（譲渡行）・§3（保険 2 の自動フラグ）・§6 AC-07・§8 P3・§9 B-4・02 §5・01 §4.2 | 当面は**規約第 17 条（一般の権利義務譲渡禁止）＋手動の再審査（シスアド `flag-review` API）**で足りる。専用の自動イベントは Phase 2。※`review_flag` 列・`MANUAL` 手動フラグ・審査解決フローは初期スコープに残る（オーナー変更の**自動**購読だけが Phase 2） |

> **関連の親機能側 Phase 2 保留**: [F20.1](../F20.1_entitlement_billing/README.md) の**営利自動切替**（org_type 自動変異）も同じくマスター決定で Phase 2 保留。ベータ特典の付与（BETA_GRANT）は元より org_type を変異させない設計（AC-22b）ゆえ、営利自動切替の保留は本書に影響しない。

**核フローの非依存（確認済み）**: ベータ特典の自動/手動付与・`isEntitled` を通した判定・USER スコープ限定による売買対策・称号バッジ授与・退会連携は、**オーナー変更の自動イベントに依存しない**（検知は「保険」であって主防壁ではない・§3）。イベントを Phase 2 に送っても、`review_flag` はシスアド手動 `flag-review`（02 §4）で立てられ、初期スコープは完結する。
...
| 譲渡 | **禁止**（`transferable=false` 固定・CHECK 制約） | 同左＋**オーナー変更で review_flag**（§3）**【自動イベントは Phase 2 保留・初期は手動 flag-review】** |
...
| 保険 2（検知） | 所有者変更の兆候で **`review_flag`** を立て運営審査に回す（チーム/組織特典: **オーナー（ADMIN）変更イベント**で自動フラグ。02 §5）**【Phase 2 保留】専用の自動イベント（B-4）は初期スコープ外。初期は規約第 17 条＋シスアド手動 `flag-review`（02 §4）で代替**（マスター 2026-07-08）。**検知は保険であり主防壁ではない**（誤検知は審査で解消・フラグ中も権利は有効のまま） |
...
3. **ベータ特典専用条項は現規約に存在しない → 新設が必要（規約改訂要件）**。盛り込むべき要素:
   ...
   - 条文実体は `landing.json` の `landing.legal.terms.sections`（6 言語）への追補として実装。**条番号は現行最終条の次で仮置きし、規約改訂 PR で確定**する。規約改訂はマスター承認事項。**B-1=マスター御裁可済（2026-07-08）＝条文は別 PR で起草**（本設計は要件の記録まで・第 17 条が一般受け皿ゆえ本実装は非ブロック）。

---

## 6. 受け入れ条件（AC）

> **【Phase 2 タグ】** `[P2]` を付した AC（AC-07＝オーナー変更**自動**イベントによる review_flag）は初期スコープの試練対象外（マスター 2026-07-08・冒頭 Phase 2 保留ブロック）。初期スコープでは `review_flag` を**シスアド手動 `flag-review`**で立てる経路（`review_reason='MANUAL'`）を試練対象とし、AC-08（フラグ中も権利有効）・AC-20（審査解決）は初期スコープに残る。
...
| AC-07 | 正常 `[P2]` | **【Phase 2】**チームのオーナー（ADMIN）変更イベント発生 → 当該チームの有効な `beta_grants.review_flag=true`・`review_reason='OWNER_CHANGED'` になり、運営に通知が届く（**専用の自動イベントは Phase 2 保留・初期試練対象外**。初期は手動 `flag-review` で `review_reason='MANUAL'` を立てる＝AC-08/20 は初期スコープに残る） |
...
| **P3** | 審査運用強化 | **S** | P1 | review 運用画面（**オーナー変更〔B-4〕の自動イベント連携は Phase 2 保留・初期スコープ外**・マスター 2026-07-08。初期は手動 flag-review） |
| **Phase 2** | オーナー変更 自動イベント連携 | **S** | P3・team/organization ドメイン | `TeamOwnershipTransferredEvent`／`OrganizationOwnershipTransferredEvent` の発火＋購読で `review_flag` 自動化（02 §5・B-4） |
...
## 9. 要裁可論点 → マスター御裁可済（2026-07-08）

> **2026-07-08 マスター御裁可**: B-1=条文別PR起草で確定・B-2=(a)確定・B-3=(a)確定・**B-4=新設確定だが 2026-07-08 マスター決定で初期スコープ外（Phase 2 保留）へ移動**・B-5=実装前確定条件として残置（gamification grep で確定・主フロー非依存）。

| # | 論点 | 選択肢 | **御裁可済（2026-07-08）** |
|---|---|---|---|
| **B-1** | 規約改訂（ベータ特典専用条項の新設・§5） | 条文案を規約改訂 PR で提示 | **✅ 確定**: 規約条項は**別 PR で起草**（§5 の 4 要素）→マスター承認後に 6 言語反映。**第 17 条が一般受け皿ゆえ本実装は非ブロック**（条文起草は別タスク）。**条文案は [`docs/legal/beta_perk_terms_clause_proposal.md`](../../legal/beta_perk_terms_clause_proposal.md) に起草済み（新「第27条（ベータテスト特典）」・6 言語対訳／マスター承認＋弁護士レビュー待ち）** |
| **B-2** | チーム/組織特典の 2 年後更新の運用 | (a) シスアド一括延長操作のみ／(b) 自動更新オプトイン | **✅ (a) 確定**。自動更新しない・延長は都度アナウンス→シスアド一括操作 |
| **B-3** | 特典取消時のバッジ剥奪 | (a) 剥奪しない／(b) 剥奪する | **✅ (a) 確定**。剥奪しない（バッジは活動実績の称号で権利ではない。不正取得と断定時のみ手動剥奪を運用で許す） |
| **B-4** `[P2]` | オーナー変更イベントの実装 | team ドメインに `TeamOwnershipTransferredEvent` が現存しない場合、新設 publish が必要 | **✅ 新設確定 → ただし 2026-07-08 マスター決定で Phase 2 保留（初期スコープ外）へ移動**（冒頭 Phase 2 保留ブロック）。**初期スコープでは規約第 17 条＋シスアド手動 flag-review が代替**（機能は塞がらない）。専用の自動イベント publish/購読は Phase 2 で実装（実装時に既存イベント有無を再確認） |
| **B-5** | ベータ称号 system badge の scope 取り扱い（§4） | `badges.scope_type/scope_id` NOT NULL・プラットフォーム横断 badge の前例なし | 🔧 **実装前確定条件として残置**（御裁可）: 実装着手前に gamification の scope クエリ・上限制約を grep して sentinel scope（`PLATFORM`/`0`）の可否を確定。主フロー非依存ゆえ🟢を阻害しない（§9.1） |

### 9.1 実装前確定条件（設計はブロックしないが実装着手前に決める）

| 条件 | 内容 | それまでの運用 |
|---|---|---|
| **個人 activeDays の計測源** | F10.8 は USER スコープ非対応（§7）。`audit_logs` の `LOGIN_SUCCESS` を第一候補に日別在籍を数える経路を確定する | `min_active_days=NULL` で `membershipTenureDays` のみで自動付与（02 §2・§3）。**2026-07-28 追記**: 閾値自体は `INDIVIDUAL` のみ 14 を投入済み（V168）。計測経路（`LoginActivityQueryService`）は `audit_logs` の `LOGIN_SUCCESS` を**ユーザー各自の TZ で日境界を切って**数える実装で結線済み（02 §3.1）。本番有効化は引き続き `mannschaft.beta.auto-grant.enabled=false` と規約レビューでゲートする |
