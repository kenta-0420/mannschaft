...
> **2026-07-08 マスター御裁可**: B-1=条文別PR起草で確定・B-2=(a)確定・B-3=(a)確定・B-4=新設確定・B-5=実装前確定条件として残置（gamification grep で確定・主フロー非依存）。

| # | 論点 | 選択肢 | **御裁可済（2026-07-08）** |
|---|---|---|---|
| **B-1** | 規約改訂（ベータ特典専用条項の新設・§5） | 条文案を規約改訂 PR で提示 | **✅ 確定**: 規約条項は**別 PR で起草**（§5 の 4 要素）→マスター承認後に 6 言語反映。**第 17 条が一般受け皿ゆえ本実装は非ブロック**（条文起草は別タスク）。**条文案は [`docs/legal/beta_perk_terms_clause_proposal.md`](../../legal/beta_perk_terms_clause_proposal.md) に起草済み（新「第27条（ベータテスト特典）」・6 言語対訳・PR #〈本 PR〉／マスター承認＋弁護士レビュー待ち）** |
...
| **B-4** | オーナー変更イベントの実装 | team ドメインに `TeamOwnershipTransferredEvent` が現存しない場合、新設 publish が必要 | **✅ 新設確定**。team ドメインに最小のイベント publish を新設（クロスドメインはイベント駆動）。**それまでは手動 flag-review が代替**（機能は塞がらない・実装時に既存イベント有無を再確認） |
