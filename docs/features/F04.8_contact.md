...
#### 認可とエラー契約（2026-07-30 確定 / 認可根治戦役 第2波 ロットA）

ID を受け取るエンドポイントは、**entity 由来の当事者 ID**（申請は `target_id` / `requester_id`、
連絡先削除はフォルダ所有者、招待トークンは発行者、事前拒否は設定の所有者）と認証主体を照合する。
当事者以外には**不存在と同じ 404** を返し、対象の存在有無を秘匿する。

| エンドポイント | 権限 | 当事者以外 |
|---|---|---|
| `DELETE /api/v1/contacts/{userId}` | フォルダ所有者 | 404 `CONTACT_015` |
| `POST /api/v1/contact-requests/{requestId}/accept` | 申請の宛先 | 404 `CONTACT_006` |
| `POST /api/v1/contact-requests/{requestId}/reject` | 申請の宛先 | 404 `CONTACT_006` |
| `DELETE /api/v1/contact-requests/{requestId}` | 申請の送信者 | 404 `CONTACT_006` |
| `DELETE /api/v1/contact-request-blocks/{blockedUserId}` | 設定の所有者 | 404 `CONTACT_010` |
| `DELETE /api/v1/contact-invite-tokens/{id}` | トークン発行者 | 404 `CONTACT_014` |
| `GET /api/v1/contact-invite-tokens/{token}/qr` | トークン発行者 | 404 `CONTACT_014` |
| `GET /api/v1/{teams,organizations}/{id}/members/contactable` | §4.7 の公開範囲 | 403 `CONTACT_007` |

> `CONTACT_006/010/014/015` は `ERROR_CODE_STATUS_MAP` で 404、`CONTACT_007` は 403 に割り当てる。
> 契約は `ContactScopeContractIT` で固定している。
...
- バックエンド: 共通ユーティリティ `com.mannschaft.app.common.qr.BrandedQrImageWriter`（`com.google.zxing:core` ベース）で生成
- 中央に白背景/グレー角丸枠/グレー太字「Mannschaft」バッジを合成。誤り訂正レベルは常に H 固定（バッジ被覆下でもスキャン可能）
- 招待（`InviteService.generateInviteQrCode`）と同一ユーティリティに統一済み（2026-07-06〜）。個別の ECL 指定（旧: M）は撤去
