...
- **退会時の失効**：外部Stripe通信をDBトランザクション内へ持ち込まない。DBを先に終端化し、専用作業行と再試行バッチでStripeを収束させる。
  1. 当該ユーザーが受益者の `ACTIVE` / `PAST_DUE` 契約だけを即時 `CANCELLED` にし、`membership_beneficiary_withdrawal_cancellations` に作業行を保存してから Stripe Subscription を即時 cancel する。Stripe失敗・処理途中停止は同じ退会attempt由来の冪等キーで再試行する。他の受益者の契約は変更しない。退会取消後も有料契約を自動復旧せず、再加入は本人の明示操作とする。
  2. 当該ユーザーが払い手の契約は従来どおり期末解約を予約し、利用期間を即時に奪わない。
