...
2026-05-18 11:49（JST）、本番運用前のステージング検証中に発生した認証メール SES 失敗事故（受信者: `hideharu215@yahoo.co.jp`、テンプレ種別: `EMAIL_VERIFICATION_TOKEN`）が本書起草の直接の発端である。SES sendEmail() が `MessageRejectedException` を返したにもかかわらず、呼び出し側 `AuthEmailEventListener.onSendEmailVerificationRequested()` は例外を log.warn() で吸収するのみで、ユーザーには「メールを送信しました」と表示され、認証 token は完全に喪失した。同じ token はワンタイム生成のため復元不能であり、ユーザーは「メールが届かない」と問い合わせざるを得ない状況に陥った。
...
|---------------------------|--------------------------|----------------------------------------------------------------------|
| auth ドメイン             | AuthEmailEventListener  | EMAIL_VERIFICATION_TOKEN / PASSWORD_RESET_TOKEN / 2FA_ENABLED_NOTICE |
| billing ドメイン          | OverdueInvoiceService   | 滞納督促メール                                                       |
| event-watch ドメイン      | EventWatchNotifier      | 緊急閉鎖通知 / 委任投票リマインド                                    |
| chat ドメイン             | DirectMailService       | SYSTEM_ADMIN → ユーザー 1:1 通知 / 重要事項説明書送付完了通知       |
| reservation ドメイン      | ConfirmableNotification | 予約確定 / キャンセル通知                                            |
| pointcard ドメイン        | PointCardEventListener  | ポイント付与 / 失効リマインド                                        |
| organization ドメイン     | OrganizationInvitationService | 組織招待                                                          |
| repair-plan ドメイン      | RepairPlanLongtermNotifier | 長期修繕計画レビュー期限通知                                       |
...
erDiagram
    email_outbox {
        ...
        VARCHAR_64 template_kind "EMAIL_VERIFICATION_TOKEN 等"
...
CREATE TABLE email_outbox (
    ...
    -- テンプレート識別
    template_kind       VARCHAR(64)   NOT NULL          COMMENT 'EMAIL_VERIFICATION_TOKEN / PASSWORD_RESET_TOKEN 等',
...
| organization_id            | NULL 許容                                                  | 認証メール (EMAIL_VERIFICATION_TOKEN / PASSWORD_RESET_TOKEN) は組織未確定でも送信される   |
...
public record EmailOutboxRequest(
    String templateKind,         // 必須: 例 "EMAIL_VERIFICATION_TOKEN"
...
```json
{
  ...
  "templateKind": "EMAIL_VERIFICATION_TOKEN",
  "locale": "ja",
...
public class AuthEmailEventListener {
    ...
    public void onSendEmailVerificationRequested(SendEmailVerificationRequestedEvent event) {
        emailOutboxService.enqueue(new EmailOutboxRequest(
            ...
            "auth",
            event.tokenId().toString(),    // sourceEventId
            null,                          // idempotencyKey 自動生成
            event.userId(),
            null                           // organizationId
        ));
    }
...
| retry_count | 次回までの遅延 | 累積経過時間 | 状態          |
|-------------|----------------|--------------|---------------|
| 0 (初回失敗) | 10 秒          | 10 秒        | PENDING       |
| 1           | 30 秒          | 40 秒        | PENDING       |
| 2           | 2 分           | 2 分 40 秒   | PENDING       |
| 3           | 10 分          | 12 分 40 秒  | PENDING       |
| 4           | 30 分          | 42 分 40 秒  | PENDING       |
| 5           | 2 時間         | 2 時間 42 分 | PENDING       |
| 6 到達      | -              | -            | DEAD_LETTER に遷移 |

最大保持期間は約 2 時間 42 分。それ以上の遅延を許容するケース（例: SES アカウント停止が 1 日続く）は SYSTEM_ADMIN が手動で再キューする運用とする。
...
|----|------------------------------------------------------|----------------------------------------|-------------------------------------|------------|-------------------------------------------------------------------|
| 1  | `auth.email.AuthEmailEventListener#onEmailVerification` | メール認証 raw token を含む確認メール   | `EMAIL_VERIFICATION_TOKEN`          | **18-b 最優先** | raw token 喪失問題の直接の発端。本書起草の起点                  |
| 2  | `auth.email.AuthEmailEventListener#onPasswordReset`    | パスワードリセット raw token            | `PASSWORD_RESET_TOKEN`              | **18-b 最優先** | raw token 喪失問題の直接の発端                                  |
| 3  | `auth.email.AuthEmailEventListener#on2FAEnabled`       | 二要素認証有効化通知                    | `TWO_FACTOR_ENABLED_NOTICE`         | **18-b 最優先** | token は含まないが認証ドメイン統一移行                          |
| 4  | `billing.invoice.InvoiceEmailService#sendNew`         | 新規請求書通知                          | `INVOICE_ISSUED`                    | 18-c       | 添付PDF URL を payload に含めて Worker でレンダリング           |
| 5  | `billing.invoice.InvoiceEmailService#sendReminder`    | 支払期日リマインド                      | `INVOICE_PAYMENT_REMINDER`          | 18-c       | -                                                                 |
| 6  | `billing.invoice.OverdueInvoiceService`               | 滞納督促 (複数受信者ループ)             | `INVOICE_OVERDUE_NOTICE`            | 18-c       | 複数受信者 → outbox では 1 record = 1 宛先に展開              |
| 7  | `chat.dm.DirectMailService#sendSystemMail`            | SYSTEM_ADMIN → ユーザー 1:1 通知       | `SYSTEM_DIRECT_MAIL`                | 18-c       | F09.17 EMAIL チャネルとは別経路 (Phase 18-f で再統合)          |
| 8  | `chat.dm.ConfirmableNotification#deliver`             | 確認必須通知 (複数受信者ループ)         | `CONFIRMABLE_NOTIFICATION`          | 18-c       | 1 record = 1 宛先に展開、各々 sourceEventId 別                  |
| 9  | `event-watch.notifier.EventWatchNotifier#sendDigest`   | 委任投票ダイジェスト                    | `PROXY_VOTE_DIGEST`                 | 18-c       | -                                                                 |
| 10 | `event-watch.notifier.EventWatchNotifier#sendReminder` | 委任投票リマインド                      | `PROXY_VOTE_REMINDER`               | 18-c       | -                                                                 |
| 11 | `pointcard.event.PointCardEventListener#onIssued`     | ポイント付与通知                        | `POINTCARD_ISSUED`                  | 18-c       | フラグ駆動の冪等性は呼び出し側に温存、outbox の idempotency_key とは別レイヤ |
| 12 | `event-watch.emergency.EmergencyClosureService`       | 緊急閉鎖通知 (複数受信者ループ)         | `EMERGENCY_CLOSURE_NOTICE`          | **18-b 最優先** | 緊急性高 + 複数受信者で SES 失敗時の影響が大きい                |
| 13 | `organization.invitation.OrganizationInvitationService` | 組織招待                                | `ORGANIZATION_INVITATION`           | 18-c       | invitation_token の喪失問題 (#1 と同類)                          |
| 14 | `repair-plan.notifier.RepairPlanLongtermNotifier`     | 長期修繕計画レビュー期限通知            | `REPAIR_PLAN_REVIEW_DUE`            | 18-c       | -                                                                 |
...
- `EmailOutboxWorker` (`@Scheduled` + `@SchedulerLock`)
...
- #1 EMAIL_VERIFICATION_TOKEN (auth)
- #2 PASSWORD_RESET_TOKEN (auth)
- #3 TWO_FACTOR_ENABLED_NOTICE (auth)
- #12 EMERGENCY_CLOSURE_NOTICE (event-watch、複数受信者展開含む)
...
- **2026-05-18**: 初稿。本日 11:49 (JST) に発生した認証メール SES 失敗事故（受信者: `hideharu215@yahoo.co.jp`、テンプレ種別: `EMAIL_VERIFICATION_TOKEN`）を契機に起草。F09.17 既存設計とは独立した基盤として位置付け、将来統合パスを §12 で担保。未解決事項ゼロで設計完了。実装は Phase 18-a 〜 18-e の 5 フェーズに分割、Phase 18-f は別軍議で対応。
