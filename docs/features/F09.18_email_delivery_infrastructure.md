...
- `software.amazon.awssdk.core.exception.SdkClientException` (ネットワーク一時障害: "Unable to execute HTTP request" 等)
- HTTP 5xx 系全般
...
- cause チェーンに `"Unable to load credentials"` を含む例外 (`SdkClientException` 等) — **AWS 認証情報未設定（2026-06-20 追加）**

> **認証情報ロード失敗の取り扱い**（`SesExceptionClassifier` 実装 §7.2 改訂 / 2026-06-20）:
> `SdkClientException("Unable to load credentials from any of the providers in the chain ...")` は
> 恒久的な設定ミスであり、何度リトライしても解消しない。以前は一時失敗扱いで 6 回（約42分）の
> 無駄なリトライ後に DEAD_LETTER 化されアラートが大幅に遅延していた。
> 本改訂で `SesExceptionClassifier.isPermanent()` が例外の cause チェーンを辿り、
> いずれかに `"Unable to load credentials"` が含まれれば即座に永久失敗と判定する。
> これにより認証情報ミス時は即 DEAD_LETTER 化・即アラートとなる。
> 一方、ネットワーク一時障害（`"Unable to execute HTTP request"` 等）は引き続き一時失敗（リトライ）のまま変更しない。

ShedLock 失敗時の挙動: lock を取得できなかった pod は `poll()` を return するだけ。次の `fixedDelay=10s` 後の起動でリトライ。

### 7.2.1 EmailTransport 抽象と simulate モード（2026-06-20 追加）

#### 背景

dev/local 環境では AWS SES 認証情報が無いため、以前は `EmailOutboxServiceImpl` が直接 `SesV2Client` を注入して `sendEmail()` を呼び、`SdkClientException: Unable to load credentials` で必ず失敗し、リトライ 6 回上限で DEAD_LETTER が蓄積していた（37 件蓄積 → CRITICAL アラート常時発火）。

SQS リスナー側は `@ConditionalOnProperty(name = "mannschaft.ses.sqs.queue-name", matchIfMissing = false)` で local/test で起動しない設計なのに、**送信 Worker / 送信クライアントには local ゲートが無かった**ことが原因の非対称である。

#### 設計

送信経路を `EmailTransport` インターフェースで抽象化し、`mannschaft.email.simulate` プロパティで実装を切り替える:

| `simulate` 設定 | 有効になる Bean | 挙動 |
|---|---|---|
| `false` または未設定（既定） | `SesEmailTransport` | 実 SES v2 で送信（prod/staging の従来動作） |
| `true` | `LoggingEmailTransport` | ログ出力のみ（宛先マスキング）、疑似 messageId `"SIMULATED-{UUID}"` を返す |

```yaml
# application.yml（既定 false = 実SES）
mannschaft:
  email:
    simulate: ${MANNSCHAFT_EMAIL_SIMULATE:false}

# application-local.yml（local プロファイルで上書き）
mannschaft:
  email:
    simulate: true

# application-test.yml / application-ci.yml
mannschaft:
  email:
    simulate: true
```

#### `EmailTransport` インターフェース

```java
public interface EmailTransport {
    /**
     * @return SES MessageId（simulate 時は "SIMULATED-{UUID}"）
     *         送信失敗時は実装が例外を投げ、呼び出し側が SesExceptionClassifier で判定する
     */
    String send(String toAddress, String subject, String htmlBody);
}
```

- **`SesEmailTransport`**: `@ConditionalOnProperty(name="mannschaft.email.simulate", havingValue="false", matchIfMissing=true)` — 現状の `sendViaSes` ロジックをそのまま移植
- **`LoggingEmailTransport`**: `@ConditionalOnProperty(name="mannschaft.email.simulate", havingValue="true")` — 例外を投げない（=必ず SENT 化）

`EmailOutboxServiceImpl` は `SesV2Client` の直接注入を削除し、`EmailTransport emailTransport` を注入する。`sendViaSes()` メソッドは廃止。`classifier.isPermanent(ex)` の分岐・metrics・notifyDeadLetter は完全に温存する（SES 例外は `EmailTransport.send()` を通じてそのまま伝播する）。
...
| `delivery_success_rate` (直近24h)         | < 0.98             | < 0.95             | WARN: ダッシュボード / CRITICAL: SYSTEM_ADMIN プッシュ + ErrorReport |
| `queue_depth_dead_letter` (直近24h)       | > 5                | > 10               | WARN: ダッシュボード / CRITICAL: SYSTEM_ADMIN プッシュ + ErrorReport |

> **DEAD_LETTER アラートの時間窓（`EmailOutboxAlertChecker` 実装 §10 改訂 / 2026-06-20）**:
> `checkDeadLetterDepth()` は以前、全期間累計の DEAD_LETTER 件数（`countByStatus`）で判定していた。
> これにより過去に蓄積した死信が永久に CRITICAL ログを出し続ける問題があった
>（`checkSuccessRate` は既に `minusHours(24)` 窓を使っており非対称だった）。
> 本改訂で `countByStatusSince(DEAD_LETTER, now - 24h)` を使い、**直近24h の DEAD_LETTER 件数**で判定する。
> 閾値（> 10 件で CRITICAL）は変更しない。
> ログ文言も「直近24h の DEAD_LETTER が {} 件 (閾値 {})」と24h窓であることを明示するよう更新した。
...
| 16 | `reservation/event/ReservationRecipientEmailEventListener.java` | 予約通知メール (F03.4 機能D・予約成立時に登録宛先へ「日時＋メニュー＋予約者名」を送信) | `RESERVATION_RECEIVED_NOTIFY`  | 18-c       | `source_domain='reservation'`、`source_event_id="reservation-notify:{reservationId}:{email}"` で冪等（userId=null→"0"）。1 record = 1 宛先。非ユーザー宛先（店代表アドレス等）可。スルー方式（subject/body を payloadVars に詰める） |

> **注（機能D で1件追加・合計16箇所）**: 上表は当初「既存 14 箇所」だが、`GUARDIANSHIP_PROGRESSION_NOTICE`（switch のみ）・`DIRECT_MAIL_AD`（#15）・`RESERVATION_RECEIVED_NOTIFY`（#16・機能D）を後続で追加している。`EmailTemplateKind` enum・`EmailOutboxServiceImpl.renderTemplate` の switch・本マトリクスの3点は常に一致させること（enum/switch/§11 のドリフト防止）。
...
- **2026-06-20 (SesExceptionClassifier 堅牢化 + DEAD_LETTER アラート 24h 窓化)**:
  - §7.2 失敗種別判定: `SesExceptionClassifier.isPermanent()` が cause チェーンを辿り `"Unable to load credentials"` を含む例外を永久失敗に分類するよう拡張。AWS 認証情報未設定時の無駄なリトライ（6回・約42分）を排除し、即 DEAD_LETTER 化・即アラートとする。一時的ネットワーク障害はリトライ維持。
  - §10 アラート閾値: `EmailOutboxAlertChecker.checkDeadLetterDepth()` が全期間累計（`countByStatus`）から直近24h（`countByStatusSince`）に変更。過去の死信が永久に CRITICAL を発し続ける問題を解消。`checkSuccessRate` との対称性を実現。閾値（> 10 件）は変更なし。
