...
> **実装フェーズ**: Phase 3
> **最終更新**: 2026-06-02
> **関連ドキュメント**: 組織・チームスケジュール → `F03.1_schedule_shared.md` / 外部連携 → `F08_external_integration.md`
...
> 個人スコープ（`ScopeType.PERSONAL`）の予定操作は、チーム/組織スケジュールの変更フィード（[`F03.18_schedule_activity_feed.md`](F03.18_schedule_activity_feed.md)）の対象外。

### カレンダーUI付加機能

| 機能 | 説明 |
|------|------|
| **祝日表示** | ユーザープロフィールの `country_code`（→ `F01.1_auth.md` Section 3）をもとに、フロントエンドの `date-holidays` ライブラリ（100カ国以上対応）で祝日を取得してカレンダーに表示する。`country_code` が NULL の場合は `locale` から推定（`ja`→`JP` 等）。祝日は日付番号・祝日名ともに赤で表示 |
| **開始時刻連動** | 開始時刻を入力すると終了時刻が自動で1時間後にセットされる。日付をまたぐ場合は終了日を翌日に自動更新 |
...
個人予定に対するリマインダー通知設定。1予定につき最大3件まで設定可能。機能55 第一陣（V72.003）で絶対日時指定（ABSOLUTE）に対応した。

```
personal_schedule_reminders
├── id                    BIGINT UNSIGNED AUTO_INCREMENT PK
├── schedule_id           BIGINT UNSIGNED NOT NULL  FK → schedules(id) ON DELETE CASCADE
├── remind_before_minutes INT UNSIGNED NULL         -- 相対指定：開始N分前（RELATIVE時に使用。ABSOLUTE時はNULL可）
├── remind_at             DATETIME NULL             -- 絶対日時（ABSOLUTE時に使用。RELATIVE時はNULL）
├── reminder_kind         VARCHAR(10) NOT NULL DEFAULT 'RELATIVE'  -- 指定方式（RELATIVE/ABSOLUTE）。既定はRELATIVE（後方互換）
├── notified              BOOLEAN NOT NULL DEFAULT FALSE  -- 通知送信済みフラグ
├── created_at            DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
└── UNIQUE KEY uq_reminder (schedule_id, remind_before_minutes)
```

> V72.003 での変更点: `remind_at DATETIME NULL` / `reminder_kind VARCHAR(10) NOT NULL DEFAULT 'RELATIVE'` を追加。`remind_before_minutes` を NOT NULL → NULL 可に変更（ABSOLUTE 時は未使用のため）。

**実効時刻の解決ロジック**
- `RELATIVE`: `effectiveRemindAt = schedules.start_at - remind_before_minutes 分`
- `ABSOLUTE`: `effectiveRemindAt = remind_at`

**`PersonalScheduleReminderBatchService` による根治（機能55 第二陣）**

機能55 以前は個人予定リマインダーも起動バッチが存在せず永遠に発火しなかった（非稼働状態）。機能55 第二陣で `PersonalScheduleReminderBatchService`（`@Scheduled fixedDelay=60秒`・ShedLock `personalScheduleReminderBatch`）を新設し、1分間隔で以下を実行するよう根治した:
- `notified = FALSE` かつ `effectiveRemindAt <= NOW()` かつ `schedules.deleted_at IS NULL` のリマインダーを取得
- 個人予定の所有者（`schedules.user_id`）へ通知を送信
- `notified = TRUE` に更新

> - リマインダーは冪等（`notified = FALSE` 条件で重複通知を防止）
...
> V3.016 は V3.007（schedules）および V1.005（users）完了後に実行。F03.1_schedule_shared.md のマイグレーション一覧と共有する。

```sql
V3.018__add_personal_schedule_columns.sql
  -- schedules テーブルへの変更:
  --   color VARCHAR(7) NULL ADD COLUMN
  -- personal_schedule_reminders テーブルの作成（remind_before_minutes NOT NULL / notified / created_at）
```

> V3.018 は V3.016 完了後に実行。

```sql
-- 機能55 第一陣（PR #1256 main マージ済み）
V72.003__alter_personal_schedule_reminders_add_absolute.sql
  -- personal_schedule_reminders テーブルへの変更:
  --   remind_at DATETIME NULL ADD COLUMN（絶対日時指定・ABSOLUTE 時に使用）
  --   reminder_kind VARCHAR(10) NOT NULL DEFAULT 'RELATIVE' ADD COLUMN（指定方式）
  --   remind_before_minutes を NOT NULL → NULL 可に変更（ABSOLUTE 時は未使用のため）
```

> V72.003 は V3.018（personal_schedule_reminders テーブルが存在すること）完了後に実行。
...
```json
{
  ...
  "color": "#4A90D9",
  "reminders": [10, 60],
  "absolute_reminders": ["2026-04-10T12:00:00"],
...
>
> **`reminders`**: 予定開始の何分前に通知するかを配列で指定（最大3件）。省略時はリマインダーなし。後方互換のため維持。
>
> **`absolute_reminders`**（機能55）: 絶対日時指定のリマインダーを配列で指定（最大3件）。各要素は未来の日時であること。`reminders`（相対）との合計が最大5件を超えると 400 エラー（`CreatePersonalScheduleRequest.MAX_TOTAL_REMINDERS = 5`）。
...
> - 詳細 GET（`GET /api/v1/me/schedules/{id}`）では `detailed_reminders` フィールドが追加される（機能55）。相対・絶対の両方を `reminder_kind` 付きで返す（一覧 GET では null）。`reminders`（分数の配列）は後方互換のため維持されるが、絶対指定リマインダーはここに載らない。`detailed_reminders` で統合参照すること

**`GET /api/v1/me/schedules/{id}` 追加フィールド（詳細のみ）**
```json
{
  "detailed_reminders": [
    { "id": 1, "reminder_kind": "RELATIVE", "remind_before_minutes": 10, "remind_at": null, "notified": false },
    { "id": 2, "reminder_kind": "RELATIVE", "remind_before_minutes": 60, "remind_at": null, "notified": false },
    { "id": 3, "reminder_kind": "ABSOLUTE", "remind_before_minutes": null, "remind_at": "2026-04-10T12:00:00", "notified": false }
  ]
}
```
...
### リマインダー通知バッチ（機能55 根治後）

`PersonalScheduleReminderBatchService`（`@Scheduled fixedDelay=60秒`・ShedLock `personalScheduleReminderBatch`）が毎分実行する。

**根治経緯**: 機能55 以前は起動バッチが存在せず、個人予定リマインダーは永遠に発火しなかった（非稼働状態）。機能55 第二陣で本バッチを新設してこの欠落を根治した。
...
2. 各リマインダーの実効時刻を解決する:
   - reminder_kind = RELATIVE: effectiveRemindAt = s.start_at - remind_before_minutes 分
   - reminder_kind = ABSOLUTE: effectiveRemindAt = r.remind_at
3. effectiveRemindAt > NOW() の場合はスキップ（まだ時刻に達していない）
4. 対象リマインダーごとに個人予定所有者（s.user_id）へ通知を送信
5. notified = TRUE に UPDATE
```
...
| 日付 | 変更内容 |
|------|---------|
| 2026-06-02 | 機能55 個人リマインダー絶対日時対応・バッチ根治の設計書同期（PR #1256/#1261/#1262/#1265）: (1) `personal_schedule_reminders` テーブル定義に `remind_at`（絶対日時）・`reminder_kind`（RELATIVE/ABSOLUTE）カラムを追加、`remind_before_minutes` の NULL 可化を反映。(2) リクエストに `absolute_reminders` フィールドを追加（最大3件・相対との合算最大5件）。(3) 詳細 GET レスポンスに `detailed_reminders`（相対・絶対両方を reminderKind 付きで露出）を追加。(4) `PersonalScheduleReminderBatchService` による根治（従来非稼働だった事実・実効時刻解決ロジック・ShedLock 名）を記載。(5) Flyway 節に V72.003 を記録 |
| 2026-04-15 | カレンダーUI付加機能を概要セクションに追記: ①祝日表示（`date-holidays` ライブラリ使用・`country_code` 連動・100カ国以上対応）②開始時刻連動で終了時刻を自動1時間後セット |
