...
| event_type | トリガー | metadata 例 |
|-----------|---------|------------|
| `POINT_CARD_CREATED` | カード追加成功時（`POST /api/v1/point-cards`） | `{"provider_code": "tokyu_point", "card_id": "01928a3e-4b2f-7a8c-9d12-..."}` |
| `POINT_CARD_DELETED` | カード物理削除成功時（`DELETE /api/v1/point-cards/{id}`） | `{"provider_code": "tokyu_point", "card_id": "01928a3e-4b2f-7a8c-9d12-..."}` |
...
- F18 の `barcode_value` / `nickname` / `memo` は **AES-256-GCM で暗号化保存される個人機密** であり、監査ログの `metadata` に **絶対に含めない**こと。
- `last4` も含めない（4 桁単独では特定不可だが、`provider_code` と組み合わせると識別力が出るため）。
- 含めてよい識別子は `provider_code`（プロバイダー側コード、PII ではない）、`card_id` / `group_id`（UUIDv7、それ単体では特定不可）、`card_count` / `is_enabled` / `terms_version` のみ。
