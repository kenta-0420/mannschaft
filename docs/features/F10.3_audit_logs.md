...
> **観測系の役割分担**: 本機能は「セキュリティ操作の監査記録」を担当する。**システム監視は F10.5（パフォーマンス）/ F10.6（エラー）で分担**する（性能指標・例外集約・アラート通知は本機能の責務外）。
...
| `REPAIR_PLAN` | マンション修繕長期計画（F08.8） | `SCENARIO_CREATED` / `BID_CARD_MOVED` / `PACK_GENERATED` / `EXTERNAL_AGENT_DELEGATION_GRANTED` 等 |
| `POINT_CARD` | 個人ポイントカードウォレット（F18） | `POINT_CARD_CREATED` / `POINT_CARD_DELETED` / `POINT_CARD_VIEWED` / `POINT_CARD_GROUP_CREATED` / `POINT_CARD_GROUP_DELETED` / `POINT_CARD_SETTINGS_UPDATED` |
...
| `ACCOUNT_UNLOCKED` | 管理者によるアカウントロック手動解除（`reason=ADMIN_ACTION` 固定）。Valkey TTL 自然失効による自動解除はイベントを記録しない | `{"reason": "ADMIN_ACTION"}` |
...
| `SEAL_REGENERATED` | 電子印鑑の再生成（手動または姓変更による自動） | Phase 5 以降 |
| `SEAL_STAMPED` | 電子印鑑の押印実行 | Phase 5 以降 |
| `SEAL_STAMP_REVOKED` | 電子印鑑の押印取消（本人） | Phase 5 以降 |
| `SEAL_STAMP_FORCE_REVOKED` | 電子印鑑の押印取消（ADMIN強制） | Phase 5 以降 |

#### TODO カスタムステータスラベル + キャッチボール (F02.3.1)

> `user_id` = 操作者。ラベル CRUD は `team_id` / `organization_id` に当該スコープを記録。個人ラベルは `user_id` のみ（`team_id` / `organization_id` は NULL）。`TODO_HANDED_OFF` は当該 TODO のスコープに応じて `team_id` または `organization_id` を記録。

| event_type | トリガー | metadata 例 |
|-----------|---------|------------|
| `TODO_STATUS_LABEL_CREATED` | カスタムステータスラベルの新規作成（`POST /todo-status-labels`） | `{"label_id": 11, "scope_type": "TEAM", "scope_id": 100, "name": "レビュー中", "bucket": "IN_PROGRESS", "color": "#fbbf24"}` |
| `TODO_STATUS_LABEL_UPDATED` | カスタムステータスラベルの更新（`PUT /todo-status-labels/{id}`）| `{"label_id": 11, "scope_type": "TEAM", "scope_id": 100, "changed_fields": ["name", "color"], "old_name": "レビュー中", "new_name": "レビュー待ち"}` |
| `TODO_STATUS_LABEL_DELETED` | カスタムステータスラベルの論理削除（`DELETE /todo-status-labels/{id}`）| `{"label_id": 11, "scope_type": "TEAM", "scope_id": 100, "name_snapshot": "レビュー中", "bucket": "IN_PROGRESS"}` |
| `TODO_HANDED_OFF` | キャッチボール送出（`POST /todos/{todoId}/handoff`） | `{"todo_id": 42, "handoff_id": 88, "from_assignee_user_ids": [5], "to_user_ids": [12, 15], "previous_status": "IN_PROGRESS", "previous_status_label_id": 2, "new_status": "IN_PROGRESS", "new_status_label_id": 11, "has_message": true}` |

**補足**:
- `TODO_HANDED_OFF` の `metadata.has_message` はメッセージの有無のみ（メッセージ本文は audit_logs に記録しない。プライバシーと容量の観点）。
- ラベル CRUD イベントは `is_system_default = true` のラベルへの操作（403 で拒否される）はそもそも記録されない。
- ラベル名は監査調査で必須（後で削除されると元の名前が分からなくなる）のため `name` / `old_name` / `new_name` / `name_snapshot` を平文記録。これは PII ではなく業務識別子の扱い。

#### マンション修繕長期計画ダッシュボード (F08.8)

> `user_id` = 操作者。`team_id` / `organization_id` には当該スコープを記録。シナリオ・カンバン・申し送りパック・委任の重要操作を全て監査対象とする。`/scenarios/simulate`（保存しない再計算 API）は監査ログ爆発防止のため記録しない（保存系のみ記録）。

| event_type | トリガー | metadata 例 |
|-----------|---------|------------|
| `SCENARIO_CREATED` | シミュレーションシナリオ新規保存（`POST /repair-plan/scenarios`）| `{"scenario_id": 42, "scope_type": "ORGANIZATION", "scope_id": 7, "engine_version": "v1.0.0", "content_sha256": "ab12...", "name": "値上げ案A"}` |
| `SCENARIO_UPDATED` | シナリオ更新（`PATCH /repair-plan/scenarios/{id}`、locked_at IS NULL のみ） | `{"scenario_id": 42, "changed_fields": ["name", "params_json"], "old_content_sha256": "ab12...", "new_content_sha256": "cd34..."}` |
| `SCENARIO_LOCKED_FOR_PROPOSAL` | シナリオロック（`publish-as-announcement` 内部処理で `locked_at` セット） | `{"scenario_id": 42, "version_no": 1, "locked_at": "2026-05-09T10:30:00Z", "content_sha256": "cd34..."}` |
| `SCENARIO_PROPOSAL_CONVERTED` | シナリオ → F02.8 告知ドラフト変換完了（`POST /repair-plan/scenarios/{id}/publish-as-announcement`） | `{"scenario_id": 42, "version_no": 1, "announcement_id": 100, "proposed_resolution_no": "R6-第3号議案"}` |
| `BID_CARD_CREATED` | カンバン業者見積カード追加（`POST /repair-plan/quote-kanbans/{kanbanId}/cards`） | `{"card_id": 88, "kanban_id": 12, "vendor_id": 5, "vendor_name_snapshot": "○○塗装工業", "stage": "REQUESTED"}` |
| `BID_CARD_MOVED` | カード stage 遷移（`POST /repair-plan/quote-cards/{id}/move`） | `{"card_id": 88, "kanban_id": 12, "from_stage": "UNDER_REVIEW", "to_stage": "SHORTLISTED"}` |
| `BID_DEADLINE_OPENED` | 入札締切時刻到達によりカンバン業者間比較ビューを解放（cron） | `{"kanban_id": 12, "bid_deadline_at": "2026-05-15T17:00:00Z", "card_count": 5}` |
| `BID_VENDOR_SELECTED` | カード SELECTED 確定 → property_work_packages.selected_vendor_id 更新（Service 経由） | `{"card_id": 88, "kanban_id": 12, "vendor_id": 5, "work_package_id": 200, "amount": 11500000}` |
| `PACK_GENERATED` | 申し送りパック PDF 生成完了（非同期ジョブ AFTER_COMMIT） | `{"pack_id": 33, "scope_type": "ORGANIZATION", "scope_id": 7, "term_year": 2026, "pdf_sha256": "ef56...", "pii_level": "STANDARD", "r2_key": "repair_plan/organization/7/packs/33/ef56....pdf"}` |
| `PACK_DOWNLOADED` | 申し送りパック署名付き URL からのダウンロード実行 | `{"pack_id": 33, "downloader_user_id": 12, "viewer_watermark": "for: 田中理事 / 2026-05-09 14:32", "ip_address": "203.0.113.1"}` |
| `TIMELINE_EXPORTED` | 地層タイムラインの PDF/CSV エクスポート | `{"scope_type": "ORGANIZATION", "scope_id": 7, "format": "pdf", "year_from": 2006, "year_to": 2026, "row_count": 142}` |
| `EXTERNAL_AGENT_DELEGATION_GRANTED` | 管理会社への機能別委任付与（`POST /repair-plan/delegations`） | `{"delegation_id": 9, "agent_user_id": 50, "agent_company_name": "○○管理(株)", "delegation_type": "REPAIR_PLAN_MANAGE", "valid_until": "2027-03-31T23:59:59Z"}` |
| `EXTERNAL_AGENT_DELEGATION_REVOKED` | 委任失効（`DELETE /repair-plan/delegations/{id}` または有効期限到来 cron） | `{"delegation_id": 9, "agent_user_id": 50, "delegation_type": "REPAIR_PLAN_MANAGE", "revoked_reason": "MANUAL"}` |

**補足**:
- `SCENARIO_*` イベントの `content_sha256` は `SHA-256(params_json || computed_summary_json || engine_version)` で算出。改ざん検出時の照合に使用。
- `BID_CARD_MOVED` は前進・後退（取り消し）両方で記録。`from_stage` / `to_stage` で遷移方向を判定可能。
- `PACK_DOWNLOADED` の `viewer_watermark` は実際に PDF 内に焼き付けられた透かし文字列を平文記録（漏洩追跡のため）。閲覧者の氏名は業務識別子として扱う（PII フラグなし）。
- `EXTERNAL_AGENT_DELEGATION_*` は区分所有法上の管理会社業務委託契約の電子記録。`granted_by` / `revoked_by` は理事長のみ可能であり、不正委任を追跡する重要証跡。
- 詳細仕様は `F08.8_repair_longterm_dashboard.md` §5・§6 参照。

#### 個人ポイントカードウォレット (F18)

> `user_id` = 操作者（本人）。個人スコープ機能のため `team_id` / `organization_id` は常に NULL（Phase 1）。`target_user_id` も常に NULL（自分のカードのみが対象）。Phase 2 で organization 自店発行カードに関するイベント（スタンプ押印等）を追加する際に `organization_id` を埋める想定。

| event_type | トリガー | metadata 例 |
|-----------|---------|------------|
| `POINT_CARD_CREATED` | カード追加成功時（`POST /api/v1/point-cards`） | マッチ時: `{"provider_code": "tokyu_point", "provider_matched": true, "card_id": "01928a3e-4b2f-7a8c-9d12-..."}` ／ マッチなし（自由入力）: `{"provider_code": null, "provider_matched": false, "card_id": "01928a3e-4b2f-7a8c-9d12-..."}` |
| `POINT_CARD_DELETED` | カード物理削除成功時（`DELETE /api/v1/point-cards/{id}`） | `{"provider_code": "tokyu_point", "provider_matched": true, "card_id": "01928a3e-4b2f-7a8c-9d12-..."}` または `{"provider_code": null, "provider_matched": false, "card_id": "01928a3e-4b2f-7a8c-9d12-..."}` |
| `POINT_CARD_VIEWED` | グループ提示モード開始時のみ（`GET /api/v1/point-cards/groups/{id}` 経由）。個別カード詳細取得（`GET /api/v1/point-cards/{id}`）では発火しない（呼び出し頻度が高くログ爆発を招くため） | `{"group_id": "01928a3e-...", "card_count": 4}` |
| `POINT_CARD_GROUP_CREATED` | グループ作成成功時（`POST /api/v1/point-cards/groups`） | `{"group_id": "01928a3e-...", "card_count": 0}` |
| `POINT_CARD_GROUP_DELETED` | グループ削除成功時（`DELETE /api/v1/point-cards/groups/{id}`） | `{"group_id": "01928a3e-..."}` |
| `POINT_CARD_SETTINGS_UPDATED` | ユーザー設定更新時（`PUT /api/v1/point-cards/settings`）。オプトイン切替・規約同意・WebAuthn 要求設定 | `{"is_enabled": true, "terms_version": "v1.0.0"}` |

**補足（metadata 取り扱い注意 — 暗号化対象を含めない）**:
- F18 の `display_name` / `barcode_value` / `nickname` / `memo` は **AES-256-GCM で暗号化保存される個人機密** であり、監査ログの `metadata` に **絶対に含めない**こと。
- `last4` も含めない（4 桁単独では特定不可だが、`provider_code` と組み合わせると識別力が出るため）。
- 含めてよい識別子は `provider_code`（プロバイダー側コード、PII ではない。**案 B 改修により fuzzy match マッチ結果に依存して `null` になりうる**）、`provider_matched`（fuzzy match の有無を示すブール、運用分析用）、`card_id` / `group_id`（UUIDv7、それ単体では特定不可）、`card_count` / `is_enabled` / `terms_version` のみ。
- **【案 B】**`POINT_CARD_CREATED` / `POINT_CARD_DELETED` の `provider_code` は、ユーザーが自由入力で登録したカード（プリセット未使用 + fuzzy match マッチなし）の場合は `null` となる。`provider_matched` ブールを併記することで、運営マスタの拡充判断材料（自由入力カードの登録比率）を取得可能にする。
- 個別カード提示（`GET /api/v1/point-cards/{id}`）では監査ログを記録しない方針。グループ提示モードという「意図的な一括提示行為」だけを `POINT_CARD_VIEWED` で 1 件記録し、`card_count` で対象枚数を残す（カードごとに N 件記録するとログ爆発のため）。
- F18 の保持期間は本機能標準の 2 年で十分（個人ウォレットの調査ニーズはこの範囲でカバー可能）。
- 詳細仕様は `F18_point_card_wallet.md` §10・§11 参照。
...
- **保持ポリシー**: 2年間 DB で保持。2年超過分はアーカイブ（**Cloudflare R2 `audit-archive/` プレフィックス**）へ移動後 DB から削除。アーカイブ先（バケット名 `mannschaft-storage`・パス設計 `audit-archive/{year}/{month}/`）はインフラ構成が固まる **Phase 2〜3 で共通インフラ定義として確定**し、バッチ実装もその時点で行う。`idx_al_created_at` インデックスにより効率的なバッチ処理が可能（実装上の支障なし）。アーカイブ済みログの検索は **Cloudflare Workers 経由のオブジェクト一覧取得・フィルタリング**で対応する想定（インフラ選定時に確定）。アーカイブ対象データは R2 Infrequent Access ストレージクラスの利用を検討する
...
- [x] 1年超過分のアーカイブ先（R2 バケット名・パス設計）を Phase 10 開始前に確定する → **Phase 2〜3 でインフラ構成確定と同時に決定**する方針に変更（Phase 10 まで先送りしない。現時点では DB ストレージに余裕があり先行設計は不要。インフラ選定後に Cloudflare R2 のバケット名 `mannschaft-storage`・パス設計 `audit-archive/{year}/{month}/` を共通インフラ定義として確定し、アーカイブバッチも同時に実装する。Section 6 更新済み）
...
| 日付 | 変更内容 |
|------|---------|
| 2026-05-14 | F18 案 B 改修追従: `POINT_CARD_CREATED` / `POINT_CARD_DELETED` の metadata 例を更新。`provider_code` を Nullable に変更（自由入力カードでは `null`）、`provider_matched` ブールフィールドを新設（fuzzy match の有無）。`display_name` も暗号化対象に追加され metadata 禁止リストに明記。詳細は F18_point_card_wallet.md §11 参照 |
| 2026-05-14 | F18（個人ポイントカードウォレット）連携対応: イベントカテゴリ定義に `POINT_CARD` カテゴリを追加。イベントタイプカタログに新セクション「個人ポイントカードウォレット (F18)」を追加し、6 種のイベント（`POINT_CARD_CREATED` / `POINT_CARD_DELETED` / `POINT_CARD_VIEWED` / `POINT_CARD_GROUP_CREATED` / `POINT_CARD_GROUP_DELETED` / `POINT_CARD_SETTINGS_UPDATED`）と metadata 例を定義。`barcode_value` / `nickname` / `memo` / `last4` の暗号化対象データを metadata に含めない方針を明記。個別カード提示は監査ログを記録せずグループ提示モードのみを `card_count` 付きで 1 件記録する設計を明文化（ログ爆発防止）。詳細は F18_point_card_wallet.md §11 参照 |
| 2026-05-07 | 監視系クロス参照追加: §1 概要に「監視は F10.5（パフォーマンス）/ F10.6（エラー）で分担」と1行追記。本機能はセキュリティ監査記録専任、性能・エラー観測は F10.5 / F10.6 で別軸として整備する役割分担を明確化（F10.5 / F10.6 設計書の同時整備に伴う追記） |
| 2026-04-11 | R2 移行最終仕上げ: Section 6 保持ポリシーの「S3 互換ストレージ」→ Cloudflare R2 `mannschaft-storage` バケット内 `audit-archive/{year}/{month}/` プレフィックスに変更。アーカイブ済みログ検索方針を「Amazon Athena / S3 Select」→「Cloudflare Workers 経由のオブジェクト一覧取得・フィルタリング」に変更。R2 Infrequent Access ストレージクラス利用の検討を追記。未解決事項 #3 の「S3 バケット名」→「R2 バケット名」に変更 |
| 2026-05-06 | F02.3.1（TODO カスタムステータスラベル + キャッチボール）対応: イベントタイプカタログに新セクション「TODO カスタムステータスラベル + キャッチボール (F02.3.1)」を追加。4種のイベント `TODO_STATUS_LABEL_CREATED` / `TODO_STATUS_LABEL_UPDATED` / `TODO_STATUS_LABEL_DELETED` / `TODO_HANDED_OFF` とそれぞれの metadata 例を定義。`TODO_HANDED_OFF` のメッセージ本文は audit_logs に記録せず `has_message` フラグのみ記録する方針を明記。ラベル名（`name` / `old_name` / `new_name` / `name_snapshot`）は調査必須項目として平文記録。詳細は F02.3.1_todo_status_labels_and_handoff.md §6.7 参照 |
| 2026-03-12 | UX・保守性改善10項目: ① F01 新イベント4件をカタログに追加（`LOGOUT_SESSION` / `DEVICE_FINGERPRINT_MISMATCH` / `NEW_DEVICE_LOGIN` / `PASSWORD_SETUP`）② `event_type` のカンマ区切り複数指定を全 API に追加 ③ イベントカテゴリ定義（`AuditEventCategory` enum）と `event_category` クエリパラメータを追加 ④ `AuditEventType` enum によるアプリケーション層での型管理方針を明記 ⑤ ADMIN 向けスコープ付きログ参照 API 枠を追加（`GET /teams/{id}/audit-logs` / `GET /organizations/{id}/audit-logs`、Phase 3 以降） ⑥ アーカイブ済みログの検索方針（Cloudflare Workers 経由オブジェクト一覧取得・フィルタリング）を保持ポリシーに追記 ⑦ ページネーション方式を確定: admin=オフセット維持（件数把握・ページジャンプ有用）、me+スコープ付き=カーソルベース（他ドキュメントと統一） ⑧ テーブルパーティショニングは Phase 1 見送り・将来検討として明記（月別 RANGE パーティション候補） ⑨ 書き込み冪等性は Phase 1 で fire-and-forget 維持を明記（将来 Transactional Outbox 検討） ⑩ `metadata` スキーマは設計書に詳細定義せず Java sealed interface + record で型安全性を担保する方針を明記 |
...
| 2026-03-08 | アーカイブ先確定タイミングを変更: 「Phase 10 開始前」→「Phase 2〜3 でインフラ構成確定と同時に決定」。Section 6 保持ポリシーを更新（Phase 10 以降の記述を削除し Phase 2〜3 での確定を明記。アーカイブ先は Cloudflare R2 `mannschaft-storage` バケット内 `audit-archive/` プレフィックス）。未解決事項を解決済みに更新 |
...
| 2026-02-21 | `target_user_id` 説明にバッチ操作ケースを追記。`ACCOUNT_UNLOCKED` を管理者手動解除のみに限定（Valkey TTL 自然失効はイベント対象外と明記）。`MFA_DISABLED` のトリガー条件を明確化。`me/audit-logs` にもソート順を追記。セキュリティ考慮事項の email_hash 対象を両イベントに更新。Flyway に FK 追加タイミング設計（Phase 1 では INT のみ・FK は後続 migration）を追記。`EMAIL_VERIFIED` 未解決事項を対応済みに更新 |
