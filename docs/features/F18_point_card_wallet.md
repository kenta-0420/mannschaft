...
| ✅ プロバイダーロゴ画像の権利確認 | **頭文字アイコン継続採用で確定（2026-05-18 マスター判断）**。各社ロゴは商標 / 著作物として権利確認・許諾取得コストが大きい一方、`CardTile.vue` の頭文字アイコンフォールバック + プロバイダーマスタ `color_hex` カラムによる背景色での視覚識別で実用上の判別性は十分と判定。F18 v1 ではロゴ画像を導入せず、頭文字 + 背景色での識別に統一する。将来ロゴ導入する場合は (a) 各社個別許諾取得 (b) フェアユース範囲の法務確認 (c) ロゴ非対応プロバイダーの混在運用 のいずれかが前提となる |
...
| 2026-05-18 | **プロバイダーロゴ画像の権利確認 方針確定（マスター判断）**: 各社ロゴは商標 / 著作物として権利確認・許諾取得コストが大きい一方、既存実装の `CardTile.vue` 頭文字アイコンフォールバック + `point_card_providers.color_hex` 背景色による視覚識別で実用上の判別性は十分と評価。F18 v1 ではロゴ画像を導入しない方針で確定し、§16 未解決事項テーブルから「プロバイダーロゴ画像の権利確認 SOP」を ✅ 解決済みカテゴリに移動。これにより F18 設計書の未解決事項は「iOS Safari Wake Lock 実機 QA」1 件（運営タスク）と「残高型 Permission 駆動化（SELF_ISSUED_BALANCE v2 再開時に統合実装）」「SELF_ISSUED_BALANCE 機能凍結（v2 法務整備待ち）」の凍結 2 件のみとなり、**F18 v1 の実装・設計判断は完全クローズ**。将来ロゴ導入が必要となった場合は (a) 各社個別許諾取得 (b) フェアユース範囲の法務確認 (c) ロゴ非対応プロバイダーの混在運用 のいずれかが前提となる |
