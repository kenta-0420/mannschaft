...
> **ステータス**: 🟢 設計完了
> **最終更新**: 2026-06-13（**二重検分反映: SimpleBroker は購読中セッションへの後追い切断機構が無く可視性降格後も次回再接続まで差分配信が届きうる限界を §J.3.3 に明記。ペイロード最小化の二重防御で実害を限定**）
...
- **可視性降格後の後追い切断ギャップ（既知の限界・二重防御で実害限定）**: SimpleBroker（インメモリ・J.5）は**購読中セッションへの後追い切断機構を持たない**ため、購読成立後に対象 match の可視性が降格（例: 公開→非公開へ変更）しても、**当該セッションが次に再接続するまでは差分配信が届きうる**（購読認可は SUBSCRIBE 時点の一回判定で、配信のたびに再評価はしない）。ただし上記の**ペイロード最小化（機微情報を元々含まない）**が二重防御として働くため、仮に降格後の差分が届いても漏れるのは「公開可能な試合進行情報（得点・イベント種別・選手表示名）」に限られ**実害は限定的**である。再接続時はスナップショット再取得（J.4）で `canView` を再評価するため、降格は次回接続で反映される。配信のたびに全購読者の可視性を再判定する厳密化は、外部ブローカー移行（J.5）と併せて将来余地（MVP の観戦は試合進行情報のみで機微情報を持たないためブロッカーではない）。
