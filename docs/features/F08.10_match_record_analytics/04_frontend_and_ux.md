...
|------|------|------------------|------|--------|
| **入口 1: 大会（リーグ/トーナメント）** | TOURNAMENT / LEAGUE | 大会ページの対戦表で、**自動生成済みの対戦カード（fixture）を押下**（カードは事前生成済・05 §H） | カード由来（相手・日時は fixture から確定済） | → `live.vue` タイムライン |
...
- **得点 = 3 タップ**: [得点ボタン] → [得点者を選手グリッドから選択] → [任意でアシスト者選択 or スキップ]（アシスト連携は G.2a の連鎖モデル）。
- **交代 = 3 タップ**: [交代ボタン] → [OUT 選手] → [IN 選手]。SUB_OUT/SUB_IN の 2 イベントを 1 操作で生成。
- **カード = 3 タップ**: [カードボタン] → [選手] → [黄/赤]（赤・2 枚目黄は out 確定・02 §E）。**黄/赤を確定するとそのまま理由コードの選択式リストに繋がる**（任意・スキップ可・G.2c）。
- **タイムライン各行に記録チーム識別インジケーター**（自チーム/相手チームのアイコン・薄背景）を表示（G.10）。
- **タイムライン各行に「理由・メモ」（`note`）と連鎖（`linked_event_id`）を視覚化**して表示する（G.2a・G.2b）。

#### イベント種別の選択 UI（プリセット＋その他）

- イベント種別は**選択式**: よく使う種別をプリセット大ボタンで提示する。`[得点]` `[アシスト]` `[警告]` `[交代]` の **4 プリセット ＋ `[その他]`**。
- 既存の「得点・交代・カード」の 3 タップ大ボタン（画面下部固定）と統合し、**よく使う 4 種＋「その他」の選択 UI** に拡張する（3 タップの速さは維持）。
- `[その他]` を選ぶと **自由入力**（`custom_label`＝ラベル名 ＋ `note`＝理由メモ手書き）を受け付ける（`event_type=OTHER`・01 §D.2）。スコア・出場時間には影響しない（01 §D.2 表）。
- 各イベントには **「選手（背番号/名前）」＋「理由・メモ」自由記述枠（`note`）** を付帯できる（任意）。例: アシストに「7 番、コーナーキックから」。

#### G.2a イベントの時系列連鎖（双方向・GOAL ⇔ ASSIST）

得点とアシストは**それぞれ固有の選手・背番号・理由（`note`）を持つ 2 つの独立イベント**であり、`linked_event_id`（01 §B.2 自己参照）で双方向に連鎖させる。**速い道**と**物語る道**のどちらからでも同じ連鎖が作れる。

- **速い道（得点起点）**: `[得点]` → 得点者選択 → **「＋アシストを紐付け」** → アシスト者＋理由（`note`）。GOAL と ASSIST の 2 イベントを生成し、相互に `linked_event_id` で連鎖。
- **物語る道（アシスト起点）**: `[アシスト]` → 選手＋理由（`note`）→ **「→ 得点へつなぐ」** → 得点者。逆順でも同じ連鎖（ASSIST と GOAL を `linked_event_id` で結ぶ）。
- いずれも **2 つの独立イベント**（各々 `note`・選手・背番号を持つ）を `linked_event_id` で結ぶ。アシストは「得点のサブステップ」ではなく**固有の `note`/選手/背番号を持つ独立イベント**として扱う。
- **集計は二重計上しない**: 02 §F の集計はイベント単体カウント（GOAL=得点者の goals、ASSIST=アシスト者の assists）であり、`linked_event_id` は表示・関連付けのメタ情報（01 §B.2）。
- 3 タップの速さは維持しつつ、連鎖（紐付け）は**任意の拡張ステップ**（スキップ可）とする。

#### G.2b タイムライン表示での連鎖の束ね

- `linked_event_id` で連鎖したイベントは、タイムライン上で**視覚的に束ねて**表示する（例: `7番 アシスト ⤵ / 9番 得点`）。
- 連鎖の各行はそれぞれの `note`（理由）を表示する（「7 番 アシスト（コーナーキックから）⤵ / 9 番 得点」）。
- 連鎖の一方を削除しても他方は残る（`ON DELETE SET NULL`・01 §B.2）。残った側は連鎖表示が解除され単独イベントとして表示される。

#### G.2c 警告・退場の理由コード選択（JFA 標準・選択式＋補足メモ）【マスター御裁可】

警告/退場を選んだら、**標準理由コードを選択式リスト**で提示する（01 §D.5 のカタログ・`card_reason_code`）。構造化コード＋補足の自由記述（`note`）の**両方**を 1 イベントに付けられる。

- **警告（黄・2 枚目黄）**→ `CautionCode` C1〜C8 を選択式リストで提示（**各コード＋日本語短ラベル**: 例「C2 ラフプレー」）。`SECOND_YELLOW`（2 枚目の警告による退場）は退場でもあるため **CS（警告 2 回）**も併せ提示する（01 §D.5.3）。
- **退場（赤・一発）**→ `SendingOffCode` S1〜S6 を選択式リストで提示（例「S2 乱暴な行為」）。一発退場では CS（警告 2 回）は提示しない（01 §D.5.3）。
- **さらに補足の自由記述（`note`）枠を併記**する（例「背後からのチャージ」）。コードで分類しきれない事情はここに書く。
- **最短操作（ADHD 配慮）**: 「**コードをタップ → 確定**」で完了できる短い操作にする。3 タップ（カード→選手→黄/赤）の速さは維持し、理由コード選択は**その流れに続く任意ステップ**（スキップ可）。
- **デフォルトは未選択可**（後で補完できる）。ただし**公式戦（記録係あり・03 §C.1）では理由コードの記録を推奨**する旨を UI で案内する（必須にはしない）。
- コード記号（C1 等）は言語非依存・固定。短ラベルは i18n（`match.card_reason.*`・G.6）で 6 言語表示する。

#### G.2d タイムライン表示での理由コード表示

- タイムライン各行で、警告/退場イベントは **カードアイコン（色＋形状）＋理由コード＋短ラベル＋選手**を束ねて表示する（例: **「🟨 C2 ラフプレー（7 番）」**）。
- 理由コード未選択のカードは従来どおりカード＋選手のみ表示（例「🟨（7 番）」）。
- 補足の `note` があれば併記する（例「🟨 C2 ラフプレー（7 番・背後からのチャージ）」）。
...
- **よく使うイベント大ボタン**（得点・アシスト・警告・交代 の 4 プリセット＋「その他」）を画面下部に固定配置（種別選択 UI・G.2）。
...
- **理由コード短ラベルも `match.json` 内の `match.card_reason.*` namespace に含める**（C1〜C8 / S1〜S6 / CS の説明文を 6 言語で翻訳・コード記号自体は言語非依存）。`match.json` は既に `nuxt.config.ts` の `files` 配列へ登録するため、**理由コード用に新規ファイル登録は不要**（match.json に内包）。

| namespace | 用途 |
|-----------|------|
| `match.list` | 一覧（フィルタ・W/D/L ラベル・進行中バッジ・空状態） |
| `match.create` | 作成フォーム（クイックスタート・相手・モード・日時・会場） |
| `match.live` | ライブ記録（ボタン・タイマー状態・undo・409 トースト・相手分依頼メッセージ・**種別選択／その他／理由・メモ（note）／連鎖（紐付け・つなぐ）ラベル／理由コード選択 UI のラベル・推奨案内**） |
| `match.analytics` | 分析画面（指標名・チャート凡例・期間・空状態・自己ベスト） |
| `match.event_type` | イベント種別ラベル（GOAL→「得点」・**OTHER→「その他」**等） |
| `match.card_type` | カード種別ラベル（YELLOW→「警告」等・形状ラベル併用） |
| `match.card_reason` | 警告/退場の理由コード短ラベル（`C1`…`C8` / `S1`…`S6` / `CS`・01 §D.5）。**コード記号は言語非依存・固定、説明文（短ラベル）を 6 言語翻訳**。例: `match.card_reason.C2`→ja「ラフプレー」 |
