...
> **ステータス**: 🟢 設計完了
> **最終更新**: 2026-06-13（**二重検分反映: §8.4 タイムラインでのファウル理由コード表示を追記＝color 非依存・形状/アイコン併用でファウルコード PF/SF/OF/TF/UF/DF を選手とともに表示**）
...
### §8.4 タイムライン表示でのファウル理由コード表示

コア 04 §G.2d（タイムライン表示での理由コード表示・骨格はコア）のバスケ具体例。サッカーのカード（§8.4 of 01_soccer）に相当する。

- タイムライン各行で、ファウルイベント（PERSONAL_FOUL / TECHNICAL_FOUL / FOUL_OUT）は **ファウルアイコン（形状）＋ファウル種別＋理由コード（PF/SF/OF/TF/UF/DF・§5）＋選手**を束ねて表示する。例: 「⚠ PF パーソナルファウル（24 番）」「⛔ DF ディスクォリファイ（10 番）→ 退場」。
- **色非依存（コア 04 §G.12 準拠）**: 色だけに依存せず、**形状・アイコン・テキストラベル（コード＋短ラベル）を併用**する（テクニカルファウルとパーソナルファウルをアイコン形状で区別・退場 FOUL_OUT は専用アイコン）。色覚多様性に配慮し、Okabe-Ito 等の色盲フレンドリーパレットを用いる。
- 理由コード未選択のファウルはアイコン＋種別＋選手のみ表示（コードは任意・コア 04 §G.2c）。補足の `note`（自由記述）があれば併記。
- 5 個目の PERSONAL_FOUL に達した選手は、当該行で**退場（FOUL_OUT）への遷移を促す表示**（累積ファウル数バッジ）を併せて出す（§8.1）。
- 短ラベルは i18n（`match.card_reason`・§9）で 6 言語表示。コード記号（PF/TF 等）は言語非依存・固定。
...
本書はコア [01_soccer.md](./01_soccer.md) §10 の手順に従い、`Sport.BASKETBALL` 追加・event_type 集合（§2・器に FIELD_GOAL_2 等を追加）・period（§3・QUARTER/OVERTIME）・スコア重み付き合算（§4）・反則コード（§5・別カタログ）・統計（§6・points/rebounds 等）・ポジション（§7・PG〜C）・UX（§8・連鎖 §8.2／ファウル理由選択 §8.3／**タイムラインでのファウル理由コード表示 §8.4（color 非依存・形状/アイコン併用）**／クォーター timer 別 composable §8.5／チャート §8.6）・i18n（§9）を定義した。コアのテーブル・出場時間算出・集計枠組み・権限・IDOR・F00 可視性・WebSocket 観戦の骨格は一切再実装しない（器拡張＝`MatchEventType` への値追加のみ）。
