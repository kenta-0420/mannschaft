...
| 1-B | Entity／enum | `MatchEntity`/`MatchEventEntity`/`PlayerAppearanceEntity`（UuidV7Entity 継承・子は org_id/deleted_at 無し）・`MatchKind`/`MatchStatus`(POSTPONED 含む)/`MatchEventType`(PENALTY_SHOOTOUT 含む) 等 enum・`SportEventCatalog`（案 A） |
...
| 2-D | Controller/DTO | `MatchController`/`MatchEventController`/`MatchStatsController` ＋ Request DTO（jakarta.validation・owning/recorded_by はサーバー導出・03 §C.4a/C.4b） |
