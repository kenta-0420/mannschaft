...
> **ステータス**: 🟢 実装完了（BE・FE）。`/試練`（red 作成）→ `/出陣`（green 化）まで完了。FE の免除 UI（一覧＋免除操作。§12.1 の文言）も実装済み。**債務者（キャンセル料を負っている本人）向けの未払い一覧は未着手**（マスター裁定・別途起票予定）
> **最終更新**: 2026-08-13
...
| 16 | i18n | `recruitment.ts` × 6 言語 | 免除操作・状態表示の文言。**免除の確認 UI の文案は §12.1 に従う（言い切り禁止）** |
| 17 | 新設 API | `recruitment.controller` | `GET /api/v1/recruitment-cancellation-records`（免除 UI のための一覧。マスター裁定 2026-08-13・§12.2） |
| 18 | 新設メソッド | `RecruitmentCancellationRecordQueryService` | 一覧の絞り込み（listing で候補を安く減らし、**escrow の受取先で最終判定**・キーセットページング・§12.2 / §12.3） |
| 18b | 新設メソッド | `ConnectChargeService` | `filterPayeeSettlementManaged(...)`（受取先側の権限を**一括**判定する入口。`isPayeeSettlementManager` と判定分岐を共有・§12.2） |
| 19 | FE | `frontend/app/pages/me/recruitment-cancellation-fees/index.vue` | 免除 UI の一覧画面 |
| 20 | FE | `frontend/app/components/recruitment/RecruitmentCancellationFeeWaiveModal.vue` | 免除の確認モーダル（§12.1 の文案） |
...
- キー・ロケールファイルの配置は既存の i18n ルールに従う。**実際の配置は `common.json` ではなく、募集ドメイン専用のロケールファイル `frontend/app/locales/{ja,en,zh,ko,es,de}/recruitment.ts`**（`nuxt.config.ts` に個別登録済み。既存の募集ドメインの文言もここに集約されている）。**6 言語すべてに追加**（未翻訳ならまず日本語と同じ値で可）。UI への直書きは禁止。キーは `recruitment.cancellationFeeWaive.confirmDialog.message`。

### 12.2 一覧 API のドメイン境界と二段構え（マスター裁定 2026-08-13）

免除 UI には一覧取得 API が要る（当初本書の実装成果物には含まれていなかった漏れ）。以下をマスターが裁定した。

1. **一覧の対象は受取先側（精算管理者・受取先本人）＋ `SYSTEM_ADMIN` に限る。** 債務者（キャンセル料を負っている本人）向けの未払い一覧は**本波では作らない**（別画面の話であり、混ぜると両方が中途半端になる。マスターが別途起票）。
2. **一覧に出す記録の受取先は、payment ドメイン（escrow）が持つ受取先が権威である。**

#### 裁定 2 の是正（2026-08-13・外部検分による差し戻し）

当初の裁定は「`recruitment_listings` が自ドメインに受取先情報を持つので、それで絞れば payment ドメインを読まずに済む」というものだった。**これは誤りであり、撤回する。**

`RecruitmentListingEntity#updateForEdit`（`:361-393`）は **受取先（`payeeKind`/`payeeUserId`）を募集の作成後に変更できる**。したがって listing の受取先を一覧の権威にすると:

- escrow 上の債権者が A のまま、募集の受取先を B へ変更すると
- 一覧は「現在の listing の受取先」で絞るため、**B に A の債権記録（債務者 ID・金額・状態）が見える**
- 免除の実行自体は escrow 基準で 403 になるため債権操作は防げるが、**閲覧の漏えいが残る**

**是正後の要件（動かさない）**: 一覧に出す記録の権威ある受取先は escrow が持つ受取先でなければならない。可変な listing の受取先を権威にしてはならない。

実装は以下で構成する:

| 層 | 役割 | 位置づけ |
|---|---|---|
| `ConnectChargeService#findSourceIdsWithPayeeSettlementManaged` | 操作者が受取先である募集 ID を **escrow から導出**する | 事前絞り込みの候補集合を作る |
| `RecruitmentCancellationRecordRepository#findChunkOfPayeeCandidates` | 上記の募集 ID の `IN` で候補を DB 側で安く減らす | **最適化であって権威ではない**（Javadoc に明記） |
| `ConnectChargeService#filterPayeeSettlementManaged` | escrow の payee で**最終的に絞る** | **権威**。免除 API の `isPayeeSettlementManager` と判定分岐を共有（`isActorManagerOfPayeeAccount` に一本化）し、閲覧と実行で判断が割れないようにする |
| `#findChunkForSystemAdmin` | 無絞り込み | `SYSTEM_ADMIN` は受取先に関わらず全件 |

- **recruitment から escrow のテーブル／Repository は直接読まない**（§3.4・D-5 番人）。payment ドメインの入口メソッドを 1 本呼び、自分が渡した識別子のうち許されたものの集合（`EscrowSourceRef`）だけを受け取る。
- **性能**: 行ごとに payment へ問い合わせるとページ内件数ぶんのラウンドトリップになるため、**一括で解決する入口**にした。escrow は `source_id IN (...)` の 1 往復、受取先口座は `findAllById` の 1 往復、権限判定は**受取先口座ごとに一度だけ**。問い合わせ回数はページ内件数ではなく、ページ内の相異なる受取先の数にしか比例しない。
- **escrow を引けない記録は受取先側には見えない**（§10.2 の最終行と同じ扱い。`SYSTEM_ADMIN` のみ）。
- **事前絞り込みの候補集合も escrow から導出する。** listing の可変な受取先で候補を絞ってはならない——絞ると、受取先を差し替えた瞬間に**本来の債権者が権威ある判定に到達する前に候補から外れ、自分の記録を見失う**（漏えいの裏返しの取りこぼし。契約 IT の実測で露見した）。**事前絞り込みは「権威ある集合を必ず包含する（上位集合である）」ことが安全の条件**であり、それを満たせるのは escrow 側だけである。
- 募集単位の絞り込みだけでは足りない（同一募集でも参加者ごとに escrow は別行）。参加者単位の最終判定は `filterPayeeSettlementManaged` が必ず行う。

3. **免除の実行時にも改めて検証する。** 一覧で絞ったから免除側の検証を省略してよい、は禁止（`RecruitmentCancellationFeeWaiveService` が `isPayeeSettlementManager` を必ず呼ぶ）。

既定の絞り込み状態は免除可能な `PENDING`/`FAILED`/`UNCOLLECTIBLE`（§10.1 の対象状態表と同一）。

### 12.3 一覧のページング方式

**キーセットページング**（`(cancelledAt, id)` の一意な複合キー・カーソル方式）を採る。OFFSET ページングは本件では成立しない。理由は 2 つあり、どちらか一方でも致命である:

1. **母集合が縮む** — 既定の絞り込み（免除可能な 3 状態）では、免除した行がその場で絞り込みから外れる。ページ番号を進める方式だと、縮んだぶんだけ後続の行が OFFSET の網から漏れて読み飛ばされる。
2. **アプリ層で後段の絞り込みが入る** — DB から取った 1 ページを escrow 基準で絞るため、DB の「N 件目から」とアプリの「N 件目」が一致しない。OFFSET はそもそも意味を持たない。

「ページ 0 固定のドレイン」は採らない。それが正しいのは**処理した行が必ず絞り込みから外れる**場合に限られるが、本 EP は `status` を指定して `WAIVED`/`PAID` を閲覧でき、その場合は免除しても行が母集合に残り続ける（ページ 0 を返し続けて先へ進めない）。

**ソートキーは一意にする。** `cancelledAt` 単独では同一時刻の行の順序が不定になり、ページ境界で行が重複・欠落する。末尾に主キー `id` を足して決着させる。

レスポンスは既存の `CursorPagedResponse`（`meta.nextCursor` / `hasNext` / `limit`）を用いる。**総件数は返さない** — DB の絞り込みの後にアプリ層の絞り込みが入る以上、DB だけで数えた総件数は必ず過大な嘘になる。数えられない値を返すより、続きの有無と位置だけを正直に返す。

入力検証: `size` は `@Min(1)`/`@Max(100)`、壊れた `cursor` は 400。いずれも 500 にしない（`@Validated` は付けない——Spring 6.1+ の組込みメソッド検証を抑止してしまう。`ActivityController` の同事故を参照）。
