...
| 18 | 新設メソッド | `RecruitmentCancellationRecordQueryService` | 一覧のドメイン境界内絞り込み（`recruitment_listings` の JOIN のみで受取先を判定。escrow は読まない・§12.2） |
...
2. **一覧の絞り込みは payment ドメイン（escrow）を一切読まない。** `recruitment_cancellation_records.listing_id` から同一ドメインの `recruitment_listings`（`payeeKind`/`payeeUserId`/`scopeId`）を JOIN するだけで受取先を判定できる（§3.4 のクロスドメイン禁止に抵触しない）。
3. **ただし一覧は「絞り込まれた閲覧」であり、認可の最終的な権威ではない。** listing の受取先情報と escrow の受取先が万が一食い違っても、一覧に見えるだけでは免除できない——免除の実行は既存どおり `RecruitmentCancellationFeeWaiveService` が `ConnectChargeService#isPayeeSettlementManager` で必ず再検証する（二段構え。逆に「一覧で絞ったから免除側の検証を省略してよい」は禁止）。

実装は `RecruitmentCancellationRecordRepository#findVisibleToPayee`（TEAM/ORG は `AccessControlService` の候補スコープ絞り込み＋権限判定、USER は本人一致）と `#findAllForSystemAdmin`（無絞り込み）の 2 クエリで構成する。既定の絞り込み状態は免除可能な `PENDING`/`FAILED`/`UNCOLLECTIBLE`（§10.1 の対象状態表と同一）。
