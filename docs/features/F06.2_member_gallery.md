...
| `thumbnail_s3_key` | VARCHAR(500) | YES | NULL | サムネイルの R2 オブジェクトキー。PHOTO は短辺300px の画像、VIDEO は動画のポスターフレーム JPEG（Cloudflare Workers で非同期生成） |
...
- 物理削除
- **写真の後処理**: Cloudflare Workers（または Cloudflare Images）で短辺300px へリサイズしてサムネイル生成。`thumbnail_s3_key` に R2 オブジェクトキーを格納
- **動画の後処理**: Cloudflare Workers（ffmpeg.wasm）または Cloudflare Stream 連携で (a) メタデータ抽出（duration / codec / resolution）、(b) 00:01 時点のポスターフレームを JPEG 化してサムネイルとして R2 に書き戻し、(c) `processing_status = READY` に更新。失敗時は FAILED に遷移してフロントで再生不可表示
...
- **サイズ上限**:
  - **PHOTO**: 1枚あたり **20MB**（アプリケーション層でバリデーション）
  - **VIDEO**: 1本あたり **500MB**（アプリケーション層でバリデーション。F04.1 の 200MB より緩め、ギャラリーは長尺の試合動画等の蓄積用途を想定）
- `cover_photo_id = NULL` かつメディアが0件のアルバムは、フロントエンドでプレースホルダー画像を表示する
- **クォータ計上**: PHOTO / VIDEO ともに `file_size` が当該スコープのストレージクォータに計上される
...
| `file_size` | Long | Yes | ファイルサイズ（バイト）。PHOTO: 最大 20MB、VIDEO: 最大 500MB |

**処理内容**
1. 権限チェック + `allow_member_upload` 検証
2. `content_type` と拡張子の整合性チェック
3. `file_size` 上限チェック（PHOTO: 20MB / VIDEO: 500MB）
4. `StorageQuotaService.checkQuota()` で空き容量確認（不足時 409）
5. R2 オブジェクトキー生成: `gallery/{scope_type}/{scope_id}/album-{albumId}/{media_type|lowercase}-{uuid}.{ext}`
...
{
  "data": {
    "upload_url": "https://{accountId}.r2.cloudflarestorage.com/mannschaft-media/gallery/TEAM/1/album-8/video-abc123.mp4?X-Amz-Signature=...",
...
| ステータス | 条件 |
|-----------|------|
| 400 | `media_type` が不正 / `content_type` が非対応 / `file_size` 上限超過 |
...
6. Cloudflare Workers を非同期起動:
   - PHOTO: EXIF 抽出 → GPS 等プライベート情報ストリップ → HEIC → JPEG 変換 → 短辺300px サムネイル生成 → `processing_status = READY`
   - VIDEO: メタデータ抽出（duration / codec / resolution）→ 00:01 時点ポスターフレーム抽出 → JPEG サムネイル R2 書き戻し → `processing_status = READY`
...
| ステータス | 条件 |
|-----------|------|
| 400 | ファイルサイズ超過（PHOTO 20MB / VIDEO 500MB）/ 非対応形式 / `s3_key` が R2 に存在しない |
| 403 | 権限不足（MEMBER で allow_member_upload = false） |
| 404 | アルバムが存在しない |
| 409 | ストレージクォータ不足 |
...
```
1. フロントエンドが POST /gallery/albums/{id}/media/upload-url で
   Cloudflare R2 の Presigned PUT URL を取得
   - media_type（PHOTO / VIDEO）・content_type・file_size を指定
   - バックエンドが StorageQuotaService.checkQuota() でクォータ事前確認
2. ブラウザから R2 へ直接アップロード（PUT {upload_url}）
3. POST /gallery/albums/{id}/photos で写真/動画メタデータをバックエンドに登録
   - 各 s3_key を R2 HeadObject で実在確認
   - クォータ再チェック → recordUpload() で加算
   - photos INSERT（processing_status = PENDING）
4. Cloudflare Workers（非同期）:
   a. PHOTO:
      - EXIF 情報抽出（撮影日時、幅、高さ）
      - EXIF GPS 情報のストリップ（位置情報漏洩防止）
      - HEIC → JPEG 変換（該当する場合）
      - サムネイル生成（短辺300px）
      - photos レコードを UPDATE（thumbnail_s3_key, taken_at, width, height, content_type, processing_status = READY）
   b. VIDEO:
      - メタデータ抽出（duration / codec / resolution / 撮影日時）
      - GPS 等プライベートメタデータのストリップ
      - 00:01 時点のポスターフレームを JPEG で抽出し R2 に書き戻し
      - photos レコードを UPDATE（thumbnail_s3_key, duration_seconds, video_codec, width, height, taken_at, processing_status = READY）
...
- 動画は 1本 最大500MB のため、少数でも容量を占めやすい点を UI で注意喚起
```
...
- 処理:
  ...
  2. photo_albums: 関連する photos をカスケード削除
     - 全メディアの s3_key + thumbnail_s3_key を非同期バッチで R2 削除
     - StorageQuotaService.recordDeletion() で使用量減算（写真・動画のバイト合計）
  3. 物理 DELETE 実行
...
- **ファイルサイズ制限**: 写真は1枚あたり20MB、動画は1本あたり500MB、1アルバムあたり合計500件を上限
...
- **Presigned URL の安全性**: R2 Presigned PUT URL の有効期限は写真10分・動画15分。`file_key` は UUID ベースで推測不可能
...
V6.021__create_photos.sql                  -- photos テーブル作成（media_type / processing_status を含む）
