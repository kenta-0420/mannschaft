...
> **Cookie 属性ポリシーの横断仕様**: Cookie の `Secure` 属性は環境変数 `MANNSCHAFT_COOKIE_SECURE`（本番 `true`）で制御し、発行/削除で属性を統一する。詳細は [`docs/security/02_cookie_and_session.md`](../security/02_cookie_and_session.md) を参照。なお現行実装では access_token を Cookie 発行し refresh_token は `@CookieValue` 読み取り + DB ハッシュ管理であり、本表のデュアルモード設計との差分は同文書 §3 に記録。
