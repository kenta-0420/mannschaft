...
> **実装フェーズ**: Phase 1
> **最終更新**: 2026-05-19
> **関連ドキュメント**: F01.9 年齢確認・保護者同意機能（F01.9_age_verification_parental_consent.md）、F09.18 メール配信基盤、F10.3 監査ログ
...
> **F01.9 で追加される関連テーブル（詳細は F01.9_age_verification_parental_consent.md 参照）**
> - `parental_consent_links` — 未成年ユーザーと保護者の紐付け（UUIDv7 PK・BINARY(16)、V11.167）
> - `age_group_settings` — 年齢区分ごとの機能フラグ・テーマ設定（6行固定マスタ、V11.168〜169）
...
| `display_name` | VARCHAR(50) | NO | — | 外部公開コンテンツ専用のニックネーム。チーム内・組織内の表示には `lastName + firstName`（実名）を使用すること |
...
| `country_code` | CHAR(2) | YES | NULL | ISO 3166-1 alpha-2 国コード（例: `JP`・`US`・`DE`）。カレンダー祝日表示に使用。NULL の場合は `locale` から推定（`ja`→`JP`、`en`→`US`、`zh`→`CN`、`ko`→`KR`、`es`→`ES`、`de`→`DE`）|
| `birth_date` | VARBINARY(255) | YES | NULL | 生年月日（AES-256-GCM 暗号化）。V17.001 で追加済み。F01.9 で登録時必須化。年齢判定・`PENDING_PARENTAL_CONSENT` 遷移の基準 |
| `status` | ENUM | NO | `PENDING_VERIFICATION` | `PENDING_VERIFICATION` / `ACTIVE` / `FROZEN` / `ARCHIVED` / `PENDING_PARENTAL_CONSENT`（F01.9追加） |
| `last_login_at` | DATETIME | YES | NULL | アーカイブ判定・非アクティブ検出 |
| `reminder_sent_at` | DATETIME | YES | NULL | 将来拡張用（現在未使用）|
...
| `PENDING_PARENTAL_CONSENT` | メール確認済み・保護者同意待ち（F01.9） | 不可（423） |
| `ACTIVE` | 通常利用中 | 可 |
...
- `status=PENDING_PARENTAL_CONSENT` のユーザーが FROZEN になった場合、凍結解除後は `ACTIVE` ではなく `PENDING_PARENTAL_CONSENT` に戻す（保護者同意のスキップを防ぐ）。実装: 凍結前の status を別フィールドに保持するか、解除時に `parental_consent_links` の有効同意レコード存在で判定する（詳細は F01.9 参照）
- `birth_date` は AES-256-GCM 暗号化。登録時は平文で受け取り、サービス層で暗号化してから保存する（F01.9 で必須化）
...
- 登録後7日以内に認証が完了しなかった場合、`AuthCleanupBatchService` によりユーザーアカウントが論理削除される
...
> **注（F09.18 連携）**: 認証メール送信は F09.18 outbox 経由（Phase 18-b で移行）。raw token は `EncryptionService` (AES-256-GCM) で暗号化して `email_outbox.payload_json` に保存される。SES 失敗時は指数バックオフでリトライされ、最大 2.5 時間以内に届く。届かない場合は DEAD_LETTER に移行し SYSTEM_ADMIN に通知される。ユーザーは「認証メール再送」UI でいつでも新しいトークン付きで再送請求できる。
...
> **Cookie 属性ポリシーの横断仕様**: Cookie の `Secure` 属性は環境変数 `MANNSCHAFT_COOKIE_SECURE`（本番 `true`）で制御し、発行/削除で属性を統一する。詳細は [`docs/security/02_cookie_and_session.md`](../security/02_cookie_and_session.md) を参照。**現行実装（2026-05-26〜）では access_token / refresh_token の両方をサーバーが Cookie 発行（`Set-Cookie`）し、login/refresh 成功で body にも返却（モバイル互換）、logout で両 Cookie を maxAge=0 でクリアする**（本表のデュアルモード設計に整合）。refresh_token はサーバー側で DB に SHA-256 ハッシュ管理。
...
| POST | `/api/v1/auth/webauthn/reauthenticate-begin` | 必要 | WebAuthn 再認証開始（重要操作の事前認証 challenge 取得）|
| POST | `/api/v1/auth/webauthn/reauthenticate-complete` | 必要 | WebAuthn 再認証完了（重要操作の事前認証署名検証）|
...
- `display_name`: 必須・1〜50文字（外部公開スコープ専用のニックネーム。チーム内・組織内では実名が表示される）
- `birth_date`: **必須（F01.9）**・`YYYY-MM-DD` 形式・未来日付禁止・100年以上前の日付禁止。18歳未満の場合は保護者同意フロー（F01.9）に遷移する
...
```
※ `mfa_session_token` は Valkey に5分間保持する一時トークン。`/auth/2fa/validate` に渡す。
...
- 対象セッションの Access Token を無効化するため、Valkey に `blacklist:jti:{jti}` を登録（TTL = `exp - 現在時刻`）
...
- Valkey に無効化タイムスタンプを設定（`user_invalidated_at:{user_id}` = 現在 Unix timestamp、TTL 900秒）
...
```
※ `challenge` は Valkey に TTL 5分で保存（リプレイ攻撃防止）。`allowCredentials` にはユーザーが登録したデバイスの `credential_id` 一覧を返す。
...
> フィールドを省略した場合は既存値を維持する（PATCH セマンティクス。フィールドが存在しない = 変更なし）。
> `null` を明示的に送った場合はそのフィールドを NULL に更新する（`nullable` なフィールドのみ）。

**バリデーション**
| フィールド | ルール |
|-----------|--------|
| `last_name` / `first_name` | 1〜50文字 |
| `display_name` | 1〜50文字 |
| `locale` | BCP 47 タグ（許可値: `ja`・`en`・`zh`・`ko`・`es`・`de`） |
| `timezone` | IANA タイムゾーン名（Java の `ZoneId.of()` で検証）|
| `country_code` | ISO 3166-1 alpha-2（英大文字2文字）。不正値の場合は 400 Bad Request |
| `postal_code` | 最大20文字 |

**レスポンス（200 OK）**: 更新後のプロフィール全体（`GET /api/v1/users/me` と同一スキーマ）

**エラーレスポンス**
| ステータス | 条件 |
|-----------|------|
| 400 | バリデーションエラー（不正な `country_code`・`locale`・`timezone` 等）|
| 401 | 未認証 |
...
4. Valkey でロック状態を確認（5回失敗で30分ロック）
5. bcrypt でパスワード検証（失敗 → Valkeyカウンタをインクリメント・audit_logs に LOGIN_FAILED を記録。カウンタが5回に達した時点でアカウントをロック状態に設定し、audit_logs に ACCOUNT_LOCKED を記録）
6. 2FA が有効か確認:
   6a. 2FA 無効: Access Token + Refresh Token を発行して返す
   6b. 2FA 有効: mfa_session_token を Valkey に保存（TTL 5分）して返す
7. ログイン成功時: last_login_at 更新・Valkeyカウンタリセット・audit_logs に記録
...
| `jti` | String | UUID v4。**単一デバイスログアウト**時に Valkey ブラックリストへ登録するキー |
| `roles` | String[] | プラットフォームレベルのロール。**基底 `MEMBER` ＋ SYSTEM_ADMIN ユーザーは `SYSTEM_ADMIN` を追加**（例: `["MEMBER","SYSTEM_ADMIN"]`）。per-scope ロール（ADMIN/DEPUTY_ADMIN/SUPPORTER）は載せない |
...
組織・チーム内のロール（ADMIN / DEPUTY_ADMIN / SUPPORTER 等）は **コンテキスト依存**（どの組織のADMINか）のため JWT に埋め込まない。
リクエスト処理時に DB から動的に解決する。

```
JWT の roles: ["MEMBER"]（SYSTEM_ADMIN なら ["MEMBER","SYSTEM_ADMIN"]）
  └─ per-scope ロールは Service 層 / SpEL ガードで users ↔ memberships / user_roles を都度参照し
     「この操作に対してこのユーザーがこの組織/チームで何のロールを持つか」を解決
     （MEMBER/SUPPORTER は memberships.role_kind、ADMIN/DEPUTY_ADMIN 等の権限ロールは user_roles 参照）
```

**SYSTEM_ADMIN のみ JWT に持つ**ことで、プラットフォーム管理APIのフィルタリング（`hasRole('SYSTEM_ADMIN')`）を DB ラウンドトリップなしに行える。
SYSTEM_ADMIN 判定は発行時に `user_roles`（`existsSystemAdminByUserId`）を参照し、**リフレッシュ時も再判定**する。

> **roles claim を載せる方式（boolean ではなく配列追加）**: `roles` 配列に文字列 `"SYSTEM_ADMIN"` を追加する方式を採用する。`JwtAuthenticationFilter` の既存変換 `new SimpleGrantedAuthority("ROLE_" + role)` にそのまま乗り、SecurityConfig の `hasRole("SYSTEM_ADMIN")` がコード変更なしで機能するため。boolean claim（`system_admin: true`）は変換経路の新設が必要なため採用しない。
>
> **実装と設計の同期に関する注記（2026-05-30）**: 上記は **設計上の正典**であり、ロール認可基盤の根治（[docs/security/03 ロール・権限モデル](../security/03_role_authority_model.md)）で実装をこの設計に合わせる。それ以前の実装は全発行経路が `["MEMBER"]` 固定で SYSTEM_ADMIN を載せておらず、かつ `@EnableMethodSecurity` 未有効のため `@PreAuthorize` が実機で無効だった（病巣の詳細・段階計画は 03 を参照）。なお旧記述にあった `["USER"]` は `roles` シードに存在しないロール名のため誤記とし、基底ロールは **`MEMBER`** に統一する。

**Access Token 無効化戦略（2方式の使い分け）**

| 方式 | Valkey キー | TTL | 使用するイベント |
|------|-----------|-----|----------------|
...
- Access Token の有効期限は最長 15分。無効化タイムスタンプを設定した 15分後には、それ以前に発行された全 Access Token が自然失効するため、Valkey キーも不要になる。TTL = 900秒 で自動回収する。
...
1. mfa_session_token を Valkey で検証（存在しない・期限切れ → 400）
...
1. Valkey から challenge を取得・使用済みチェック（リプレイ攻撃防止）
...
3. challenge（ランダム 32バイト）を生成し Valkey に一時保存（TTL 5分）
...
6. Valkey から challenge を取得・使用済みチェック（リプレイ攻撃防止）
...
4. Valkey で再利用チェック: `totp_used:{user_id}:{totp_code}` が存在する → 400（再利用拒否）
5. 検証成功: Valkey に `totp_used:{user_id}:{totp_code}` を登録（TTL 90秒）
   ※ TTL 90秒 = 3ウィンドウ分（コードが有効でありうる最大時間）
6. 以降の処理（トークン発行 / 2FA有効化 / バックアップコード再生成）へ進む
```

**バックアップコードの再利用防止**
バックアップコードは `two_factor_auth.backup_codes`（JSON 配列）に格納されており、使用時に配列から削除するため DB レベルで自動的に使い捨てとなる。Valkey 管理は不要。
...
7. Valkey に `user_invalidated_at:{user_id}` = 現在 Unix timestamp（TTL 900秒）を設定 → 全 Access Token を即時無効化
...
3. current_password を bcrypt で検証（失敗 → 401。失敗回数は Valkey でカウント・ログイン失敗カウンタとは別管理）
...
8. Valkey に `user_invalidated_at:{user_id}` = 現在 Unix timestamp（TTL 900秒）を設定 → 全 Access Token を即時無効化
...
3. 全 Refresh Token を失効・Valkey に `user_invalidated_at:{user_id}` を設定（TTL 900秒）
...
| ログイン失敗制限 | 同一IP: 1分10回・5回失敗でアカウント30分ロック（Valkey 管理） |
| パスワードリセット | 同一IP: 1分3回まで |
| TOTP 秘密鍵暗号化 | AES-256（`AES_ENCRYPTION_KEY` 環境変数）|
| TOTP 再利用防止 | 検証成功時に `totp_used:{user_id}:{code}` を Valkey へ登録（TTL 90秒）。同一コードの再送信を拒否 |
...
| （ログイン失敗） | 5回失敗で30分ロック | 同一アカウント | Valkey 管理。ACCOUNT_LOCKED 記録 |
...
V13.013__add_country_code_to_users.sql  -- users.country_code CHAR(2) NULL 追加
```
...
| 日付 | 変更内容 |
|------|---------|
| 2026-04-16 | `users` テーブルに `country_code CHAR(2) NULL` を追加（V13.013）。`GET・PUT /api/v1/users/me` のレスポンス・リクエストに `country_code` を追加。カレンダー祝日表示機能（フロントエンド `date-holidays` ライブラリ・100カ国以上対応）向けの国設定。NULL 時は `locale` から推定（`ja`→`JP` 等）。`PUT /api/v1/users/me` の詳細仕様（リクエストボディ・バリデーション）セクションを新規追加。Flyway V13.013 を Section 7 に追記 |
...
| 2026-02-19 | JWT ペイロード仕様を追加。標準クレーム（sub/jti/roles/exp/iat/iss）を定義。Refresh Token は opaque token（非JWT）と明記。Valkey ブラックリストのキー形式・TTL戦略を定義 |
...
| 2026-05-30 | JWT roles claim 仕様を認可基盤完全根治に同期。基底 `MEMBER` ＋ SYSTEM_ADMIN ユーザーは `["MEMBER","SYSTEM_ADMIN"]` を発行（roles 配列に追加・boolean claim ではない）。リフレッシュ時も再判定。`["USER"]` 表記のゆれを `MEMBER` に統一。実装と設計の乖離（全経路 `["MEMBER"]` 固定・`@EnableMethodSecurity` 未有効）を [docs/security/03 ロール・権限モデル](../security/03_role_authority_model.md) へ正典化したことを注記 |
