...
| 12 | `CIRCULATION_DOCUMENT` | (visibility 概念なし、`circulation_recipients` ACL で判定) | ACL固定 (CUSTOM + recipients) | C |
...
- **`CIRCULATION_DOCUMENT`**: visibility 概念なし。`toStandard` は `CUSTOM` を返し、`evaluateCustom` で `circulation_recipients` テーブルに `(document_id, user_id)` レコードがあるかを判定する **ACL 固定戦略**。
  ...
  - Projection には `scopeType/scopeId` を載せて将来拡張に備える。recipients は別 SQL で都度取得する（バルク N+1 回避は `MembershipBatchQueryService` 拡張で後日対応＝**Phase C スコープ外**、Phase D 以降の課題）。
- **`COMMENT`**: Phase C は **CirculationComment 限定**で実装する（CirculationDocument の comment ACL ＝親文書の recipients に従属）。既存の他 Comment 実体（ActivityComment / IncidentComment / TodoComment / WorkflowRequestComment / ProxyVoteMotionComment / SharedFileComment）は **Phase C スコープ外**。汎用化は Phase D 以降の親 Resolver 整備（PHOTO_ALBUM / FILE_ATTACHMENT 等）と一括で別軍議。
...
Survey の `ResultsVisibility.AFTER_RESPONSE` / `AFTER_CLOSE` を Resolver の `evaluateCustom` で扱う際、null 値での評価は **必ず fail-closed = `false` を返す** こと:

| CUSTOM サブ型 | null 条件 | 返却値 | 理由 |
|---|---|---|---|
| `AFTER_CLOSE` | `expiresAt` が null（締切未設定） | `false` | 「閉じた後に公開」と定義されているため、閉じる契機がない場合は永久に「未閉鎖」とみなして閲覧不可 |
| `AFTER_RESPONSE` | `userId` が null（匿名アクセス） | `false` | 「自分が回答した後に閲覧可」と定義されているため、回答主体が特定できない場合は閲覧不可 |
| `VIEWERS_ONLY` | `viewers` 集合が null/空 | `false` | 「指定リスト内のみ閲覧可」と定義されているため、リストが未確定な場合は閲覧不可 |
...
| 関所 | 入口 | 統一方法 |
|---|---|---|
| (1) 通知 | `ScheduleAttendanceService.openAttendanceSolicitation` / `SurveyPublishNotificationListener` | `NotificationService.createNotificationPreAuthorized` / `NotificationHelper.notifyAllPreAuthorized` で `canView`（`isAccessible`/`filterAccessible`）をスキップ。受信者は配信母集団で事前認可済み。**既存 `createNotification`/`notifyAll` の Phase F ガードは他通知のため不変**。 |
...
**(B) 通知レグレッションの根治**: SURVEY 通知が `ResultsVisibility` 軸を含む `canView` で誤 deny され、配信母集団に属する直属一般メンバー・配下チームメンバーへ届かなかった問題を、関所(1)の専用経路化で根治（番人テスト `SurveyPublishNotificationListenerTest` が `notifyAllPreAuthorized` 経由を担保）。

**`submitResponse` ALL 認可穴**: 旧実装は `DistributionMode.TARGETED` のみ `existsBySurveyIdAndUserId` で弾き、`ALL` は認可ゼロ＝組織外の任意ユーザーが `surveyId` を知れば回答できた。`ALL` 時にスコープ準拠の `checkMembershipOrDescendant`（ORG=トグル準拠母集団 / TEAM=直接所属）を追加して塞いだ。
...
#### CirculationDocument ACL 判定アルゴリズム（案 A 確定）

`CIRCULATION_DOCUMENT` の `evaluateCustom` 内で適用する ACL 判定:

```
canView(documentId, viewerUserId) :=
    SystemAdmin                                       (D-13 高速パス、これは Abstract 側で短絡)
 OR EXISTS (
        SELECT 1 FROM circulation_recipients
         WHERE document_id = :documentId
           AND user_id     = :viewerUserId
    )
```

- 親 ORG/TEAM の所属チェックは **行わない**（recipients に登録された時点で配信対象として確定済の前提）。
- recipients 取得は別 SQL（バルク N+1 回避は `MembershipBatchQueryService` 拡張で Phase D 以降に実施＝Phase C スコープ外）。
- Projection には `scopeType/scopeId` を載せておき、SystemAdmin 判定や監査用途には対応可能とする。
- 削除済 document（`deleted_at IS NOT NULL`）は SystemAdmin 以外 fail-closed（既存 §7.5 status ガード規約に従う）。
...
| **F** | ✅ 完了 | 2026-05-04 | 通知配信中央集約ガード（§11.5/§12.7） |
...
| **C** | priority 2 機能の Resolver 実装 (BULLETIN_THREAD, TOURNAMENT, RECRUITMENT_LISTING, JOB_POSTING, SURVEY, CIRCULATION_DOCUMENT, COMMENT (CirculationComment 限定)) | 2 週 | A |
...
**対象**: BULLETIN_THREAD / TOURNAMENT / RECRUITMENT_LISTING / JOB_POSTING / SURVEY / CIRCULATION_DOCUMENT / COMMENT (CirculationComment 限定)

§3.3 対応表との整合をとり、Phase C は **7 機能**。Phase B と同手順だが、以下 3 機能は通常パターン外:

- **`SURVEY`**: ResultsVisibility の時間軸条件 (AFTER_RESPONSE/AFTER_CLOSE/VIEWERS_ONLY) を `evaluateCustom` でハンドリングする必要があり、テストパターン多め。null 値の境界条件は §5.1.4 を参照。
- **`CIRCULATION_DOCUMENT`**: ACL 固定戦略（§3.3/§11.2）。`circulation_recipients` テーブルへの直接照会で判定。Projection と recipients は別 SQL（バルク化は Phase D 以降）。
- **`COMMENT`**: Phase C は **CirculationComment 限定**。親文書 (CirculationDocument) の可視性に従属し、内部で `ContentVisibilityChecker.canView(CIRCULATION_DOCUMENT, parentId, userId)` を呼ぶ（§D-16）。他 Comment 実体は Phase D 以降の汎用化軍議に委ねる。

**工数目安**: 1.5 週 → **2 週**（CIRCULATION_DOCUMENT の ACL 設計と COMMENT の親従属実装の追加分を考慮）

**触れるファイル概算**: 各機能 5 ファイル × 7 機能 = **35 ファイル新規 + 7 ファイル改修**
...
class VisibilityArchitectureTest {
    ...
    /**
     * D-17: AbstractContentVisibilityResolver のサブクラスは @Transactional 禁止。
     * CGLIB プロキシ + final テンプレートメソッドの組み合わせで NPE を起こすため。
     * (PR#345 マージ済)
     */
    @ArchTest
    static final ArchRule abstractContentVisibilityResolver_subclasses_must_not_be_transactional =
        noClasses().that().areAssignableTo(AbstractContentVisibilityResolver.class)
            .and().areNotAssignableFrom(AbstractContentVisibilityResolver.class)
...
### 13.9 Resolver 結合テストは ContentVisibilityChecker 経由必須 (Phase B 教訓)

**Phase B 教訓** (PR#319 BlogPost が CI green でマージされてしまい、後段 PR#320 で本番経路 NPE が発覚した事例)。

Resolver の結合テスト（`*VisibilityResolverIntegrationTest`）は、**必ず `ContentVisibilityChecker.canView(...)` 経由**で書くこと。Repository を直接呼んで Projection の整合性だけを検証するテストは禁止。

| アンチパターン（禁止） | 推奨（必須） |
|---|---|
| `repository.findVisibilityProjectionsByIdIn(...)` を直接呼んで戻り値を assert | `checker.canView(ReferenceType.X, contentId, userId)` を呼んで boolean を assert |
| Resolver を `@Autowired` で取り出し `resolver.canView(...)` を呼ぶ | `ContentVisibilityChecker` を `@Autowired` し ファサード経由で呼ぶ |

**理由**: Resolver を直接呼ぶと Spring の CGLIB プロキシ経路を踏まないため、D-17 で説明した `@Transactional` 起因の NPE バグが **テストでは検出できない** が **本番で死ぬ**。ファサード経由ならレジストリ → プロキシ → 実装という本番コールパスを忠実に踏めるので、CGLIB 罠を含む組み立てミスがテストで露呈する。

**推奨テンプレート**: `VisibilityCheckerTestSupport` のヘルパに加え、結合テスト用の `VisibilityCheckerIntegrationTestSupport`（実 Bean を `@Autowired` するベースクラス）を Phase A 成果物に同梱。各機能の結合テストはこれを継承して書く。

`TEST_CONVENTION.md` に上記を規約化（Phase B 教訓を踏まえ v0.4 で追記）。
...
### D-17: AbstractContentVisibilityResolver のサブクラスは @Transactional 禁止 (✅, v0.4 追加)

**Phase B 教訓** (PR#320/321 で発覚 → PR#344/347 で根治済、ArchUnit ガード PR#345 マージ済)。

**判断**: `AbstractContentVisibilityResolver` のサブクラス（個別 Resolver 実装）には `@Transactional`（クラスレベル / メソッドレベル問わず）を **付けてはならない**。

**根本原因の連鎖**:

1. `@Transactional` を付与した Spring Bean は、Spring が **CGLIB プロキシ化** する（インタフェース実装でない場合の標準挙動）。
2. CGLIB プロキシは Objenesis でコンストラクタを bypass してインスタンスを生成するため、プロキシインスタンス自体の **フィールドはすべて `null`** になる（プロキシは委譲先 target に転送するだけが本来の責務）。
3. ところが `AbstractContentVisibilityResolver.canView` 等のテンプレートメソッドは `final` 宣言されており、**CGLIB が override できない**。
4. 結果として `final` メソッド内の処理は **プロキシインスタンス上で（target ではなく）実行** される。
5. プロキシインスタンスのフィールドは null なので、`this.membershipBatchQueryService` 等の参照が **`NullPointerException`** で死ぬ。

**正しい配置**:

- トランザクション境界は **下層の Repository / `MembershipBatchQueryService` が自前で持つ** ため、Resolver 自体に `@Transactional` は **不要**。
- どうしても Resolver スコープでトランザクションを張りたい場合は、`final` を解除するのではなく **別 Service クラスに切り出して** `@Transactional` をそちら側に付ける。

**強制機構**:

- ArchUnit ルール `abstractContentVisibilityResolver_subclasses_must_not_be_transactional` を §13.5 に追加（PR#345 マージ済）。
- クラス自身およびメソッド両方に `@Transactional` 付与禁止を検査。

**実例**: PR#320 (BlogPostVisibilityResolver) / PR#321 (EventVisibilityResolver) で同症状発覚 → PR#344/347 で `@Transactional` 削除により根治。
