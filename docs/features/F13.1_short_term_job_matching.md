...
| 既存機能 | 関連内容 |
|---|---|
| F01.2 組織・チーム・メンバー・ロール | `team_members.role` enum に **`JOBBER` 追加**、`MANAGE_JOBS` 権限の詳細化 |
...
(A) 既存 Mannschaft ユーザーを JOBBER として招待する場合:
  ...
  3. 招待された側が受諾 → team_members に (user_id, team_id, role='JOBBER', joined_at, invited_by) を INSERT
...
- 招待拒否時: `team_members` には INSERT されず、監査ログに `JOB_JOBBER_INVITATION_DECLINED` を記録
...
- 応募時点で当該チームの `team_members` に JOBBER として自動加入（JOIN-ON-APPLY 方式）するか、応募のみで加入しない（APPLY-ONLY 方式）かは **Requester 側が求人作成時に選択**（`job_postings.auto_join_jobber_on_apply` BOOLEAN フィールド）
...
Requester / ADMIN が業務終了後に各 Worker の業務時間を入力
  ...
  ↓
Worker に「⏱ 業務時間の確認依頼」通知 (`JOB_TIME_CONFIRMATION_REQUESTED`, 強制配信)
  ↓
...
- `team_members.role` enum に `JOBBER` 追加
...
| （既存、修正）`team_members` | **`role` enum に `JOBBER` を追加** | — |
...
##### `team_members`（F01.2 所管）

- `role` ENUM に **`JOBBER`** を追加
  - 変更後: `ENUM('SYSTEM_ADMIN','ADMIN','DEPUTY_ADMIN','MEMBER','SUPPORTER','JOBBER','GUEST','PUBLIC')`
  - F01.2 設計書にも連動追記（§17.3 参照）
...
erDiagram
    ...
    teams ||--o{ team_members : "has_members_incl_jobber"
...
> - `teams ↔ team_members (role=JOBBER)` — チームと JOBBER は多対多
...
-- V13.020__alter_team_members_add_jobber_role.sql
ALTER TABLE team_members
  MODIFY COLUMN role ENUM('SYSTEM_ADMIN','ADMIN','DEPUTY_ADMIN','MEMBER','SUPPORTER','JOBBER','GUEST','PUBLIC') NOT NULL;
...
3. `team_members (user_id, team_id, role='JOBBER')` を INSERT（UPSERT、既に `MEMBER/SUPPORTER` の場合は 409）
...
Phase 13.1.11: 運用ダッシュボード・リコンシリエーション・Geolocation 削除バッチ
    ↓
Phase 13.1.12: 【第三版】JOBBER ロール基盤（team_members ENUM 拡張 + JobberProfile + 招待フロー）
    ↓
...
backend/src/main/resources/db/migration/
  ...
  V13.020__alter_team_members_add_jobber_role.sql,        ← 第三版
...
| **F01.2 `team_members.role` enum への JOBBER 追加** | §5.2 / §17.3 で V13.020 マイグレーション明記。既存データへの影響なし（enum 拡張のみ） |
