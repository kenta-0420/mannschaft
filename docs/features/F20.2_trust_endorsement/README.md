...
| ファイル | 内容 |
|---|---|
| `README.md`（本書） | 概要・中核モデル（一方向 DAG の信任グラフ／状態機械／アンカー／資格条件）・受け入れ条件 AC・スコープ・状態遷移図・段階ロードマップ・論点の確定（§11 マスター御裁可済）・変更履歴 |
...
- [ ] 厳密な非巡回（acyclicity）検証（**§11-1 マスター御裁可済＝(a) 採用**・「信任元は認証済み」不変条件のみで未認証リングを阻止・サイクル検出はしない）
...
- 信任は**方向を持つ**（A が B を信任する ≠ B が A を信任する）。「相互認証」ではない。A→B と B→A が両方立つこと（巡回）は**許容する**（§11-1 マスター御裁可済＝(a)・サイクル検出はしない）。
...
> 閾値は「安全側の既定」を置き、運用で緩める前提。値は**マスター御裁可済（2026-07-08・暫定値）**でベータ実データで調整する前提（§11-2）。config 外部化済みで無停止変更可。
...
4. **連鎖は Y までの 1 段で止める**。Y が `UNDER_REVIEW` になっても、Y が発行した信任は**無効化しない**（既存の被信任先を守る）。ただし Y は `UNDER_REVIEW` の間**新規の信任は発行できない**（§11-3 マスター御裁可済＝(b)・発行資格は `state=CERTIFIED` 厳密一致のみ）。Y の被信任先へは波及させない。
...
## 11. 論点の確定（マスター御裁可済・2026-07-08）

以下 7 論点は**すべてマスター御裁可済（2026-07-08）**。各々の**確定選択肢**を実装の正とする（推奨どおり確定）。

| # | 論点 | 確定（マスター御裁可済 2026-07-08） |
|---|---|---|
| **11-1** | 信任グラフの厳密な非巡回（acyclicity）を強制するか | **確定＝(a)**「信任元は認証済み」不変条件のみ。**サイクル検出はしない**（認証済み同士の A→B・B→A や巡回は許容）。未認証リングの自己認証は (a) で構造的に阻止できる。§9 の「探索不要」原則・1000 万規模コストのため (b) は採らない。「一方向 DAG」は「アンカー起点で認証が外へ伝播する」意味と整理する |
| **11-2** | 資格の既定値（N ヶ月・M 人・年間上限） | **確定＝推奨値（暫定・マスター御裁可済）**: 設立 **6 ヶ月**＋アクティブメンバー **5 人**＋年間上限 **10 件**。**ベータ運用の実データで調整する前提**の暫定値。`mannschaft.trust.*` で config 外部化済みのため後から無停止変更可（§3.3） |
| **11-3** | `UNDER_REVIEW` 団体は信任を発行できるか | **確定＝(b) 発行不可**。信任発行資格（§3.3）は `state=CERTIFIED`（＋アンカー）**厳密一致のみ**。`UNDER_REVIEW`（信頼が揺らいでいる団体）には新規信任を発行させない。ただし §3.7 の連鎖で `UNDER_REVIEW` に落ちた団体が**過去に発行した**信任は 1 段制限で無効化しない（既存の被信任先は守る・非対称は §3.7／状態遷移表に明記済） |
| **11-4** | 閾値 T=3 の変更可能性 | **確定＝3 ＋定数化**（`mannschaft.trust.certification-threshold=3`）。運用変更の余地のみ残す |
| **11-5** | 1 団体が受けられる信任の上限（incoming 側） | **確定＝(a) 無制限**。incoming は無制限で件数を信頼の厚みとして公開表示。上限は発行側（outgoing）の年間上限のみ |
| **11-6** | endorsee 側の同意（受任の承諾）フローの要否 | **確定＝(a) 同意不要 ＋ (c) を将来拡張**。信任は endorser の一方向の対外表明で、endorsee 側の承諾フローは設けない（自然拡大を阻害しないため）。公開面 opt-out（(c)）は**将来拡張点**として温存（[03 §9.1](03_security.md)）。**存在オラクル対策**（(a) でも endorsee は「endorser 管理者から F00 可視」の場合のみ許可し、不可視・不在を同一応答 `TRUST_007` に統一）は確定仕様（[03 §3](03_security.md)・[02 §2.3](02_api_design.md)） |
| **11-7** | PRIVATE 団体が当事者の信任の公開範囲 | **確定＝(b) 安全側**。相手方が F00 可視（PUBLIC）の信任のみ公開面に出す（F19.1 の非公開原則と矛盾させない）。**件数 `validEndorsementCount` は除外に関わらず全件カウント**（[03 §4.1](03_security.md)） |

> 全 7 論点はマスター御裁可済（2026-07-08）。実装（軍議→試練→出陣）はこの確定選択肢を正として進める。11-2 の資格既定値（6 ヶ月／5 人／10 件）は暫定値であり、config 外部化により後から無停止で調整できる。
...
| 2026-07-08 | **精査第2パス反映**。(重大) 01 CREATE TABLE 本体/CHECK/COMMENT/ER 図の `revoke_reason` を `ENDORSER_DELETED` 込み 4 値に統一。(中位) 02 §3.3 取消フロー返却コード分離（非存在=TRUST_007／取消済み=TRUST_008）＋認可を状態判定の前に配置・AC-24 に F00 可視条件追記・削除カスケード耐障害性を整合バッチ孤児信任検出で補償（02 §5.4.1/03 §5/§8）。(低) 02 §8 AC 対応列更新・公開 state 丸め注記・`TrustBadgeVisibility.publicState`・削除団体宛て信任の年間上限 12 ヶ月残留の許容明記・03 §4.1 言い切り |
| 2026-07-08 | **実機 E2E 耐性クリティック反映**。(高) H1 テナント基底矛盾解消＝`AbstractTenantAwareRepository` 非継承→素の `JpaRepository`＋`findByOrganizationId`（`deleted_at` 無しの `EscrowTransactionRepository` 前例準拠・継承すると起動時 PropertyReferenceException で全 E2E 即死）／H2 アンカー資格条件の明示（アンカーは条件1のみ免除・信任発行には設立6ヶ月/precision非NULL/DISTINCT5人が別途必要）＋実機 E2E 最小 seed レシピ（§10.1・鶏卵問題の解）。(中位) M1 日時シリアライズ規約（§0.1・既定 Jackson `LocalDateTimeTimezoneSerializer` 準拠）／M2 レート制限実結線（03 §6.1・`common.ratelimit`）／M3 通知実チャネル（§8・`NotificationHelper.notify`＋`NotificationType` 新値・観測点=`notifications` テーブル type 行）／M4 direction 契約（incoming/outgoing 常存在・対象外は `[]`・省略時 BOTH）。(低) L1 `ScopeType` 完全修飾名・L2 orphan 判定を team/org 読み取り Service 経由。全精査完了によりステータス 🟢 設計完了へ |
| 2026-07-08 | **要裁可論点 §11 のマスター御裁可を反映（11-1〜11-7 確定・実装待ちへ）**。11-1=(a) 非巡回は不変条件のみ・サイクル検出せず／11-2=推奨値確定（設立6ヶ月・メンバー5人・年間10件／暫定・ベータ実データで調整）／11-3=(b) UNDER_REVIEW は信任発行不可／11-4=閾値3＋定数化／11-5=(a) incoming 無制限／11-6=(a) endorsee 同意不要＋(c) 公開面 opt-out は将来拡張／11-7=(b) PRIVATE 当事者の信任は相手が F00 可視の分のみ公開・件数は全件。§11 見出しを「要裁可」→「マスター御裁可済（2026-07-08）」に更新・03 §9.1/§9.2 を確定済みに更新。ステータス 🟢 設計完了（マスター御裁可済・実装待ち）へ |
