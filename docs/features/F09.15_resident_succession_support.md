...
| メソッド | パス | 認可 | 用途 |
|---|---|---|---|
| GET | `/api/v1/succession/covenant-templates` | ADMIN/MEMBER | 誓約テンプレート一覧（現行 version） |
...
| GET | `/api/v1/succession/pre-registrations/me` | MEMBER | 自分の事前登録の取得（封緘状態でも本人は閲覧可） |
| PUT | `/api/v1/succession/pre-registrations/me` | MEMBER | 自分の事前登録の更新（封緘維持） |
| GET | `/api/v1/succession/pre-registrations/{id}` | ADMIN（UNSEALED 時のみ）| 事前登録の閲覧（unseal_audit_views に記録） |
| POST | `/api/v1/succession/residents/{residentId}/death-status` | ADMIN | 死亡状態の入力（SUSPECTED/CONFIRMED/CANCELLED_FALSE_ALARM）|
| POST | `/api/v1/succession/unseal-requests` | ADMIN | 封緘解除申請の起票 |
| POST | `/api/v1/succession/unseal-requests/{id}/first-approve` | ADMIN/DEPUTY_ADMIN(`MANAGE_SUCCESSION_UNSEAL`)| 一次承認 |
| POST | `/api/v1/succession/unseal-requests/{id}/second-approve` | ADMIN/DEPUTY_ADMIN(`MANAGE_SUCCESSION_UNSEAL`)| 二次承認（72h TTL カウントダウン開始） |
| POST | `/api/v1/succession/unseal-requests/{id}/reject` | ADMIN/DEPUTY_ADMIN | 承認プロセス拒否 |
| GET | `/api/v1/succession/unseal-requests/{id}/audit-views` | ADMIN | 開封中閲覧履歴 |
| GET | `/api/v1/succession/delinquency-escalations` | ADMIN/DEPUTY_ADMIN | 5 段階エスカ進行中の一覧（理事長ダッシュボード） |
| POST | `/api/v1/succession/delinquency-escalations/{id}/freeze` | ADMIN | エスカレーション凍結（弁護士介入等） |
| POST | `/api/v1/succession/delinquency-escalations/{id}/resolve` | ADMIN | エスカレーション解決（手動クローズ） |
| POST | `/api/v1/succession/legal-filings` | ADMIN | 法的手続き起票（テンプレ PDF 生成） |
| GET | `/api/v1/succession/legal-filings/{id}/evidence-zip` | ADMIN | 区分所有法 8 条証拠 ZIP のダウンロード URL（Pre-signed・1h・ADMIN のみ） |
| POST | `/api/v1/succession/legal-filings/{id}/evidence-rebuild` | ADMIN | 証拠 ZIP の再生成 |
| GET | `/api/v1/succession/frozen-account-guidance` | ADMIN | 凍結口座汎用テンプレ + FAQ |
...
#### POST `/api/v1/succession/unseal-requests/{id}/second-approve`
...
#### POST `/api/v1/succession/residents/{residentId}/death-status`
...
- 検証 API: `POST /succession/covenants/{id}/verify` — 保存中 PDF を取得し再ハッシュ + トークン検証
...
- F00 ContentVisibilityResolver に `reference_id_uuid BINARY(16) NULL` 並列カラムを追加（V0.xxx 採番は実装時調整）
- F00 設計書側 §5.1.4 CUSTOM 値運用規約に「UUIDv7 reference 系は `reference_id_uuid` を使う」追記
- Flyway 採番予約調整: 本機能 V9.020〜V9.026 を予約
...
- F08.2 PaymentStatusChangedEvent 購読
- 5 段階バッチ
- F03.6 ORG_WIDE 横展開
- 死亡疑い自動起票
...
| バージョン | 内容 |
|---|---|
| V9.020 | `resident_registry` 拡張 5 カラム（ALTER TABLE）|
| V9.021 | `succession_covenants` テーブル新設 |
| V9.022 | `succession_pre_registrations` テーブル新設 |
| V9.023 | `unseal_requests` テーブル新設（DB CHECK 含む）|
| V9.024 | `unseal_audit_views` テーブル新設 |
| V9.025 | `delinquency_escalations` テーブル新設 |
| V9.026 | `legal_filings` テーブル新設 + `MANAGE_SUCCESSION_UNSEAL` permission 追加 |

> V9.020〜V9.026 は **仮予約**。実装着手時に Phase 9 系の他機能（F09.16 V9.030〜V9.036、その他）との連番衝突を確認し、必要に応じてシフトする。

### F08.2 / F03.6 / F03.12 / F04.10 / F09.1 / F00 への追記スキーマ変更

| 変更 | Flyway |
|---|---|
| F00 `reference_id_uuid BINARY(16) NULL` 並列カラム追加 | S0 で先行（採番は実装時調整） |
