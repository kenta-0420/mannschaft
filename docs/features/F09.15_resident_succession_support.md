...
> **ステータス**: 🟢 設計完了 + ✅ v1 実装完了（S0〜S6）
> **実装フェーズ**: Phase 9（v1 実装フェーズ完遂 / 残: S6-D 法的監修・v2 国際化）
> **最終更新**: 2026-05-16
...
> `/api/v1/succession/...` として設計されたが、現実装は **組織スコープ**
> `/api/v1/organizations/{orgId}/succession/...` に統一されている。
> 該当 Controller: `UnsealRequestController` (6 件) / `LegalFilingController` (6 件) /
> `DelinquencyEscalationController` (4 件) / `SuccessionCovenantController` (2 件)。
> 下記のパス記載は旧設計のものであり、次フェーズで全行を `/api/v1/organizations/{orgId}/succession/...`
> に書き換える PR が必要。
