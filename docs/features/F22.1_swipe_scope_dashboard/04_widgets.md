...
> **管理者レンズ（F10.1.1）**: ADMIN / DEPUTY_ADMIN 向けの管理者ウィジェット群（`ADMIN_*` プレフィックス。予約/予算/承認待ち/メンバー統計/アラート/通報/モジュール/コンソール導線）は、本書の `SWIPE_*` メンバー向け 8 枚とは別の名前空間で定義され、チーム/組織パネルの**レンズトグル**で切り替えて表示する。一覧と可視性は [F10.1.1/02_admin_lens_widgets.md](../F10.1.1_team_org_admin_console/02_admin_lens_widgets.md) を参照。横スワイプの軸（スコープ）は不変で、第4タブは追加しない。
