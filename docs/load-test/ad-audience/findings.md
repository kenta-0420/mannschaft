...
|---|---|---|---|---|---|
| 10,000 | 3,021 | 2,000 | 45 | 1,000 | 396 |
| 30,000 | 5,579 | 6,000 | 62 | 3,000 | 287 |
| 50,000 | 11,515 | 10,000 | 161 | 5,000 | 1,139 |

（2回の実行結果があり、上表は増幅測定を含む最終実行（第5.5節と同一実行）の値。両実行とも
`matchedCount`/`resultCount` は完全一致しており、所要時間はDBキャッシュ状態・JITウォームアップの
影響で数十〜数百ms揺れる。件数に対して桁が変わるような悪化は両実行とも見られなかった）。
...
```
member_count=10000  heap_delta_bytes=-995736  bytes_per_user=-995
member_count=30000  heap_delta_bytes=-32504   bytes_per_user=-10
member_count=50000  heap_delta_bytes=-57664   bytes_per_user=-11
```

（別実行では `+1,000,624` のように正の値が出たこともあったが、同じ規模・同じ処理でも
実行のたびに符号や桁がぶれること自体が「単発測定はノイズに支配されている」ことの証拠である）。
...
第5.3節の (a) 案とは別に、単発計測の弱点（GCノイズが対象より大きい）そのものを克服する
追加の実測も試みた。同一の複合クエリを `resolve()` 相当の経路で50回繰り返し、
**その全結果 `Set` をリストに保持したまま**（GCで回収されないようにする）ヒープ差分を取り、
「保持したエントリ総数」で割る方式である（第5.3節でいったん不採用とした案(b)の変種だが、
母集団規模を上げる代わりに反復回数を増やして対象を大きくしたため、縮小版の趣旨は維持している）。

```
member_count=10000  amp_repeat=50 amp_total_users=50,000  amp_heap_delta_bytes=4,313,896  amp_bytes_per_user=86
member_count=30000  amp_repeat=50 amp_total_users=150,000 amp_heap_delta_bytes=9,252,416  amp_bytes_per_user=61
member_count=50000  amp_repeat=50 amp_total_users=250,000 amp_heap_delta_bytes=15,145,040 amp_bytes_per_user=60
```

**今回は正の値で安定し、実測として成立した。** 対象総数（5万〜25万件）を GC ノイズ（±1MB程度）より
十分大きくしたことで符号が安定し、対象規模が大きいケースほど1エントリあたりの値が
60バイト付近に収束している（小さいケースでの86バイトは、対象総数がまだ相対的に小さく
ノイズの影響が残っているためとみられる）。

**この実測値（約60〜61バイト/エントリ、大きいサンプルでの値を採用）は、第5.4節で計算のみで
出した約64バイト/エントリの理論値と高い精度で一致する。** これは (a) の計算根拠が
妥当であったことの実測による裏付けであり、以降の外挿は**実測で裏付けられた60バイト/エントリ**を
採用する（計算のみの64バイト/エントリより実測に近い値を優先する）。
...
実測比率（複合ケース: memberCount の 10%）を、第5.5節で実測により裏付けられた
60バイト/エントリで延伸すると:

| 規模 | resultCount（10%） | 推定メモリ（60B/entry・実測裏付け） |
|---|---|---|
| 500,000 | 50,000 | 約 3.0 MB |
| 1,000,000 | 100,000 | 約 6.0 MB |
...
| 規模 | peak時の概算エントリ数（3セット合計・本試験の分布比率） | 推定メモリ（60B/entry・実測裏付け） |
|---|---|---|
| 500,000 | 約 491,000 | 約 29.4 MB |
| 1,000,000 | 約 983,000 | 約 59.0 MB |
...
`N かける memberCount かける 60B` に達するため、セグメント数が多い・かつ広いキャンペーンが
将来的に増える場合は再検証が必要。
...
- **ヒープの単発測定**: 第5.2節の通り、単発の `resolve()` 呼び出し前後のヒープ差分はGCノイズに
  埋もれて実測できなかった（第5.5節の増幅測定で1エントリあたりのコストは実測できたが、
  これは「50回分の合計を割った平均値」であり、単発呼び出し1回の瞬間的なヒープ挙動そのものを
  直接見たわけではない）。
- **50万〜100万件規模でのJVM挙動そのもの**: 第6節の数値は「1エントリあたり約60バイト」という
  実測裏付け済みの係数を件数に掛けた計算値であり、実際に50万〜100万件を投入したJVMで
  ヒープを直接確認したものではない（縮小版の趣旨どおり、その規模の本走は行っていない）。
