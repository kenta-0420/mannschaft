...
**走査の時間制限はプリエンプティブである。** 「走査が終わってから経過時間を測って閾値と比べる」形は、**ハングしたときにだけ働かない番人**になる（制御が戻らないので比較に到達せず、CI のジョブ上限まで居座る）。捕まえたい破滅的バックトラックの再発はまさにこの経路で起こるため、`assertTimeoutPreemptively` で別スレッド実行し上限で**打ち切る**。走査は引数と局所変数しか触らず `ThreadLocal`・Spring コンテキストを使わないので、別スレッド実行で不安定にならない。
