...
## §11. 2026-09-01 軍議＋Codex 検分2巡 改訂裁定

> **🏯 マスター裁定（2026-09-01）**
>
> 2026-09-01 軍議で本設計書の未解決観点（§10.6 DEPUTY_ADMIN 扱い・§10.11 承継方式）を再検討し、Codex による検分2巡を経て以下を承認する。**§10 の裁定履歴は撤回ではなく、以下の内容で上書き改訂する**（該当箇所は本節を参照する形で読み替えること）。

### 11.1 §10.6 / §10.11 の変更 — purge 例外経路限定の自動承継（案 C′）を採用

- **§10.6「DEPUTY_ADMIN はスコープ外」を変更**: DEPUTY_ADMIN も承継候補として扱う（下記 11.2 の候補資格を満たす場合）。ただし `checkLastAdmin` ガード自体（ADMIN 文字列ハードコード判定）は変更しない。DEPUTY_ADMIN は「候補者」としてのみ本節の対象に加わる。
- **§10.11「自動昇格（案 C）不採用」の理由（本人同意なき権限付与）は、purge（アカウント物理削除）経路に限っては別のトレードオフで再評価する**: purge 経路は既に §10.11 裁定で「承諾スキップの強制委譲（forceTransferForPurge）」を採用済であり、退会者本人の30日タイムリミットに阻害されず完結する必要がある。この既存の強制委譲の器を使い、**委譲先の選定を「退会者が UX で個別に指名する」から「システムが優先順位に従って自動選定する」へ変更**する。これが案 C′（案 C の purge 限定版）である。
  - 通常のオーナー委譲（§10.11 前半で確定済の承諾型オファー）はそのまま維持し、本節の変更対象ではない。
  - 案 C を全面不採用とした §4.2 の理由（本人同意なき権限譲渡の法的リスク）は、purge 例外経路においては「30日以内に必ず ADMIN 不在を解消しなければならない」という制約下でのやむを得ない措置として、マスターが個別に再評価し承認した。一般の委譲フロー（案 B の手動指名 UX）における不採用理由は変更しない。

### 11.2 承継候補の優先順位と資格条件

**優先順位:**

1. 対象スコープの **DEPUTY_ADMIN のうち候補資格を満たす最古参**（`user_roles.created_at` 昇順）
2. DEPUTY_ADMIN が候補資格者ゼロの場合、**MEMBER のうち候補資格を満たす最古参**
3. 同一の `created_at` で並ぶ場合は **`user_roles.id`（または `user_id`）昇順** でタイブレーク
4. 候補資格者が 1 人もいない場合（＝他メンバー 0 人、または残りメンバー全員が資格を満たさない）は組織/チームを **archive**

**候補資格条件（すべて満たすこと）:**

- 対象スコープに現役在籍していること（`membership` が `active`。除名予定・招待中は対象外）
- 退会予定でないこと（`users.deleted_at IS NULL` かつ `withdrawal_requested_at IS NULL` — 退会受付中の別ユーザーを承継先に選ばない）
- 匿名化済・利用停止中でないこと（`users.anonymized_at IS NULL` かつ利用停止フラグ OFF）

この資格判定は「候補選定時点」だけでなく、§12 のロック下で **実行直前に再検証**する（11.2 の資格が選定後に崩れるケースへの対処は §12.6 を参照）。

**強制委譲の記録:**

- `forceTransferForPurge` 実行時、監査ログに `forced=true` を記録する（既存 §10.11 裁定を踏襲）
- 昇格された利用者へ通知を送る（「先任者の退会に伴い、あなたが組織 X の管理者に自動で指名されました」の主旨）
- 昇格を望まない場合、承継者は**既存の承諾型オファー（§10.11 前半で確定済の通常委譲フロー）を使って自主的に第三者へ再委譲できる**。他に委譲先候補が誰もいない場合は archive を選択できる（archive 操作自体は既存 API を使用、§3 案 B / §5.6 参照）。強制委譲そのものへの「辞退」専用 API は新設しない（既存資産の再利用で足りるため）。

### 11.3 AC — TEAM 昇格の ORGANIZATION 越境禁止

案 C′ の自動承継は **スコープ単位で独立して完結**し、TEAM スコープでの ADMIN 自動昇格が同一ユーザーの ORGANIZATION 権限（別スコープ）へ波及しないことを受け入れ条件とする。具体的には:

- `forceTransferForPurge(scopeId, scopeType, ...)` は `scopeType` ごとに独立実行し、TEAM の承継処理が ORGANIZATION 側の `user_roles` 行を一切変更しない（既存アーキテクチャ思想 §「クロスドメイン FK 禁止・ドメイン間は ID 参照のみ」と整合）
- 受け入れテストとして「同一ユーザーが TEAM A の ADMIN かつ ORGANIZATION B の ADMIN のケースで、TEAM A のみ退会 purge → TEAM A の承継後も ORGANIZATION B の ADMIN 権限に変化がない」を追加する（§8.2 統合テストへ追記）

---

## §12. ①-0 直列化・状態機械（2026-09-01 新設）

親設計書 Phase B-1 の `RolePurgeEventListener` および本節 §11 の自動承継が同時に走る前提を踏まえ、「最後の ADMIN 判定」から「変更実行（委譲/降格/除名/archive）」までを直列化し、レースコンディションによる ADMIN 数 0 の一時的発生すら防ぐ。

### 12.1 不変条件

> **他メンバー 1 人以上のスコープについて、DB コミット単位で ADMIN 数 0 を許さない。**

「他メンバー 0 人」スコープ（＝ purge によりスコープが空になるケース）は archive によって「ADMIN 不在の組織」自体が実質消滅するため、この不変条件の例外として扱う（既存 §1.4・§3 の整理と整合）。

### 12.2 直列化の実装方針

- スコープ単位ロック（`scopeType` + `scopeId` の組）で「最後の ADMIN 判定 → 変更実行」を直列化する
- 実装方式は次のいずれかとし、**実装 PR 着手時に確定する**（本節では両案を許容案として記載）:
  1. 既存 `AdminRoleMutationLockService`（存在する場合）を拡張し、スコープキーでの分散ロック/悲観ロックを提供する
  2. `user_roles` のスコープ該当行に対する `SELECT ... FOR UPDATE` で悲観ロックを取得する
- いずれの方式でも、ロック粒度は「スコープ単位」（`scopeType` + `scopeId`）とし、ユーザー単位やロール単位のロックでは不変条件を保証できない点に注意する

### 12.3 ロック取得順序（デッドロック回避）

退会者が複数スコープで ADMIN を兼務している場合、purge 処理は **退会者の全 ADMIN スコープを一括列挙**した上で、`scopeType` → `scopeId` の**昇順**でロックを取得する。個別スコープを都度ロックする実装（列挙せず処理順に取得）は、他の直列化経路（委譲・除名等）との間でロック順序が食い違いデッドロックを招くため禁止する。

### 12.4 ADMIN 変動の全経路を同一ロックへ収容

以下の ADMIN 変動を起こしうる全経路が、同一のスコープロックを通ることを設計上の必須要件とする:

| # | 経路 |
|---|---|
| 1 | 退会受付（`requestWithdrawal` 内の §2.4 系 409 判定 — 11.2/12.9 参照） |
| 2 | purge（`AccountPurgedEvent` → `RolePurgeEventListener` → `removeMemberWithoutAdminCheck` / `forceTransferForPurge`） |
| 3 | 通常の委譲（承諾型オファー `acceptOffer`） |
| 4 | 除名（`removeMember`） |
| 5 | 自主脱退（`leaveScope`） |
| 6 | 降格（`changeRole`） |
| 7 | 招待承諾によるロール確定（新規 ADMIN 招待の受諾） |

**この網羅をコードレベルで保証する手段として、ArchUnit 番人を新設する方針とする**（「ADMIN ロールを変更しうるメソッドは、直列化ロックの取得なしに `user_roles` の UPDATE/DELETE を発行してはならない」相当のルール。具体的なルール記述・対象パッケージ境界は実装 PR 側で確定する）。

### 12.5 purge × cancel-withdrawal の勝敗

退会取消（`cancel-withdrawal`）と purge 開始が競合するケースの勝敗を明示する:

- **purge 開始マーク後は cancel 不可**（cancel-withdrawal は 409 を返す）
- **purge 開始マーク前は cancel が勝つ**（purge 自体が起動しない）

「開始マーク」の実体（`users.purge_started_at` 相当のカラム、または既存 `purged_at` の先行フラグ運用）は実装 PR で確定する。

### 12.6 冪等性とロック下の候補再検証

- purge の自動承継処理は **冪等キー = `scope + userId(退会者) + purgeId`** を持ち、`AccountPurgedEvent` の再配送（at-least-once 配送を前提）が起きても昇格・通知・監査ログの重複を発生させない
- 候補選定（§11.2）は「選定時点」と「ロック取得後の実行直前」で **再検証** する。候補選定後に候補者自身が退会・利用停止に陥るレースに対処するため、ロック下で 11.2 の資格条件を再チェックし、資格を失っていれば次点候補（または archive）へフォールバックする
- purge 開始マーク後にクラッシュ（プロセス強制終了・デプロイ中断等）した場合、補正バッチ（§12.8 相当、または専用の再開バッチ）が同一冪等キーで処理を再開できることを設計要件とする

### 12.7 退会受付・退会確定・purge 実行の三段階チェック

既存 §5.2 のシーケンスでは「退会受付時（`requestWithdrawal`）」のみが 409 判定点として記述されていたが、**直列化ロックを前提に判定点を二重化**する:

1. **退会受付時**（`requestWithdrawal`）: §2.4（下記 §12.9）のガードで 409 を返す。ここはロックなしの事前チェック（UX 用の早期拒否）
2. **退会確定・purge 実行時**（`AccountPurgedEvent` 処理内）: スコープロックを取得した上で **同一ロック下で再判定** する。退会受付から purge 実行までの間（最大30日）にスコープの他メンバー構成が変化しうるため、受付時の判定を実行時にそのまま信用しない

### 12.8 既存データ検出バッチ

既存 §3 案 A の「安全弁」に加え、既に ADMIN 不在で残っている既存データを検出・是正する夜次バッチを新設する。詳細は §13 参照。

---

## §13. AdminlessScopeSuccessionBatchService（既存データ検出バッチ）

村ドメイン `VillageHeadmanSuccessionBatchService`（§1.3）を踏襲した実装とする。

| 項目 | 内容 |
|---|---|
| クラス名 | `AdminlessScopeSuccessionBatchService`（新設） |
| 実行タイミング | 夜次（既存 `VillageHeadmanSuccessionBatchService` の UTC 03:00 系スケジュールに準拠する想定。実装 PR で確定） |
| チャンクサイズ | 500 |
| 排他制御 | ShedLock（複数インスタンス同時実行防止。既存踏襲） |
| 手動発火 | 可（運用者が緊急是正できる口を用意する） |
| ロック整合 | §12 のスコープロックを **同じ手順で** 通す（バッチ専用の別ロック経路を作らない） |
| 承継ロジック | §11.2 の優先順位・候補資格をそのまま適用 |
| 昇格の実装経路 | `AccountPurgedEvent` の issuer（退会者コンテキスト）が存在しないため、`assignRole` を直接呼び `forced=true` 監査を記録する**専用メソッド**（`RoleService` に新設、例: `assignRoleForBatchSuccession` 相当）を使う。既存の `forceTransferForPurge`（退会者コンテキスト前提）とは別メソッドとして分離する |
| AC | 「バッチ処理対象時点で ADMIN 数 0 のスコープが 0 件になっていること」を受け入れ条件とする |

---

## §14. 退会受付ガードの具体化（既存 §2.4 系の改訂）

既存 §5.4「新規 API / メソッド」の `findLastAdminScopes` を以下の通り具体化する（§10.6 改訂＝DEPUTY_ADMIN を候補に含める前提の反映を含む）。

- `UserRoleRepository#findLastAdminScopes(userId)` は、「退会予定ユーザーが ADMIN であり、かつ当該スコープに §11.2 の候補資格を満たす他メンバーが 1 人もいない」スコープではなく、**引き続き「ADMIN 数=1（自分のみ）」のスコープを機械的に列挙する**（承継候補の有無判定は UX 側・バッチ側の責務として分離する。Repository は「対応が要るスコープの列挙」に専念する）
- 退会受付時（`requestWithdrawal`）、他メンバー 1 人以上の lastAdmin スコープが残っていれば **409 Conflict** を返し、`deletion-preview` レスポンスに以下を列挙する:
  - `scopeType`
  - `scopeId`
  - `scopeName`
  - `otherMembersCount`
- FE（`SettingsDeletionPreviewDialog.vue`）は「後任指名（承諾型オファー）」または「アーカイブ」を **全件処理し終えるまで削除ボタンを無効化**する（既存 §5.1 モックのフロー通り）
- **他メンバー 0 人のスコープはブロックしない**。退会受付をそのまま通し、purge 実行時に archive する（既存 §1.4/§3 案 B の整理を踏襲。ユーザーに無用な追加操作を強いない）

---

## §15. force-unarchive の条件追記（既存 §10.12 の具体化）

既存 §10.12 裁定（SYSTEM_ADMIN 用 force-unarchive エンドポイント新設）に以下を追記する:

- 権限は **SYSTEM_ADMIN 限定**、**監査ログ必須**（既存裁定どおり変更なし）
- **追記事項**: unarchive 実行時、対象スコープに現役 ADMIN が存在するかを検証する。**ADMIN が 1 人も存在しない場合、unarchive 単体では完了させず、同一操作の一部として管理者指名（招待）を同時に要求する**。具体的には unarchive API のリクエストに「初期 ADMIN として指名するユーザー」を必須パラメータとして含めるか、unarchive 直後に招待フローへ強制遷移させるかのいずれかとし、詳細は実装 PR で確定する。これにより「unarchive したがまた ADMIN 不在」という再発を構造的に防ぐ。

---

## §16. 総括 — 未解決事項の解消状況

本節（§11〜§15）の追記により、既存 §10.6・§10.11 は以下のとおり読み替える:

| 節 | 旧記述 | 2026-09-01 改訂による変更 |
|---|---|---|
| §10.6 | DEPUTY_ADMIN の扱いは本軍議スコープ外 | **2026-09-01 改訂により変更**。§11.1/§11.2 のとおり、DEPUTY_ADMIN は purge 例外経路の承継候補として扱う |
| §10.11 前半（承諾型オファー） | 変更なし。既存裁定（2026-07-18）のまま維持 | 変更なし |
| §10.11 後半（purge 経路の強制委譲） | 承継先はユーザーが手動指名（案 B の UX） | **2026-09-01 改訂により変更**。承継先の選定を §11.2 の優先順位に基づく自動選定へ変更（案 C′）。強制委譲の器（`forceTransferForPurge`・`forced=true` 監査）自体は既存裁定を踏襲 |

§10 本体の表・裁定文言は削除せず、上表を参照する形で本節へ誘導する。
...
| 2026-09-01 | **§11〜§16 実装 PR #3040 で AC1〜AC12 を green 化**。`RoleSuccessionService`（承継候補選定・forceTransferForPurge・promoteForBatchSuccession・forceAssignInitialAdminOnUnarchive）／`AdminlessScopeSuccessionBatchService`（§13夜次バッチ）／`PurgeStartGuard` + `users.purge_started_at`（V197・§12.5）／`SystemAdminScopeForceUnarchiveController`（§15）／`GDPR_011`・`GDPR_012` を実装。①-0 直列化は既存 `AdminRoleMutationLockService`（`SELECT...FOR UPDATE`方式）を新規経路（forceTransferForPurge・batch・force-unarchive）に横展開する形で再利用し、新規ロックサービスは起こさなかった（既存 `changeRole`/`removeMember`/`leaveScope`/`transferOwnership` が既にこの経路で直列化済と判明したため）。冪等キーは設計書の `purgeId` そのものではなく DB 実状態（scope の ADMIN 行の有無）で代替（`AccountPurgedEvent` が purgeId 相当を持たないため）。詳細は PR #3040 本文参照 | 足軽（出陣） |
| 2026-09-01 | **2026-09-01 軍議＋Codex 検分2巡・マスター御裁可反映**: §11〜§16 を新設。(a) §10.6/§10.11 を purge 例外経路限定の自動承継（案 C′）に変更 — 優先順位 DEPUTY_ADMIN 最古参→MEMBER 最古参（同率 ID 昇順）、候補資格（現役在籍・退会予定でない・匿名化/停止中でない）、forced=true 監査＋通知＋既存承諾型オファーでの辞退導線、TEAM→ORGANIZATION 非波及を AC 化。(b) ①-0 直列化・状態機械（§12）を新設 — 不変条件（他メンバー1人以上で ADMIN 数0を許さない）、スコープ単位ロック、全7経路の同一ロック収容＋ArchUnit番人方針、ロック順序（scopeType→scopeId昇順）、purge×cancel-withdrawal 勝敗、冪等キー、候補再検証、三段階チェック。(c) 退会受付ガード具体化（§14）・force-unarchive 条件追記（§15）・既存データ検出バッチ `AdminlessScopeSuccessionBatchService`（§13）を新設。ステータスを 🟡設計段階 → 🟢設計確定（実装待ち）に更新。既存 §10 裁定履歴は削除せず、§16 に読み替え表を追記する形で保持 | 足軽（軍議反映・ドキュメントのみ PR）|
