...
| **1:1 従属表（主キーが親の UUIDv7 そのもの）** | 親テーブルの行に高々 1 行が従属し、独立発番の代理キーを持たない表。主キーに親の UUIDv7 をそのまま使う（DDL に `id` 列が存在しない）。原則 6 の意図（各ノードが独立して発番できる）は親の採番で既に満たされており、代理キーを足すと「親 1 行につき 1 行」を別途 UNIQUE で守る必要が生じて不変条件の担保が弱くなる。例: `active_billing_contract_operation_pointers`（`PRIMARY KEY (contract_id)`・契約操作 Saga の lease・V196） | 親の主キー（UUIDv7）をそのまま |

判定基準: 「テナントごと・ユーザーごとに行が増えていくテーブル」=原則 6 適用。
「全テナント共通で読み取られるテーブル」「行が 1 件で固定のテーブル」「主キーが親の UUIDv7 そのものである 1:1 従属表」=例外。

いずれの例外も、番人 `EntityUuidV7ConventionArchTest` の凍結ストアへ<b>理由つきで</b>登録すること
（登録は「違反隠蔽」ではなく「設計是認例外の正規化」である。理由を Javadoc に書かない登録は認めない）。
