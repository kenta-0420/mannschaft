...
> **追加前提（精査第 1 パスで確定）**: relay 有効化より前に **Valkey の転送時暗号化（TLS）を有効化**する（§8.6）。現状 `transit_encryption_enabled = false` のため、relay を先に ON にするとチャット本文・通知本文（PII）が**平文で Valkey を流れる新経路**が生まれる。TLS 化は隊 3（IaC）のスコープ。
...
- **実装点と登録順**: 捕捉は brokerChannel の `ChannelInterceptor`（`preSend`・メッセージ非改変）で行い、`@Order(Ordered.LOWEST_PRECEDENCE)` を明示して**既存インターセプタの最終段**に固定する（暗黙順序に依存しない）。
...
- `RelayEnvelope.originNodeId` に**発信ノードの一意 ID**（起動時生成 UUID または ECS タスク ID）を入れ、受信側は `originNodeId == 自ノード ID` なら**破棄**（自分が publish したものを再注入しない）。
...
- **AC-2（マルチノード user・認可）**: `convertAndSendToUser` が、対象ユーザーの**接続ノードに関わらず**到達する。かつ**他ユーザーには漏洩しない**。検証手順を固定する: ユーザー A を**ノード B のみ**に接続し、**ノード A から** A 宛に送信 → ノード A 上の別ユーザー（受信者 C）には届かず、ノード B の A に届くこと。
...
- **AC-8（無停止デプロイの観測）**: 段階 2 の設定変更後、ローリングデプロイ中に維持していた WS 接続で「切断→クライアント自動再接続」以外の断が発生しない。デプロイ中の HTTP 5xx ゼロ・deployment circuit breaker 非発火。
...
- **API 契約**: **REST 変更なし**。クライアントの購読 destination（`/topic/...`・`/user/.../queue/...`）は**全て不変**。STOMP エンドポイント（`/ws`・`/ws/signage`）・SockJS も不変。**FE 改修ゼロ**。OpenAPI（`docs/openapi.json`）への影響なし。
  - **ただし挙動変化の周知（FE 隊・実機 E2E）**: Principal 根治により、**これまで実際には届いていなかった `/user/queue/notifications` のリアルタイム通知が初めて実配信される**。FE は購読済みで改修不要だが、通知トースト・バッジ等の表示頻度が実際に変わるため FE 隊へ周知し、実機 E2E の確認項目に含める（AC-5 と対）。
...
#### 7.1.2 AC-2 / AC-5 / AC-7（user 宛・Principal・実 STOMP スタック）— 2 コンテキスト @SpringBootTest

- 単一 JVM 内で `@SpringBootTest(webEnvironment = RANDOM_PORT)` 相当のフルコンテキストを **2 つ**（`mannschaft.websocket.relay.node-id` を別値・ポートは RANDOM_PORT 自動採番）起動し、**1 個の Valkey Testcontainer を共有**する（両コンテキストに同一の Valkey 接続先を注入）。
- 実 STOMP クライアント（`WebSocketStompClient`）で各ノードに CONNECT し、`UserDestinationMessageHandler` / `SimpUserRegistry` / SimpleBroker を含む**実スタック**で検証する: AC-2（到達＋非漏洩・手順は §5 AC-2 の固定手順）・AC-5（registry 登録＋同一ソース一致）・AC-7（サイネージ topic のマルチノード到達）。
- **TestContext キャッシュへの配慮**: 1 つ目はフレームワーク管理のコンテキストとしてよいが、**2 つ目は `SpringApplicationBuilder` 等で明示起動・テスト終了時に明示 close** し、フレームワークの TestContext キャッシュに残さない。テストクラス全体も `@DirtiesContext(classMode = AFTER_CLASS)` で後始末し、他テスト（`AbstractMySqlIntegrationTest` 系）へのキャッシュ分裂波及を防ぐ。
...
- 遅延の直接計測（publish→他ノード再注入の片道時間）は**ノード間クロックスキューで不正確なため採用しない**。代替: `relay.publish.count` 対 `relay.receive.count` の**突合（件数整合・滞留検知）** ＋ **Valkey RTT**（PING レイテンシ）を監視する。補助として同一ノード内 self-roundtrip（自ノード publish→自ノード受信を同一クロックで計測）を用いてよい
...
- **relay 有効化の前提条件（セキュリティ・必須）**: 現状 `transit_encryption_enabled = false`（`modules/data/main.tf` 198 行）。relay を有効化すると**チャット本文・通知本文（PII）が平文で Valkey を流れる新経路**が生まれるため、**relay ON より前に転送時暗号化を有効化**する（隊 3 スコープに含める）:
  1. ElastiCache: `transit_encryption_enabled = true`（in-place 変更可否・`transit_encryption_mode` の段階適用要否は実装時に AWS 仕様を確認）
...
| **隊 2: 試練（テスト先行）** | 受け入れ条件 → red テスト | §7.2 の Principal red（`/queue` 登録済みの切り分けアサート含む）/ §7.1.1 手組み結合（broadcast・ループ防止）/ §7.1.2 2 コンテキスト実スタック（user 宛・Principal・サイネージ）/ 非回帰確認 |
| **隊 3: IaC** | ECS deployment 設定 + Valkey TLS 化 | §8.6 の `deployment_*` 変更 + circuit breaker + コメント書き換え + **`transit_encryption_enabled=true` 化と `spring.data.redis.ssl` 有効化（relay ON の前提条件・§8.6）**。Graviton #2219 先行の上に積む・Cloudflare Tunnel 隊と協調 |
...
- **A.4** タイピング（§2.2 表 #3）の中継除外オプションを実装するか — **presence は中継必須で確定**（§2.2 注記・§8.3）。タイピング除外は明示的な機能劣化（別ノードのタイピング表示欠落）を伴うため、段階 2 の計測後にマスター判断（§8.3）。
...
- **A.6** user 経路のリレーマーカーが `UserDestinationMessageHandler` の解決ホップを経てもヘッダ保持されるか — 実装時にテストで確認（保持されなくても §4.2.1 の destination 判定で二重配信は防止される・§4.4）。
- **A.7** ElastiCache `transit_encryption_enabled` の in-place 変更可否（`transit_encryption_mode` によるオンライン段階適用が必要か）— 隊 3 が AWS 仕様を確認して適用手順（要ダウンタイムの有無含む）を確定（§8.6）。
