...
| 層 | 説明 | 例 |
|----|------|----|
| **個人アカウント** | ユーザー単位。実名（姓・名）＋ニックネーム（display_name、外部公開スコープ専用）＋愛称2（nickname2、nullable）を登録。複数チーム/組織にマルチ所属可能。QR会員証を保有。組織・チームに属さない個人利用も可能 | 田中太郎 |
...
Mannschaft は「チーム・組織内は実名表示」を基本思想とする。
通常の SNS は匿名性が高く排他性を生みやすく、LINE も実名でないため「誰？」となりやすい。
内部コミュニケーションは実名で行うことで、つながりの質を高める。

| 表示スコープ | 表示形式 | 説明 |
|---|---|---|
| チーム内・組織内（メンバー一覧・メンション・チャット） | `lastName + firstName`（実名） | 誰が書いたか・誰に頼むかが一目でわかる |
| フレンドチーム・上位/下位組織とのやり取り | アイコン + `lastName + firstName`（実名） | 他組織でも顔と名前で識別できる |
| 外部公開コンテンツ（タイムライン・ブログ・組織外共有） | `display_name`（ニックネーム） | 個人情報保護・プライバシー配慮 |

- `display_name`（`users.display_name`）は**外部公開スコープ専用**のニックネームである
- `NameResolverService` と `DisplayScope` 列挙型が名前解決を担い、スコープに応じて自動で切り替える
- `user_social_profiles` テーブルは匿名投稿用の別名プロフィールを管理する（任意機能）
- 実装時は `NameResolverService.resolve(userId, scope)` を使用し、直接 `display_name` を参照しないこと
...
- **予約作成（機能55）**: 予定作成時に「○月○日になったら出欠募集を開始」「○月○日になったらアンケートを自動公開」を予約可能。1分間隔バッチが指定時刻に処理を実行する
- **リマインダー相対/絶対両対応（機能55）**: 「開始60分前」（相対）または「○月○日○時」（絶対）のリマインダーを設定可能。共有予定・個人予定ともに対応
...
- スタッフ別・日付別の一覧表示
- **Phase 1 MVP 完了**（2026-04-24 / PR #128）: スケジュール・スロット・希望提出・変更依頼・勤務制約の基盤 API 実装、5段階希望強度（PREFERRED=+100 / WEAK_REST=-30 ほか）、マイシフト画面／デフォルト可否プロファイル設定画面を提供。Phase 2 で自動割当本体（Strategy パターン・貪欲法 MVP）と PDF 出力に着手予定
...
- **エラーレポート管理（F12.5 Phase 2）**: ユーザーから送信されたエラーレポートの一覧・Kanban・詳細確認（スタックトレース・発生ページ・ユーザーエージェント・任意コメント）。ステータス管理（未対応 / 対応中 / 解決済み）に加え、ワークフロー段階管理（INVESTIGATION_STARTED → ROOT_CAUSE_IDENTIFIED → FIX_IN_PROGRESS → TEST_COMPLETED → RELEASED）、担当者アサイン、操作履歴・コメント、タイムライン（occurrences + activities）、AI 自動分析（Claude Haiku 4.5、月次予算 5,000 円）、GitHub Issue 自動作成（PII 除去 + 内部 URL 付与）に対応。詳細: `docs/features/F12.5_frontend_error_tracking.md` §12
...
| 9 | 住民台帳、プロモーション配信、駐車場区画、LINE/SNS連携、施設予約、ダイレクトメール、広告、コルクボード、Webhook/外部API、デジタルサイネージ、メール配信基盤 | [F09.1](docs/features/F09.1_resident_registry.md), [F09.2](docs/features/F09.2_promotion_targeting.md), [F09.3](docs/features/F09.3_parking.md), [F09.4](docs/features/F09.4_line_sns.md), [F09.5](docs/features/F09.5_facility_booking.md), [F09.6](docs/features/F09.6_direct_mail.md), [F09.7](docs/features/F09.7_advertising.md), [F09.8](docs/features/F09.8_corkboard.md), [F09.9](docs/features/F09.9_webhook_api.md), [F09.10](docs/features/F09.10_digital_signage.md), [F09.18](docs/features/F09.18_email_delivery_infrastructure.md) |
