...
| 方式 | 概要 |
|------|------|
| フリープラン | デフォルト機能（33個）＋ 選択式モジュール最大10個まで無料 |
...
| 29 | 記念日管理 | チーム記念日の登録・年次繰り返し・リマインド通知（0〜90日前） |
| 30 | コイントス・抽選 | コイン/サイコロ/カスタム選択肢の3モード抽選ツール・チャット共有・履歴 |
| 31 | 当番管理 | 日替わり/週替わり/月替わりの当番ローテーション・メンバー割り当て |
| 32 | 在席管理 | メンバーの外出・帰着ステータス追跡・行き先・帰着予定時刻・履歴・統計 |
| 33 | 買い物リスト | チーム共有の買い物リスト・テンプレート・チェック機能・アーカイブ |
...
| 1 | QR会員証 | ○ | ○ | ○ | |
| 2 | 会費・決済 | ○ | ○ | - | |
| 3 | 予約管理 | ○（チーム横断ビュー） | ○（枠管理） | ○（予約のみ） | 組織: 配下チームの予約ページへのナビゲーション / チーム: 予約枠の作成・管理 / 個人: 予約・キャンセル・履歴閲覧 |
| 4 | サービス履歴 | - | ○ | ○ | チーム: 記録管理 / 個人: 自分の履歴閲覧 |
| 5 | 備品管理 | ○ | ○ | - | |
| 6 | ギャラリー | ○ | ○ | - | |
| 7 | シフト管理 | ○（チーム横断ビュー） | ○（作成・管理） | ○（希望提出） | 組織: 配下チームのシフトページへのナビゲーション / チーム: シフト作成・編集・確定 / 個人: 希望日時の提出・確定シフトの確認 |
...
※ デフォルト機能（33個）は全テンプレートで常時有効のため表に含まない
...
#### 29. 記念日管理
- チームの記念日（設立記念、大会優勝日等）を登録・管理
- **年次繰り返し**: 毎年自動でリマインドを送信（ON/OFF切替可能）
- **通知タイミング**: 0〜90日前の範囲で通知時期を設定可能
- 直近の記念日を「もうすぐの記念日」として一覧表示（残り日数付き）
- 作成・編集・削除は ADMIN/DEPUTY_ADMIN のみ

#### 30. コイントス・抽選
- チーム内の意思決定を支援するランダム抽選ツール
- **3モード**: コイン（表/裏）、サイコロ（1〜6）、カスタム（自由選択肢を2つ以上設定）
- アニメーション付きの抽選演出
- 任意で質問（お題）を設定可能
- 結果をチャットに共有する機能
- 過去の抽選履歴を閲覧可能（タイムスタンプ・モード・結果）

#### 31. 当番管理
- チームメンバーの当番ローテーションを管理
- **3つのローテーション周期**: 日替わり / 週替わり / 月替わり
- 当番ごとにアイコン（絵文字）・メンバー順序・開始日を設定
- 「今日の当番」を一覧表示
- 当番の有効/無効切替が可能
- 作成・編集・削除は ADMIN/DEPUTY_ADMIN のみ

#### 32. 在席管理
- チームメンバーの在席（在/不在）ステータスをリアルタイム追跡
- **ステータスタブ**: 全メンバーの現在のステータス（在席/外出中）を一覧表示。行き先・帰着予定時刻を表示
- **履歴タブ**: 外出・帰着イベントのログ（タイムスタンプ・行き先・メッセージ）
- **統計タブ**: 総イベント数・外出回数・帰着回数・延滞数、メンバー別統計テーブル
- 「外出する」ボタン: 行き先・帰着予定時刻・メッセージを入力して記録
- 「帰着する」ボタン: 任意メッセージ付きでワンクリック帰着記録

#### 33. 買い物リスト
- チーム共有の買い物リストを複数作成・管理
- **リスト管理**: 新規作成・編集・削除・テンプレート化・アーカイブ
- **アイテム管理**: 商品名・数量・メモの登録、チェックボックスで購入済みマーク
- チェック済みアイテムの一括クリア機能
- テンプレートリストを元に繰り返し利用可能
- 作成・編集・削除は ADMIN/DEPUTY_ADMIN のみ
...
#### 3. 予約管理
- **組織アカウント（チーム横断ビュー）**: 配下チームの一覧を表示し、各チームの予約管理ページへナビゲート
...
#### 7. シフト管理
- **組織アカウント（チーム横断ビュー）**: 配下チームの一覧を表示し、各チームのシフト管理ページへナビゲート
