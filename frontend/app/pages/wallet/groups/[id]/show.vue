...
/**
 * 設定 `requireBiometricOnShow=true` のときに呼ぶ。OS の生体認証 / PIN ダイアログを
 * navigator.credentials.get で表示し、ユーザーのジェスチャ確認を取る。
 *
 * <p>本来はサーバー側で 5 分以内の WebAuthn 完了確認を行う設計（§9・POINT_CARD_009）だが、
 * バックエンドに「再認証専用エンドポイント」が現状存在せず completeWebAuthnLogin は
 * トークンを再発行してしまうため、S5 範囲では「クライアント側ジェスチャ確認」までを行う。
 * サーバー検証はバックエンドに新エンドポイント追加後に実施する（残懸念）。</p>
 *
...
async function verifyBiometric(): Promise<boolean> {
  ...
  try {
    const beginRes = await authApi.beginWebAuthnLogin()
    ...
    // ユーザーキャンセル / タイムアウト / authenticator なし
