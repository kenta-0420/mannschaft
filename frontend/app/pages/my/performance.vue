  ...
  <div class="mx-auto max-w-3xl">
    ...
    <template v-else-if="data">
      <div class="grid gap-4 md:grid-cols-3">
        <div v-for="(value, key) in data.metrics" :key="key" class="rounded-xl border border-surface-200 bg-surface-0 p-4 text-center">
      ...
      <div v-if="data.records?.length" class="mt-6">
        ...
        <div class="flex flex-col gap-2">
          <div v-for="r in data.records" :key="r.id" class="rounded-xl border border-surface-200 bg-surface-0 p-4">
    ...
    <div v-else class="py-12 text-center"><i class="pi pi-chart-bar mb-3 text-4xl text-surface-300" /><p class="text-surface-400">パフォーマンスデータがありません</p></div>
  </div>
</template>
