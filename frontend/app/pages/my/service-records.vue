...
const { getMyRecords } = useServiceRecordApi()
...
<template>
  <div class="mx-auto max-w-3xl">
    ...
    <div v-else class="flex flex-col gap-3">
      <div v-for="r in records" :key="r.id" class="rounded-xl border border-surface-200 bg-surface-0 p-4">
      ...
      <div v-if="records.length === 0" class="py-12 text-center"><i class="pi pi-list mb-3 text-4xl text-surface-300" /><p class="text-surface-400">サービス履歴がありません</p></div>
    </div>
  </div>
</template>
