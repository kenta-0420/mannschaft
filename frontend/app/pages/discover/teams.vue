...
import type { PublicTeamSearchResult, SpringPage } from '~/types/public'
import { PREFECTURES } from '~/constants/prefectures'

/**
 * F19.1 Phase 4 公開チーム検索ページ。
 *
...
const { t } = useI18n()
const { searchPublicTeams } = usePublicApi()
...
// 検索結果
const { data: resultsPage, pending, refresh } = await useAsyncData<SpringPage<PublicTeamSearchResult>>(
  'discover-teams',
  () =>
    searchPublicTeams({
...
const teams = computed(() => resultsPage.value?.content ?? [])
...
// SEO
useSeoMeta({
  title: () => t('public.discover.teams.title'),
  description: () => t('public.meta.ogDescriptionDefault'),
})

useSeoPublicPage({
  canonicalPath: '/discover/teams',
  title: () => t('public.discover.teams.title'),
  description: () => t('public.meta.ogDescriptionDefault'),
})
</script>
    ...
    <section aria-labelledby="filter-heading" class="rounded-lg bg-surface-50 p-4 dark:bg-surface-800">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
        ...
        <div class="flex-1">
          ...
          <InputText
            v-model="keyword"
            :placeholder="t('public.discover.teams.searchPlaceholder')"
            class="w-full"
