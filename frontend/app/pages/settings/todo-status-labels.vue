...
async function loadLabels() {
  loading.value = true
  try {
    const res = await labelApi.listLabels('me')
    labels.value = res.data
  } catch (e) {
    errorHandler.handleApiError(e, 'todo-status-labels:list')
  } finally {
    loading.value = false
  }
}
...
async function handleSave() {
  ...
  try {
    if (editTarget.value) {
      ...
      await labelApi.updateLabel('me', undefined, editTarget.value.id, body)
      ...
      await labelApi.createLabel('me', undefined, body)
...
function handleSaveError(error: unknown) {
  ...
  errorHandler.handleApiError(error, 'todo-status-labels:save')
}
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await labelApi.deleteLabel('me', undefined, deleteTarget.value.id)
...
function handleDeleteError(error: unknown) {
  ...
  errorHandler.handleApiError(error, 'todo-status-labels:delete')
}
...
<template>
  <div class="mx-auto max-w-2xl">
    ...
    <template v-else>
      ...
      <section>
        <h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-surface-500">
          {{ t('todo.statusLabel.management.userSection') }}
