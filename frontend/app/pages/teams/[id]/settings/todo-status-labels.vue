...
async function loadLabels() {
  ...
  try {
    ...
    errorHandler.handleApiError(e, 'team-todo-status-labels:list')
  } finally {
    loading.value = false
  }
}
...
async function handleSave() {
  ...
  try {
    if (editTarget.value) {
      ...
      await labelApi.updateLabel('team', teamId, editTarget.value.id, body)
      ...
      await labelApi.createLabel('team', teamId, body)
...
function handleSaveError(error: unknown) {
  ...
  errorHandler.handleApiError(error, 'team-todo-status-labels:save')
}
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await labelApi.deleteLabel('team', teamId, deleteTarget.value.id)
...
function handleDeleteError(error: unknown) {
  ...
  errorHandler.handleApiError(error, 'team-todo-status-labels:delete')
}
...
<template>
  <div class="mx-auto max-w-2xl">
    ...
    <template v-else>
      ...
      <section>
        <h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-surface-500">
          {{ t('todo.statusLabel.management.teamSection') }}
