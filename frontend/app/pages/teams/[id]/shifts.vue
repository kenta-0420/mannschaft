...
// シフト表作成フォーム
const createForm = ref({ title: '', periodStart: null as Date | null, periodEnd: null as Date | null })
const creating = ref(false)

async function createSchedule() {
  if (!createForm.value.title.trim() || !createForm.value.periodStart || !createForm.value.periodEnd) return
  creating.value = true
  try {
    await shiftApi.createShiftSchedule(teamId, {
  ...
  catch { notification.error('作成に失敗しました') }
  ...
  <div>
    ...
    <TabView v-model:active-index="activeTab">
      <TabPanel header="シフト表">
        <ShiftScheduleList :team-id="teamId" :can-manage="isAdminOrDeputy" @select="onScheduleSelect" @create="showCreateDialog = true" />
      </TabPanel>
    ...
    <!-- シフト表作成ダイアログ -->
    <Dialog v-model:visible="showCreateDialog" header="シフト表を作成" :style="{ width: '420px' }" modal>
      <div class="flex flex-col gap-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <DatePicker v-model="createForm.periodStart" date-format="yy/mm/dd" class="w-full" show-icon />
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium">終了日</label>
            <DatePicker v-model="createForm.periodEnd" date-format="yy/mm/dd" class="w-full" show-icon />
          </div>
        </div>
      </div>
      <template #footer>
