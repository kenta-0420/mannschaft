...
<template>
  <div class="mx-auto max-w-6xl">
    <div class="mb-6 flex items-center justify-between">
      ...
      <Button label="データエクスポート" icon="pi pi-download" severity="secondary" @click="showExport = true" />
    </div>
      ...
      <div class="mb-6 grid gap-4 md:grid-cols-4">
        <Card>
          <template #content>
            ...
            <p class="text-3xl font-bold text-primary">{{ data.summary.totalViews.toLocaleString() }}</p>
          </template>
        </Card>
        <Card>
          <template #content>
        ...
        <Card>
          <template #content>
            ...
            <p class="text-3xl font-bold text-green-600">{{ data.summary.memberViews.toLocaleString() }}</p>
          </template>
        </Card>
        <Card>
          <template #content>
            <p class="text-sm text-surface-500">ゲスト閲覧</p>
            <p class="text-3xl font-bold text-blue-600">{{ data.summary.guestViews.toLocaleString() }}</p>
          </template>
        </Card>
      </div>
