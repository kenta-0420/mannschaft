...
async function saveProfile() {
  try {
    const res = await api.updateProfile(teamId.value, {
...
async function saveOfficer() {
  try {
    if (editingOfficer.value) {
      const res = await api.updateOfficer(teamId.value, editingOfficer.value.id, officerForm.value)
      ...
      const res = await api.createOfficer(teamId.value, officerForm.value)
...
async function deleteOfficer(officer: TeamOfficerResponse) {
  try {
    await api.deleteOfficer(teamId.value, officer.id)
...
async function saveOfficerOrder() {
  try {
    ...
    await api.reorderOfficers(teamId.value, { orders })
...
async function saveCf() {
  try {
    if (editingCf.value) {
      const res = await api.updateCustomField(teamId.value, editingCf.value.id, cfForm.value)
      ...
      const res = await api.createCustomField(teamId.value, cfForm.value)
...
async function deleteCf(field: TeamCustomFieldResponse) {
  try {
    await api.deleteCustomField(teamId.value, field.id)
...
async function saveCfOrder() {
  try {
    ...
    await api.reorderCustomFields(teamId.value, { orders })
