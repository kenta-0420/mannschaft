...
function applyProfile(data: TeamProfileResponse) {
...
async function saveProfile() {
  try {
    const res = await api.updateProfile(teamSlug.value, {
...
const editingOfficer = ref<TeamOfficerResponse | null>(null)
...
function openEditOfficer(officer: TeamOfficerResponse) {
...
async function saveOfficer() {
  try {
    if (editingOfficer.value) {
      const res = await api.updateOfficer(teamSlug.value, editingOfficer.value.id, officerForm.value)
      ...
      const res = await api.createOfficer(teamSlug.value, officerForm.value)
...
async function deleteOfficer(officer: TeamOfficerResponse) {
  try {
    await api.deleteOfficer(teamSlug.value, officer.id)
...
async function saveOfficerOrder() {
  try {
    ...
    await api.reorderOfficers(teamSlug.value, { orders })
...
const editingCf = ref<TeamCustomFieldResponse | null>(null)
...
function openEditCf(field: TeamCustomFieldResponse) {
...
async function saveCf() {
  try {
    if (editingCf.value) {
      const res = await api.updateCustomField(teamSlug.value, editingCf.value.id, cfForm.value)
      ...
      const res = await api.createCustomField(teamSlug.value, cfForm.value)
...
async function deleteCf(field: TeamCustomFieldResponse) {
  try {
    await api.deleteCustomField(teamSlug.value, field.id)
...
async function saveCfOrder() {
  try {
    ...
    await api.reorderCustomFields(teamSlug.value, { orders })
...
async function loadData() {
  ...
  try {
    ...
    const [profileRes, officersRes, fieldsRes] = await Promise.all([
      api.getProfile(teamSlug.value),
      api.getOfficers(teamSlug.value, true),
      api.getCustomFields(teamSlug.value, true),
    ])
