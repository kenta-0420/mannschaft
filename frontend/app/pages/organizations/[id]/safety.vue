...
async function loadChecks() {
  loading.value = true
  try {
    const res = await safetyApi.listSafetyChecks('organization', orgId, { size: 20 })
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <div class="lg:col-span-2">
        <div v-if="selectedCheckId" class="rounded-xl border border-surface-200 bg-surface-0 p-4 dark:border-surface-700 dark:bg-surface-800">
          <SafetyResultsDashboard scope-type="organization" :scope-id="orgId" :check-id="selectedCheckId" />
        </div>
