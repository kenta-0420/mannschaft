...
async function loadLabels() {
  ...
  try {
    ...
    errorHandler.handleApiError(e, 'org-todo-status-labels:list')
  } finally {
    loading.value = false
  }
}
...
async function handleSave() {
  ...
  try {
    if (editTarget.value) {
      ...
      await labelApi.updateLabel('organization', orgId, editTarget.value.id, body)
      ...
      await labelApi.createLabel('organization', orgId, body)
...
function handleSaveError(error: unknown) {
  ...
  errorHandler.handleApiError(error, 'org-todo-status-labels:save')
}
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await labelApi.deleteLabel('organization', orgId, deleteTarget.value.id)
...
function handleDeleteError(error: unknown) {
  ...
  errorHandler.handleApiError(error, 'org-todo-status-labels:delete')
}

onMounted(async () => {
  await Promise.all([loadLabels(), loadPermissions()])
})
</script>
...
<template>
  <div class="mx-auto max-w-2xl">
    ...
    <template v-else>
      ...
      <section>
        <h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-surface-500">
          {{ t('todo.statusLabel.management.organizationSection') }}
