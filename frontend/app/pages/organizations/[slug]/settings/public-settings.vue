...
/**
 * F19.1 Phase 7: 組織公開設定ページ。
 *
 * - timelinePostsPublic: タイムライン投稿の公開設定
 * - publicEventsEnabled: イベントの公開設定
  ...
  <div class="mx-auto max-w-2xl space-y-6 p-4">
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div class="flex-1">
          <label class="block text-sm font-medium" for="org-timeline-posts-public">
      ...
      <div class="flex items-start justify-between gap-4">
        <div class="flex-1">
          <label class="block text-sm font-medium" for="org-public-events-enabled">
            {{ t('public.publicSettings.publicEventsEnabled') }}
          </label>
          <p class="mt-1 text-xs text-surface-500">
            {{ t('public.publicSettings.publicEventsEnabledDescription') }}
          </p>
        </div>
        <ToggleSwitch
          id="org-public-events-enabled"
          v-model="publicEventsEnabled"
          data-testid="toggle-public-events-enabled"
        />
      </div>
