 ...
 * 監査ログを読み込む。
 *
 * BE（`/api/v1/organizations/{orgId}/audit-logs`）は `CursorPagedResponse` を返すため総件数を持たない。
...
async function loadLogs(cursor?: string) {
  ...
  try {
    const res = await auditLogApi.listByOrg(orgSlug.value, { cursor, limit: PAGE_SIZE })
