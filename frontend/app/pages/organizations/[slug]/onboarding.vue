...
async function loadData() {
  ...
  try {
    ...
    const [tmpl, prog] = await Promise.all([
      onboardingApi.listTemplates('organization', orgSlug.value),
      onboardingApi.listProgresses('organization', orgSlug.value),
    ])
...
async function handleSave(data: { name: string; description: string; deadlineDays: number | null; reminderDaysBefore: number | null; isOrderEnforced: boolean; isAdminNotifiedOnComplete: boolean; isTimelinePostedOnComplete: boolean; presetId?: number; steps: import('~/types/onboarding').CreateStepRequest[] }) {
  ...
  try {
    if (editingTemplate.value) {
      ...
      await onboardingApi.createTemplate('organization', orgSlug.value, body)
...
async function handleRemind() {
  try {
    await onboardingApi.sendReminder('organization', orgSlug.value)
