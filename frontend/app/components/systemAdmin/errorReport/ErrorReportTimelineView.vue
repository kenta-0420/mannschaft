  ...
  <section class="space-y-3">
    ...
    <ul v-else class="space-y-2">
      <li
        ...
        :key="`${item.type}-${idx}-${item.occurredAt}`"
        class="flex gap-3 rounded-lg border border-surface-200 bg-surface-0 p-3 text-sm dark:border-surface-700 dark:bg-surface-800"
      >
        ...
        <div class="min-w-0 flex-1">
          ...
          <div v-if="item.type === 'OCCURRENCE'" class="mt-1 break-all text-xs text-surface-600 dark:text-surface-300">
            {{ item.pageUrl }}
          </div>
          <div v-else class="mt-1 space-y-1">
            <div v-if="item.content" class="whitespace-pre-wrap text-sm">{{ item.content }}</div>
