...
import type { OwnReturnStayPlan, ReturnStayPlanRequest, ReturnStayPlanType } from '~/composables/returnStayPlan/useReturnStayPlanApi'
import { getOwnReturnStayPlanStatus } from '~/composables/returnStayPlan/useReturnStayPlanApi'
interface TeamOption { id: number; name: string; slug?: string }
...
const editDialogPrefectureFocus = ref<HTMLSelectElement | null>(null)
...
async function focusEditDialog() { await nextTick(); (isActive.value ? editDialogPrefectureFocus.value : editDialogFocus.value)?.focus() }
...
async function remove(plan: OwnReturnStayPlan) { if (!window.confirm(t('returnStayPlan.form.confirmDelete'))) return; try { await api.remove(plan.id); plans.value = plans.value.filter((item) => item.id !== plan.id); notification.success(t('returnStayPlan.form.deleted')) } catch { notification.error(t('returnStayPlan.form.deleteFailed')) } }
onMounted(load)
defineExpose({ focusEditDialog })
</script>
<template>
  <DashboardWidgetCard :title="$t('returnStayPlan.widget.title')" icon="pi pi-map-marker" :scrollable="false">
    <template #actions><Button data-testid="return-stay-plan-add" icon="pi pi-plus" text rounded size="small" :aria-label="$t('returnStayPlan.form.add')" @click="resetForm()" /></template>
    <div v-if="loading" class="p-4 text-sm text-surface-500">{{ $t('common.loading') }}</div>
    <div v-else-if="loadError" class="flex items-center gap-2 p-4 text-sm text-red-600"><span>{{ $t('returnStayPlan.form.loadFailed') }}</span><Button text size="small" :label="$t('common.retry')" @click="load" /></div>
  ...
  <Dialog v-model:visible="dialogVisible" modal closable close-on-escape dismissable-mask class="return-stay-edit-dialog" :header="editing ? $t('returnStayPlan.form.editTitle') : $t('returnStayPlan.form.addTitle')" :style="{ width: 'min(34rem, calc(100vw - 2rem))' }" :breakpoints="{ '640px': '100vw' }" @show="focusEditDialog">
    <form class="grid gap-4" @submit.prevent="save"><Message v-if="formError" severity="error" :closable="false">{{ formError }}</Message>
      ...
      <label class="flex items-center gap-2"><input v-model="form.isPublished" type="checkbox">{{ $t('returnStayPlan.form.published') }}</label><div class="flex justify-end gap-2"><Button type="button" text :label="$t('common.cancel')" :disabled="saving" @click="dialogVisible = false" /><Button type="submit" icon="pi pi-check" :label="$t('returnStayPlan.form.save')" :loading="saving" /></div>
    </form>
  </Dialog>
</template>
