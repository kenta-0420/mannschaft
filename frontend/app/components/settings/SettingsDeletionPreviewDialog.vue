...
async function loadPreview() {
  ...
  try {
    ...
    notification.error(t('settings.deletion_preview.fetch_error'))
  } finally {
...
<template>
  <Dialog
    :visible="visible"
    :header="$t('settings.deletion_preview.dialog_title')"
    :modal="true"
  ...
  >
    <div class="space-y-5">
      <p class="font-medium text-red-600">
        {{ $t('settings.deletion_preview.confirm_message') }}
      </p>
      ...
      <template v-else-if="preview">
        <div v-if="preview.warnings?.length" class="rounded-lg border border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800 dark:bg-yellow-900/20">
          <p class="mb-1 text-sm font-semibold text-yellow-700 dark:text-yellow-400">{{ $t('settings.deletion_preview.warning_title') }}</p>
        ...
        <div>
          <h3 class="mb-2 text-sm font-semibold">{{ $t('settings.deletion_preview.deleted_data_title') }}</h3>
          <DataTable
            :value="deletedRows"
            size="small"
            class="text-sm"
            :row-hover="true"
          >
            <Column field="category" :header="$t('settings.deletion_preview.table_category')" />
            <Column field="count" :header="$t('settings.deletion_preview.table_count')">
              <template #body="{ data }">{{ $t('settings.deletion_preview.table_count_unit', { count: data.count }) }}</template>
            </Column>
          </DataTable>
        </div>

        <div v-if="anonymizedRows.length">
          <h3 class="mb-2 text-sm font-semibold">{{ $t('settings.deletion_preview.anonymized_data_title') }}</h3>
          <DataTable
            ...
            :row-hover="true"
          >
            <Column field="entity" :header="$t('settings.deletion_preview.table_entity')" />
            <Column field="field" :header="$t('settings.deletion_preview.table_field')" />
          </DataTable>
        </div>

        <p v-if="preview.retentionDays" class="text-xs text-surface-500">
          {{ $t('settings.settings.delete_account.retention_note', { days: preview.retentionDays }) }}
        </p>
      </template>

      <div v-if="hasPassword" class="flex flex-col gap-2">
        <label for="deletePassword" class="text-sm font-semibold">
          {{ $t('settings.settings.delete_account.password_confirm_label') }}
        </label>
        <Password
          ...
          toggle-mask
          fluid
          :placeholder="$t('settings.settings.delete_account.password_placeholder')"
        />
      </div>
    </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <Button
          translate="no"
          :label="$t('settings.deletion_preview.cancel_button')"
          severity="secondary"
          @click="cancel"
        />
        <Button
          translate="no"
          :label="$t('settings.deletion_preview.delete_button')"
          severity="danger"
          icon="pi pi-trash"
