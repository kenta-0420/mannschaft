...
interface Position { id: number; name: string; description: string | null; requiredCount: number; color: string | null; sortOrder: number }
...
function openEdit(pos: Position) {
  editing.value = pos
  form.value = { name: pos.name, description: pos.description ?? '', requiredCount: pos.requiredCount, color: pos.color ?? '#6366f1' }
  showDialog.value = true
}
...
async function save() {
  ...
  try {
    if (editing.value) {
      await shiftApi.updatePosition(props.teamId, editing.value.id, form.value)
    } else {
      await shiftApi.createPosition(props.teamId, form.value)
    }
...
async function remove(id: number) {
  ...
  await shiftApi.deletePosition(props.teamId, id)
...
<template>
  <div>
    ...
    <div v-else-if="positions.length > 0" class="space-y-2">
      <div v-for="pos in positions" :key="pos.id" class="flex items-center gap-3 rounded-lg border border-surface-200 p-3 dark:border-surface-700">
        ...
        <Button icon="pi pi-trash" text rounded size="small" severity="danger" @click="remove(pos.id)" />
      </div>
    </div>
    <DashboardEmptyState v-else icon="pi pi-id-card" message="ポジションはまだありません" />

    <Dialog v-model:visible="showDialog" :header="editing ? 'ポジション編集' : 'ポジション追加'" :style="{ width: '400px' }" modal>
      <div class="flex flex-col gap-3">
        ...
        <div>
          ...
          <div class="flex gap-2">
            <button v-for="c in colorOptions" :key="c" class="h-8 w-8 rounded-full border-2" :class="form.color === c ? 'border-primary ring-2 ring-primary/30' : 'border-surface-300'" :style="{ backgroundColor: c }" @click="form.color = c" />
          </div>
        </div>
      </div>
      <template #footer>
