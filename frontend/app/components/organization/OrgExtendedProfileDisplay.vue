...
const profile = ref<OrganizationProfileResponse | null>(null)
...
async function loadData() {
  ...
  try {
    const [profileRes, officersRes, fieldsRes] = await Promise.all([
      api.getProfile(props.orgId),
      api.getOfficers(props.orgId, false),
      api.getCustomFields(props.orgId, false),
    ])
  ...
  >
    <div class="mb-3 flex items-center justify-between">
      ...
      <Button
        ...
        size="small"
        text
        @click="navigateTo(`/organizations/${orgId}/extended-profile`)"
      />
    </div>
