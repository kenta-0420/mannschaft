...
interface CounterStatus {
  id: number; name: string; isActive: boolean; currentTicket: string | null
  waitingCount: number; estimatedWaitMinutes: number
}
interface Status { counters: CounterStatus[]; totalWaiting: number; averageWaitMinutes: number }
...
async function load() {
  try {
    const res = await queueApi.getQueueStatus(props.scopeType, props.scopeId)
  ...
  <div>
    ...
    <div v-else-if="status">
      ...
      <div class="mb-6 grid grid-cols-2 gap-4">
        <div class="rounded-xl border border-surface-200 bg-surface-0 p-4 text-center dark:border-surface-700 dark:bg-surface-800">
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
        ...
        >
          <div class="mb-2 flex items-center justify-between">
            ...
            <Tag :value="counter.isActive ? '稼働中' : '停止'" :severity="counter.isActive ? 'success' : 'secondary'" rounded />
          </div>
          <div v-if="counter.currentTicket" class="mb-2 rounded-lg bg-white p-2 text-center dark:bg-surface-900">
