...
export function useLegalFilingApi() {
  ...
  async function listByOrganization(orgId: number) {
    return api<{ data: LegalFiling[] }>(
      `/api/v1/organizations/${orgId}/succession/legal-filings`,
    )
  }

  async function listByResident(orgId: number, residentRegistryId: number) {
    return api<{ data: LegalFiling[] }>(
      `/api/v1/organizations/${orgId}/succession/legal-filings/by-resident/${residentRegistryId}`,
    )
  }

  async function createLegalFiling(orgId: number, body: CreateLegalFilingRequest) {
    return api<{ data: LegalFiling }>(
      `/api/v1/organizations/${orgId}/succession/legal-filings`,
      { method: 'POST', body },
    )
  ...
  async function getById(orgId: number, legalFilingId: string) {
    return api<{ data: LegalFiling }>(
      `/api/v1/organizations/${orgId}/succession/legal-filings/${legalFilingId}`,
    )
  }

  async function buildEvidencePackage(orgId: number, legalFilingId: string) {
    return api<{ data: LegalFiling }>(
  ...
  async function getEvidenceDownloadUrl(orgId: number, legalFilingId: string) {
