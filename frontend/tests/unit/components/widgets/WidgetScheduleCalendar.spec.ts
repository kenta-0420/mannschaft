...
describe('WidgetScheduleCalendar.vue', () => {
  it('初期表示で当月の listSchedules が呼ばれ events が CalendarGrid に渡る', async () => {
    ...
    const now = new Date()
    const expected = expectedMonthRange(now.getFullYear(), now.getMonth() + 1)
    const callArgs = scheduleApiMock.listSchedules.mock.calls[0]!
    expect(callArgs[0]).toBe('team')
    expect(callArgs[1]).toBe(42)
    expect(callArgs[2]).toMatchObject({ from: expected.from, to: expected.to })
  ...
  it('次月ボタンクリックで currentMonth が進み再フェッチされる', async () => {
    ...
    expect(scheduleApiMock.listSchedules).toHaveBeenCalledTimes(2)
    ...
    const expected = expectedMonthRange(nextYear, nextMonth)
    const callArgs = scheduleApiMock.listSchedules.mock.calls[1]!
    expect(callArgs[0]).toBe('organization')
