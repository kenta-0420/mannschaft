...
describe('PageHeader.vue', () => {
  ...
  it('既定（back 未指定）で戻るリンクが描画される', async () => {
    const wrapper = await mountSuspended(PageHeader, {
      props: { title: 'ページタイトル' },
    })
  ...
  it(':back="false" で戻るリンクが描画されない', async () => {
    const wrapper = await mountSuspended(PageHeader, {
  ...
  it('back-to 指定で戻るリンクが NuxtLink になり href が一致', async () => {
    const wrapper = await mountSuspended(PageHeader, {
  ...
  it('デフォルト slot の内容がタイトル行に描画される（slot 非干渉）', async () => {
    const wrapper = await mountSuspended(PageHeader, {
      props: { title: 'ページタイトル' },
