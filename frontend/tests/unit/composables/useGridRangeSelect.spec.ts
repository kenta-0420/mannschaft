...
describe('useGridRangeSelect — 純粋関数（§6.6.3）', () => {
   ...
   *
   * **設計書内の矛盾についての記録（要裁可）**: AC-21d の例は「9:07 → 10:22 で 9:00 – 10:30」と
   * 書いているが、§6.6.3 が正とする `Math.round` では 10:22 の最近傍境界は 10:15 である
   * （10:22-10:15 = 7分 < 10:30-10:22 = 8分）。10:30 へ丸まる最初の分は **10:23**。
   * ここでは規範である §6.6.3（最近傍丸め）を実装の正とし、AC-21d の例示値は
   * 1分ぶんの記述誤りとして扱う。切り捨て(floor)にすれば 10:22 → 10:30 も作れるが、
   * それは §6.6.3 が「9:59 から下へなぞると 9:45 開始になり直感に反する」として
   * 明確に禁じている丸め方であり、採らない。
   */
