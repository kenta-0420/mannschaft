...
import OrgMessagingCampaignsIndexPage from '~/pages/organizations/[slug]/advertiser/messaging-campaigns/index.vue'

/**
 * CMP-260922-2045 第2陣 G5 の根治テスト。
 *
 * `pages/organizations/[slug]/advertiser/messaging-campaigns/index.vue` はキャンペーン取得が
 ...
 *
 * 検証観点:
 *   OM-001 取得失敗時に advertiser-messaging-campaigns-error-state が描画される
 *   OM-002（対照）取得成功・0件時はエラー状態を出さない
 *   OM-003 再試行が初回表示と同じ取得関数（load）を呼ぶ
 */
...
beforeAll(async () => {
  ...
  const warmup = await mountSuspended(OrgMessagingCampaignsIndexPage)
  warmup.unmount()
})

describe('pages/organizations/[slug]/advertiser/messaging-campaigns/index.vue — 取得失敗時のエラー状態', () => {
  beforeEach(() => {
    listCampaigns.mockReset()
  })

  it('OM-001: 取得失敗時に advertiser-messaging-campaigns-error-state が描画される', async () => {
    listCampaigns.mockRejectedValue(new Error('network error'))
    const wrapper = await mountSuspended(OrgMessagingCampaignsIndexPage)
    await flushMicrotasks()

    expect(wrapper.find('[data-testid="advertiser-messaging-campaigns-error-state"]').exists()).toBe(true)
  })

  it('OM-002（対照）: 取得成功・0件時はエラー状態を出さない', async () => {
    listCampaigns.mockResolvedValue({ data: [], meta: { total: 0, page: 0, size: 20, totalPages: 0 } })
    const wrapper = await mountSuspended(OrgMessagingCampaignsIndexPage)
    await flushMicrotasks()
  ...
  it('OM-003: 再試行が初回表示と同じ取得処理（load）を呼ぶ', async () => {
