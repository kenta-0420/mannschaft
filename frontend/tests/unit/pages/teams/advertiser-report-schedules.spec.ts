...
import TeamReportSchedulesPage from '~/pages/teams/[slug]/advertiser/report-schedules.vue'

/**
 * CMP-260922-2045 第2陣 G5 の根治テスト（組織版と対になるチーム版）。
 *
 * `pages/teams/[slug]/advertiser/report-schedules.vue` は定期レポート一覧の取得が
 * 失敗しても `schedules` を空配列にリセットするだけで、空状態文言へそのまま落ちていた。
 * エラー専用状態（DashboardErrorState / report-schedules-error-state）で修正した。
 *
 * 検証観点:
 *   TR-001 取得失敗時に report-schedules-error-state が描画される
 *   TR-002（対照）取得成功・0件時はエラー状態を出さない
 *   TR-003 再試行が初回表示と同じ取得関数（load）を呼ぶ
 */
...
beforeAll(async () => {
  ...
  const warmup = await mountSuspended(TeamReportSchedulesPage)
  warmup.unmount()
})

describe('pages/teams/[slug]/advertiser/report-schedules.vue — 取得失敗時のエラー状態', () => {
  beforeEach(() => {
    getReportSchedules.mockReset()
  })

  it('TR-001: 取得失敗時に report-schedules-error-state が描画される', async () => {
    getReportSchedules.mockRejectedValue(new Error('network error'))
    const wrapper = await mountSuspended(TeamReportSchedulesPage)
    await flushMicrotasks()

    expect(wrapper.find('[data-testid="report-schedules-error-state"]').exists()).toBe(true)
  })

  it('TR-002（対照）: 取得成功・0件時はエラー状態を出さない', async () => {
    getReportSchedules.mockResolvedValue({ data: [] })
    const wrapper = await mountSuspended(TeamReportSchedulesPage)
    await flushMicrotasks()
  ...
  it('TR-003: 再試行が初回表示と同じ取得処理（getReportSchedules）を呼ぶ', async () => {
