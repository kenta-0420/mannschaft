...
const BE = process.env.API_BASE_URL ?? 'http://localhost:8081'
...
test('親組織なしチームで記録開始からタイムラインへ遷移する', async ({ page }) => {
  ...
  // FC東京U-18 の管理者アカウントでログインし、対象チームを実際の画面導線から選ぶ。
  await page.goto('/teams', { waitUntil: 'commit' })
  await waitForHydration(page)
  const teamCardHeading = page.getByRole('heading', { name: 'テストチーム', exact: true })
  await teamCardHeading.waitFor({ state: 'visible', timeout: 180_000 })
  await teamCardHeading.click()
  await page.waitForURL(/\/teams\/team-000092\/?$/, { timeout: 180_000 })

  const matchesLink = page.locator('a[href="/teams/team-000092/matches"]').first()
  await matchesLink.waitFor({ state: 'visible', timeout: 180_000 })
  await matchesLink.click()
  await page.waitForURL(/\/teams\/team-000092\/matches\/?$/, { timeout: 180_000 })
  await page.getByRole('button', { name: '＋試合を記録', exact: true }).click()
  ...
  await expect(page.getByRole('heading', { name: 'ライブ記録' })).toBeVisible({ timeout: 180_000 })
  await expect(page.getByText('タイムライン', { exact: true })).toBeVisible({ timeout: 180_000 })
