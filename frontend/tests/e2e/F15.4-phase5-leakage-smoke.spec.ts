 ...
 *   B. 404 ケース網羅
 *      - 不在 ID / 削除済み / archived / visibility=GUESTS_AND_ABOVE（旧PRIVATE・旧ORGANIZATION_ONLY）
 *      - エラーページにチーム情報が漏れない
 */
...
const NOT_FOUND_CASES: ReadonlyArray<{ label: string; teamId: number }> = [
  ...
  { label: 'visibility=GUESTS_AND_ABOVE チーム（旧PRIVATE）', teamId: 9203 },
  { label: 'visibility=GUESTS_AND_ABOVE チーム（旧ORGANIZATION_ONLY）', teamId: 9204 },
