...
-- 日時列は TIMESTAMP(6)。Entity 側は Instant（起きた瞬間）で保持するため、
-- タイムゾーンを持たない DATETIME と組むとサーバーTZでずれる。前例は job_qr_tokens。
...
CREATE TABLE village_invitations (
    ...
    expires_at                TIMESTAMP(6)     NOT NULL                                COMMENT '無期限の招待を作れないようNOT NULL',
