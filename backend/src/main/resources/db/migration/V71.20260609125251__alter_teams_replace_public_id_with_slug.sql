-- teams テーブル: public_id を slug に置き換え
-- slug は人間可読なカスタム識別子（3〜30文字の英数字ハイフン）
ALTER TABLE teams ADD COLUMN slug VARCHAR(30) NULL;
...
UPDATE teams SET slug = CONCAT('team-', LPAD(id, 6, '0'))
WHERE CHAR_LENGTH(slug) < 3 OR slug IS NULL OR slug = '' OR slug = '-';

-- slug が30文字超の場合は切り詰め
UPDATE teams SET slug = LEFT(slug, 30) WHERE CHAR_LENGTH(slug) > 30;
...
ALTER TABLE teams MODIFY COLUMN slug VARCHAR(30) NOT NULL;
ALTER TABLE teams ADD UNIQUE INDEX uq_teams_slug (slug);

-- public_id カラムを削除
ALTER TABLE teams DROP INDEX idx_teams_public_id;
ALTER TABLE teams DROP COLUMN public_id;
