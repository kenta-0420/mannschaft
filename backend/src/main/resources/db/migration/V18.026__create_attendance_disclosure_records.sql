CREATE TABLE attendance_disclosure_records (
    ...
    evaluation_id BIGINT NOT NULL COMMENT '対象評価ID',
    student_user_id BIGINT NOT NULL COMMENT '対象生徒のユーザーID',
    ...
    decided_by BIGINT NOT NULL COMMENT '判断者のユーザーID（担任等）',
    decided_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '判断日時',
    notification_id BIGINT NULL COMMENT '通知配信レコードID（DISCLOSED時のみ・将来拡張）',
    PRIMARY KEY (id),
