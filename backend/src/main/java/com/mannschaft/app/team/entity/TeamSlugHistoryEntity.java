...
/**
 * チーム slug リネーム履歴エンティティ（F01.2 §5.9.5）。
 *
 * <p>チームの slug を変更した際に「旧 slug」を記録する。旧 URL アクセスを新 slug へ
 * 301 リダイレクトするための解決元データ（{@code old_slug → 現 team.slug}）であり、
 * かつ他チームが同じ slug を再利用できないよう恒久的に予約する役割を持つ。</p>
 *
 * <p>原則準拠:</p>
 * <ul>
 *   <li>新規テーブルゆえ主キーは UUIDv7（原則 6・{@link UuidV7Entity} 継承）。id は BINARY(16)。</li>
 *   <li>{@code teamId} は teams.id への ID 参照のみ。クロスドメイン FK は張らない（原則 1）。
 *       参照整合性はアプリ層で保証する。</li>
