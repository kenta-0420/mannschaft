...
/**
 * F09.16 居住実態管理・見守りのエラーコード定義（S3-B 追加分）。
 */
@Getter
...
public enum ResidenceStatusErrorCode implements ErrorCode {
    ...
    SNAPSHOT_NOT_FOUND("RESIDENCE_STATUS_001", "アクティビティスナップショットが見つかりません", Severity.WARN),

    /** 本人はアクティビティスコアを閲覧できない */
    SNAPSHOT_SELF_ACCESS_FORBIDDEN("RESIDENCE_STATUS_002",
            "本人はアクティビティスコアを閲覧できません（ADMIN/WATCHER のみ）", Severity.WARN),

    /** ダッシュボード閲覧権限がない（ADMIN/DEPUTY_ADMIN のみ） */
    DASHBOARD_ACCESS_FORBIDDEN("RESIDENCE_STATUS_003",
            "ダッシュボードは ADMIN/DEPUTY_ADMIN のみ閲覧できます", Severity.WARN),

    /** アクティビティスナップショット閲覧権限がない */
    SNAPSHOT_ACCESS_FORBIDDEN("RESIDENCE_STATUS_004",
