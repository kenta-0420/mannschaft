package com.mannschaft.app.admin.batch.event;

import com.mannschaft.app.admin.entity.BatchJobLogEntity;

import java.time.Instant;

/**
 * F10.X 第一陣 — バッチ失敗イベント。
 *
 * <p>{@link com.mannschaft.app.admin.batch.BatchExecutionAspect} が
 * {@link com.mannschaft.app.admin.batch.BatchEndpoint @BatchEndpoint} 付きメソッドが
 * 例外で終了した際に発火する。受信側（{@link com.mannschaft.app.admin.batch.BatchEventListener}）は
 * F12.5 へのエラーレポート起票と SYSTEM_ADMIN 通知配信を非同期で行う。</p>
 *
 * @param name       バッチ識別子
 * @param log        書き込んだ {@link BatchJobLogEntity}
 * @param cause      失敗原因の例外
 * @param occurredAt 発火時刻
 */
public record BatchFailedEvent(
        String name,
        BatchJobLogEntity log,
        Throwable cause,
        Instant occurredAt) {
}
