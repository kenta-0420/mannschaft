...
public interface BulletinThreadRepository extends JpaRepository<BulletinThreadEntity, Long> {
     ...
     * 全文検索でスレッドを取得する。
     */
    @Query(value = "SELECT * FROM bulletin_threads WHERE deleted_at IS NULL AND scope_type = :scopeType AND scope_id = :scopeId AND MATCH(title, body) AGAINST(:keyword IN BOOLEAN MODE)",
            countQuery = "SELECT COUNT(*) FROM bulletin_threads WHERE deleted_at IS NULL AND scope_type = :scopeType AND scope_id = :scopeId AND MATCH(title, body) AGAINST(:keyword IN BOOLEAN MODE)",
            nativeQuery = true)
