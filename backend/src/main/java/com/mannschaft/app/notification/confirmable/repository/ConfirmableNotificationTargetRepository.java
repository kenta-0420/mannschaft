...
public interface ConfirmableNotificationTargetRepository
     ...
     */
    @Query(value =
            "WITH RECURSIVE send_org_tree (id, depth) AS ( "
            + "    SELECT o0.id, 0 FROM organizations o0 "
            + "      JOIN confirmable_notifications n ON n.scope_id = o0.id "
            + "      WHERE n.id = :notificationId AND n.scope_type = 'ORGANIZATION' AND o0.deleted_at IS NULL "
            + "  UNION ALL "
            + "    SELECT c0.id, p0.depth + 1 FROM organizations c0 "
            + "      JOIN send_org_tree p0 ON c0.parent_organization_id = p0.id "
            + "      WHERE c0.deleted_at IS NULL AND p0.depth < :maxDepth "
            + "), send_active_org_teams (team_id) AS ( "
            + "    SELECT DISTINCT tom0.team_id FROM team_org_memberships tom0 "
            + "      WHERE tom0.organization_id IN (SELECT id FROM send_org_tree) AND tom0.status = 'ACTIVE' "
            + "), valid_org_target_ids (id) AS ( "
            // CMP-260920-1040是正（Codex P1・§8.1）: ORGANIZATION ターゲットは「送信組織を頂点とする現在のツリー」
            // に含まれるものだけを採用する（ターゲット自身が別の親の下へ移った場合は0人として扱う・AC-38）。
            + "    SELECT t.target_id FROM confirmable_notification_targets t "
            + "      WHERE t.confirmable_notification_id = :notificationId AND t.target_type = 'ORGANIZATION' "
            + "        AND t.target_id IN (SELECT id FROM send_org_tree) "
            + "), valid_team_target_ids (id) AS ( "
            // TEAM ターゲットは、組織スコープの送信では「送信組織ツリー内のいずれかの組織に現時点で ACTIVE
            // 所属している」チームだけ、チームスコープの送信では自チームだけを採用する（§8.1・AC-37・AC-39）。
            + "    SELECT t2.target_id FROM confirmable_notification_targets t2 "
            + "      JOIN confirmable_notifications n2 ON n2.id = t2.confirmable_notification_id "
            + "      WHERE t2.confirmable_notification_id = :notificationId AND t2.target_type = 'TEAM' "
            + "        AND ( "
            + "          (n2.scope_type = 'ORGANIZATION' AND t2.target_id IN (SELECT team_id FROM send_active_org_teams)) "
            + "          OR (n2.scope_type = 'TEAM' AND t2.target_id = n2.scope_id) "
            + "        ) "
            + "), org_tree (id, depth) AS ( "
            + "    SELECT o.id, 0 FROM organizations o "
            + "      JOIN valid_org_target_ids v ON v.id = o.id "
            ...
            + "    SELECT id FROM valid_team_target_ids "
     ...
     *
     * @param sendScopeType         送信スコープ種別（{@code ORGANIZATION}/{@code TEAM}。§8.1 と同じ判定に揃える）
     * @param sendScopeId           送信スコープID
    ...
    @Query(value =
            "WITH RECURSIVE send_org_tree (id, depth) AS ( "
            + "    SELECT o0.id, 0 FROM organizations o0 "
            + "      WHERE o0.id = :sendScopeId AND :sendScopeType = 'ORGANIZATION' AND o0.deleted_at IS NULL "
            + "  UNION ALL "
            + "    SELECT c0.id, p0.depth + 1 FROM organizations c0 "
            + "      JOIN send_org_tree p0 ON c0.parent_organization_id = p0.id "
            + "      WHERE c0.deleted_at IS NULL AND p0.depth < :maxDepth "
            ...
            + "        AND o.id IN (SELECT id FROM send_org_tree) "
            + "  UNION ALL "
            + "    SELECT c.id, p.depth + 1 FROM organizations c "
            + "      JOIN org_tree p ON c.parent_organization_id = p.id "
            + "      WHERE c.deleted_at IS NULL AND p.depth < :maxDepth "
            + "), team_targets (team_id) AS ( "
            + "    SELECT :sendScopeId AS team_id "
            + "      WHERE :sendScopeType = 'TEAM' AND :sendScopeId IN (:teamTargetIds) "
            + "    UNION "
            + "    SELECT sat.team_id FROM send_active_org_teams sat "
            + "      WHERE :sendScopeType = 'ORGANIZATION' AND sat.team_id IN (:teamTargetIds) "
            + "), active_org_teams (team_id) AS ( "
            + "    SELECT DISTINCT tom.team_id FROM team_org_memberships tom "
            + "      WHERE tom.organization_id IN (SELECT id FROM org_tree) AND tom.status = 'ACTIVE' "
            + ") "
    ...
    long countAdHocTargetRecipients(
            @Param("sendScopeType") String sendScopeType,
            @Param("sendScopeId") Long sendScopeId,
