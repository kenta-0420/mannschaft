...
public enum SignageErrorCode implements ErrorCode {
    ...
    /**
     * アクセスが拒否されました（トークン無効/IP制限・400）。
     *
     * <p><b>「無効化済み」「有効期限切れ」「存在しない」を意図的に同一コードへ畳んでいる。
     * 分割・ステータス分けをしてはならない。</b>これは意味が割れているのではなく、トークンの
     * 失敗理由を呼び出し側に区別させないための設計判断である
     * （{@code SignageAccessTokenService#validateToken} の Javadoc 参照）。
     * ステータスを分ければ「そのトークンは実在するのか」を外部から判定できる存在オラクルになる。
     * 畳み込みによって秘匿を達成しているため、ステータスは既定の 400 のままとする
     * （{@code SignageScopeContractIT} が3状況とも 400 を契約として固定している）。</p>
     */
