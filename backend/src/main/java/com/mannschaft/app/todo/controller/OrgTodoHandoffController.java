...
/**
 * 組織スコープ TODO キャッチボール API（F02.3.1 Phase 2）。
 */
@RestController
@RequestMapping("/api/v1/organizations/{orgId}/todos/{todoId}")
@Tag(name = "TODO キャッチボール（組織）", description = "F02.3.1 組織TODO の引き渡し操作と履歴取得")
@RequiredArgsConstructor
public class OrgTodoHandoffController {
    ...
    public ResponseEntity<ApiResponse<TodoHandoffResponse>> handoff(
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(handoffService.handoff(
                TodoScopeType.ORGANIZATION, orgId, todoId, request, SecurityUtils.getCurrentUserId()));
    }

    /**
     * 組織 TODO のキャッチボール履歴を取得する。
     */
    @GetMapping("/handoffs")
    ...
    public ResponseEntity<ApiResponse<List<TodoHandoffResponse>>> listHistory(
        ...
        return ResponseEntity.ok(handoffService.listHistory(
                TodoScopeType.ORGANIZATION, orgId, todoId, SecurityUtils.getCurrentUserId()));
    }
}
