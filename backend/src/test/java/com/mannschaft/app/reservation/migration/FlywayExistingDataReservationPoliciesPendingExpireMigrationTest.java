 ...
 * V171（仮押さえ自動失効のチーム設定追加・F03.4.5 §6.3）を含む全マイグレーションがクラッシュせず
 ...
 * マスター確定事項としている。V171 は backfill UPDATE を書かず、MySQL の
 ...
 * その場合は V171 に明示的な backfill UPDATE を足して根治すること — 「既存チームでは黙って無効」は
 * 設計意図と乖離した静かな機能欠落であり、隠してはならない。</p>
 *
 * <p>本テストは <b>V171 直前まで適用 → ALTER 前スキーマの reservation_policies 行をシード →
 * 残り（V171 含む）を適用</b> という既存データ経路を再現する。
...
@DisplayName("Flyway 既存データ reservation_policies.pending_expire_hours 追加（V171）番人テスト")
class FlywayExistingDataReservationPoliciesPendingExpireMigrationTest {

    /**
     * V171（本 PR の ALTER）の直前まで適用するためのターゲット。
     *
     * <p>Flyway の {@code target} は<b>実在するバージョンと完全一致</b>していなければならず、
     * 「169 系の上限」を表す架空の値（例 {@code 169.99999999999999}）を渡すと
     * {@code FlywayException: No migration with a target version ... could be found} で失敗する
     * （実測）。よって既存の兄弟番人テストと同様に実在バージョンを固定する。</p>
     *
     * <p><b>保守メモ</b>: 本定数より後のマイグレーションが増えても、
     * 「pending_expire_hours 列がまだ無い時点で既存行をシードする」という検証意図は保たれる
     * （シード前に適用済みの範囲が狭まるだけ）。</p>
     */
    private static final String PRE_PENDING_EXPIRE_TARGET = "169.20260728100504";
    ...
    @Test
    @DisplayName("既存policies行_pending_expire列なし状態からV171適用_既存行が既定24で充足される")
    void 既存データを持つDBでV171が安全に適用される() throws Exception {
        // given: V171 の直前（V169 系まで）を適用 ＝ pending_expire_hours 列を持たない状態
        Flyway pre = Flyway.configure()
                .dataSource(MYSQL.getJdbcUrl(), MYSQL.getUsername(), MYSQL.getPassword())
                .locations("classpath:db/migration")
                .outOfOrder(false)
                .target(MigrationVersion.fromVersion(PRE_PENDING_EXPIRE_TARGET))
                .load();
        ...
        try (Connection conn = DriverManager.getConnection(
            ...
            assertThat(columnExists(conn, "reservation_policies", "pending_expire_hours"))
                    .as("V171 適用前は pending_expire_hours 列が存在しないこと").isFalse();
        ...
        // when: 残りのマイグレーション（V171 含む）を適用する
        ...
        assertThat(restResult.success).as("V171 を含む残りのマイグレーションが成功すること").isTrue();
