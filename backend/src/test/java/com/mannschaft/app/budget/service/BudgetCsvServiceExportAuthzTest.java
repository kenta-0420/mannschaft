...
/**
 * {@link BudgetCsvService#export} の認可単体テスト（CMP-260917-2102 Phase 1 の追撃）。
 *
 * <p>実機で MEMBER が CSV を全件エクスポートできることを確認済み（BUDGET_009等と異なり
 * checkMembership止まりだった）。予算はスコープ問わずDEPUTY_ADMIN限定のため無条件
 * checkAdminOrAbove に是正する。</p>
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("BudgetCsvService 認可単体テスト（export）")
class BudgetCsvServiceExportAuthzTest {
    ...
    @InjectMocks
    private BudgetCsvService service;
    ...
    void member_isForbidden() {
        ...
        assertThatThrownBy(() -> service.export(FISCAL_YEAR_ID))
    ...
    void admin_isAllowed() {
        ...
        assertThatCode(() -> service.export(FISCAL_YEAR_ID)).doesNotThrowAnyException();
    }
}
