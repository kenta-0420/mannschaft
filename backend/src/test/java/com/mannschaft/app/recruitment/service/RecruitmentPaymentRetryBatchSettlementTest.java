 ...
 *
 * <p><b>AC-21 の観測点</b>: 「キーセットページングが維持されること」と「レコード 1 件あたりの徴収呼び出しが
 * 1 回を超えないこと」を固定する。escrow の引き当てと経路判定は payment ドメインの内部で完結するため
 * （§3.4）、recruitment 側のバッチが escrow をまとめて読む設計は採らない。1 件につき Stripe 呼び出しが
 * 1 回必要である以上、件数分のラウンドトリップは構造的に不可避であり、これは N+1 問題ではなく仕事の量
 * そのものである（一括取得しても Stripe 呼び出しは減らない）。</p>
 */
