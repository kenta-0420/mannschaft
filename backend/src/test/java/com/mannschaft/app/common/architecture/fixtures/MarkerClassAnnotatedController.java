...
/**
 * fixture: クラスレベルに {@link AuthorizedInService} 監査済マーカーを付与した Controller。
 *
 * <p>Mapping メソッド自体は無印だが、<b>宣言クラスへのマーカー付与により認可シグナルあり
 * （合格）</b>と判定されるべきケース（{@code getOwner().isAnnotatedWith} 経路の担保）。
 * {@code @PreAuthorize} のクラスレベル付与（{@link org.springframework.web.bind.annotation.RestController}）
 * と完全対称にクラスレベルのマーカーも認可済みとして扱われることを保証する。
 */
@AuthorizedInService
@RestController
@RequestMapping("/fixtures/marker-class-annotated")
public class MarkerClassAnnotatedController {
