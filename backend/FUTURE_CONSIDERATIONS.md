...
- パーミッション評価のキャッシュ戦略（Valkey）も早期に検討

### 3. リアルタイム通信の負荷
- チャット + 通知 + プッシュ通知が全てWebSocket/Valkey経由
- 同時接続数が増えた時のValkeyブローカーのスケーリング設計が必要
...
`shift_slots.assigned_user_ids`（JSON 配列）は現行設計で問題なく動作するが、`GET /shifts/my` がユーザーのシフトを全チームから横断検索する際に `JSON_CONTAINS` のフルスキャンがボトルネックになる可能性がある。Valkey キャッシュ（`mannschaft:cache:user-shifts:{userId}`、TTL 30分）で緩和するが、キャッシュミス時のレイテンシ突発悪化が懸念される。
...
**移行判断基準**: チーム数 × シフト枠数が増加し、Valkey キャッシュミス時のクエリが 100ms を超えるようになった場合に移行を検討する。
...
- [ ] 開発環境のDocker Compose構成（MySQL + Valkey + MinIO）
...
### LINE Rich Menu（Phase 10+）

LINE BOT 画面下部に固定表示されるカスタムメニュー。Phase 9 の LINE 連携（F09.4）で利用パターンが定着した後、実際のユーザーニーズに基づいてメニュー構成を決定する。

**検討事項**
- [ ] Rich Menu のテンプレートパターン（「出欠確認」「スケジュール表示」「通知設定」等のプリセット）
- [ ] チーム/組織ごとのメニュー項目カスタマイズ UI
- [ ] Rich Menu API でのプログラマティック作成 vs LINE Official Account Manager での手動作成
- [ ] メニューの切り替え（ユーザーの LINE 連携スコープに応じて異なるメニューを表示）
- [ ] Rich Menu 画像の自動生成（チームロゴ・カラーテーマの反映）

### X（旧 Twitter）フィード連携（収益安定後）

F09.4 LINE/SNS 連携で Instagram フィードのみ Phase 9 で実装。X フィード連携は API 利用料（Basic プラン月$100〜）が個人開発段階では負担が大きいため延期。収益に余裕ができた段階で `sns_feed_configs.provider = 'X'` として追加する。DB・API 設計は拡張可能な状態で維持済み。

**実装開始の判断基準**
- [ ] 月間収益が X API Basic プラン料金（月$100）を安定的に上回っている
- [ ] X フィード表示へのユーザーニーズが確認されている（要望数・アンケート等）

**プラン選定の判断基準（実装時）**
- [ ] Free プラン（月1,500リクエスト）: Valkey キャッシュ（TTL 15分）併用で小規模運用なら十分
- [ ] Basic プラン（月$100、10,000リクエスト）: 標準的な運用。読み取り専用フィード表示に推奨
- [ ] Pro プラン（月$5,000）: ハッシュタグ検索・高度なフィルタリングが必要な場合のみ。Mannschaft の規模では過剰
- [ ] コスト転嫁の検討: 利用チーム数が増えて運営費吸収が困難になった場合、SNS 連携を選択式モジュールに移行して課金対象とするか
...
- [ ] 非アクティブアーカイブバッチの設計と実装
  ...
  - チーム・組織（12ヶ月 / 全メンバーの最新ログインが基準）
    ...
    - [ ] チーム/組織の Valkey キャッシュ（`team:modules:{id}` 等）削除処理
...
以下は Phase 11 の設計完了後に検討する追加機能候補。優先度順に記載。

### AI アシスタント（組織専用チャットボット）
- **概要**: ナレッジベース（F11.4）・スケジュール・FAQ を情報源にメンバーの質問に自動回答する RAG ベースのチャットボット
- **技術**: pgvector でベクトルDB、Claude API で回答生成（F06.3 タイムラインダイジェストと同一基盤）
- **ROI**: ADMIN の問い合わせ対応負荷を劇的に軽減。全テンプレートで需要あり
- **検討事項**: RAG の精度チューニング、ハルシネーション対策、回答できない質問の ADMIN エスカレーション、API コスト管理

### CRM / 顧客管理
- **概要**: 顧客との関係性全体を管理。来訪履歴・満足度・離反リスク・誕生日フォロー等
- **連携先**: F07.1 サービス記録、F09.6 ダイレクトメール、F09.2 プロモーション
- **対象**: 整骨院、美容室、ジム、レストラン（BtoC ビジネス系テンプレート）
- **検討事項**: F07.1 との機能境界（サービス記録は施術履歴、CRM は関係性管理）、セグメント配信のフィルタ条件設計

### プロジェクト管理（カンバン / ガント）
- **概要**: 複数人が関わるプロジェクトの可視化。F02.3 TODO の拡張ではなく独立機能
- **機能**: カンバンボード（ドラッグ＆ドロップ）、ガントチャート（依存関係付き）、マイルストーン、F11.1 承認連携
- **対象**: 企業、学校（行事準備）、マンション管理組合（修繕プロジェクト）
- **検討事項**: F02.3 TODO との棲み分け（TODO は個人タスク、プロジェクト管理はチーム横断）

### 動画管理・ライブ配信連携
- **概要**: 練習動画・試合ハイライト・セミナー録画の管理。F06.2 ギャラリー（写真）の動画版
- **技術**: S3 + CloudFront + MediaConvert（HLS トランスコード）。ライブ配信は YouTube Live / Zoom URL 埋め込み
- **対象**: スポーツチーム、学校、コミュニティ
- **検討事項**: ストレージコスト（動画は写真の100倍以上）、トランスコードの非同期パイプライン設計

### チーム間マーケットプレイス
- **概要**: 同一組織内のチーム間で備品貸し借り・ノウハウ共有・合同イベント企画を仲介
- **連携先**: F08.1 マッチング、F07.3 備品管理
- **対象**: 大規模組織（複数チームを束ねる組織）

### 音声メモ / ボイスメッセージ
- **概要**: F04.2 チャットに音声メッセージ機能を追加。移動中のテキスト入力が困難な場面で活用
- **技術**: MediaRecorder API → S3 アップロード → 再生 UI。音声認識（Whisper API）でテキスト変換オプション
- **対象**: スポーツチーム（コーチ→選手への指示）、企業（移動中の報告）

### リアルタイム位置共有
- **概要**: イベント当日の集合場所案内、マラソン大会の選手追跡。期間限定（イベント開始〜終了のみ）
- **連携先**: F11.5 イベント管理
- **技術**: Geolocation API + WebSocket。プライバシー配慮（共有は明示的 opt-in、イベント終了で自動停止）
- **対象**: スポーツチーム、アウトドアコミュニティ
