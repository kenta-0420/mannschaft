...
- **`valid_from` は DB に NULL を保存しない**。省略時はアプリ層で `paid_at` の日付（DATE）を自動セットしてから INSERT する。「paid_at 当日から有効」という事実を DB レベルで明示するため NULL を避ける
...
- 同一ユーザー・同一 payment_item に対する有効な PAID レコードの重複はアプリ層で防止する（409）。判定条件: `status = 'PAID' AND (valid_until IS NULL OR valid_until >= CURDATE())`（支払い方法 STRIPE / MANUAL 問わず同一条件で判定）
...
| POST | `/api/v1/teams/{id}/payment-items/{itemId}/payments/{paymentId}/refund` | 必要（ADMIN）| 全額返金実行（Stripe 払い戻し）|
| POST | `/api/v1/organizations/{id}/payment-items/{itemId}/payments/{paymentId}/refund` | 必要（ADMIN）| 組織支払いの全額返金実行 |
...
| ステータス | 条件 |
|-----------|------|
| 409 | 対象ユーザーの有効な PAID レコードが既に存在する（判定: `status = 'PAID' AND (valid_until IS NULL OR valid_until >= CURDATE())`）|
...
| Stripe イベント | 処理内容 |
|----------------|---------|
| `checkout.session.completed`（`mode=payment`）| `member_payments.status` を PENDING → PAID に更新、`paid_at` / `stripe_payment_intent_id` / `valid_until` を設定 |
...
3. イベント種別を判定:
   ...
   c. status = REFUNDED、stripe_refund_id = refund.id、refunded_at = NOW() で UPDATE
   d. audit_logs に PAYMENT_REFUNDED（アプリ内返金後の Webhook）または
...
2. 操作者が当該チームの ADMIN か確認 → でなければ 403
...
6. Stripe API で全額返金を同期呼び出し（Stripe 先）:
     POST /v1/refunds {
       ...
       metadata: { member_payment_id: "<ID>", refunded_by: "<ADMIN user_id>" }
     }
...
8. audit_logs に PAYMENT_REFUNDED を記録（returned_by = 操作 ADMIN の user_id）
9. 200 OK を返す
...
[webhook: charge.refunded の冪等処理]
  ※ アプリ内返金後、Stripe から charge.refunded webhook が届く。
    ...
    b. status が既に 'REFUNDED' ならスキップ（冪等）
    c. REFUNDED でない場合（Stripe ダッシュボードから直接返金された場合）は
       step 7 と同様に status / stripe_refund_id / refunded_at を更新し
...
| 項目 | 仕様 |
|------|------|
| 返金ボタン | ADMIN の支払い詳細画面にのみ表示（`payment_method = 'STRIPE'` かつ `status = 'PAID'` の場合のみ活性）|
...
**Phase 3 追加マイグレーション（返金機能）**
```
V3.007__add_refund_columns_to_member_payments.sql
