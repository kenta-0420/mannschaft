...
| `service_record_fields` | チームごとのカスタムフィールド定義 | なし |
...
**制約・備考**
- 1チームあたりのカスタムフィールド上限: **20件**（アプリ層で検証）
- フィールド削除時: `service_record_values` の対応レコードもカスケード削除する（孤立データ防止）
...
| `field_id` | BIGINT UNSIGNED | NO | — | FK → service_record_fields（ON DELETE CASCADE）|
...
- `field_id` ON DELETE CASCADE: フィールド定義削除時に値も自動削除
...
**制約・備考**
- 1チームあたりのテンプレート上限: **デフォルト10件**（有料プランで上限引き上げ。具体的な段階・上限数は Phase 8 で確定）。上限値はアプリ層で設定から取得し、ハードコードしない
- 論理削除: `deleted_at DATETIME nullable`（SoftDeletableEntity 適用）
...
- `field_id` ON DELETE CASCADE: フィールド定義削除時にテンプレート値も自動削除
...
teams (1) ──── (N) service_record_templates
...
| GET | `/api/v1/teams/{teamId}/service-records/templates` | 必要 | テンプレート一覧取得 |
| POST | `/api/v1/teams/{teamId}/service-records/templates` | 必要 | テンプレート作成 |
...
テンプレート一覧を `sort_order` 順で取得。記録作成画面のテンプレート選択ドロップダウンに使用する。
...
- **1,000件超**: 非同期ジョブとして実行し、完了後にダウンロードリンクを通知で送信。レスポンスは `202 Accepted` + ジョブ ID を返す
...
6. スタッフへ「○○さんが記録にリアクションしました」通知（任意設定）
```
...
- **カスタムフィールド削除**: フィールド削除時は確認ダイアログで「既存の〇件の値も削除されます」と警告表示（フロントエンド実装）
...
- **カスタムフィールド値の XSS 対策**: TEXT 型の値はレスポンス時にサニタイズする
...
- [ ] `service_record_fields` の並び替え API（sort_order の一括更新）のエンドポイント設計
- [ ] リアクション通知をスタッフに送るか（メンバーの反応を把握する目的）、送る場合の通知チャネル（プッシュ / アプリ内のみ）
- [ ] テンプレートの組織レベル共有（複数チームで同じテンプレートを使い回す）の要否
