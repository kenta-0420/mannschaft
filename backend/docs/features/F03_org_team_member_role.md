...
- `parent_organization_id` の循環参照はアプリケーション層で防ぐ（最大5階層）
...
2. parent_organization_id が指定された場合: 循環参照・深さ（5階層）チェック
...
4. invite_tokens に UPDATE SET revoked_at = NOW()
   ...
   （削除済みチームへの参加導線を即時遮断）
5. audit_logs に TEAM_DELETED を記録
...
4. invite_tokens に UPDATE SET revoked_at = NOW()
   ...
   （削除済み組織への参加導線を即時遮断）
5. audit_logs に ORGANIZATION_DELETED を記録
6. 204 No Content を返す
```

> - 当該組織に所属する子組織・チームは削除しない（子チームはそのまま存続し、フィルタリングは `organization_id` が論理削除済みかアプリ層で判定）
> - 子チームの `invite_tokens` は本フローでは失効させない（チームが独立して存続する可能性があるため）。チームを合わせて削除する場合はチーム削除フローを別途実行する
...
`organizations.parent_organization_id`（自己参照 FK）により最大5階層の組織ツリーを構成する。
...
- **最大5階層**: depth 0（トップ）〜 depth 4（末端）。depth が 4 の組織を親とする組織の作成は 422 エラー。チームは depth に関係なく任意の組織に所属可
- **深度チェック**: 組織作成・`parent_organization_id` 更新時に祖先を遡って depth を計算し、5階層を超える場合は 422（`"組織階層の最大深さ（5階層）を超えています"`）を返す
...
- [x] 組織階層の最大深さ（現在3階層固定）をシステム設定として管理可能にするかを確定する → **最大5階層（depth 0〜4）に拡張、アプリ層固定**。全国規模の連盟〜地域支部〜部会までを網羅するには3階層では不足するため。システム設定化は採用しない（深度削減時に既存組織が超過するケースの処理が複雑になるため）。超過時は 422 を返す。WITH RECURSIVE CTE の `depth < 4` も更新
...
| 2026-03-08 | 組織階層の最大深さを3階層→5階層（depth 0〜4）に拡張: 全国規模連盟〜地域支部構造に対応するため。システム設定化は不採用（深度削減時の既存超過問題を避けるため）。階層構造の説明・例・`depth < 2` → `depth < 4`（CTE）・循環参照チェック・組織作成フローを更新。超過時 422 エラーメッセージを明記 |
