...
- 組織の論理削除時: ON DELETE CASCADE により紐付く全レコードが自動削除（参加導線も消える）
...
|------------------|---------|
| `PUBLIC` | 常に返す |
| `ORGANIZATION_ONLY` | 常に返す（この組織のメンバーが前提のため）|
...
5. team_org_memberships を DELETE WHERE team_id = チームID
   （ON DELETE CASCADE により自動削除されるが、明示的に記載）
...
> - `team_org_memberships` の当該組織エントリは `ON DELETE CASCADE` により自動削除される（チームの所属記録が消えるが、チーム自体は存続）
...
6. user_roles に当該スコープのエントリが既存か確認 → 存在すれば 409
7. user_roles に INSERT（role_id = invite_tokens.role_id）
8. invite_tokens.used_count を +1 UPDATE
9. audit_logs に TEAM_MEMBER_JOINED または ORGANIZATION_MEMBER_JOINED を記録
10. 200 OK を返す
```
...
7. 権限グループのクリーンアップ:
   ...
   - 変更後ロールが DEPUTY_ADMIN または MEMBER（前ロールと同系の場合）: 保持
...
| ブロック操作（`/blocks` 系）| ADMIN によるアカウント管理目的 |
...
- **親リソース認可チェック（チーム・子組織作成時）**: 子リソースを作成する際は、作成者が**親リソース**に対する ADMIN 権限を持つことを必ず確認する。具体的には:
  - `POST /teams`（`organization_id` 指定時）: 指定組織に対して ADMIN 権限が必要
  - `POST /organizations`（`parent_organization_id` 指定時）: 指定親組織に対して ADMIN 権限が必要
  - 親リソースが存在しない・論理削除済みの場合は 404、権限不足は 403 を返す（存在チェックと権限チェックを分けることで情報漏洩を防ぐ）
...
- **ブロック済みユーザーの招待参加防止**: `POST /invite/{token}/join` では有効性チェックの直後にブロック済みチェックを行う。`team_blocks` / `organization_blocks` に対象ユーザーのエントリが存在する場合は 403 を返し、招待トークンを保持していてもブロックをバイパスして参加できないようにする
