...
2. リクエストボディをバリデーション
3. organization_id が指定された場合（組織配下チームとして作成）:
   a. 対象組織が存在・未削除か確認 → 存在しない / 論理削除済みの場合は 404
   b. 操作者が当該組織の ADMIN（または SYSTEM_ADMIN）か確認 → ADMIN 未満は 403
      ※ DEPUTY_ADMIN への委任（org-scope の CREATE_TEAM 権限）は Phase 3 以降で検討する
4. teams に INSERT
...
> - `organization_id` を指定しない場合（独立チーム）: 認証済みユーザーであれば誰でも作成可
> - MEMBER が組織配下にチームを作成する機能は設けない（チーム作成は組織の管理行為であるため）
