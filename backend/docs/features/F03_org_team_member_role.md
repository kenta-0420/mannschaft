...
| `team_permission_groups` | DEPUTY_ADMIN 用権限グループ（ADMIN が定義）| あり |
| `team_permission_group_permissions` | 権限グループ↔パーミッション紐付け | なし |
...
| `org_type` | ENUM('NONPROFIT', 'FORPROFIT') | NO | 'FORPROFIT' | 組織種別（課金ロジックに影響）|
| `org_type_verified` | BOOLEAN | NO | FALSE | NONPROFIT 審査承認済みフラグ（SYSTEM_ADMIN が操作）|
...
- `org_type_verified = FALSE` かつ `org_type = NONPROFIT` の場合、課金ロジックは FORPROFIT として扱う（審査中）
...
**シードデータ（V2.015__seed_role_permissions.sql）**
...
合計レコード数: 11 + 11 + 11 + 3 = **36件**

**制約・備考**
- **SYSTEM_ADMIN**: 全11件を保持するが、権限チェックは JWT 判定に統一（runtime で DB 参照しない）。シードは監査・将来対応のため投入する
- **ADMIN**: 全11件を保持。`DELETE_OTHERS_CONTENT` を含む全パーミッションを行使可能
- **DEPUTY_ADMIN**: role_permissions は**天井（ceiling）定義**として機能する。ADMIN が権限グループに追加できるのは DEPUTY_ADMIN の天井に含まれるパーミッションのみ。runtime での権限解決は role_permissions を参照せず `user_permission_groups` のみを使用する（権限グループ未割り当ての DEPUTY_ADMIN は実効パーミッション 0）
- **`DELETE_OTHERS_CONTENT`**: DEPUTY_ADMIN の天井に含める（ADMIN が権限グループで明示的に付与可能にするため）。ただし、いかなる**デフォルト権限グループにも含めない**。ADMIN が意図的に追加した場合のみ有効
- **MEMBER**: `MANAGE_SCHEDULES` / `MANAGE_FILES` / `MANAGE_POSTS` の3件（基本参加行動）。Phase 3 以降で ADMIN による制限機能を追加予定
...
| `name` | VARCHAR(100) | NO | — | グループ名（例: 受付・安否確認）|
| `description` | VARCHAR(500) | YES | NULL | 説明 |
...
#### `team_permission_group_permissions`

| カラム名 | 型 | NULL | デフォルト | 説明 |
|---------|---|------|-----------|------|
| `id` | BIGINT UNSIGNED | NO | AUTO_INCREMENT | PK |
| `group_id` | BIGINT UNSIGNED | NO | — | FK → team_permission_groups |
| `permission_id` | BIGINT UNSIGNED | NO | — | FK → permissions |

**インデックス**
```sql
UNIQUE KEY uq_tpgp_group_permission (group_id, permission_id)
INDEX idx_tpgp_permission (permission_id)
```
...
|---------|---|------|-----------|------|
| `id` | BIGINT UNSIGNED | NO | AUTO_INCREMENT | PK |
| `user_id` | BIGINT UNSIGNED | NO | — | FK → users（DEPUTY_ADMIN 対象ユーザー）|
| `group_id` | BIGINT UNSIGNED | NO | — | FK → team_permission_groups |
...
- 割り当て対象ユーザーは当該チームの DEPUTY_ADMIN のみ（アプリ層でバリデーション）
...
teams (1) ──── (N) team_permission_groups
team_permission_groups (1) ──── (N) team_permission_group_permissions
permissions (1) ──── (N) team_permission_group_permissions
users (N) ──── (M) team_permission_groups       ※ via user_permission_groups
...
| GET | `/api/v1/teams/{id}/permission-groups` | 必要（ADMIN）| 権限グループ一覧 |
| POST | `/api/v1/teams/{id}/permission-groups` | 必要（ADMIN）| 権限グループ作成 |
...
| PUT | `/api/v1/teams/{id}/members/{userId}/permission-groups` | 必要（ADMIN）| DEPUTY_ADMIN への権限グループ一括設定 |
...
> `permission_groups` は `role = DEPUTY_ADMIN` のユーザーのみ返す。他ロールは空配列。
...
3. organizations に INSERT（org_type_verified = FALSE）
4. user_roles に (user_id=作成者, role_id=ADMIN, organization_id=新組織ID) を INSERT
5. org_type = NONPROFIT の場合: 審査依頼通知を SYSTEM_ADMIN へ送信（通知機能実装後）
6. audit_logs に ORGANIZATION_CREATED を記録
7. 201 Created を返す
```
...
7. 変更後ロールが DEPUTY_ADMIN でない場合: user_permission_groups を全削除
...
5. ロールが DEPUTY_ADMIN →
     user_permission_groups（当該ユーザー・チーム）
       → team_permission_group_permissions
       → permissions
     を結合して実効パーミッションセットを取得
     ※ role_permissions は参照しない（権限グループ未割り当ての場合は実効パーミッション 0）
6. ロールが MEMBER →
     role_permissions WHERE role_id = MEMBER（MANAGE_SCHEDULES / MANAGE_FILES / MANAGE_POSTS）
...
7. audit_logs に TEAM_MEMBER_REMOVED を記録（metadata: {"reason": "BLOCK"}）
...
6. audit_logs に ORGANIZATION_MEMBER_REMOVED を記録（metadata: {"reason": "BLOCK"}）
7. 204 No Content を返す
```

### DEPUTY_ADMIN 権限の 3 層制御

README 記載の 3 層制御に対応する設計:

1. **SYSTEM_ADMIN が天井を設定**: `role_permissions` の DEPUTY_ADMIN 行が「付与可能なパーミッションの上限（天井）」。ADMIN は天井に含まれるパーミッションのみを権限グループに追加できる
2. **ADMIN が権限グループを構成**: `team_permission_group_permissions` で天井内のパーミッションを選択して名前付きグループを作成（権限グループ作成UIは DEPUTY_ADMIN の `role_permissions` を取得してリスト表示する）
3. **ADMIN がユーザーへ割り当て**: `user_permission_groups` でユーザーと権限グループを紐付け

> **`DELETE_OTHERS_CONTENT` の扱い**: DEPUTY_ADMIN の天井（role_permissions）に**含める**。これにより ADMIN が権限グループに明示的に追加することが可能になる。ただし、いかなるデフォルト権限グループにも含めない。ADMIN が意図的に付与した場合のみ有効。
...
- **組織種別変更**: `org_type` の FORPROFIT → NONPROFIT 変更は SYSTEM_ADMIN の承認が必要（悪用防止）
...
- **レートリミット**: 招待参加 (`POST /invite/{token}/join`)、チームフォロー (`POST /teams/{id}/follow`)、組織フォロー (`POST /organizations/{id}/follow`) それぞれに Bucket4j で 10 req/min per user を適用（乱用防止）
...
- [ ] 組織レベルの DEPUTY_ADMIN 権限グループ（`organization_permission_groups`）の必要性を Phase 2 開始前に確認する（現設計はチームスコープのみ）
