...
| `q` | String | — | 名前の部分一致検索（`display_name` を前方一致/部分一致で検索）|
...
> - `member_count` はそのチームの現在のメンバー数（`user_roles WHERE team_id = X` の件数。論理削除済みユーザーを除外）
> - 返却順: `joined_at` 昇順
...
> - `member_count` はその組織の直接所属メンバー数（`user_roles WHERE organization_id = X` の件数）
> - 返却順: `joined_at` 昇順
...
権限解決ロジックは全 API リクエストで実行されるため、Redis キャッシュによるパフォーマンス最適化を行う。
...
| ストア | Redis（Spring Cache + `@Cacheable`）|
