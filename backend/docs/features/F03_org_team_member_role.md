...
INDEX idx_tpg_team_role (team_id, target_role)   -- ロール別グループ一覧の取得用
```
...
- 割り当て対象ユーザーは当該チームの DEPUTY_ADMIN または MEMBER（アプリ層でバリデーション）
- 割り当て時に `team_permission_groups.target_role` と対象ユーザーのロールが一致することを確認する（MEMBER に DEPUTY_ADMIN 用グループを割り当てることは不可）
...
6. ロールが MEMBER →
   ...
   b. user_permission_groups（当該ユーザー・チーム）
        → team_permission_groups WHERE target_role = 'MEMBER'
        → team_permission_group_permissions → permissions
...
README 記載の 3 層制御は DEPUTY_ADMIN と MEMBER の両ロールに適用される。
...
2. **ADMIN が権限グループを構成**: `team_permission_groups（target_role = 'DEPUTY_ADMIN'）` + `team_permission_group_permissions` で天井内のパーミッションを選択して名前付きグループを作成
...
2. **ADMIN が権限グループを構成**: `team_permission_groups（target_role = 'MEMBER'）` + `team_permission_group_permissions` で天井内のパーミッションを選択してグループを作成
