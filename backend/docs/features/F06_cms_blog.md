...
```
1. ADMIN がチーム/組織のカスタムフィールドを定義（例: 得点, 結果, 出場時間）
2. 活動記録作成時、定義済みのカスタムフィールドに値を入力
3. is_required = true のフィールドは入力必須
4. カスタムフィールドの削除は is_active = false で論理無効化
   → 既存の活動記録に紐付いた値は保持される
5. NUMBER 型フィールドは統計 API で合計・平均・最小・最大を自動集計
6. SELECT 型フィールドは選択肢ごとの分布を集計
```
...
V6.008__create_activity_templates.sql       -- activity_templates テーブル作成
V6.009__create_activity_template_fields.sql -- activity_template_fields テーブル作成
V6.010__seed_official_activity_templates.sql -- 公式テンプレート初期データ投入
```

**マイグレーション上の注意点**
- `blog_posts` は `teams`, `organizations`, `users` テーブルに依存（Phase 1 で作成済み）
- FULLTEXT インデックスは InnoDB で作成（MySQL 5.6+ で対応済み）
- `activity_custom_values.custom_field_id` の FK は RESTRICT のため、マイグレーション順序に注意
- `V6.010` の公式テンプレート初期データは SYSTEM_ADMIN が管理画面から追加・編集可能なため、最小限の見本のみ投入（5〜10件程度）
...
- [ ] ⑤活動記録のカスタムフィールド上限数: チームあたりのフィールド定義数に上限を設けるか（推奨: 20個）
