...
- **記録ソース区分の自動設定**: `source` はエンドポイントに応じてサーバー側で自動セット。リクエストボディには含めない。`POST /records` / `POST /records/bulk` → `ADMIN`、`POST /records/self` → `SELF`、`POST /schedules/{id}/performance/records/bulk` → `SCHEDULE`
