...
- **保持ポリシー**: 2年間 DB で保持。2年超過分はアーカイブ（S3 互換ストレージ）へ移動後 DB から削除。アーカイブ先（バケット名・パス設計）はインフラ構成が固まる **Phase 2〜3 で共通インフラ定義として確定**し、バッチ実装もその時点で行う。`idx_al_created_at` インデックスにより効率的なバッチ処理が可能（実装上の支障なし）
