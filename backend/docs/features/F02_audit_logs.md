...
**ページネーション方式**: オフセットベース（`page` / `size`）。SYSTEM_ADMIN は件数把握・特定ページへのジャンプが有用なため、カーソルベースではなくオフセットを採用する。一般ユーザー向け（`/users/me/audit-logs`）およびスコープ付き（`/teams/{id}/audit-logs` 等）はカーソルベースを採用する。
...
| `cursor` | String | ページネーション用カーソル（`id` ベースの Base64 エンコード値）|

**レスポンス（200 OK）**

※ `created_at` の降順（最新順）で返す。カーソルベースページネーション。
...
**クエリパラメータ**: `GET /users/me/audit-logs` と同等（`event_type` / `event_category` / `from` / `to` / `limit` / `cursor`）に加え、`user_id` で特定メンバーに絞り込み可能。
...
- **冪等性**: Phase 1 は fire-and-forget 方式を採用する。アプリケーションがイベント発行直後にクラッシュした場合、監査ログが欠落するリスクがあるが、発生頻度は極めて低いため許容する。データ量・信頼性要件が高まった場合は Transactional Outbox パターンへの移行を検討する
...
- `metadata` の JSON スキーマ（各イベントの必須/オプションキー）は **設計書には詳細定義しない**。設計書上は「metadata 例」に留める
- アプリケーション層で **Java の sealed interface + record** によりイベントごとの metadata 型を定義し、コンパイル時に型安全性を担保する
- これにより設計書の肥大化を防ぎつつ、実装時のバリデーション漏れを防止する

```java
// 実装イメージ（設計書への詳細記載は不要）
public sealed interface AuditMetadata permits
    EmptyMetadata, LoginFailedMetadata, AccountLockedMetadata, ... {
}
public record LoginFailedMetadata(String reason) implements AuditMetadata {}
public record AccountLockedMetadata(String reason, Instant unlockAt) implements AuditMetadata {}
```
...
- **テーブルパーティショニング**: Phase 1 時点ではデータ量が少ないためパーティショニングは適用しない。データ量の増加に応じ（目安: 数千万行規模）、`created_at` による月別 RANGE パーティションの導入を検討する。パーティショニングにより2年超過分の DROP PARTITION が瞬時になり、期間指定クエリのスキャン範囲も限定される
...
| 日付 | 変更内容 |
|------|---------|
| 2026-03-12 | UX・保守性改善10項目: ① F01 新イベント4件をカタログに追加（`LOGOUT_SESSION` / `DEVICE_FINGERPRINT_MISMATCH` / `NEW_DEVICE_LOGIN` / `PASSWORD_SETUP`）② `event_type` のカンマ区切り複数指定を全 API に追加 ③ イベントカテゴリ定義（`AuditEventCategory` enum）と `event_category` クエリパラメータを追加 ④ `AuditEventType` enum によるアプリケーション層での型管理方針を明記 ⑤ ADMIN 向けスコープ付きログ参照 API 枠を追加（`GET /teams/{id}/audit-logs` / `GET /organizations/{id}/audit-logs`、Phase 3 以降） ⑥ アーカイブ済みログの検索方針（Athena / S3 Select）を保持ポリシーに追記 ⑦ ページネーション方式を確定: admin=オフセット維持（件数把握・ページジャンプ有用）、me+スコープ付き=カーソルベース（他ドキュメントと統一） ⑧ テーブルパーティショニングは Phase 1 見送り・将来検討として明記（月別 RANGE パーティション候補） ⑨ 書き込み冪等性は Phase 1 で fire-and-forget 維持を明記（将来 Transactional Outbox 検討） ⑩ `metadata` スキーマは設計書に詳細定義せず Java sealed interface + record で型安全性を担保する方針を明記 |
