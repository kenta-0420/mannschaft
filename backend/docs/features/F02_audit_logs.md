...
- 以下のイベントは `session_hash = NULL`（セッションが存在しないため）:
  - バッチ処理起因のイベント（`USER_ARCHIVED` / `TEAM_ARCHIVED` / `PENDING_USER_CLEANED_UP` 等）
  - `ACCOUNT_LOCKED`（ログイン前のブルートフォース検出のため、セッションなし）
  - `LOGIN_FAILED`（認証失敗のためセッション未発行）
  - `PASSWORD_RESET_REQUESTED`（メールアドレス入力のみ・セッションなし）
...
- **保持ポリシー**: 1年間 DB で保持。1年超過分はアーカイブ（S3 互換ストレージ）へ移動後 DB から削除。アーカイブ先（バケット名・パス設計）はインフラ構成が固まる **Phase 2〜3 で共通インフラ定義として確定**し、バッチ実装もその時点で行う。`idx_al_created_at` インデックスにより効率的なバッチ処理が可能（実装上の支障なし）
