...
- ファンアウト制御: 大規模チーム（N人同期）では1スケジュール変更で N件の Google API 呼び出しが発生する。Google Calendar Batch API（最大50件/バッチリクエスト）を使用してAPI呼び出し数を削減する。@Async で非同期実行し、メインスレッドへの影響を防ぐ
...
8. スケジュールを2段階クエリで取得（`deleted_at IS NULL` + min_view_role フィルタを適用）:
   a. 未来スケジュール: start_at >= TODAY かつ start_at <= TODAY + 12ヶ月 かつ deleted_at IS NULL
      ORDER BY start_at ASC  LIMIT 500
   b. 過去スケジュール: start_at >= TODAY - 2ヶ月 かつ start_at < TODAY かつ deleted_at IS NULL
...
V3.013__create_user_google_calendar_connections_table.sql
  -- personal_sync_enabled, last_sync_error_type/message/at を CREATE TABLE に含む（ALTER TABLE 不要）
