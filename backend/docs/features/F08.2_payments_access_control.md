...
| MEMBER | 自分の支払い状況参照、Stripe 決済の実行（自己支払い・寄付）|
...
| `display_order` | SMALLINT UNSIGNED | NO | 0 | 表示順序（小さいほど上。ADMIN がドラッグ＆ドロップ等で並び替え可能）|
| `grace_period_days` | SMALLINT UNSIGNED | NO | 0 | 有効期限後の猶予期間（日数）。0 = 猶予なし（即時アクセス遮断）。猶予期間中はアクセスを維持しつつ「支払い期限超過」バナーを表示 |
...
| `stripe_receipt_url` | VARCHAR(512) | YES | NULL | Stripe 発行のレシート URL（`charge.receipt_url` から取得。メンバーの支払い証明用）|
...
- **DONATION 型の特性**: 寄付はアクセス制御に紐付けない（`team_access_requirements` / `content_payment_gates` に設定不可）。`valid_from` は他の型と同様に `paid_at` の日付をセット（「いつ寄付したか」の記録として有用）。`valid_until` は NULL（無期限 — アクセス制御に使用しないため期限の概念がない）。ADMIN が寄付項目を作成し、SUPPORTER+ が自発的に Checkout で決済する。寄付履歴は `member_payments` に記録され、ADMIN が支払い状況一覧で確認可能
- 有効判定: `status = 'PAID'` AND (`valid_until IS NULL` OR `DATE_ADD(valid_until, INTERVAL pi.grace_period_days DAY) >= CURDATE()`)。`grace_period_days = 0` の場合は従来と同一（`valid_until >= CURDATE()`）
...
| POST | `/api/v1/teams/{id}/payment-items/{itemId}/payments/bulk` | 必要（ADMIN / MANAGE_PAYMENTS）| 手動支払い一括記録（複数メンバー同時）|
| DELETE | `/api/v1/teams/{id}/payment-items/{itemId}/payments/{paymentId}` | 必要（ADMIN）| 支払い記録取り消し（CANCELLED）|
| POST | `/api/v1/teams/{id}/payment-items/{itemId}/remind` | 必要（ADMIN）| 未払いメンバーへ一括リマインド通知送信 |
| GET | `/api/v1/teams/{id}/payment-items/{itemId}/payments/export` | 必要（ADMIN / MANAGE_PAYMENTS）| 支払い状況 CSV エクスポート |
| GET | `/api/v1/teams/{id}/payment-summary` | 必要（ADMIN / MANAGE_PAYMENTS）| チーム支払いサマリー（項目別の集計情報）|
...
| POST | `/api/v1/organizations/{id}/payment-items/{itemId}/payments/bulk` | 必要（ADMIN / MANAGE_PAYMENTS）| 組織手動支払い一括記録 |
| DELETE | `/api/v1/organizations/{id}/payment-items/{itemId}/payments/{paymentId}` | 必要（ADMIN）| 組織支払い記録取り消し |
| POST | `/api/v1/organizations/{id}/payment-items/{itemId}/remind` | 必要（ADMIN）| 組織未払いメンバーへ一括リマインド通知送信 |
| GET | `/api/v1/organizations/{id}/payment-items/{itemId}/payments/export` | 必要（ADMIN / MANAGE_PAYMENTS）| 組織支払い状況 CSV エクスポート |
| GET | `/api/v1/organizations/{id}/payment-summary` | 必要（ADMIN / MANAGE_PAYMENTS）| 組織支払いサマリー |
...
| GET | `/api/v1/me/payment-requirements` | 必要 | 自分に課されている未払い項目一覧（何を支払う必要があるか）|
| POST | `/api/v1/webhooks/stripe` | 署名検証のみ | Stripe Webhook 受信 |
| POST | `/api/v1/admin/stripe/reconcile/{paymentId}` | 必要（SYSTEM_ADMIN）| Stripe 状態との手動再同期（Webhook 欠損時のリカバリ）|
...
> - `is_active` は省略可（デフォルト `true`）。`grace_period_days` は省略可（デフォルト `0`）。`true` の場合はバックエンドが Stripe Product / Price を自動生成する
...
| 422 | `type` の変更を試みた（イミュータブル。変更が必要な場合は削除して再作成）|
| 422 | `stripe_price_id` 手動指定時に Stripe 上の金額・通貨と不一致 |
...
| ステータス | 条件 |
|-----------|------|
| 403 | 操作者が当該チーム/組織の SUPPORTER 以上のロールを持たない（GUEST は支払い対象外）|
...
> - `receipt_url`: Stripe 決済の場合に Stripe が発行するレシートページの URL。`payment_method = 'MANUAL'` の場合は `null`。メンバーが支払い証明として参照可能
...
| ステータス | 条件 |
|-----------|------|
| 403 | 権限不足（ADMIN または MANAGE_PAYMENTS 権限を持つ DEPUTY_ADMIN でない）|
...
#### `POST /api/v1/teams/{id}/payment-items/{itemId}/payments/bulk`（一括手動記録）

複数メンバーの手動支払いを1リクエストで記録する。イベント時の現金徴収等で利用。
...
> - 最大50件まで（超過は 400）
> - 各エントリのバリデーションは単件の手動記録と同一（`valid_from` / `valid_until` 省略時は自動計算）
> - 1件でも重複（409 条件に該当）がある場合はそのエントリをスキップし、成功分のみ処理する（部分成功を許容）
...
> - `skipped` には重複やバリデーションエラーのエントリが `{"user_id": X, "reason": "ALREADY_PAID"}` 形式で含まれる
> - 組織版（`/organizations/{id}/payment-items/{itemId}/payments/bulk`）も同一仕様

---

#### `POST /api/v1/teams/{id}/payment-items/{itemId}/remind`（手動リマインド送信）

指定した支払い項目の未払いメンバーに一括リマインド通知を送信する。
...
> - 未払い判定: `member_payments` に `status = 'PAID' AND (valid_until IS NULL OR valid_until >= CURDATE())` のレコードが存在しないメンバー
> - `type = 'DONATION'` の payment_item ではリマインド不可（422。寄付は任意のため催促しない）
> - レートリミット: 1 req/hour per payment_item（大量送信防止）
> - 組織版（`/organizations/{id}/payment-items/{itemId}/remind`）も同一仕様
...
| 403 | 権限不足（ADMIN でない）|
| 404 | 支払い項目が存在しない / 論理削除済み |
| 422 | `type = 'DONATION'`（寄付にはリマインド不可）|
| 429 | レートリミット超過（1時間以内に同一項目で再送信）|

---

#### `GET /api/v1/teams/{id}/payment-items/{itemId}/payments/export`（CSV エクスポート）

支払い状況を CSV 形式でダウンロードする。会計・確定申告用途。

**レスポンス（200 OK）**
```
Content-Type: text/csv; charset=UTF-8
Content-Disposition: attachment; filename="payments_2025年度年会費_20260401.csv"
```

CSV カラム: `メンバーID`, `メンバー名`, `ステータス`, `支払い金額`, `通貨`, `支払い方法`, `支払い日時`, `有効期間開始`, `有効期間終了`, `備考`

> - 全メンバー（未払い含む）を出力。未払いメンバーは `ステータス = UNPAID`、金額等は空欄
> - BOM 付き UTF-8（Excel での文字化け防止）
> - 組織版（`/organizations/{id}/payment-items/{itemId}/payments/export`）も同一仕様

---

#### `GET /api/v1/teams/{id}/payment-summary`（支払いサマリー）

チーム内の全支払い項目の集計情報を返す。ADMIN ダッシュボード用。
...
> - `items` は `display_order` 昇順でソート
> - 論理削除済みの payment_items は除外
> - `total_collected` は `status = 'PAID'` の `amount_paid` 合計（REFUNDED / CANCELLED は含まない）
> - 組織版（`/organizations/{id}/payment-summary`）も同一仕様

---

#### `GET /api/v1/me/payment-requirements`（未払い項目一覧）

ログインユーザー自身に課されている未払い項目の一覧を返す。チーム・組織横断。
  ...
  "data": [
    {
      ...
      "is_overdue": true,
      "overdue_since": "2026-03-31"
    }
  ]
}
```

> - `requirement_type`: `TEAM_ACCESS`（チーム全体ロック）/ `ORG_ACCESS`（組織全体ロック）/ `CONTENT_GATE`（コンテンツ単位ゲート）
> - `is_overdue`: 過去に PAID だったが `valid_until` を過ぎた場合 `true`。初回未払いは `false`
> - `overdue_since`: `is_overdue = true` の場合、期限切れになった `valid_until` の日付
> - `stripe_price_id` が存在する場合、フロントエンドが Checkout への導線を表示できる
> - `type = 'DONATION'` は対象外（アクセス要件に設定不可のため）

---

#### `POST /api/v1/admin/stripe/reconcile/{paymentId}`（Stripe 手動再同期）

Webhook 欠損や内部エラーにより DB と Stripe の状態が乖離した場合に、SYSTEM_ADMIN が手動で同期する。
...
> - `stripe_checkout_session_id` から Stripe Checkout Session を取得し、`payment_intent.status` を確認
> - Stripe 側で `succeeded` → DB を `PAID` に更新（Webhook 受信時と同一のロジックを実行）
> - Stripe 側で `expired` → DB を `CANCELLED` に更新
> - DB と Stripe が既に一致している場合は `reconciled: false` を返す（変更なし）
> - audit_logs に `PAYMENT_MANUALLY_RECONCILED` を記録
...
| 404 | 支払い記録が存在しない |
| 422 | `payment_method = 'MANUAL'`（Stripe 決済のみ対象）|
| 502 | Stripe API 通信失敗 |
...
3. イベント種別を判定:
   ...
   h. stripe_receipt_url = charge.receipt_url を保存（Stripe の Charge オブジェクトから取得）
   i. status = PAID、paid_at = NOW() で UPDATE
   j. audit_logs に PAYMENT_COMPLETED を記録
   ...
   d. 全額返金の場合:
      ...
      audit_logs に PAYMENT_REFUNDED（アプリ内返金後の Webhook）または
...
4. type ≠ DONATION の場合: 対象ユーザーの PAID かつ有効期間内の member_payments が既存か確認 → あれば 409
   ...
   type = DONATION の場合: 重複チェックをスキップ（同一ユーザーが同一寄付項目に複数回支払い可能）
...
2. 要件が存在する場合、各 payment_item_id に対して
   payment_items から grace_period_days を取得し、以下で判定:
   member_payments WHERE user_id = リクエスト者
     AND payment_item_id = X
     AND status = 'PAID'
     AND (valid_until IS NULL OR DATE_ADD(valid_until, INTERVAL pi.grace_period_days DAY) >= CURDATE())
   が存在するか確認
3. 未払い（猶予期間も超過）の要件が1件でもある → ロック状態（全コンテンツを返さずロック画面を表示）
4. 猶予期間中（valid_until < CURDATE() かつ valid_until + grace_period_days >= CURDATE()）の要件がある場合:
   → アクセスは許可するが「🔶 {payment_item.name} の支払い期限を超過しています」バナーを表示する情報をレスポンスに含める

[コンテンツ単位ゲートの確認]
5. 全体ロックをパスした場合、表示対象コンテンツに対して
   content_payment_gates WHERE content_type = X AND content_id = Y を取得
6. ゲートが存在する場合: 上記と同様に member_payments + grace_period_days を確認
7. 未払い（猶予期間も超過）の場合:
...
> ```sql
> -- 未払い × 完全非公開のコンテンツを除外（grace_period_days を考慮）
> AND NOT EXISTS (
>   SELECT 1 FROM content_payment_gates cpg
>   JOIN payment_items pi ON pi.id = cpg.payment_item_id
...
>         AND mp.status          = 'PAID'
>         AND (mp.valid_until IS NULL
>              OR DATE_ADD(mp.valid_until, INTERVAL pi.grace_period_days DAY) >= CURDATE())
>     )
> )
> ```
...
有効判定（Section 5「アクセス制御解決ロジック」より）:
  ...
  AND (valid_until IS NULL
       OR DATE_ADD(valid_until, INTERVAL pi.grace_period_days DAY) >= CURDATE())

→ valid_until + grace_period_days < CURDATE() のレコードはバッチ実行の有無にかかわらずリアルタイムにアクセス不可
→ 猶予期間中（valid_until < CURDATE() かつ valid_until + grace_period_days >= CURDATE()）はアクセス可だが警告バナーを表示
...
- **レートリミット**: `POST /payment-items/{id}/checkout` に Bucket4j で 5 req/min per user を適用（二重決済防止）。`POST /payment-items/{id}/remind` に 1 req/hour per payment_item を適用（リマインド大量送信防止）
...
| 2026-03-12 | UX・保守性改善 10件: ① `POST /payments/bulk` 一括手動記録 API 追加（最大50件・部分成功許容） ② `POST /payment-items/{itemId}/remind` 手動リマインド送信 API 追加（1 req/hour レートリミット・DONATION 除外） ③ `GET /me/payment-requirements` 未払い項目一覧 API 追加（requirement_type で全体ロック/コンテンツゲートを区別・猶予超過フラグ付き） ④ `GET /payments/export` CSV エクスポート API 追加（BOM 付き UTF-8・未払いメンバー含む全件出力） ⑤ `GET /payment-summary` ADMIN 向けサマリー API 追加（項目別 paid/unpaid/collected 集計） ⑥ 全一覧 API をカーソルベースページネーションに統一（支払い状況一覧・me/payments・コンテンツゲート一覧） ⑦ `POST /admin/stripe/reconcile/{paymentId}` 手動再同期 API 追加（SYSTEM_ADMIN 専用・Webhook 欠損リカバリ） ⑧ `payment_items` に `display_order SMALLINT UNSIGNED` 追加（表示順序制御） ⑨ `payment_items` に `grace_period_days SMALLINT UNSIGNED DEFAULT 0` 追加（猶予期間。アクセス制御解決ロジック・SQL フィルタ・有効判定式・バッチ連携を全箇所更新） ⑩ `member_payments` に `stripe_receipt_url VARCHAR(512)` 追加（Webhook で charge.receipt_url を保存・me/payments レスポンスに含める） |
