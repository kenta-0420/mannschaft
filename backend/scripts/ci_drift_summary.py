#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""API drift CI サマリ生成スクリプト（Stage 3 第一陣 1-β / 案 B warning-only）。
...
主要設計:
    ...
    - 警告のみで PR を fail させないため、必ず exit 0
"""
...
def build_comment(main_data: dict[str, object], pr_data: dict[str, object]) -> str:
    """PR コメント本文を組み立てる。"""
    lines: list[str] = []
    ...
    lines.append(
        "_このチェックは警告のみで PR をブロックしません。詳細は "
        "`docs/internal/api_drift_baseline.md` を参照。_"
    )
    lines.append("")
...
def main() -> int:
    ...
    body = build_comment(main_data, pr_data)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(body, encoding="utf-8")
