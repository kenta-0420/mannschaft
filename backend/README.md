...
運営側（SYSTEM_ADMIN）が機能モジュールを開発・提供し、チーム管理者がカタログから必要なモジュールを選択して有効化する。

### 仕組み

- **モジュールカタログ方式**: 運営側が提供する機能モジュールの中から、チームごとに必要なものを選択
- **運営による随時追加**: 新しい機能モジュールは運営側が開発・追加
- **テンプレート**: 業種別に推奨モジュールの組み合わせをプリセットとして用意。チーム作成時に選択すると推奨モジュールが自動有効化される
- **管理者によるON/OFF変更**: テンプレート選択後も、チーム管理者がモジュールを自由に追加・削除可能
...
| 2 | ダッシュボード | 個人/チーム/組織ダッシュボード・ウィジェットカスタマイズ |
| 3 | TODO管理 | タスク管理・担当者割り振り・進捗管理 |
| 4 | タイムライン | X風UIのチーム内投稿・交流 |
| 5 | CMS（ブログ・お知らせ） | 外部公開/メンバー限定ブログ・お知らせ配信 |
| 6 | チャット | Slack風階層スレッド・絵文字リアクション・グループチャット |
| 7 | 会費・決済 | 月会費/年会費/都度払い・集金管理 |
| 8 | プッシュ通知 | リマインド・LINE連携・WebSocket通知 |
| 9 | アンケート・投票 | カスタム設問・匿名/記名・集計グラフ |
| 10 | ファイル共有 | ドキュメント共有・フォルダ管理・アクセス権限 |
| 11 | 検索 | グローバル検索・フィルタリング |
| 12 | 通報・モデレーション | 不適切コンテンツ通報・レビュー・BAN対応 |
| 13 | メンション | @ユーザー名 通知・メンション一覧 |
| 14 | 監査ログ | 管理者操作履歴の自動記録・閲覧 |
| 15 | データエクスポート | CSV/JSON一括エクスポート |
| 16 | 多言語対応 (i18n) | 日本語/英語切替 |
| 17 | スケジュール・出欠管理 | カレンダー・Googleカレンダー同期・出欠回答 |
| 18 | 予約管理 | 時間枠予約・スタッフ別枠・リマインド |
| 19 | サービス履歴 | 施術/来店/対応履歴・カスタムフィールド |
| 20 | 活動記録 | 活動内容記録・カスタムフィールド・統計 |
| 21 | 掲示板 | カテゴリ別・ツリー構造返信・重要度5段階・既読機能 |
| 22 | マッチング・対外交流 | 対戦/交流相手募集・NGチーム設定 |
| 23 | パフォーマンス管理 | カスタムデータ記録・統計ダッシュボード |
| 24 | メンバー紹介 | メンバー一覧・プロフィール・拡張フィールド |
| 25 | 備品管理 | 在庫管理・貸出ステータス |
| 26 | ギャラリー | 写真アルバム・アップロード・閲覧権限 |

※ 運営側が随時新モジュールを追加予定

### テンプレート × 推奨モジュール対応表（プリセット）
...
※ テンプレート選択で推奨モジュールが自動有効化。管理者が自由にON/OFF変更可能
※ 有効モジュール数が10を超える場合は有料プランが必要
