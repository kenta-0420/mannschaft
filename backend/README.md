...
| 層 | 説明 | 例 |
|----|------|----|
| **個人アカウント** | ユーザー単位。実名登録＋表示用ニックネーム。複数チーム/組織にマルチ所属可能。QR会員証を保有。組織・チームに属さない個人利用も可能 | 田中太郎 |
| **チームアカウント** | チーム/店舗/教室等の運用単位。都道府県・市区町村を設定し地域検索可能。テンプレート選択で機能モジュールを決定。**組織に属さない独立チームも可能** | FCマンシャフト / 田中整骨院 / 3年B組 |
...
- 開発フローの制約（AI/人間の区別）は `.claudecode.md` §16 を参照すること。